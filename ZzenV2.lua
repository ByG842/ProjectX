...
Components.Textbox = (function()
	...
	return function(Parent, Acrylic)
		...
		Creator.AddSignal(Textbox.Input.Focused, function()
			Update()
			-- 🎬 Rework Pass 3: เส้น indicator ไล่ขยาย/เลื่อนนุ่มๆ แทนการสลับค่าทันที (ใช้ร่วมกันทุก Textbox ในไลบรารี)
			TweenService:Create(Textbox.Indicator, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				Size = UDim2.new(1, -2, 0, 2),
				Position = UDim2.new(0, 1, 1, 0),
				BackgroundTransparency = 0,
			}):Play()
			TweenService:Create(Textbox.Frame, TweenInfo.new(0.18), {
				BackgroundTransparency = Acrylic and 0.82 or 0,
			}):Play()
...
ElementsTable.Slider = (function()
	...
	function Element:New(Idx, Config)
		...
		local SliderDotScale = New("UIScale", { Scale = 1 }) -- Rework Pass 3
		SliderDotScale.Parent = SliderDot

		local function SetGrabbed(grabbed)
			TweenService:Create(SliderDotScale, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
				Scale = grabbed and 1.35 or 1,
			}):Play()
			TweenService:Create(SliderFrame.Border, TweenInfo.new(0.15), {
				Transparency = grabbed and 0.15 or 0.5,
			}):Play()
			Creator.OverrideTag(SliderFrame.Border, { Color = grabbed and "Accent" or "ElementBorder" })
		end
		...
		Creator.AddSignal(SliderDot.InputBegan, function(Input)
			...
			then
				Dragging = true
				SetGrabbed(true) -- 🎬 Rework Pass 3: จุดโตขึ้นเวลาจับลาก + ขอบ element เรือง Accent
			end
		end)
...
ElementsTable.Keybind = (function()
	...
	function Element:New(Idx, Config)
		...
		-- ⌨️ ไอคอนคีย์บอร์ดเล็กๆ นำหน้า text ให้ดูรู้ทันทีว่าเป็น keybind (Rework Pass 3)
		local KeybindIcon
		pcall(function()
			local iconId = Library:GetIcon("keyboard")
			if iconId then
				KeybindIcon = New("ImageLabel", {
					Image = iconId,
					Size = UDim2.fromOffset(14, 14),
					BackgroundTransparency = 1,
					LayoutOrder = 1,
		...
		-- ปุ่ม x เคลียร์ค่าเป็น None (Rework Pass 3: ฟีเจอร์ใหม่)
		...
		local KeybindRow = New("Frame", {
			...
			KeybindIcon,
			(function() KeybindDisplayLabel.LayoutOrder = 2 return KeybindDisplayLabel end)(),
			ClearButton,
		})

		local KeybindGlowStroke = New("UIStroke", {
			Transparency = 0.45,
			ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
			ThemeTag = {
				Color = "InElementBorder",
		...
		-- 🎬 hover feedback เบาๆ (Rework Pass 3)
		Creator.AddSignal(KeybindDisplayFrame.MouseEnter, function()
			if Picking then return end
			TweenService:Create(KeybindDisplayFrame, TweenInfo.new(0.15), { BackgroundTransparency = 0.78 }):Play()
		end)
		Creator.AddSignal(KeybindDisplayFrame.MouseLeave, function()
			if Picking then return end
			TweenService:Create(KeybindDisplayFrame, TweenInfo.new(0.15), { BackgroundTransparency = 0.88 }):Play()
		end)

		-- 🎬 pulse + ขอบเรือง Accent ตอน keybind ถูกกด/ทำงาน (Rework Pass 3)
		local function PulseActive(isOn)
			Creator.OverrideTag(KeybindGlowStroke, { Color = isOn and "Accent" or "InElementBorder" })
			TweenService:Create(KeybindGlowStroke, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Transparency = isOn and 0 or 0.45,
			}):Play()
			TweenService:Create(KeybindDisplayFrame, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				BackgroundTransparency = isOn and 0.75 or 0.88,
			}):Play()

			local pulseScale = KeybindDisplayFrame:FindFirstChildOfClass("UIScale")
			if not pulseScale then
				pulseScale = New("UIScale", { Scale = 1 })
				pulseScale.Parent = KeybindDisplayFrame
			end
			pulseScale.Scale = 0.94
			TweenService:Create(pulseScale, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
		end
		...
		function Keybind:SetValue(Key, Mode)
			...
			Keybind.Value = Key
			Keybind.Mode = Mode
			ClearButton.Visible = Key ~= "None" and Key ~= nil -- Rework Pass 3
		end
		...
		function Keybind:DoClick()
			PulseActive(Keybind.Mode == "Toggle" and Keybind.Toggled or true) -- 🎬 Rework Pass 3: feedback ตอนกด/ทำงานจริง
			if Keybind.Mode == "Toggle" then
				task.delay(0.16, function()
					if Keybind.Mode == "Toggle" and not Keybind.Toggled then
						PulseActive(false)
					end
				end)
			else
				task.delay(0.16, function() PulseActive(false) end)
			end
		...
		Creator.AddSignal(KeybindDisplayFrame.InputBegan, function(Input)
			...
			then
				...
				-- 🎬 Rework Pass 3: ขอบเรือง Accent กะพริบเบาๆ ตอนกำลังรอกดปุ่มใหม่ ให้รู้ว่า "กำลังฟังอยู่"
				Creator.OverrideTag(KeybindGlowStroke, { Color = "Accent" })
				TweenService:Create(KeybindGlowStroke, TweenInfo.new(0.15), { Transparency = 0 }):Play()
				...
				Event = UserInputService.InputBegan:Connect(function(Input)
					...
					EndedEvent = UserInputService.InputEnded:Connect(function(Input)
						...
						then
							...
							Keybind.Value = Key
							ClearButton.Visible = Key ~= "None" -- Rework Pass 3

							-- 🎬 เลิก glow ตอนเลือกเสร็จ + pulse เด้งยืนยันสั้นๆ
							Creator.OverrideTag(KeybindGlowStroke, { Color = "InElementBorder" })
							TweenService:Create(KeybindGlowStroke, TweenInfo.new(0.2), { Transparency = 0.45 }):Play()
							local confirmScale = KeybindDisplayFrame:FindFirstChildOfClass("UIScale")
							if not confirmScale then
								confirmScale = New("UIScale", { Scale = 1 })
								confirmScale.Parent = KeybindDisplayFrame
							end
							confirmScale.Scale = 1.08
							TweenService:Create(confirmScale, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
...
ElementsTable.Colorpicker = (function()
	...
	function Element:New(Idx, Config)
		...
		-- 🎬 Rework Pass 3: กรอบ Accent เรืองแสง + สเกลโตนิดๆ ตอน hover swatch สีในแถว element
		local DisplayFrameStroke = New("UIStroke", {
			Transparency = 1,
			Thickness = 1.5,
			ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
			ThemeTag = { Color = "Accent" },
		})
		local DisplayFrameScale = New("UIScale", { Scale = 1 })
		...
		Creator.AddSignal(DisplayFrame.MouseEnter, function()
			TweenService:Create(DisplayFrameStroke, TweenInfo.new(0.15), { Transparency = 0.2 }):Play()
			TweenService:Create(DisplayFrameScale, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 1.1 }):Play()
		end)
		Creator.AddSignal(DisplayFrame.MouseLeave, function()
			TweenService:Create(DisplayFrameStroke, TweenInfo.new(0.15), { Transparency = 1 }):Play()
			TweenService:Create(DisplayFrameScale, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 1 }):Play()
		end)
		...
		local function CreateColorDialog()
			...
			for _, PresetColor in ipairs(PRESETS) do
				...
				local SwatchScale = New("UIScale", { Scale = 1 }) -- Rework Pass 3
				...
				Creator.AddSignal(Swatch.MouseEnter, function()
					...
					TweenService:Create(SwatchScale, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 1.12 }):Play()
				end)

				Creator.AddSignal(Swatch.MouseLeave, function()
					TweenService:Create(Stroke, TweenInfo.new(0.12), { Transparency = 0.5, Thickness = GetStyleProperty("BorderThickness") }):Play()
					TweenService:Create(SwatchScale, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 1 }):Play()
				end)

				Creator.AddSignal(Swatch.MouseButton1Click, function()
					Hue, Sat, Vib = Color3.toHSV(PresetColor)
					Display()
					-- 🎬 pop ยืนยันตอนเลือก preset (Rework Pass 3)
					SwatchScale.Scale = 0.85
					TweenService:Create(SwatchScale, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1.12 }):Play()
				end)
			end
		...
		function Colorpicker:Display()
			...
			-- 🎬 Rework Pass 3: ไล่สีนุ่มๆ แทนการสลับสีทันที ตอนกด Done/SetValue
			TweenService:Create(DisplayFrameColor, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				BackgroundColor3 = Colorpicker.Value,
...
ElementsTable.MiniBar = (function()
	...
	function Element:New(Idx, Config)
		...
		local _pctCountToken = 0 -- Rework Pass 3: กันหลาย SetValue ยิงซ้อนกันแล้วเลขวิ่งชนกัน

		function Bar:SetValue(Value)
			Value = math.clamp(Value, self.Min, self.Max)
			local fromPct = (self.Value - self.Min) / math.max(self.Max - self.Min, 1)
			self.Value = Value
			...
			-- 🎬 Rework Pass 3: เลข % ไล่วิ่งขึ้น/ลงให้เข้ากับ fill bar แทนการเด้งเลขทันที
			_pctCountToken = _pctCountToken + 1
			local myToken = _pctCountToken
			task.spawn(function()
				local duration, steps = 0.3, 12
				for i = 1, steps do
					if myToken ~= _pctCountToken then return end
					local t = i / steps
					local eased = 1 - (1 - t) * (1 - t)
					local current = fromPct + (pct - fromPct) * eased
					PctLabel.Text = math.floor(current * 100 + 0.5) .. "%"
					task.wait(duration / steps)
				end
				if myToken == _pctCountToken then
...
ElementsTable.Alert = (function()
	...
	function Element:New(Idx, Config)
		...
		local RootScale = New("UIScale", { Scale = 0.9 }) -- 🎬 Rework Pass 3: ใช้ทำ pop-in ตอนโผล่มา

		local RootStroke = New("UIStroke", { Color = style.bg, Transparency = 0.3, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })

		local DismissBtn
		if Config.Dismissible then -- 🎁 Rework Pass 3: ฟีเจอร์ใหม่ ปิด Alert เองได้
		...
		local Root = New("Frame", {
			...
			BackgroundTransparency = 1, -- เริ่มจากใสสนิทแล้วค่อย fade เข้า
			...
			New("Frame", {
				BackgroundTransparency = 1,
				AutomaticSize = Enum.AutomaticSize.Y,
				Size = UDim2.new(1, Config.Dismissible and -44 or -22, 0, 0),
				LayoutOrder = 2,
			}, {
		...
		-- 🎬 Rework Pass 3: pop-in + fade เข้าตอนสร้าง แทนการโผล่มาทื่อๆ ทันที
		TweenService:Create(Root, TweenInfo.new(0.28, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { BackgroundTransparency = 0.75 }):Play()
		TweenService:Create(RootStroke, TweenInfo.new(0.28), { Transparency = 0.3 }):Play()
		TweenService:Create(RootScale, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()

		local destroyed = false
		local function animateOutThenDestroy()
			if destroyed then return end
			destroyed = true
			TweenService:Create(Root, TweenInfo.new(0.2), { BackgroundTransparency = 1 }):Play()
			TweenService:Create(RootStroke, TweenInfo.new(0.2), { Transparency = 1 }):Play()
			TweenService:Create(RootScale, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), { Scale = 0.9 }):Play()
			task.delay(0.22, function()
		...
		if DismissBtn then
			Creator.AddSignal(DismissBtn.MouseEnter, function() TweenService:Create(DismissBtn, TweenInfo.new(0.12), { TextTransparency = 0.3 }):Play() end)
			Creator.AddSignal(DismissBtn.MouseLeave, function() TweenService:Create(DismissBtn, TweenInfo.new(0.12), { TextTransparency = 0 }):Play() end)
			Creator.AddSignal(DismissBtn.MouseButton1Click, animateOutThenDestroy)
		end

		if Config.Duration then -- 🎁 Rework Pass 3: ปิดตัวเองอัตโนมัติหลังผ่านไปกี่วิ
			task.delay(Config.Duration, animateOutThenDestroy)
		end

		function Alert:Destroy() animateOutThenDestroy() end
...
ElementsTable.ActionButton = (function()
	...
	function Element:New(Idx, Config)
		...
		local CopyBtnScale = New("UIScale", { Scale = 1 }) -- Rework Pass 3
		CopyBtnScale.Parent = CopyBtn
		...
		Creator.AddSignal(CopyBtn.MouseButton1Click, function()
			...
			-- feedback: เปลี่ยนข้อความชั่วคราว + pop เด้งยืนยัน + สีเขียวแป๊บนึง (Rework Pass 3)
			CopyBtn.Text = copiedLabel
			CopyBtnScale.Scale = 0.88
			TweenService:Create(CopyBtnScale, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
			Creator.OverrideTag(CopyBtn, { BackgroundColor3 = "Accent" })
			TweenService:Create(CopyBtn, TweenInfo.new(0.15), { BackgroundTransparency = 0.5 }):Play()
			task.delay(resetDelay, function()
				if CopyBtn and CopyBtn.Parent then
					CopyBtn.Text = idleLabel
					TweenService:Create(CopyBtn, TweenInfo.new(0.25), { BackgroundTransparency = 0.85 }):Play()
				end
			end)
...
ElementsTable.ButtonGroup = (function()
	...
	function Element:New(Idx, Config)
		...
		BG.Buttons = {} -- Rework Pass 3: เก็บ handle ปุ่มแต่ละอันไว้ให้ควบคุมทีหลังได้ (เช่น SetLoading)

		for i, btn in ipairs(Config.Buttons) do
			local BtnScale = New("UIScale", { Scale = 1 }) -- Rework Pass 3

			local Spinner = New("ImageLabel", { -- Rework Pass 3: ใช้กับ BG:SetLoading(i, true)
				Image = Library:GetIcon("loader-2") or "",
				Size = UDim2.fromOffset(14, 14),
				Position = UDim2.fromScale(0.5, 0.5),
				AnchorPoint = Vector2.new(0.5, 0.5),
				BackgroundTransparency = 1,
			...
			local B = New("TextButton", {
				...
				NewCorner("SmallCorner"), -- 🐛 Rework Pass 3: เดิมไม่มีมุมโค้งเลย ทำให้ปุ่มดูเหลี่ยมแปลกๆ ต่างจาก element อื่นทั้งไลบรารี
			...
			Creator.AddSignal(B.MouseButton1Click, function()
				-- 🎬 Rework Pass 3: กดแล้วเด้งป๊อปนิดๆ ให้รู้สึกกดโดนจริง
				BtnScale.Scale = 0.9
			...
			BG.Buttons[i] = { Frame = B, Text = B }

			BG.Buttons[i].SetLoading = function(_, isLoading) -- 🎁 Rework Pass 3: ฟีเจอร์ใหม่ โชว์ spinner หมุนตอนกดแล้วรอ callback ทำงาน
				Spinner.Visible = isLoading
				B.TextTransparency = isLoading and 1 or 0
				B.AutoButtonColor = not isLoading
				if isLoading then
					task.spawn(function()
...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		for _, item in ipairs(Config.Items) do
			local chipScale = New("UIScale", { Scale = 1 }) -- Rework Pass 3
			...
			Creator.AddSignal(chip.MouseButton1Click, function()
				...
				-- 🎬 press-bounce สั้นๆ ตอนกด (Rework Pass 3)
				chipScale.Scale = 0.9
				TweenService:Create(chipScale, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
...
ElementsTable.Stepper = (function()
	...
	function Element:New(Idx, Config)
		...
		local ValueScale = New("UIScale", { Scale = 1 }) -- Rework Pass 3
		ValueScale.Parent = ValueDisplay

		local function set(v)
			v = math.clamp(CleanFloat(v, ST.Step), ST.Min, ST.Max)
			ST.Value = v
			ValueDisplay.Text = tostring(v) .. Config.Suffix
			-- 🎬 Rework Pass 3: ป๊อปนิดๆ ตอนค่าเปลี่ยน ให้รู้สึกมี feedback ทุกครั้งที่กด
			ValueScale.Scale = 1.15
			TweenService:Create(ValueScale, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
...
ElementsTable.ToggleGroup = (function()
	...
	function Element:New(Idx, Config)
		...
		-- 🎬 Rework Pass 3: แผ่น Accent เลื่อนไถลไปหาปุ่มที่เลือก แทนการ fade ทีละปุ่มแบบเดิม
		...
		local function updateBtns(selected, instant)
			for _, b in ipairs(Btns) do
				...
				if active then
					local goal = {
						Size = UDim2.fromOffset(b.Frame.AbsoluteSize.X, 24),
						Position = UDim2.fromOffset(b.Frame.Position.X.Offset, 2),
					}
					if instant then
						Highlight.Size = goal.Size
						Highlight.Position = goal.Position
					else
						TweenService:Create(Highlight, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), goal):Play()
					end
				end
			end
		end
		...
		for i, opt in ipairs(Config.Options) do
			...
			-- 🎬 hover เบาๆ เฉพาะปุ่มที่ยังไม่ active (Rework Pass 3)
			Creator.AddSignal(Btn.MouseEnter, function()
				if TG.Value ~= opt then
					TweenService:Create(Lbl, TweenInfo.new(0.15), { TextColor3 = Color3.fromRGB(230, 230, 230) }):Play()
				end
			end)
			Creator.AddSignal(Btn.MouseLeave, function()
				if TG.Value ~= opt then
					TweenService:Create(Lbl, TweenInfo.new(0.15), { TextColor3 = Creator.GetThemeProperty("SubText") }):Play()
				end
			end)
		...
		task.spawn(function()
			...
			RunService.Heartbeat:Wait()
			updateBtns(TG.Value, true)
		end) -- รอ layout คำนวณตำแหน่ง/ขนาดปุ่มจริงก่อนค่อยวาง Highlight แบบ instant (กัน AbsoluteSize ยังไม่นิ่ง)
...
ElementsTable.LiveLabel = (function()
	...
	function Element:New(Idx, Config)
		...
		function LL:SetText(text)
			text = text or ""
			local changed = text ~= self.Value -- Rework Pass 3: flash เฉพาะตอนค่าเปลี่ยนจริง กันกระพริบรัวตอน SetText ค่าเดิมซ้ำๆ
			self.Value      = text
			ValueLabel.Text = self.Value
			Pill.Visible    = self.Value ~= ""

			if changed and Pill.Visible then
				-- 🎬 Rework Pass 3: กะพริบสว่างวูบสั้นๆ ตอนค่าอัปเดต ให้เห็นชัดว่า "เพิ่งขยับ" สมกับชื่อ Live
				local flashScale = Pill:FindFirstChildOfClass("UIScale")
				if not flashScale then
					flashScale = New("UIScale", { Scale = 1 })
					flashScale.Parent = Pill
				end
				TweenService:Create(Pill, TweenInfo.new(0.08), { BackgroundTransparency = 0.35 }):Play()
				flashScale.Scale = 1.08
				task.delay(0.08, function()
					if Pill and Pill.Parent then
						TweenService:Create(Pill, TweenInfo.new(0.25), { BackgroundTransparency = 0.72 }):Play()
						TweenService:Create(flashScale, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 1 }):Play()
					end
				end)
			end
