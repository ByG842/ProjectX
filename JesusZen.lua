...
Components.Element = (function()
	...
	return function(Title, Desc, Parent, Hover, Options)
		...
		-- [ จุดที่แก้ ] เอา TextColor3/BackgroundColor3 hardcode ออกจาก TitleLabel — ไม่ใช่บั๊กที่ทำให้สีค้าง
		-- จริงๆ (เช็คแล้วพบว่า Creator.AddThemeObject เรียก UpdateTheme() ทันทีตอน register ซึ่งจะ sync
		-- สีจาก ThemeTag ทับ hardcode นี้อยู่ดีตั้งแต่เฟรมแรก) แต่เป็นโค้ดซ้ำซ้อน/ทำให้อ่านสับสนว่าตกลง
		-- สีมาจากไหนกันแน่ — ลบทิ้งให้เหลือแหล่งความจริงเดียวคือ ThemeTag เพื่อความสะอาดของโค้ด
		...
		-- [ จุดที่แก้ ] เอา TextColor3 hardcode ออกเหมือนกัน (เหตุผลเดียวกับ TitleLabel ด้านบน — ThemeTag
		-- sync ทับอยู่แล้ว ไม่ใช่บั๊ก) ส่วนฟอนต์ "Ubuntu" hardcode ตรงนี้ต่างออกไปนิดหน่อย: มันไม่ได้ผูกกับ
		-- ThemeTag เลย (ThemeTag รองรับแค่ TextColor3/BackgroundColor3/ImageColor3 ไม่รองรับ FontFace) แต่
		-- ทุก TextLabel จะ auto-register เข้า StyleRegistry.Texts อยู่ดีจาก Creator.New (ใช้ fontKey="FontMedium"
		-- เป็น default) ซึ่งจะ sync ฟอนต์ทับตอน ApplyStyle() ถูกเรียก (เช่นตอนสลับ Minecraft) เหมือนกัน — เปลี่ยน
		-- มาใช้ GetStyleProperty("FontRegular") ตรงๆ ให้สอดคล้องกับที่อื่นทั้งไฟล์ แทนที่จะ hardcode Font.new(...)
