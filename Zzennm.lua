...
ElementsTable.RadioGroup = (function()
	...
	function Element:New(Idx, Config)
		...
		-- 🎨🎬 Rework 2: การ์ดแต่ละแถวมีพื้นผิวถาวร (ไม่โล่งว่าง) + แถบบอกสถานะด้านซ้าย + glow นุ่มๆ หลังจุดวงกลมตอนเลือก
		local IdleFill, HoverFill, SelectedFill = 0.955, 0.9, 0.86
		local IdleStroke, SelectedStroke = 0.8, 0.3

		local function UpdateRadio(pulseOpt)
			for _, btn in pairs(Buttons) do
				...
				local dotTi = TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
				local fadeTi = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
				local barTi = TweenInfo.new(0.32, Enum.EasingStyle.Back, Enum.EasingDirection.Out)

				Creator.OverrideTag(btn.Card, { BackgroundColor3 = isSelected and "Accent" or "Element" })
				Creator.OverrideTag(btn.CardStroke, { Color = isSelected and "Accent" or "InElementBorder" })

				TweenService:Create(btn.Outer, dotTi, {
					BackgroundTransparency = isSelected and 0.05 or 0.9,
					Size = isSelected and UDim2.fromOffset(18, 18) or UDim2.fromOffset(16, 16),
				}):Play()
				TweenService:Create(btn.Inner, dotTi, {
					BackgroundTransparency = isSelected and 0 or 1,
					Size = isSelected and UDim2.fromOffset(9, 9) or UDim2.fromOffset(5, 5),
				}):Play()
				TweenService:Create(btn.Glow, dotTi, {
					BackgroundTransparency = isSelected and 0.7 or 1,
					Size = isSelected and UDim2.fromOffset(28, 28) or UDim2.fromOffset(16, 16),
				}):Play()

				TweenService:Create(btn.Card, fadeTi, {
					BackgroundTransparency = isSelected and SelectedFill or IdleFill,
				}):Play()
				TweenService:Create(btn.CardStroke, fadeTi, {
					Transparency = isSelected and SelectedStroke or IdleStroke,
				}):Play()
				TweenService:Create(btn.IndicatorBar, barTi, {
					Size = isSelected and UDim2.new(0, 3, 0.62, 0) or UDim2.new(0, 3, 0, 0),
					BackgroundTransparency = isSelected and 0.1 or 1,
				}):Play()
				TweenService:Create(btn.Label, fadeTi, {
					TextColor3 = isSelected and Creator.GetThemeProperty("Accent") or Creator.GetThemeProperty("Text"),
				}):Play()
				btn.Label.FontFace = isSelected and GetStyleProperty("FontBold") or GetStyleProperty("FontMedium")
			end

			-- 🎬 pop เด้งเบาๆ ที่การ์ดของตัวเลือกที่เพิ่งกด (ไม่เด้งตอน init/SetValue เงียบๆ)
			if pulseOpt then
				for _, btn in pairs(Buttons) do
					if btn.Value == pulseOpt then
						btn.CardScale.Scale = 0.97
						TweenService:Create(btn.CardScale, TweenInfo.new(0.24, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
					end
				end
			end
		end
		...
		for _, opt in ipairs(Config.Options) do
			local CardStroke = New("UIStroke", {
				Transparency = IdleStroke,
				Thickness = GetStyleProperty("BorderThickness"),
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
				ThemeTag = { Color = "InElementBorder" },
			})
			local CardScale = New("UIScale", { Scale = 1 })

			-- 🎨 แถบสีบอกสถานะด้านซ้าย (เหมือนตัวเลือกที่ถูกไฮไลต์อยู่) โผล่ขึ้นมาแบบสปริงตอนเลือก
			...
			local Row = New("TextButton", {
				...
				BackgroundTransparency = IdleFill,
				Text = "",
				...
				New("UIPadding", { PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 10) }),
				New("UIListLayout", {
					FillDirection = Enum.FillDirection.Horizontal,
					SortOrder = Enum.SortOrder.LayoutOrder,
					Padding = UDim.new(0, 10),
			...
			-- 🎬 glow นุ่มๆ ขยายออกรอบจุดวงกลมตอนถูกเลือก ให้ดูมีมิติ ไม่แบนเรียบ
			...
			local BtnData = { Value = opt, Outer = Outer, Inner = Inner, Glow = Glow, Card = Card, CardStroke = CardStroke, CardScale = CardScale, IndicatorBar = IndicatorBar, Label = Label }
			table.insert(Buttons, BtnData)

			-- 🎬 hover feedback เบาๆ (การ์ดยกน้ำหนักขึ้นนิดนึงเฉพาะแถวที่ยังไม่ได้เลือก)
			Creator.AddSignal(Row.MouseEnter, function()
				local isSelected = IsMulti and Radio.Value[opt] == true or (not IsMulti and opt == Radio.Value)
				if not isSelected then
					TweenService:Create(Card, TweenInfo.new(0.15), { BackgroundTransparency = HoverFill }):Play()
				end
			end)
			Creator.AddSignal(Row.MouseLeave, function()
				local isSelected = IsMulti and Radio.Value[opt] == true or (not IsMulti and opt == Radio.Value)
				if not isSelected then
					TweenService:Create(Card, TweenInfo.new(0.15), { BackgroundTransparency = IdleFill }):Play()
				end
			end)
			Creator.AddSignal(Row.MouseButton1Down, function()
				TweenService:Create(CardScale, TweenInfo.new(0.08), { Scale = 0.98 }):Play()
			end)
			Creator.AddSignal(Row.MouseButton1Up, function()
				TweenService:Create(CardScale, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
			end)
...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		-- 🎨🎬 Rework 2: ดาว (แทนติ๊กถูก) ลอยเป็นแบดจ์มุมขวาบนแบบ overlay ไม่กินพื้นที่ layout
		-- เพราะฉะนั้นก่อนกด/หลังกด ปุ่มไม่ขยับ ไม่เว้นช่องว่างให้ล่วงหน้าอีกต่อไป
		local StarColor = Color3.fromRGB(255, 196, 61)

		local function updateChip(item, wrapper)
			local active = table.find(CH.Value, item) ~= nil
			local chipBtn = wrapper:FindFirstChild("ChipButton")
			local badge = wrapper:FindFirstChild("StarBadge")
			local stroke = chipBtn and chipBtn:FindFirstChildOfClass("UIStroke")
			local label = chipBtn and chipBtn:FindFirstChild("ChipLabel")

			local popTi = TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
			local fadeTi = TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

			if chipBtn then
				TweenService:Create(chipBtn, popTi, {
					BackgroundTransparency = active and 0.08 or 0.92,
				}):Play()
			end

			if stroke then
				TweenService:Create(stroke, fadeTi, {
					Transparency = active and 0 or 0.85,
					Thickness = active and 1.5 or GetStyleProperty("BorderThickness"),
				}):Play()
			end

			if label then
				TweenService:Create(label, fadeTi, { TextTransparency = active and 0 or 0.35 }):Play()
				label.FontFace = active and GetStyleProperty("FontBold") or GetStyleProperty("FontMedium")
			end

			if badge then
				local icon = badge:FindFirstChild("StarIcon")
				local halo = badge:FindFirstChild("Halo")
				local badgeScale = badge:FindFirstChildOfClass("UIScale")

				if icon then
					TweenService:Create(icon, fadeTi, { ImageTransparency = active and 0 or 1 }):Play()
					TweenService:Create(icon, popTi, { Rotation = active and 0 or -35 }):Play()
				end
				if halo then
					TweenService:Create(halo, fadeTi, { BackgroundTransparency = active and 0.55 or 1 }):Play()
				end
				if badgeScale then
					TweenService:Create(badgeScale, popTi, { Scale = active and 1 or 0.3 }):Play()
				end
			end
		end

		for _, item in ipairs(Config.Items) do
			local ChipScale = New("UIScale", { Scale = 1 }) -- press-pop ตอนกด
			...
			-- ปุ่มชิปตัวจริง: ไม่มีไอคอนอยู่ใน ListLayout อีกต่อไป กันไม่ให้เว้นช่องว่างล่วงหน้า
			...
			-- ⭐ แบดจ์ดาวลอยมุมขวาบน: overlay เต็มตัว ไม่ยุ่งกับ ListLayout ของปุ่ม -> ขนาดปุ่มคงที่ตลอด ไม่ว่าจะติ๊กหรือไม่
			local StarBadge = New("Frame", {
				Name = "StarBadge",
				Size = UDim2.fromOffset(18, 18),
				...
				New("UIScale", { Scale = 0.3 }),
				New("Frame", {
					Name = "Halo",
					Size = UDim2.fromScale(1.7, 1.7),
					Position = UDim2.fromScale(0.5, 0.5),
					AnchorPoint = Vector2.new(0.5, 0.5),
					BackgroundColor3 = StarColor,
					BackgroundTransparency = 1,
					ZIndex = 4,
				}, {
					NewCorner("PillCorner"),
				...
				New("ImageLabel", {
					Name = "StarIcon",
					Image = Library:GetIcon("star") or "",
					Size = UDim2.fromScale(1, 1),
					BackgroundTransparency = 1,
					ImageTransparency = 1,
					ImageColor3 = StarColor,
					Rotation = -35,
					ZIndex = 6,
				}),
			})

			local ChipWrapper = New("Frame", {
				Name = "ChipWrapper",
				Size = UDim2.fromOffset(0, 30),
				AutomaticSize = Enum.AutomaticSize.XY,
				BackgroundTransparency = 1,
			...
			ChipBtns[item] = ChipWrapper
			updateChip(item, ChipWrapper)

			-- 🎬 hover ยกตัวขึ้นนิดๆ (สเกลโตเบาๆ) เฉพาะตอนยังไม่ได้เลือก ให้รู้สึก interactive
			Creator.AddSignal(chip.MouseEnter, function()
				local active = table.find(CH.Value, item) ~= nil
				if not active then
					TweenService:Create(ChipScale, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 1.05 }):Play()
					TweenService:Create(chip, TweenInfo.new(0.15), { BackgroundTransparency = 0.85 }):Play()
				end
			end)
			Creator.AddSignal(chip.MouseLeave, function()
				local active = table.find(CH.Value, item) ~= nil
				if not active then
					TweenService:Create(ChipScale, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 1 }):Play()
					TweenService:Create(chip, TweenInfo.new(0.15), { BackgroundTransparency = 0.92 }):Play()
				end
			end)
			...
			Creator.AddSignal(chip.MouseButton1Click, function()
				...
				for it, wrp in pairs(ChipBtns) do
					updateChip(it, wrp)
				end
				-- 🎬 press-bounce สั้นๆ ยืนยันว่ากดโดนจริง (เด้งเกินตัวนิดนึงแล้วหดกลับ)
				ChipScale.Scale = 0.88
				TweenService:Create(ChipScale, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
		...
		function CH:SetValue(v)
			self.Value = v
			for it, wrp in pairs(ChipBtns) do updateChip(it, wrp) end
