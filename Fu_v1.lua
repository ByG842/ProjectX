...
for _, ElementComponent in pairs(ElementsTable) do
	Elements["Add" .. ElementComponent.__type] = function(self, Idx, Config)
		...
		-- ── Compact mode: ถ้าอยู่ใน Column ให้ลด padding element ──────────
		if self._compact and result and result.Elements and result.Elements.LabelHolder then
			local lh = result.Elements.LabelHolder
			local pad = lh:FindFirstChildOfClass("UIPadding")
			if pad then
...
-- ─── AddColumns: 2-column layout (compact / minimal) ──────────────────
-- Usage:
--   local cols = Tab:AddColumns() or SubTab:AddColumns()
--   local left  = cols:Left()
--   local right = cols:Right()
--   left:AddSection("Title"):AddToggle("id", { Title = "Hello" })

local function CreateColumnsInContainer(parentContainer, scrollContainer)
	local New = Creator.New

	-- ── ค่าขนาด compact สำหรับ Column ─────────────────────────────────
	local COL_HEADER_H   = 22   -- header สูงกว่าปกติ (28) → 22
	local COL_CARD_PAD_H = 6    -- padding ซ้ายขวาใน card
	local COL_CARD_PAD_V = 4    -- padding บนล่างใน card
	local COL_GAP        = 4    -- ระยะห่างระหว่าง section card
	local COL_EL_PAD     = 7    -- top/bottom padding ของแต่ละ element ใน column
	local COL_GAP_COLS   = 5    -- ช่องว่างระหว่าง left/right column

	-- ── Compact Section สำหรับใช้เฉพาะใน Column ────────────────────────
	local function MakeCompactSection(Title, Parent, Icon)
		local Sec = {}

		Sec.Layout = New("UIListLayout", {
			Padding = UDim.new(0, 3),
			SortOrder = Enum.SortOrder.LayoutOrder,
		})

		Sec.Container = New("Frame", {
			Size = UDim2.new(1, -(COL_CARD_PAD_H * 2), 0, 0),
			Position = UDim2.fromOffset(COL_CARD_PAD_H, COL_HEADER_H + COL_CARD_PAD_V),
			BackgroundTransparency = 1,
		}, { Sec.Layout })
		...
		-- Compact icon
		local iconImg = nil
		if Icon and not fischbypass then
			local resolved = Library:GetIcon(Icon)
			iconImg = (resolved and resolved ~= "") and resolved or nil
		end
		...
		local Divider = New("Frame", {
			...
			Position = UDim2.new(0, 6, 0, COL_HEADER_H),
			BackgroundTransparency = 0.75,
			ThemeTag = { BackgroundColor3 = "InElementBorder" },
		})
		...
		local CardFrame = New("Frame", {
			...
			Size = UDim2.new(1, 0, 0, COL_HEADER_H + COL_CARD_PAD_V),
			LayoutOrder = 7,
			Parent = Parent,
			ThemeTag = { BackgroundColor3 = "Element" },
		}, {
		...
		Sec.Root = New("Frame", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, COL_HEADER_H + COL_CARD_PAD_V + COL_GAP),
			LayoutOrder = 7,
			Parent = Parent,
		}, {
			New("UIPadding", { PaddingTop = UDim.new(0, COL_GAP) }),
			CardFrame,
		})

		-- Auto-resize
		Creator.AddSignal(Sec.Layout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
			local h = Sec.Layout.AbsoluteContentSize.Y
			Sec.Container.Size = UDim2.new(1, -(COL_CARD_PAD_H * 2), 0, h)
			local cardH = COL_HEADER_H + COL_CARD_PAD_V + h + COL_CARD_PAD_V
			CardFrame.Size = UDim2.new(1, 0, 0, cardH)
			Sec.Root.Size = UDim2.new(1, 0, 0, cardH + COL_GAP)
		end)

		return Sec
	end

	-- ── Compact Element wrapper ──────────────────────────────────────────
	-- สร้าง Frame ที่มี padding น้อยกว่าปกติ แล้ว inject เข้า Element
	local function CompactElementContainer(realContainer)
		-- wrap realContainer ใน UIPadding override ไม่ได้โดยตรง
		-- แทนที่ด้วยการทำ proxy Frame ที่มี padding compact
		local Proxy = New("Frame", {
			Size = UDim2.new(1, 0, 0, 0),
			AutomaticSize = Enum.AutomaticSize.Y,
			BackgroundTransparency = 1,
			LayoutOrder = 7,
			Parent = realContainer,
		}, {
			New("UIListLayout", {
				Padding = UDim.new(0, 2),
				SortOrder = Enum.SortOrder.LayoutOrder,
	...
	local ColumnsRoot = New("Frame", {
		...
		New("UIListLayout", {
			Padding = UDim.new(0, COL_GAP_COLS),
			FillDirection = Enum.FillDirection.Horizontal,
			SortOrder = Enum.SortOrder.LayoutOrder,
			VerticalAlignment = Enum.VerticalAlignment.Top,
		}),
	...
	local function MakeColumn(layoutOrder)
		local ColFrame = New("Frame", {
			Size = UDim2.new(0.5, -(math.ceil(COL_GAP_COLS / 2) + 1), 0, 0),
			AutomaticSize = Enum.AutomaticSize.Y,
			BackgroundTransparency = 1,
			LayoutOrder = layoutOrder,
			Parent = ColumnsRoot,
		...
		-- AddSection: ใช้ compact section แทนของปกติ
		function Col:AddSection(SectionTitle, SectionIcon)
			local Sec    = MakeCompactSection(SectionTitle, ColFrame, SectionIcon)
			local SecObj = {
				Type       = "Section",
