...
ElementsTable.Colorpicker = (function()
	...
	function Element:New(Idx, Config)
		...
		local function CreateColorDialog()
			...
			local DialogW = HasAlpha and 500 or 460
			local DialogH = 400
			...
			local function CreateInput(X, Y, Width)
				...
				Box.Frame.Size = UDim2.fromOffset(Width, 34)
				return Box
			end
			...
			local SatVibMap = New("ImageLabel", {
				...
				Position = UDim2.fromOffset(PadX, TopY),
				...
				NewCorner("SmallCorner"),
				New("UIStroke", { Transparency = 0.7, Thickness = GetStyleProperty("BorderThickness") }),
				SatCursor,
			})
			...
			local HueSliderX = PadX + MapSize + 16
			local HueSlider = New("Frame", {
				Size = UDim2.fromOffset(18, MapSize),
				Position = UDim2.fromOffset(HueSliderX, TopY),
				Parent = Dialog.Root,
			}, {
				NewCorner("PillCorner"),
			...
			local TransparencySliderX = HueSliderX + 18 + 16
			if HasAlpha then
				...
				TransparencySlider = New("Frame", {
					Size = UDim2.fromOffset(18, MapSize),
					Position = UDim2.fromOffset(TransparencySliderX, TopY),
					Parent = Dialog.Root,
					BackgroundTransparency = 1,
				}, {
					NewCorner("PillCorner"),
			...
			-- Right-hand input column: label sits above each field
			local ColX = (HasAlpha and TransparencySliderX or HueSliderX) + 18 + 22
			local ColW = DialogW - ColX - PadX
			local RowH = 56

			local function Row(n) return TopY + (n * RowH) end

			CreateInputLabel("Hex", ColX, Row(0), ColW)
			local HexInput = CreateInput(ColX, Row(0) + 18, ColW)

			CreateInputLabel("Red", ColX, Row(1), ColW)
			local RedInput = CreateInput(ColX, Row(1) + 18, ColW)

			CreateInputLabel("Green", ColX, Row(2), ColW)
			local GreenInput = CreateInput(ColX, Row(2) + 18, ColW)

			CreateInputLabel("Blue", ColX, Row(3), ColW)
			local BlueInput = CreateInput(ColX, Row(3) + 18, ColW)

			local AlphaInput
			if HasAlpha then
				CreateInputLabel("Alpha", ColX, Row(4), ColW)
				AlphaInput = CreateInput(ColX, Row(4) + 18, ColW)
			end

			-- Current / New preview swatches beneath the sat/hue controls
			local SwatchY = TopY + MapSize + 18
			local SwatchW = (HueSliderX + 18 - PadX - 10) / 2
			local SwatchH = 40
			...
			local OldColorFrameChecker = New("ImageLabel", {
				...
				TileSize = UDim2.fromOffset(40, 40),
				BackgroundTransparency = 1,
				Position = UDim2.fromOffset(PadX, SwatchY),
				Size = UDim2.fromOffset(SwatchW, SwatchH),
				Parent = Dialog.Root,
			}, {
				NewCorner("SmallCorner"),
				New("UIStroke", { Transparency = 0.75, Thickness = GetStyleProperty("BorderThickness") }),
			...
			CreateInputLabel("Current", PadX, SwatchY + SwatchH + 6, SwatchW)
			...
			local DialogDisplayFrameChecker = New("ImageLabel", {
				...
				TileSize = UDim2.fromOffset(40, 40),
				BackgroundTransparency = 1,
				Position = UDim2.fromOffset(PadX + SwatchW + 10, SwatchY),
				Size = UDim2.fromOffset(SwatchW, SwatchH),
				Parent = Dialog.Root,
			}, {
				NewCorner("SmallCorner"),
			...
			CreateInputLabel("New", PadX + SwatchW + 10, SwatchY + SwatchH + 6, SwatchW)
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- 🎨🔥 Quick Color: a bold, tap-to-apply color system. No presets, no saving —
		-- just a glowing rainbow strip and a grid of vivid swatches for instant theming.
		...
		New("TextLabel", {
			Text = "Slide the rainbow or tap a swatch — your theme updates instantly.",
			FontFace = GetStyleProperty("FontMedium"),
			TextSize = GetStyleProperty("TextSizeSm") or 12,
			TextXAlignment = Enum.TextXAlignment.Left,
			TextWrapped = true,
