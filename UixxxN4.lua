...
local Themes = {
	...
	Minecraft = {
		Name = "Minecraft",
		Accent              = Color3.fromRGB(106, 190, 65),
		AcrylicMain         = Color3.fromRGB(58, 42, 27),
		...
		Element             = Color3.fromRGB(72, 52, 32),
		...
		InputIndicatorFocus = Color3.fromRGB(106, 190, 65),
...
function Creator.GetThemeProperty(Property)
	...
	if Library.CustomThemeColors and Library.CustomThemeColors[Property] then
		return Library.CustomThemeColors[Property]
	end
...
Components.Window = (function()
	...
	return function(Config)
		...
		local ResizeGripIcon = New("ImageLabel", {
			...
			ImageTransparency = Mobile and 0.35 or 1,
			ZIndex = 21,
		})
		...
		function Window:Minimize()
			...
			local SmoothSpringParams = { frequency = 4.5, dampingRatio = 1 }
		...
		do
			...
			local ENTRANCE_EASING_STYLE = Enum.EasingStyle.Quint
...
ElementsTable.Colorpicker = (function()
	...
	function Element:New(Idx, Config)
		...
		local function CreateColorDialog()
			...
			local HasAlpha = Config.Transparency
			local DialogW = 380
			local DialogH = HasAlpha and 650 or 600
			Dialog.Root.Size = UDim2.fromOffset(DialogW, DialogH)

			local PadX = 20
			local ContentW = DialogW - (PadX * 2)

			local OrigHue, OrigSat, OrigVib = Colorpicker.Hue, Colorpicker.Sat, Colorpicker.Vib
			local OrigTransparency = Colorpicker.Transparency

			local Hue, Sat, Vib = OrigHue, OrigSat, OrigVib
			local Transparency = OrigTransparency
			...
			-- === Hero preview band: big glowing color banner with rotating rainbow border ===
			local RainbowSequence = {}
			for hue = 0, 1, 0.1 do
				table.insert(RainbowSequence, ColorSequenceKeypoint.new(hue, Color3.fromHSV(hue, 1, 1)))
			end

			local HeroGradient = New("UIGradient", {
				Color = ColorSequence.new(RainbowSequence),
				Rotation = 0,
			})

			local HeroStroke = New("UIStroke", {
				Thickness = 3,
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
			}, {
				HeroGradient,
			...
			local HeroHexLabel = New("TextLabel", {
				FontFace = GetStyleProperty("FontBold") or GetStyleProperty("FontSemiBold"),
			...
			local HeroFrame = New("Frame", {
				Size = UDim2.fromOffset(ContentW, HeroH),
				Position = UDim2.fromOffset(PadX, HeroY),
				BackgroundColor3 = Colorpicker.Value,
				Parent = Dialog.Root,
			}, {
				NewCorner("ElementCorner"),
			...
			-- === "Current" chip: tap to revert every change back to the original color ===
			local ChipY = HeroY + HeroH + 8
			local ChipH = 30

			local CurrentChipSwatch = New("Frame", {
			...
			local CurrentChipLabel = New("TextLabel", {
				FontFace = GetStyleProperty("FontMedium"),
				Text = "↺  Tap to reset to Current (#" .. Colorpicker.Value:ToHex() .. ")",
				TextSize = GetStyleProperty("TextSizeSm") or 12,
			...
			local CurrentChip = New("TextButton", {
				Text = "",
				AutoButtonColor = false,
				BackgroundTransparency = 0.85,
				Size = UDim2.fromOffset(ContentW, ChipH),
				Position = UDim2.fromOffset(PadX, ChipY),
				Parent = Dialog.Root,
				ThemeTag = { BackgroundColor3 = "Element" },
			}, {
				NewCorner("TinyCorner"),
			...
			-- === Saturation / Value map (big, full width) ===
			local MapY = ChipY + ChipH + 12
			local MapH = 170
			...
			local SatVibMap = New("ImageLabel", {
				Size = UDim2.fromOffset(ContentW, MapH),
				Position = UDim2.fromOffset(PadX, MapY),
				...
				NewCorner("ElementCorner"),
				New("UIStroke", { Transparency = 0.6, Thickness = GetStyleProperty("BorderThickness") }),
				SatCursor,
			})

			-- === Horizontal hue bar (full width, unlike a vertical side-slider) ===
			local HueY = MapY + MapH + 12
			local HueH = 22
			...
			local HueDrag = New("Frame", {
				...
				NewCorner("PillCorner"),
				New("UIStroke", { Thickness = 2, Color = Color3.fromRGB(15, 15, 15), Transparency = 0.1 }),
			})

			local HueBar = New("TextButton", {
				Text = "",
				AutoButtonColor = false,
				Size = UDim2.fromOffset(ContentW, HueH),
				Position = UDim2.fromOffset(PadX, HueY),
				Parent = Dialog.Root,
			}, {
				NewCorner("PillCorner"),
				New("UIGradient", { Color = ColorSequence.new(RainbowSequence) }),
				New("UIStroke", { Transparency = 0.6, Thickness = GetStyleProperty("BorderThickness") }),
				HueDrag,
			})

			-- === Alpha bar (optional, same horizontal style) ===
			local AlphaBar, AlphaDrag, AlphaColorLayer
			local AlphaY = HueY + HueH + 12
			if HasAlpha then
				AlphaDrag = New("Frame", {
					Size = UDim2.fromOffset(16, 16),
					AnchorPoint = Vector2.new(0.5, 0.5),
				...
				AlphaBar = New("Frame", {
					Size = UDim2.fromOffset(ContentW, HueH),
					Position = UDim2.fromOffset(PadX, AlphaY),
					Parent = Dialog.Root,
				}, {
					NewCorner("PillCorner"),
					New("ImageLabel", {
			...
			-- === Compact input grid: Hex full width, then R / G / B in three columns ===
			local GridY = (HasAlpha and (AlphaY + HueH) or (HueY + HueH)) + 18
			local LabelH = 14
			local InputH = 32

			local function CreateInputLabel(Text, X, Y, Width)
				return New("TextLabel", {
					...
					TextSize = GetStyleProperty("TextSizeSm") or 12,
					TextXAlignment = Enum.TextXAlignment.Left,
					Size = UDim2.fromOffset(Width, LabelH),
			...
			local function CreateInput(X, Y, Width)
				local Box = Components.Textbox()
				Box.Frame.Parent = Dialog.Root
				Box.Frame.Position = UDim2.fromOffset(X, Y)
				Box.Frame.Size = UDim2.fromOffset(Width, InputH)
				return Box
			end

			CreateInputLabel("Hex", PadX, GridY, ContentW)
			local HexInput = CreateInput(PadX, GridY + LabelH + 4, ContentW)

			local RowY = GridY + LabelH + 4 + InputH + 14
			local ColGap = 10
			local ColW = (ContentW - (ColGap * 2)) / 3

			CreateInputLabel("R", PadX, RowY, ColW)
			local RedInput = CreateInput(PadX, RowY + LabelH + 4, ColW)

			CreateInputLabel("G", PadX + ColW + ColGap, RowY, ColW)
			local GreenInput = CreateInput(PadX + ColW + ColGap, RowY + LabelH + 4, ColW)

			CreateInputLabel("B", PadX + (ColW + ColGap) * 2, RowY, ColW)
			local BlueInput = CreateInput(PadX + (ColW + ColGap) * 2, RowY + LabelH + 4, ColW)

			local AlphaInput
			if HasAlpha then
				local AlphaRowY = RowY + LabelH + 4 + InputH + 14
				CreateInputLabel("Alpha", PadX, AlphaRowY, ContentW)
				AlphaInput = CreateInput(PadX, AlphaRowY + LabelH + 4, ContentW)
			end
			...
			local function Display()
				...
				HueDrag.Position = UDim2.new(Hue, 0, 0.5, 0)

				HeroFrame.BackgroundColor3 = Current
				HeroHexLabel.Text = "#" .. Current:ToHex()
				local Luminance = 0.299 * Current.R + 0.587 * Current.G + 0.114 * Current.B
				HeroHexLabel.TextColor3 = Luminance > 0.6 and Color3.fromRGB(20, 20, 20) or Color3.fromRGB(255, 255, 255)

				HexInput.Input.Text = "#" .. Current:ToHex()
				RedInput.Input.Text = GetRGB()["R"]
				GreenInput.Input.Text = GetRGB()["G"]
				BlueInput.Input.Text = GetRGB()["B"]

				if HasAlpha then
					AlphaColorLayer.BackgroundColor3 = Current
					AlphaDrag.Position = UDim2.new(1 - Transparency, 0, 0.5, 0)
					AlphaInput.Input.Text = Library:Round((1 - Transparency) * 100, 0) .. "%"
				end
			end

			Creator.AddSignal(CurrentChip.MouseButton1Click, function()
				Hue, Sat, Vib = OrigHue, OrigSat, OrigVib
			...
			Creator.AddSignal(HueBar.InputBegan, function(Input)
				if
					Input.UserInputType == Enum.UserInputType.MouseButton1
					or Input.UserInputType == Enum.UserInputType.Touch
				then
			...
			if HasAlpha then
				Creator.AddSignal(AlphaBar.InputBegan, function(Input)
					if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
						while UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
							local MinX = AlphaBar.AbsolutePosition.X
							local MaxX = MinX + AlphaBar.AbsoluteSize.X
							local MouseX = math.clamp(Mouse.X, MinX, MaxX)

							Transparency = 1 - ((MouseX - MinX) / (MaxX - MinX))
							Display()
...
ElementsTable.RadioGroup = (function()
	...
	function Element:New(Idx, Config)
		...
		local function GetCallbackValue()
			...
			local selected = {}
			for _, opt in ipairs(Radio.Options) do
	...
	function Element:New(Idx, Config)
		...
		local function updateChip(item, btn)
			...
			TweenService:Create(btn, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				BackgroundTransparency = active and 0.15 or 0.92,
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- Helper: read a saved {r,g,b} table from Settings, falling back to a default Color3
		local function ReadSavedColor(SettingsKey, FallbackColor)
			local Saved = Settings[SettingsKey]
			if Saved and typeof(Saved) == "table" then
				return Color3.fromRGB(Saved.r or 200, Saved.g or 200, Saved.b or 200)
			end
			return FallbackColor
		end

		local defaultAccentColor = ReadSavedColor("AccentColor", Color3.fromRGB(200, 200, 200))
		local defaultTextColor = ReadSavedColor("TextColor", Color3.fromRGB(240, 240, 240))
		local defaultElementColor = ReadSavedColor("ElementColor", Color3.fromRGB(35, 35, 40))
		local defaultWindowColor = ReadSavedColor("WindowColor", Color3.fromRGB(20, 20, 24))

		-- === Multiple color pickers: customize the whole theme, not just Accent ===
		local AccentColorPicker = customThemeSection:AddColorpicker("CustomAccentColor", {
			Title = "Accent Color",
			Description = "The highlight color used for toggles, sliders and selections.",
		...
		local TextColorPicker = customThemeSection:AddColorpicker("CustomTextColor", {
			Title = "Text Color",
			Description = "The main text color across the interface.",
			Default = defaultTextColor,
			Callback = function(Color)
				Library:SetThemeColorOverride("Text", Color)
				Settings.TextColor = { r = math.floor(Color.R * 255), g = math.floor(Color.G * 255), b = math.floor(Color.B * 255) }
				InterfaceManager:SaveSettings()
			end,
		})

		local ElementColorPicker = customThemeSection:AddColorpicker("CustomElementColor", {
			Title = "Card Color",
			Description = "The background color of cards, sections and elements.",
			Default = defaultElementColor,
			Callback = function(Color)
				Library:SetThemeColorOverride("Element", Color)
				Settings.ElementColor = { r = math.floor(Color.R * 255), g = math.floor(Color.G * 255), b = math.floor(Color.B * 255) }
				InterfaceManager:SaveSettings()
			end,
		})

		local WindowColorPicker = customThemeSection:AddColorpicker("CustomWindowColor", {
			Title = "Window Background",
			Description = "The main background color of the entire window.",
			Default = defaultWindowColor,
			Callback = function(Color)
				Library:SetThemeColorOverride("AcrylicMain", Color)
				Settings.WindowColor = { r = math.floor(Color.R * 255), g = math.floor(Color.G * 255), b = math.floor(Color.B * 255) }
				InterfaceManager:SaveSettings()
			end,
		})

		task.defer(function()
			if Settings.AccentColor then Library:SetAccentColor(defaultAccentColor, true) end
			if Settings.TextColor then Library:SetThemeColorOverride("Text", defaultTextColor, true) end
			if Settings.ElementColor then Library:SetThemeColorOverride("Element", defaultElementColor, true) end
			if Settings.WindowColor then Library:SetThemeColorOverride("AcrylicMain", defaultWindowColor, true) end
		end)

		local function SyncPickers()
			AccentColorPicker:SetValueRGB(Library.CustomAccentColor or (Themes[Library.Theme] and Themes[Library.Theme].Accent) or Color3.fromRGB(200, 200, 200))
			TextColorPicker:SetValueRGB((Library.CustomThemeColors and Library.CustomThemeColors.Text) or (Themes[Library.Theme] and Themes[Library.Theme].Text) or Color3.fromRGB(240, 240, 240))
			ElementColorPicker:SetValueRGB((Library.CustomThemeColors and Library.CustomThemeColors.Element) or (Themes[Library.Theme] and Themes[Library.Theme].Element) or Color3.fromRGB(35, 35, 40))
			WindowColorPicker:SetValueRGB((Library.CustomThemeColors and Library.CustomThemeColors.AcrylicMain) or (Themes[Library.Theme] and Themes[Library.Theme].AcrylicMain) or Color3.fromRGB(20, 20, 24))
		end

		-- 🎨🔥 Quick Color: a bold, tap-to-apply color system for the Accent color.
		-- No presets, no saving — just a glowing rainbow strip and vivid swatches.
		local function ApplyAccent(Color)
			Library:SetAccentColor(Color)
			Settings.AccentColor = { r = math.floor(Color.R * 255), g = math.floor(Color.G * 255), b = math.floor(Color.B * 255) }
			InterfaceManager:SaveSettings()
			AccentColorPicker:SetValueRGB(Color)
		end

		local QuickColorCard = New("Frame", {
			AutomaticSize = Enum.AutomaticSize.Y,
			BackgroundTransparency = 0.1,
			Size = UDim2.new(1, 0, 0, 0),
			Parent = customThemeSection.Container,
			LayoutOrder = 50,
			ThemeTag = {
				BackgroundColor3 = "Element",
			},
		...
		local QuickColorLayout = New("UIListLayout", {
			SortOrder = Enum.SortOrder.LayoutOrder,
			Padding = UDim.new(0, 10),
			Parent = QuickColorCard,
		})

		New("TextLabel", {
			Text = "⚡ QUICK COLOR",
			FontFace = GetStyleProperty("FontBold") or GetStyleProperty("FontMedium"),
			TextSize = (GetStyleProperty("TextSizeMd") or 13) + 2,
			TextXAlignment = Enum.TextXAlignment.Left,
			TextColor3 = Color3.fromRGB(240, 240, 240),
			BackgroundTransparency = 1,
		...
		New("TextLabel", {
			Text = "Slide the rainbow or tap a swatch — your accent updates instantly.",
			FontFace = GetStyleProperty("FontMedium"),
			TextSize = GetStyleProperty("TextSizeSm") or 12,
			TextXAlignment = Enum.TextXAlignment.Left,
			TextWrapped = true,
		...
		-- Rainbow strip: tap or drag anywhere to instantly set a fully-saturated hue
		local RainbowSequence = {}
		for hue = 0, 1, 0.1 do
			table.insert(RainbowSequence, ColorSequenceKeypoint.new(hue, Color3.fromHSV(hue, 1, 1)))
		end

		local RainbowCursor = New("Frame", {
			...
			NewCorner("PillCorner"),
			New("UIStroke", { Transparency = 0.2, Thickness = 2, Color = Color3.fromRGB(15, 15, 15) }),
		})

		local RainbowStrip = New("TextButton", {
			Text = "",
			AutoButtonColor = false,
			Size = UDim2.new(1, 0, 0, 36),
			LayoutOrder = 3,
			...
			New("UIStroke", { Transparency = 0.35, Thickness = GetStyleProperty("BorderThickness"), Color = Color3.fromRGB(0, 0, 0) }),
			RainbowCursor,
		})

		local function SetHueFromX(X)
			local MinX = RainbowStrip.AbsolutePosition.X
			local MaxX = MinX + RainbowStrip.AbsoluteSize.X
			local Clamped = math.clamp(X, MinX, MaxX)
			local Hue = (Clamped - MinX) / math.max(MaxX - MinX, 1)
			RainbowCursor.Position = UDim2.new(Hue, 0, 0.5, 0)
			ApplyAccent(Color3.fromHSV(Hue, 1, 1))
		end

		Creator.AddSignal(RainbowStrip.InputBegan, function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
		...
		-- Vivid swatch grid: bold, saturated one-tap colors
		local QuickSwatches = {
			Color3.fromRGB(255, 71, 87),   -- red
			Color3.fromRGB(255, 140, 30),  -- orange
			Color3.fromRGB(255, 209, 51),  -- yellow
			Color3.fromRGB(46, 213, 115),  -- green
			Color3.fromRGB(26, 232, 202),  -- teal
			Color3.fromRGB(52, 172, 224),  -- sky
			Color3.fromRGB(58, 89, 255),   -- blue
			Color3.fromRGB(139, 92, 246),  -- violet
			Color3.fromRGB(217, 70, 239),  -- magenta
			Color3.fromRGB(255, 105, 180), -- pink
			Color3.fromRGB(255, 255, 255), -- white
			Color3.fromRGB(30, 30, 36),    -- graphite
		}

		local SwatchGrid = New("Frame", {
			AutomaticSize = Enum.AutomaticSize.Y,
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 0),
			LayoutOrder = 4,
		...
		for i, SwatchColor in ipairs(QuickSwatches) do
			...
			local ScaleMotor, SetScale = Creator.SpringMotor(1, Scale, "Scale")

			Creator.AddSignal(Swatch.MouseEnter, function() SetScale(1.18) end)
			Creator.AddSignal(Swatch.MouseLeave, function() SetScale(1) end)
			Creator.AddSignal(Swatch.MouseButton1Down, function() SetScale(0.9) end)
			Creator.AddSignal(Swatch.MouseButton1Up, function() SetScale(1.18) end)

			Creator.AddSignal(Swatch.MouseButton1Click, function()
				ApplyAccent(SwatchColor)
			end)
		end

		-- === 💾 Theme Save System: save the full custom theme combo under a name ===
		Settings.SavedThemes = Settings.SavedThemes or {}

		local function GetThemeColorTable()
			return {
				Accent  = { r = math.floor(AccentColorPicker.Value.R * 255), g = math.floor(AccentColorPicker.Value.G * 255), b = math.floor(AccentColorPicker.Value.B * 255) },
				Text    = { r = math.floor(TextColorPicker.Value.R * 255), g = math.floor(TextColorPicker.Value.G * 255), b = math.floor(TextColorPicker.Value.B * 255) },
				Element = { r = math.floor(ElementColorPicker.Value.R * 255), g = math.floor(ElementColorPicker.Value.G * 255), b = math.floor(ElementColorPicker.Value.B * 255) },
				Window  = { r = math.floor(WindowColorPicker.Value.R * 255), g = math.floor(WindowColorPicker.Value.G * 255), b = math.floor(WindowColorPicker.Value.B * 255) },
			}
		end

		local function GetSavedThemeNames()
			local names = {}
			for _, theme in ipairs(Settings.SavedThemes) do
				table.insert(names, theme.Name)
			end
			if #names == 0 then
				table.insert(names, "No themes saved")
			end
			return names
		end

		local ThemeNameInput = customThemeSection:AddInput("SaveThemeName", {
			Title = "Theme Name",
			Description = "Name your current color combo, then save it below.",
			Default = "",
			Placeholder = "e.g. Midnight Purple",
			MaxLength = 24,
		})

		local SavedThemesDropdown = customThemeSection:AddDropdown("SavedThemesSelect", {
			Title = "Saved Themes",
			Description = "Pick a saved theme to apply or delete it.",
			Values = GetSavedThemeNames(),
			Default = GetSavedThemeNames()[1],
		})

		local function RefreshThemeDropdown()
			local names = GetSavedThemeNames()
			SavedThemesDropdown:SetValues(names)
			SavedThemesDropdown:SetValue(names[1])
		end

		customThemeSection:AddActionButton("SaveTheme", {
			Title       = "Save Theme",
			Description = "Stores Accent, Text, Card and Window colors together under the name above.",
			ButtonText  = "Save",
			CopiedText  = "✓ Saved",
			CopyText    = "",
			Callback    = function()
				local Name = (ThemeNameInput.Value ~= "" and ThemeNameInput.Value)
					or ("Theme " .. (#Settings.SavedThemes + 1))
				...
				local Colors = GetThemeColorTable()
				if existing then
					existing.Colors = Colors
				else
					table.insert(Settings.SavedThemes, { Name = Name, Colors = Colors })
				end

				InterfaceManager:SaveSettings()
				RefreshThemeDropdown()
				SavedThemesDropdown:SetValue(Name)
			end,
		})

		customThemeSection:AddActionButton("ApplyTheme", {
			Title       = "Apply Theme",
			Description = "Applies the selected saved theme's colors.",
			ButtonText  = "Apply",
			CopiedText  = "✓ Applied",
			CopyText    = "",
			Callback    = function()
				local SelectedName = SavedThemesDropdown.Value
				for _, theme in ipairs(Settings.SavedThemes) do
					if theme.Name == SelectedName then
						local c = theme.Colors
						local AccentC = Color3.fromRGB(c.Accent.r, c.Accent.g, c.Accent.b)
						local TextC = Color3.fromRGB(c.Text.r, c.Text.g, c.Text.b)
						local ElementC = Color3.fromRGB(c.Element.r, c.Element.g, c.Element.b)
						local WindowC = Color3.fromRGB(c.Window.r, c.Window.g, c.Window.b)

						Library:SetAccentColor(AccentC)
						Library:SetThemeColorOverride("Text", TextC)
						Library:SetThemeColorOverride("Element", ElementC)
						Library:SetThemeColorOverride("AcrylicMain", WindowC)

						Settings.AccentColor = c.Accent
						Settings.TextColor = c.Text
						Settings.ElementColor = c.Element
						Settings.WindowColor = c.Window
						InterfaceManager:SaveSettings()
		...
		customThemeSection:AddActionButton("DeleteTheme", {
			Title       = "Delete Theme",
			Description = "Removes the selected theme from your saved list.",
			ButtonText  = "Delete",
			CopiedText  = "✓ Deleted",
			CopyText    = "",
			Callback    = function()
				local SelectedName = SavedThemesDropdown.Value
				for i, theme in ipairs(Settings.SavedThemes) do
					if theme.Name == SelectedName then
						table.remove(Settings.SavedThemes, i)
						break
					end
				end
				InterfaceManager:SaveSettings()
		...
		-- === 💡 Accent Color RGB: a silly disco light effect that cycles the accent
		-- through the rainbow. Sits right before the reset button, as requested. ===
		local RGBConnection = nil

		local AccentRGBToggle = customThemeSection:AddToggle("AccentColorRGB", {
			Title = "Accent Color RGB",
			Description = "Fun disco lighting — cycles your accent color through the rainbow.",
			Default = false,
			Callback = function(Value)
				if Value then
					local RGBHue = 0
					RGBConnection = RenderStepped:Connect(function(dt)
						RGBHue = (RGBHue + dt * 0.25) % 1
						Library:SetAccentColor(Color3.fromHSV(RGBHue, 1, 1), true)
					end)
				else
					...
					-- restore the picker's chosen accent color once the light show stops
					Library:SetAccentColor(AccentColorPicker.Value, true)
				end
			end,
		})
		...
		local ResetAccentButton = customThemeSection:AddActionButton("ResetAccentColor", {
			...
			Description = "Clears all custom colors and restores the theme's defaults.",
			ButtonText = "Reset",
			CopiedText = "✓ Reset",
			CopyText   = "",
			Callback   = function()
				if RGBConnection then
					RGBConnection:Disconnect()
					RGBConnection = nil
					AccentRGBToggle:SetValue(false)
				end

				Library:ResetAllThemeColorOverrides()
				Settings.AccentColor = nil
				Settings.TextColor = nil
				Settings.ElementColor = nil
				Settings.WindowColor = nil
				InterfaceManager:SaveSettings()
				SyncPickers()
			end,
		})
...
Library.CreateWindow = function(self, Config)
	...
	if Config.BackgroundImageTransparency == nil and Config.Theme == "Minecraft" then
		Config.BackgroundImageTransparency = 0.08
	end
...
-- 🎨 Generic theme color overrides: lets any registered theme key (Text, Element,
-- AcrylicMain, etc.) be recolored independently of Accent, for full custom theming.
Library.CustomThemeColors = Library.CustomThemeColors or {}

function Library:SetThemeColorOverride(Key, Color, Instant)
	Library.CustomThemeColors = Library.CustomThemeColors or {}
	Library.CustomThemeColors[Key] = Color
	if Instant then
		Creator.UpdateTheme()
	else
		Creator.UpdateThemeAnimated(0.3)
...
function Library:ResetThemeColorOverride(Key, Instant)
	if Library.CustomThemeColors then
		Library.CustomThemeColors[Key] = nil
	end
	if Instant then
		Creator.UpdateTheme()
	else
...
function Library:ResetAllThemeColorOverrides(Instant)
	Library.CustomThemeColors = {}
	Library.CustomAccentColor = nil
	if Instant then
		Creator.UpdateTheme()
	else
