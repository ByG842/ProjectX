...
--// Load IconsX from GitHub
local Icons = {}
local ok, result = pcall(function()
	return loadstring(game:HttpGet("https://raw.githubusercontent.com/ByG842/ProjectX/refs/heads/main/Iconsx.lua"))()
end)
...
local assets = {
	...
	-- Icons from IconsX (lucide)
	buttonImage      = GetIcon("lucide-chevron-right"),   -- > arrow for buttons
	searchIcon       = GetIcon("lucide-search"),           -- 🔍 search
	grid             = GetIcon("lucide-grid"),             -- grid pattern for color bg
	globe            = GetIcon("lucide-globe"),            -- 🌐 global settings
	transform        = GetIcon("lucide-move"),             -- ✥ drag/move icon
	dropdown         = GetIcon("lucide-chevron-down"),     -- ∨ dropdown arrow
	-- Window control buttons
	iconMinimize     = GetIcon("lucide-minus"),            -- — minimize
	iconMaximize     = GetIcon("lucide-maximize-2"),       -- ⤢ maximize
	iconClose        = GetIcon("lucide-x"),                -- ✕ close/exit
}
...
function MacLib:Window(Settings)
	...
	sidebar.Size = UDim2.new(0.325, 0, 1, -52)
	...
	-- ============================================================
	-- TITLE BAR: Logo+Title+Subtitle LEFT | Icon Buttons RIGHT
	-- ============================================================

	-- TitleBar container (52px tall, full sidebar width)
	...
	windowControls.Size = UDim2.new(1, 0, 0, 52)

	-- ── LEFT SECTION: Logo + Title + Subtitle ──────────────────
	local leftInfoSection = Instance.new("Frame")
	leftInfoSection.Name = "LeftInfoSection"
	leftInfoSection.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	leftInfoSection.BackgroundTransparency = 1
	leftInfoSection.BorderColor3 = Color3.fromRGB(0, 0, 0)
	leftInfoSection.BorderSizePixel = 0
	leftInfoSection.Position = UDim2.fromScale(0, 0)
	leftInfoSection.Size = UDim2.new(1, -90, 1, 0)

	local leftInfoLayout = Instance.new("UIListLayout")
	leftInfoLayout.Name = "LeftInfoLayout"
	leftInfoLayout.Padding = UDim.new(0, 10)
	leftInfoLayout.FillDirection = Enum.FillDirection.Horizontal
	leftInfoLayout.SortOrder = Enum.SortOrder.LayoutOrder
	leftInfoLayout.VerticalAlignment = Enum.VerticalAlignment.Center
	leftInfoLayout.Parent = leftInfoSection

	local leftInfoPadding = Instance.new("UIPadding")
	leftInfoPadding.Name = "LeftInfoPadding"
	leftInfoPadding.PaddingLeft = UDim.new(0, 14)
	leftInfoPadding.Parent = leftInfoSection
	...
	if Settings.Logo then
		...
		logoFrame.Size = UDim2.fromOffset(30, 30)
		...
		logoImage.Size = UDim2.fromOffset(35, 35)
		...
		logoImgCorner.CornerRadius = UDim.new(0, 4)
		logoImgCorner.Parent = logoImage

		logoFrame.Parent = leftInfoSection
	end

	-- Title + Subtitle stack
	local textStack = Instance.new("Frame")
	textStack.Name = "TextStack"
	textStack.Size = UDim2.fromScale(0, 1)
	textStack.AutomaticSize = Enum.AutomaticSize.X
	textStack.BackgroundTransparency = 1
	textStack.BorderSizePixel = 0
	textStack.LayoutOrder = 2

	local textStackLayout = Instance.new("UIListLayout")
	textStackLayout.Name = "TextStackLayout"
	textStackLayout.Padding = UDim.new(0, 1)
	textStackLayout.FillDirection = Enum.FillDirection.Vertical
	textStackLayout.SortOrder = Enum.SortOrder.LayoutOrder
	textStackLayout.VerticalAlignment = Enum.VerticalAlignment.Center
	textStackLayout.Parent = textStack
	...
	subtitle.Size = UDim2.fromScale(0, 0)
	subtitle.LayoutOrder = 2
	subtitle.Parent = textStack

	textStack.Parent = leftInfoSection
	leftInfoSection.Parent = windowControls

	-- ── CENTER SEARCH: absolute center of windowControls ──────
	local titleSearchFrame = Instance.new("Frame")
	titleSearchFrame.Name = "TitleSearch"
	titleSearchFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	titleSearchFrame.BackgroundTransparency = 0.93
	titleSearchFrame.BorderSizePixel = 0
	titleSearchFrame.AnchorPoint = Vector2.new(0.5, 0.5)
	titleSearchFrame.Position = UDim2.fromScale(0.5, 0.5)
	titleSearchFrame.Size = UDim2.new(0, 200, 0, 28)
	titleSearchFrame.ZIndex = 2

	local titleSearchCorner = Instance.new("UICorner")
	titleSearchCorner.CornerRadius = UDim.new(0, 6)
	titleSearchCorner.Parent = titleSearchFrame

	local titleSearchStroke = Instance.new("UIStroke")
	titleSearchStroke.Color = Color3.fromRGB(255, 255, 255)
	titleSearchStroke.Transparency = 0.88
	titleSearchStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
	titleSearchStroke.Parent = titleSearchFrame

	local titleSearchPadding = Instance.new("UIPadding")
	titleSearchPadding.PaddingLeft = UDim.new(0, 8)
	titleSearchPadding.PaddingRight = UDim.new(0, 8)
	titleSearchPadding.Parent = titleSearchFrame

	local titleSearchLayout = Instance.new("UIListLayout")
	titleSearchLayout.FillDirection = Enum.FillDirection.Horizontal
	titleSearchLayout.VerticalAlignment = Enum.VerticalAlignment.Center
	titleSearchLayout.Padding = UDim.new(0, 6)
	titleSearchLayout.SortOrder = Enum.SortOrder.LayoutOrder
	titleSearchLayout.Parent = titleSearchFrame

	local titleSearchIcon = Instance.new("ImageLabel")
	titleSearchIcon.Name = "SearchIcon"
	titleSearchIcon.Image = assets.searchIcon
	titleSearchIcon.ImageColor3 = Color3.fromRGB(160, 160, 175)
	titleSearchIcon.BackgroundTransparency = 1
	titleSearchIcon.Size = UDim2.fromOffset(13, 13)
	titleSearchIcon.LayoutOrder = 1
	titleSearchIcon.Parent = titleSearchFrame

	local titleSearchBox = Instance.new("TextBox")
	titleSearchBox.Name = "SearchBox"
	titleSearchBox.FontFace = Font.new(assets.interFont)
	titleSearchBox.Text = ""
	titleSearchBox.PlaceholderText = "Search..."
	titleSearchBox.PlaceholderColor3 = Color3.fromRGB(130, 130, 148)
	titleSearchBox.TextColor3 = Color3.fromRGB(220, 220, 230)
	titleSearchBox.TextSize = 12
	titleSearchBox.TextXAlignment = Enum.TextXAlignment.Left
	titleSearchBox.BackgroundTransparency = 1
	titleSearchBox.BorderSizePixel = 0
	titleSearchBox.ClearTextOnFocus = false
	titleSearchBox.Size = UDim2.new(1, -25, 1, 0)
	titleSearchBox.LayoutOrder = 2
	titleSearchBox.Parent = titleSearchFrame

	titleSearchFrame.Parent = windowControls

	-- Wire search to filter tab switchers
	local function filterTabs(query)
		query = query:lower()
		for _, tabData in pairs(tabs) do
			local name = tabData.switcherName and tabData.switcherName.Text:lower() or ""
			if tabData.switcherName then
				local parent = tabData.switcherName.Parent
				if parent then
					parent.Visible = query == "" or name:find(query, 1, true) ~= nil
				end
			end
		end
	end
	titleSearchBox:GetPropertyChangedSignal("Text"):Connect(function()
		filterTabs(titleSearchBox.Text)
	end)

	-- ── RIGHT SECTION: icon-style buttons (Exit / Minimize) ────
	-- Helper to create a hover-effect icon button
	local function makeIconButton(iconAssetId, layoutOrder, parentFrame)
		local btn = Instance.new("TextButton")
		btn.Name = "IconBtn_" .. layoutOrder
		btn.Text = ""
		...
		btn.MouseEnter:Connect(function()
			Tween(btn, TweenInfo.new(0.15, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.88 }):Play()
		end)
		btn.MouseLeave:Connect(function()
			Tween(btn, TweenInfo.new(0.15, Enum.EasingStyle.Sine), { BackgroundTransparency = 1 }):Play()
		end)
		btn.MouseButton1Down:Connect(function()
			Tween(btn, TweenInfo.new(0.08, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.78 }):Play()
		end)
	...
	controlsPadding.Name = "ControlsPadding"
	controlsPadding.PaddingRight = UDim.new(0, 8)
	controlsPadding.Parent = controls

	-- Minimize button
	local minimize = makeIconButton(assets.iconMinimize, 1, controls)
	minimize.Name = "Minimize"

	-- Maximize button
	local maximize = makeIconButton(assets.iconMaximize, 2, controls)
	maximize.Name = "Maximize"

	-- Exit/Close button
	local exit = makeIconButton(assets.iconClose, 3, controls)
	exit.Name = "Exit"

	-- Disable buttons listed in DisabledWindowControls
	local function setButtonEnabled(btn, enabled)
		btn.Active = enabled
		btn.Interactable = enabled
		btn.BackgroundTransparency = 1
		if btn:FindFirstChild("Icon") then
			btn.Icon.ImageTransparency = enabled and 0 or 0.85
		end
	end

	for _, btn in ipairs({minimize, maximize, exit}) do
		local isEnabled = true
		if Settings.DisabledWindowControls and table.find(Settings.DisabledWindowControls, btn.Name) then
			isEnabled = false
		end
		setButtonEnabled(btn, isEnabled)
	end
	...
	-- windowControls will be parented to base (full width) after base is constructed
	-- sidebar and content are offset down by 52px to sit below the titlebar

	-- Dummy information frame so sidebarGroup position still works
	...
	sidebarGroup.Position = UDim2.fromOffset(0, 0)
	sidebarGroup.Size = UDim2.fromScale(1, 1)
	...
	content.Position = UDim2.new(1, 0, 0, 52)
	content.Size = UDim2.new(0, (base.AbsoluteSize.X - sidebar.AbsoluteSize.X), 1, -52)
	...
	UserInputService.InputChanged:Connect(function(input)
		if resizingContent and input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			sidebar.Size = UDim2.new(0, newSidebarWidth, 1, -52)
			content.Size = UDim2.new(0, base.AbsoluteSize.X - newSidebarWidth, 1, -52)
		end
	end)
	...
	-- GlobalSettings opened via WindowFunctions:OpenGlobalSettings()
	function WindowFunctions:OpenGlobalSettings()
	...
	-- ── FLOATING MINIMIZER BUTTON (FU-style) ───────────────────
	-- สร้างปุ่มลอยสำหรับเปิดหน้าต่างขึ้นมาใหม่เมื่อพับ
	local minimizerHolder = Instance.new("Frame")
	minimizerHolder.Name = "MacLibMinimizer"
	minimizerHolder.Size = UDim2.fromOffset(44, 44)
	minimizerHolder.Position = UDim2.new(0, 20, 0, 20)
	minimizerHolder.BackgroundTransparency = 1
	minimizerHolder.ZIndex = 999999
	minimizerHolder.Visible = false

	local minimizerCorner = Instance.new("UICorner")
	minimizerCorner.CornerRadius = UDim.new(0, 12)
	minimizerCorner.Parent = minimizerHolder

	local minimizerBtn = Instance.new("TextButton")
	minimizerBtn.Name = "MinimizerButton"
	minimizerBtn.Size = UDim2.fromScale(1, 1)
	minimizerBtn.BackgroundColor3 = Color3.fromRGB(22, 22, 28)
	minimizerBtn.BackgroundTransparency = 0.05
	minimizerBtn.BorderSizePixel = 0
	minimizerBtn.Text = ""
	minimizerBtn.AutoButtonColor = false
	minimizerBtn.ZIndex = 999999

	local minimizerBtnCorner = Instance.new("UICorner")
	minimizerBtnCorner.CornerRadius = UDim.new(0, 12)
	minimizerBtnCorner.Parent = minimizerBtn

	local minimizerBtnStroke = Instance.new("UIStroke")
	minimizerBtnStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
	minimizerBtnStroke.Color = Color3.fromRGB(255, 255, 255)
	minimizerBtnStroke.Transparency = 0.85
	minimizerBtnStroke.Thickness = 1
	minimizerBtnStroke.Parent = minimizerBtn

	local minimizerIcon = Instance.new("ImageLabel")
	minimizerIcon.Name = "Icon"
	minimizerIcon.Image = Settings.Logo or "rbxassetid://10734897102"
	minimizerIcon.Size = UDim2.fromOffset(26, 26)
	minimizerIcon.Position = UDim2.fromScale(0.5, 0.5)
	minimizerIcon.AnchorPoint = Vector2.new(0.5, 0.5)
	minimizerIcon.BackgroundTransparency = 1
	minimizerIcon.ImageColor3 = Color3.fromRGB(255, 255, 255)
	minimizerIcon.ZIndex = 999999
	minimizerIcon.Parent = minimizerBtn

	local minimizerIconCorner = Instance.new("UICorner")
	minimizerIconCorner.CornerRadius = UDim.new(0, 4)
	minimizerIconCorner.Parent = minimizerIcon

	minimizerBtn.Parent = minimizerHolder
	minimizerHolder.Parent = macLib

	-- Drag support for the minimizer button
	local miniDragging = false
	local miniDragStart = nil
	local miniDragOffset = nil

	minimizerBtn.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			miniDragging = true
			miniDragStart = Vector2.new(input.Position.X, input.Position.Y)
			miniDragOffset = minimizerHolder.Position
			local conn
	...
	RunService.Heartbeat:Connect(function()
		if miniDragging and miniDragStart and miniDragOffset and minimizerHolder and minimizerHolder.Parent then
			local currentPos = UserInputService:GetMouseLocation()
			local delta = Vector2.new(currentPos.X, currentPos.Y) - miniDragStart
			local newX = miniDragOffset.X.Offset + delta.X
			local newY = miniDragOffset.Y.Offset + delta.Y
			local viewport = workspace.CurrentCamera.ViewportSize
			local size = minimizerHolder.AbsoluteSize
			newX = math.clamp(newX, 0, viewport.X - size.X)
			newY = math.clamp(newY, 0, viewport.Y - size.Y)
			minimizerHolder.Position = UDim2.new(0, newX, 0, newY)
		end
	end)
	...
	minimizerBtn.MouseButton1Click:Connect(function()
		...
		if not miniDragging then
			...
			minimizerHolder.Visible = not state
		end
	end)
	...
	local function ToggleMenu()
		...
		if not state then
			WindowFunctions:Notify({
				...
				Description = "Minimized. Press " .. tostring(MenuKeybind.Name) .. " or click the button to reopen.",
				Lifetime = 4
			})
		end
	end
