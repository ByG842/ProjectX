...
local StyleThemes = {
	Default = {
		-- UICorner — Fluent/Win11 ใช้มุมเล็กกว่าเดิม ดูคมและ "flat" ขึ้น
		WindowCorner      = UDim.new(0, 8),
		ElementCorner     = UDim.new(0, 4),
		SmallCorner       = UDim.new(0, 4),
		TinyCorner        = UDim.new(0, 2),
		PillCorner        = UDim.new(1, 0),
		-- UIStroke — เส้นบางแบบ hairline (1px) ตาม Fluent Design
		BorderThickness   = 1,
		-- Font (Gotham ใกล้เคียง Segoe UI Variable ที่สุดใน Roblox)
		...
		-- TextSize — สเกลตาม Fluent type ramp ให้ดูกระชับขึ้น
		TextSizeLg        = 14,
		TextSizeMd        = 13,
		TextSizeSm        = 12,
		TextSizeXs        = 11,
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		-- MAIN TITLEBAR FRAME — height 40px, บางแบบ Windows 11 caption bar
		TitleBar.Frame = New("Frame", {
			...
			New("Frame", {
				Name = "RightSection",
				Size = UDim2.new(0, 132, 1, 0), -- 3 ปุ่ม x 44px ชิดกันสนิทแบบ Win11 (ไม่มี padding ระหว่างปุ่ม)
				AnchorPoint = Vector2.new(1, 0),
				Position = UDim2.new(1, 0, 0, 0), -- ชิดมุมขวาบนของหน้าต่างพอดี
				BackgroundTransparency = 1,
			}, {
				New("UIListLayout", {
			...
			-- Bottom separator line (hairline บางแบบ Fluent divider)
			New("Frame", {
				BackgroundTransparency = 0.75,
				Size = UDim2.new(1, 0, 0, 1),
		...
		-- Win11 caption button: เหลี่ยมเต็มความสูง Titlebar ไม่มีมุมโค้ง, ปุ่ม Close เป็นสีแดงตอน hover
		local function RightBarButton(Icon, Parent, Callback, IsClose)
			local Button = { Callback = Callback or function() end }
			Button.Frame = New("TextButton", {
				Size = UDim2.new(0, 44, 1, 0), -- เต็มความสูง Titlebar เหมือนปุ่ม Win11 จริง
				BackgroundTransparency = 1,
				Parent = Parent,
				Text = "",
				ThemeTag = not IsClose and { BackgroundColor3 = "Hover" } or nil,
				BackgroundColor3 = IsClose and Color3.fromRGB(196, 43, 28) or nil, -- แดง Win11 close
			}, {
				New("ImageLabel", {
					Image = Icon,
					Size = UDim2.fromOffset(14, 14),
			...
			local Icon2 = Button.Frame:FindFirstChild("Icon")
			local Motor, SetTransparency = Creator.SpringMotor(1, Button.Frame, "BackgroundTransparency")

			AddSignal(Button.Frame.MouseEnter, function()
				SetTransparency(IsClose and 0 or 0.88)
				if IsClose then Icon2.ImageColor3 = Color3.fromRGB(255, 255, 255) end
			end)
			AddSignal(Button.Frame.MouseLeave, function()
				SetTransparency(1, true)
				if IsClose then Creator.OverrideTag(Icon2, { ImageColor3 = "SubText" }) end
			end)
			AddSignal(Button.Frame.MouseButton1Down, function()
				SetTransparency(IsClose and 0.15 or 0.80)
			end)
			AddSignal(Button.Frame.MouseButton1Up, function()
				SetTransparency(IsClose and 0 or 0.88)
			end)
