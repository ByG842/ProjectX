...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:SelectSubTab(SubTabIndex)
			...
			if PreviousSubTab > 0 and PreviousSubTab ~= SubTabIndex and self.SubTabs[PreviousSubTab] and self.SubTabs[SubTabIndex] then
				...
				-- ✨ Stagger children ของ SubTab
				if self.SubTabs[SubTabIndex] and self.SubTabs[SubTabIndex].ContainerFrame then
					PlayStaggerAnimation(self.SubTabs[SubTabIndex].ContainerFrame, Direction)
				end
	...
	-- ✨ Stagger: เนื้อหาภายใน tab ลอยเข้ามาทีละอัน จากซ้าย/ขวา
	local function PlayStaggerAnimation(ContainerFrame, Direction)
		...
		-- Sort by LayoutOrder so stagger matches visual order
		table.sort(children, function(a, b)
			return a.LayoutOrder < b.LayoutOrder
		end)

		local SLIDE_X    = (Direction ~= 0) and (Direction * 28) or 18
		local DELAY_STEP = 0.038   -- วินาทีระหว่างแต่ละ element

		for i, child in ipairs(children) do
			local origPos   = child.Position
			-- ✅ บันทึก BackgroundTransparency เดิมของแต่ละ element ก่อน
			local origTransp = child.BackgroundTransparency

			-- ตั้งต้น: ดัน element ออกไปด้านข้าง
			child.Position = UDim2.new(
				origPos.X.Scale,
				origPos.X.Offset + SLIDE_X,
				origPos.Y.Scale,
				origPos.Y.Offset
			)
	...
	function TabModule:SelectTab(Tab)
		...
		if PreviousTab > 0 and PreviousTab ~= Tab and TabModule.Tabs[PreviousTab] and TabModule.Tabs[Tab] then
			...
			-- ✨ Stagger: children ลอยเข้ามาทีละอัน
			if TabModule.Tabs[Tab] and TabModule.Tabs[Tab].ContainerFrame then
				PlayStaggerAnimation(TabModule.Tabs[Tab].ContainerFrame, Direction)
			end
