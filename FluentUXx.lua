...
Library.CreateWindow = function(self, Config)
	...
	-- ถ้าไม่ใส่ layout หน้าต่างจะอยู่ตรงกลางปกติ
	if Config.Layout then
		...
		-- ตำแหน่งสำหรับแต่ละ layout
		local layoutPositions = {
			top    = UDim2.fromOffset(math.floor((vp.X - winW) / 2), 0),
			bottom = UDim2.fromOffset(math.floor((vp.X - winW) / 2), math.floor(vp.Y - winH)),
			left   = UDim2.fromOffset(0, math.floor((vp.Y - winH) / 2)),
			right  = UDim2.fromOffset(math.floor(vp.X - winW), math.floor((vp.Y - winH) / 2)),
		}

		local targetPos = layoutPositions[layout]
		if targetPos then
			Window.Position = targetPos
			Window._layoutMode = layout
			if Window.Root then
				-- animate slide-in from off-screen
				local startPositions = {
					top    = UDim2.fromOffset(targetPos.X.Offset, -winH - 20),
					bottom = UDim2.fromOffset(targetPos.X.Offset, vp.Y + 20),
					left   = UDim2.fromOffset(-winW - 20, targetPos.Y.Offset),
					right  = UDim2.fromOffset(vp.X + 20,  targetPos.Y.Offset),
				}
				Window.Root.Position = startPositions[layout]
				TweenService:Create(Window.Root, TweenInfo.new(0.55, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
					Position = targetPos
				}):Play()
			end

			-- ── Docked collapse / expand button ────────────────────
			-- สร้างปุ่มพับ/ขยายที่ขอบหน้าต่างตาม layout
			task.defer(function()
				if not (Window.Root and Window.Root.Parent) then return end
				local gui = Window.Root.Parent

				-- กำหนดทิศทางปุ่มพับ
				local collapseDir = {
					top    = { anchor = Vector2.new(0.5, 0), pos = UDim2.new(0.5, 0, 1, 0),    arrow_open = "▲", arrow_close = "▼" },
					bottom = { anchor = Vector2.new(0.5, 1), pos = UDim2.new(0.5, 0, 0, 0),    arrow_open = "▼", arrow_close = "▲" },
					left   = { anchor = Vector2.new(0, 0.5), pos = UDim2.new(1, 0, 0.5, 0),    arrow_open = "◀", arrow_close = "▶" },
					right  = { anchor = Vector2.new(1, 0.5), pos = UDim2.new(0, 0, 0.5, 0),    arrow_open = "▶", arrow_close = "◀" },
				}

				local dir = collapseDir[layout]
				if not dir then return end

				local btnSize = UDim2.fromOffset(layout == "top" or layout == "bottom" and 60 or 20, layout == "left" or layout == "right" and 60 or 20)
				if layout == "top" or layout == "bottom" then
					btnSize = UDim2.fromOffset(60, 20)
				else
					btnSize = UDim2.fromOffset(20, 60)
				end

				local CollapseBtn = New("TextButton", {
					Name        = "LayoutCollapseBtn",
					Text        = dir.arrow_open,
					Size        = btnSize,
					AnchorPoint = dir.anchor,
					Position    = dir.pos,
					...
					TextSize    = 13,
					Parent      = Window.Root,
					ThemeTag    = { BackgroundColor3 = "AcrylicMain", TextColor3 = "SubText" },
				}, {
					New("UICorner", {
						CornerRadius = layout == "top" or layout == "bottom"
							and UDim.new(0, 8)
							or  UDim.new(0, 8),
					}),
					New("UIStroke", {
				...
				Creator.AddSignal(CollapseBtn.MouseButton1Click, function()
					collapsed = not collapsed
					CollapseBtn.Text = collapsed and dir.arrow_close or dir.arrow_open
					TweenService:Create(Window.Root, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
						Position = collapsed and hiddenPos[layout] or targetPos
					}):Play()
				end)
			end)
		end
