...
-- ============================================================
-- ✨ ZzenV1 UI/UX REWORK SYSTEM
-- One motion language for every element:
-- entrance + hover lift + press feedback + accent focus + safe fallbacks
-- ============================================================
local UIFX = {}
UIFX.__index = UIFX

local function motionEnabled(kind)
	local motion = Library.Motion
	if not motion or motion.Enabled == false then return false end
	if kind and motion[kind] == false then return false end
	return true
end

local function motionScale(value)
	local intensity = (Library.Motion and tonumber(Library.Motion.Intensity)) or 1
	return math.clamp(intensity, 0, 2) * value
end

function UIFX:Tween(obj, info, props)
...
function UIFX:AddScale(obj)
	if not obj then return nil end
	local scale = obj:FindFirstChild("ZzenFXScale")
	if not scale then
		scale = Instance.new("UIScale")
		scale.Name = "ZzenFXScale"
		scale.Scale = 1
		scale.Parent = obj
	end
	return scale
...
function UIFX:AttachPress(frame, options)
	if not frame or not frame:IsA("GuiObject") then return end
	options = options or {}
	local scale = self:AddScale(frame)
	local hoverScale = options.HoverScale or 1.012
	local pressScale = options.PressScale or 0.985
	local normal = 1
	local hovering = false

	-- soft accent halo: almost invisible at rest, becomes a clean focus ring on hover
	local focusStroke = frame:FindFirstChild("ZzenFXFocusStroke")
	if not focusStroke then
		focusStroke = Instance.new("UIStroke")
		focusStroke.Name = "ZzenFXFocusStroke"
		focusStroke.Thickness = 1
		focusStroke.Transparency = 1
		focusStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
		focusStroke.Color = Creator.GetThemeProperty("Accent")
		focusStroke.Parent = frame
		-- Register without forcing a full theme refresh for every element creation.
		Creator.Registry[focusStroke] = { Object = focusStroke, Properties = { Color = "Accent" } }
	end

	if motionEnabled("Hover") then
		Creator.AddSignal(frame.MouseEnter, function()
			hovering = true
			if motionEnabled("Hover") then
				local tw = self:Tween(scale, TweenInfo.new(0.18, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Scale = hoverScale})
				if tw then tw:Play() end
				local stroke = self:Tween(focusStroke, TweenInfo.new(0.18, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Transparency = 0.52})
				if stroke then stroke:Play() end
			end
		end)
		Creator.AddSignal(frame.MouseLeave, function()
			hovering = false
			local tw = self:Tween(scale, TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Scale = normal})
			if tw then tw:Play() end
			local stroke = self:Tween(focusStroke, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Transparency = 1})
			if stroke then stroke:Play() end
		end)
	end

	if motionEnabled("Press") then
		Creator.AddSignal(frame.MouseButton1Down, function()
			local tw = self:Tween(scale, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = pressScale})
			if tw then tw:Play() end
			local stroke = self:Tween(focusStroke, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Transparency = 0.3})
			if stroke then stroke:Play() end
		end)
		Creator.AddSignal(frame.MouseButton1Up, function()
			local target = hovering and hoverScale or normal
			local tw = self:Tween(scale, TweenInfo.new(0.16, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = target})
			if tw then tw:Play() end
			local stroke = self:Tween(focusStroke, TweenInfo.new(0.16, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Transparency = hovering and 0.52 or 1})
			if stroke then stroke:Play() end
		end)
	end
end

function UIFX:Entrance(frame, delayTime, distance)
	if not frame or not frame.Parent or not motionEnabled("Entrance") then return end
	if Library.Motion and Library.Motion._entranced then return end
	local scale = self:AddScale(frame)
	scale.Scale = 0.965
	local original = frame.Position
	local offset = math.floor((distance or 10) * motionScale(1))
	frame.Position = UDim2.new(original.X.Scale, original.X.Offset, original.Y.Scale, original.Y.Offset + offset)
	task.delay(delayTime or 0, function()
		if not frame.Parent then return end
		local tw = self:Tween(scale, TweenInfo.new(0.42, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Scale = 1})
		if tw then tw:Play() end
		local pos = self:Tween(frame, TweenInfo.new(0.48, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Position = original})
		if pos then pos:Play() end
	end)
end

function UIFX:Pulse(obj, strength)
	if not obj or not motionEnabled("Micro") then return end
	local scale = self:AddScale(obj)
	strength = strength or 0.94
	scale.Scale = strength
	local tw = self:Tween(scale, TweenInfo.new(0.24, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1})
	if tw then tw:Play() end
end

function UIFX:RotatePress(icon, degrees)
	if not icon or not motionEnabled("Micro") then return end
	degrees = degrees or 10
	icon.Rotation = 0
	local tw1 = self:Tween(icon, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = degrees})
	if tw1 then
		tw1:Play()
		tw1.Completed:Connect(function()
			if icon.Parent then
				local tw2 = self:Tween(icon, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Rotation = 0})
...
function UIFX:SetMotion(enabled, intensity)
	Library.Motion.Enabled = enabled ~= false
	if intensity ~= nil then
		Library.Motion.Intensity = math.clamp(tonumber(intensity) or 1, 0, 2)
	end
end

function UIFX:ResetMotion()
	Library.Motion.Enabled = true
	Library.Motion.Intensity = 1
end
...
Components.Element = (function()
	...
	return function(Title, Desc, Parent, Hover, Options)
		...
		-- ✨ universal tactile motion for every control that is built on Components.Element
		UIFX:AttachPress(Element.Frame, {
			HoverScale = Hover and 1.012 or 1.006,
			PressScale = Hover and 0.985 or 0.992,
		})
		UIFX:Entrance(Element.Frame, 0.015 + math.min((Element.Frame.LayoutOrder or 1) * 0.004, 0.12), 5)
...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		UIFX:AttachPress(Tab.Frame, { HoverScale = 1.018, PressScale = 0.98 })
		...
		function Tab:AddSubTab(Title, Icon)
			...
			UIFX:AttachPress(SubTabButton, { HoverScale = 1.015, PressScale = 0.985 })
	...
	function TabModule:SelectTab(Tab)
		...
		local selectedTabIcon = TabModule.Tabs[Tab].Frame:FindFirstChildWhichIsA("ImageLabel", true)
		if selectedTabIcon then
			UIFX:RotatePress(selectedTabIcon, 8)
		end
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		local function BarButton(Icon, Pos, Parent, Callback)
			...
			UIFX:AttachPress(Button.Frame, { HoverScale = 1.06, PressScale = 0.9 })
...
Components.Window = (function()
	...
	return function(Config)
		...
		do
			local ENTRANCE_DURATION = 0.72
			local ENTRANCE_EASING_STYLE = Enum.EasingStyle.Quint -- smooth, quick, game-like
...
Library.CreateWindow = function(self, Config)
	...
	-- Motion can be controlled per window without touching individual elements.
	Library.Motion = Library.Motion or {}
	Library.Motion.Enabled = Config.Motion ~= false
	Library.Motion.Intensity = math.clamp(tonumber(Config.MotionIntensity) or Library.Motion.Intensity or 1, 0, 2)
	Library.Motion.Entrance = Config.MotionEntrance ~= false
	Library.Motion.Hover = Config.MotionHover ~= false
	Library.Motion.Press = Config.MotionPress ~= false
	Library.Motion.Micro = Config.MotionMicro ~= false
...
-- ✨ Global motion controls — useful for scripts that want to change the feel at runtime
function Library:SetMotionEnabled(Value)
	if Library.UIFX then
		Library.UIFX:SetMotion(Value, Library.Motion and Library.Motion.Intensity or 1)
	end
end

function Library:SetMotionIntensity(Value)
	if Library.UIFX then
		Library.UIFX:SetMotion(Library.Motion and Library.Motion.Enabled ~= false, Value)
	end
end

function Library:MotionPulse(Object, Strength)
	if Library.UIFX then
		Library.UIFX:Pulse(Object, Strength)
	end
end
