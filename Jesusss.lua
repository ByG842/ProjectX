	...
	return function(Config)
			...
			New("Frame", {
				...
				New("Frame", {
					...
					(Config.SubTitle or Config.Discord) and New("Frame", {
						...
						-- [ จุดที่เพิ่ม ] Discord: ข้อความสีฟ้าคงที่ต่อท้าย SubTitle กดเพื่อ copy ลิงก์
						-- (Config.Discord = ลิงก์จริงที่จะ copy, Config.DiscordLabel = ข้อความที่โชว์ ปรับได้ default "ลิ้งดิส")
						Config.Discord and New("TextButton", {
							Name = "DiscordLabel",
							Text = Config.DiscordLabel or "ลิ้งดิส",
							AutoButtonColor = false,
							FontFace = GetStyleProperty("FontRegular"),
							TextSize = GetStyleProperty("TextSizeXs"),
							TextXAlignment = "Left",
						...
						-- [ จุดที่เพิ่ม ] ไอคอน copy เล็กๆ ต่อท้ายข้อความ Discord กดแล้ว copy ลิงก์เหมือนกัน
						Config.Discord and New("ImageButton", {
							Name = "DiscordCopyIcon",
							Image = Library:GetIcon("lucide-copy"),
		...
		if Config.Discord then
			...
			local function CopyDiscordLink()
				...
				if discordIcon then
					discordIcon.Image = Library:GetIcon("lucide-clipboard-check")
					task.delay(1.2, function()
						if discordIcon and discordIcon.Parent then
							discordIcon.Image = Library:GetIcon("lucide-copy")
						end
					end)
				end
			end
