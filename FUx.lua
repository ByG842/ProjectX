...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:SelectSubTab(SubTabIndex)
			...
			-- ✨ Match main tab: subtle 5% drift
			local SlideDistance = math.min(ContainerSize * 0.05, 22)
			...
			if PreviousSubTab > 0 and PreviousSubTab ~= SubTabIndex and self.SubTabs[PreviousSubTab] and self.SubTabs[SubTabIndex] then
				...
				-- ✨ Overdamped (dampingRatio=1) = zero bounce, crisp minimal feel
				pcall(function()
					OldSubTab.XMotor:setGoal(Spring(-Direction * SlideDistance * 0.6, { frequency = 9, dampingRatio = 1 }))
					OldSubTab.TransparencyMotor:setGoal(Spring(1, { frequency = 9, dampingRatio = 1 }))
				end)

				pcall(function()
					NewSubTab.XMotor:setGoal(Spring(0, { frequency = 9, dampingRatio = 1 }))
					NewSubTab.TransparencyMotor:setGoal(Spring(0, { frequency = 9, dampingRatio = 1 }))
				end)
	...
	function TabModule:SelectTab(Tab)
		...
		-- ✨ Minimal slide: subtle 5% shift only, feels premium not bouncy
		local SlideDistance = math.min(ContainerSize * 0.05, 22)
		...
		-- ✨ Snappier selector spring
		Window.SelectorPosMotor:setGoal(Spring(TabModule:GetCurrentTabPos(), { frequency = 10, dampingRatio = 0.9 }))
		...
		if PreviousTab > 0 and PreviousTab ~= Tab and TabModule.Tabs[PreviousTab] and TabModule.Tabs[Tab] then
			...
			-- ✨ Snap old container to resting state instantly before animating out
			OldContainer.Visible = true
			OldContainer.Position = UDim2.fromOffset(0, 0)
			OldContainer.GroupTransparency = 0
			pcall(function()
			...
			-- ✨ New page starts very close (subtle drift), fully transparent
			NewContainer.Visible = true
			NewContainer.Position = UDim2.fromOffset(Direction * SlideDistance, 0)
			NewContainer.GroupTransparency = 1
			pcall(function()
			...
			-- ✨ Old: fade out fast + barely drift away — feels light, not heavy
			pcall(function()
				OldTab.ContainerXMotor:setGoal(Spring(-Direction * SlideDistance * 0.6, { frequency = 9, dampingRatio = 1 }))
				OldTab.ContainerTransparencyMotor:setGoal(Spring(1, { frequency = 9, dampingRatio = 1 }))
			end)

			-- ✨ New: slides in with overdamped spring (no bounce), crisp & clean
			pcall(function()
				NewTab.ContainerXMotor:setGoal(Spring(0, { frequency = 9, dampingRatio = 1 }))
				NewTab.ContainerTransparencyMotor:setGoal(Spring(0, { frequency = 9, dampingRatio = 1 }))
			end)
