...
Components.Notification = (function()
	...
	function Notification:New(Config)
		...
		if Config.Type == "default" then
			-- ไม่ใช่ type สี fixed → ใช้สี Accent ของธีมปัจจุบันเสมอ
			accentColor = Creator.GetThemeProperty("Accent") or TypeColors.default
		else
			accentColor = TypeColors[Config.Type] or Creator.GetThemeProperty("Accent") or TypeColors.default
		end
		...
		local isMinecraftNotif = (Library.Theme == "Minecraft")
		local NotifTextureLayer = isMinecraftNotif and New("ImageLabel", {
		...
		if isMinecraftNotif and NewNotification.AcrylicPaint and NewNotification.AcrylicPaint.Frame then
			-- ธีม Minecraft: เอาเงานุ่มๆ ทรงโค้งออก จะได้ดูเหลี่ยมจริงๆ ไม่มีอะไรมาทำให้ดูมนอีก
			local shadow = NewNotification.AcrylicPaint.Frame:FindFirstChild("ShadowLayer")
			if shadow then shadow.ImageTransparency = 1 end
		end

		local borderColorEnd = accentColor:Lerp(Color3.new(1, 1, 1), 0.4)
		local BorderStroke = New("UIStroke", {
			...
			New("UIGradient", {
				Color = ColorSequence.new(accentColor, borderColorEnd),
				Rotation = 45,
			}),
		})
