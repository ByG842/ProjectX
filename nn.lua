...
Components.Section = (function()
	...
	return function(Title, Parent, Icon)
		...
		-- header row
		-- [ จุดที่เพิ่ม ] เปิดพื้นหลัง header ให้ไล่สี (gradient) แทนที่จะโปร่งใส 100% เหมือนเดิม —
		-- ใช้สี Accent ของธีมปัจจุบันเป็นฐาน (ไม่ hardcode สีใหม่ เพื่อให้เข้ากับทุกธีมอัตโนมัติ)
		-- แล้วไล่ความทึบจากเข้มสุดฝั่งซ้าย (ใกล้ AccentLine/ไอคอน) ไปจางจนใสสุดฝั่งขวา
		...
		local CardFrame = New("Frame", {
			...
			-- [ จุดที่แก้ ] Section ใช้มุมโค้งของตัวเอง (4px) แยกจาก ElementCorner ของปุ่ม/element อื่นๆ
			-- (ปกติ ElementCorner = 8px ทำให้ดูมนเกินไปสำหรับการ์ดใหญ่) — ล็อกค่าตรงนี้ ไม่ผูกกับ
			-- style ธีม (เช่น Minecraft ที่ตั้ง ElementCorner = 0px) เพราะ Section อยากให้เหลี่ยมมนนิดๆ
			-- เสมอ ไม่ใช่เหลี่ยมจัดเป๊ะไปตามธีม
...
Elements.AddTab = function(self, TabTitle, TabIcon)
	...
	if not self.SectionTabStrip then
		...
		-- ✅ จุดที่แก้บั๊ก: กล่องสีเทาลอยผิดตำแหน่งข้าง Banana ที่เจอ เกิดจากปุ่มยังไม่ resolve
		-- AutomaticSize (AbsoluteSize/AbsolutePosition ยังเป็น 0) ตอนที่คำนวณตำแหน่งแถบไฮไลต์ครั้งแรก
		-- แถบเลยไปโผล่ที่พิกัด (0,0) ของ strip แทนตำแหน่งจริงของปุ่ม — ซ่อนไว้ก่อนด้วย Visible = false
		-- จนกว่าจะคำนวณตำแหน่งจริงสำเร็จค่อยโชว์ กันไม่ให้เห็น "กล่องลอย" ผิดที่แบบเดิมอีก
		self.SectionTabIndicator = Creator.New("Frame", {
			Size = UDim2.fromOffset(0, 30),
		...
		self._MoveSectionTabIndicator = function(Button, Instant)
			if not Button then return end
			local strip = self.SectionTabStrip
			local indicator = self.SectionTabIndicator

			local function Apply()
				if Button.AbsoluteSize.X <= 0 then return false end
				local goalPos = UDim2.fromOffset(
					Button.AbsolutePosition.X - strip.AbsolutePosition.X,
					Button.AbsolutePosition.Y - strip.AbsolutePosition.Y
				)
				local goalSize = UDim2.fromOffset(Button.AbsoluteSize.X, Button.AbsoluteSize.Y)
			...
			-- รอ Heartbeat ทีละเฟรมจนกว่าปุ่มจะมีขนาดจริง (สูงสุด ~10 เฟรม กันลูปค้างถ้า Button ถูกทำลายไปก่อน)
			-- แทนที่จะเชื่อ task.defer ครั้งเดียวแบบเดิมซึ่งบางเครื่อง/เฟรมแรกยังไม่ทันคำนวณ layout เสร็จ
			if not Apply() then
				task.spawn(function()
					for _ = 1, 10 do
		...
		-- ปรับตำแหน่งแถบไฮไลต์ใหม่ทันทีถ้า strip เปลี่ยนขนาด (เช่น resize หน้าต่าง, สลับ TabWidth)
		-- ไม่งั้นแถบจะค้างตำแหน่งเดิมไม่ตรงกับปุ่มที่ขยับไปแล้ว
		Creator.AddSignal(self.SectionTabStrip:GetPropertyChangedSignal("AbsoluteSize"), function()
			local activeTab = self.SectionTabs[self.SelectedSectionTab]
			if activeTab then
				self._MoveSectionTabIndicator(activeTab.Button, true)
			end
		end)
	end
	...
	UpdateSectionTabAppearance(true)
