...
ElementsTable.LiveLabel = (function()
	...
	-- พื้นหลังแต่ละ type (โปร่งแสง)
	local TypeBg = {
		default = Color3.fromRGB(165, 168, 185),
		info    = Color3.fromRGB(30,  90,  140),
		success = Color3.fromRGB(20,  90,   55),
		warning = Color3.fromRGB(110,  75,   10),
		error   = Color3.fromRGB(110,  25,   25),
	}
	...
	function Element:New(Idx, Config)
		...
		-- ── ต้องใช้ frame ขยายได้ (AutomaticSize Y) ──────────────
		local LLFrame = Components.Element(Config.Title or "", Config.Description, self.Container, false, Config)
		LLFrame.Frame.Size          = UDim2.new(1, 0, 0, 0)
		LLFrame.Frame.AutomaticSize = Enum.AutomaticSize.Y
		...
		-- วางใต้ Title (LayoutOrder 10 เพื่อให้อยู่ต่ำกว่า title row)
		local BadgeWrap = New("Frame", {
			Size                   = UDim2.new(1, -20, 0, 0),
			AutomaticSize          = Enum.AutomaticSize.Y,
			Position               = UDim2.new(0, 10, 0, 0),
			BackgroundTransparency = 1,
			LayoutOrder            = 10,
			Parent                 = LLFrame.LabelHolder,  -- ใส่ใน LabelHolder เพื่อ flow ต่อจาก title
		}, {
			New("UIPadding", {
		...
		-- ข้อความจริงๆ — รองรับ wrap + หลายบรรทัด
		local ValueLabel = New("TextLabel", {
			...
			TextXAlignment   = Enum.TextXAlignment.Left,
			TextYAlignment   = Enum.TextYAlignment.Top,
			TextWrapped      = true,          -- ✅ wrap แล้ว
			RichText         = true,
			AutomaticSize    = Enum.AutomaticSize.Y,  -- ✅ ขยายตามเนื้อหา
			BackgroundTransparency = 1,
			Size             = UDim2.new(1, 0, 0, 0),
			Parent           = Pill,
		})

		-- ── Stroke บน Pill ต้องเข้าถึงได้สำหรับ SetType ──────────
		...
		function LL:SetType(t)
			...
			TweenService:Create(ValueLabel, TweenInfo.new(0.15), {
				TextColor3 = col,
			}):Play()
