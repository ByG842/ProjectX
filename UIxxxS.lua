...
Components.Window = (function()
	...
	return function(Config)
		...
		-- 🎬 Intro animation: ตอนหน้าต่างถูกสร้างขึ้นครั้งแรก (ตอนรันสคริปต์) ให้ชิ้นส่วนหลักของหน้าต่าง
		-- (แถบหัวข้อ / แถบแท็บด้านซ้าย / พื้นที่เนื้อหา) ค่อยๆ เลื่อนเข้ามาประกอบกันเป็นหน้าต่าง แบบช้าๆ นุ่มๆ
		-- พร้อมกับทั้งหน้าต่าง fade + ขยายเข้ามาเบาๆ (ใช้กลไก RootScale/RootWrapper เดิมที่มีอยู่แล้วสำหรับตอน minimize)
		do
			local IntroDuration = 0.55
			local IntroStyle = Enum.EasingStyle.Quint
			local IntroDirection = Enum.EasingDirection.Out
			local IntroSpringParams = { frequency = 3, dampingRatio = 1 } -- dampingRatio 1 = ไม่กระเด้ง ไหลลื่นๆ ช้ากว่า SmoothSpringParams ของ minimize เพราะเกิดแค่ครั้งเดียวตอนเปิด

			-- จำตำแหน่งจริง (ปลายทาง) ของแต่ละชิ้นส่วนไว้ก่อน แล้วค่อยขยับออกไปนอกกรอบเป็นจุดเริ่มต้นของอนิเมชัน
			local TitleBarTarget = Window.TitleBar.Frame.Position
			local TabFrameTarget = TabFrame.Position
			local ContainerTarget = Window.ContainerCanvas.Position
			local TabDisplayTarget = Window.TabDisplay.Position

			Window.TitleBar.Frame.Position = TitleBarTarget - UDim2.fromOffset(0, 46) -- แถบหัวข้อ: เลื่อนลงมาจากด้านบน
			TabFrame.Position = TabFrameTarget - UDim2.fromOffset(Window.TabWidth + 60, 0) -- แถบแท็บ/sidebar: เลื่อนเข้ามาจากด้านซ้าย
			Window.ContainerCanvas.Position = ContainerTarget + UDim2.fromOffset(70, 0) -- พื้นที่เนื้อหา: เลื่อนเข้ามาจากด้านขวา
			Window.TabDisplay.Position = TabDisplayTarget + UDim2.fromOffset(70, 0)

			-- กันชิ้นส่วนที่ยังเลื่อนไม่ถึงที่โผล่ล้นออกไปนอกกรอบหน้าต่างระหว่างอนิเมชัน แล้วคืนค่าเดิมตอนจบ
			local RootClipsBefore = Window.Root.ClipsDescendants
			Window.Root.ClipsDescendants = true

			-- ทั้งหน้าต่าง fade เข้า + ขยายเข้ามาเบาๆ พร้อมๆ กับชิ้นส่วนที่กำลังประกอบตัว
			Window.RootScale.Scale = 0.94
			Window.RootWrapper.GroupTransparency = 1
			if Window.RootScaleMotor then
				Window.RootScaleMotor:setGoal(Spring(1, IntroSpringParams))
			end
			if Window.RootFadeMotor then
				Window.RootFadeMotor:setGoal(Spring(0, IntroSpringParams))
			end

			TweenService:Create(Window.TitleBar.Frame, TweenInfo.new(IntroDuration, IntroStyle, IntroDirection), {
				Position = TitleBarTarget,
			}):Play()

			task.delay(0.08, function()
				if TabFrame and TabFrame.Parent then
					TweenService:Create(TabFrame, TweenInfo.new(IntroDuration, IntroStyle, IntroDirection), {
						Position = TabFrameTarget,
					}):Play()
				end
			end)

			task.delay(0.16, function()
				if Window.ContainerCanvas and Window.ContainerCanvas.Parent then
					TweenService:Create(Window.ContainerCanvas, TweenInfo.new(IntroDuration, IntroStyle, IntroDirection), {
						Position = ContainerTarget,
					}):Play()
				end
				if Window.TabDisplay and Window.TabDisplay.Parent then
					TweenService:Create(Window.TabDisplay, TweenInfo.new(IntroDuration, IntroStyle, IntroDirection), {
						Position = TabDisplayTarget,
					}):Play()
				end
			end)

			task.delay(0.16 + IntroDuration + 0.05, function()
				if Window.Root then
					Window.Root.ClipsDescendants = RootClipsBefore
				end
			end)
		end
