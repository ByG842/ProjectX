...
Components.Section = (function()
	...
	return function(Title, Parent, Icon, Description)
		local Section = {}
		...
		local HEADER_H    = hasDesc and 46 or 28  -- ความสูง header row (เพิ่มถ้ามี description)
		...
		local SectionHeader = New("Frame", {
			...
			AccentLine,
			-- Title row (horizontal: icon + text)
			New("Frame", {
			...
			-- Description label (ถ้ามี)
			hasDesc and New("TextLabel", {
				RichText = true,
				Text = Description,
...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent, Description)
		...
		function Tab:AddSubTab(Title, Icon)
			...
			if not self.SubTabHolder then
				...
				-- ข้อ 11: Divider คั่นระหว่าง SubTab row กับ content
				local SubTabDivider = New("Frame", {
			...
			function SubTab:AddSection(SectionTitle, SectionIcon, SectionDescription)
				...
				local SectionFrame = Components.Section(SectionTitle, SubTab.Container, Icon, SectionDescription)
		...
		function Tab:AddSection(SectionTitle, SectionIcon, SectionDescription)
			if self.SelectedSubTab > 0 and self.SubTabs[self.SelectedSubTab] then
				return self.SubTabs[self.SelectedSubTab]:AddSection(SectionTitle, SectionIcon, SectionDescription)
			end
			...
			local SectionFrame = Components.Section(SectionTitle, Tab.Container, Icon, SectionDescription)
			Section.Container = SectionFrame.Container
	...
	function TabModule:SelectTab(Tab)
		...
		-- อัปเดต description และ divider position
		do
			local desc = TabModule.Tabs[Tab].Description or ""
			local hasDesc = desc ~= ""
			if Window.TabDescDisplay then
				Window.TabDescDisplay.Text = desc
				Window.TabDescDisplay.Visible = hasDesc
			end
			-- ขยับ Divider ตามว่ามี description หรือไม่
			local baseY = 91  -- ตำแหน่ง Y ใต้ title (32px title + 56 offset + 3 gap)
			local divY = hasDesc and (baseY + 20) or (baseY + 3)
			if Window.TabHeaderDivider then
				Window.TabHeaderDivider.Position = UDim2.fromOffset(Window.TabWidth + 26, divY)
			end
			-- ปรับ ContainerCanvas ให้เลื่อนตาม
			if Window.ContainerCanvas then
				local contentY = divY + 8
				Window.ContainerCanvas.Position = UDim2.fromOffset(Window.TabWidth + 26, contentY)
				Window.ContainerCanvas.Size = UDim2.new(1, -Window.TabWidth - 32, 1, -(contentY - 0))
			end
		end
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		-- ── MAIN TITLEBAR FRAME — height 52px ────────────────────
		TitleBar.Frame = New("Frame", {
			...
			New("Frame", {
				Name = "LeftSection",
				Size = UDim2.new(1, -320, 1, 0),  -- เว้นช่องขวาให้ search+buttons
			...
			-- ── RIGHT SECTION (SearchBox + Min/Max/Close) ──────────
			New("Frame", {
				Name = "RightSection",
			...
			-- ── Bottom divider — ชัดขึ้น ───────────────────────────
			New("Frame", {
				BackgroundTransparency = 0.25,  -- เพิ่มความชัดจาก 0.55 → 0.25
				Size = UDim2.new(1, 0, 0, 2),   -- หนาขึ้นจาก 1px → 2px
		...
		-- ── Build RightSection: SearchBox → Min → Max → Close ─────
		local rightSection = TitleBar.Frame:FindFirstChild("RightSection")

		-- Search Box ใน TitleBar
		local TitleSearchFrame = New("Frame", {
			Name = "TitleSearchFrame",
		...
		-- เชื่อม TitleSearchInput กับ Window search ถ้า ShowSearch เปิดอยู่
		local titleSearchInput = TitleSearchFrame:FindFirstChild("TitleSearchInput")
		if titleSearchInput then
			Creator.AddSignal(titleSearchInput:GetPropertyChangedSignal("Text"), function()
				local win = Config.Window or Library.Window
				if win and win.UpdateElementVisibility then
					win.UpdateElementVisibility(titleSearchInput.Text or "")
				end
			end)
			Creator.AddSignal(UserInputService.InputBegan, function(input, gp)
				if gp then return end
				if input.KeyCode == Enum.KeyCode.Escape and titleSearchInput:IsFocused() then
					titleSearchInput.Text = ""
					titleSearchInput:ReleaseFocus()
				end
			end)
		end
		...
		TitleBar.CloseButton.Frame.LayoutOrder = 4

		TitleBar.SearchFrame = TitleSearchFrame
		TitleBar.SearchInput = titleSearchInput
...
Components.Window = (function()
	...
	return function(Config)
		...
		local SearchFrame = New("Frame", {
			...
			BackgroundTransparency = 0.7,
			ZIndex = 10,
			Visible = false,  -- ซ่อนแล้ว เพราะ Search ย้ายไปอยู่ใน TitleBar แล้ว
		...
		local TabFrame = New("Frame", {
			Size = UDim2.new(0, Window.TabWidth, 1, -31),
		...
		-- ── Tab Header: Title + Description + Divider ────────────
		-- Title ของ Tab ที่แสดงตรงบริเวณ Content
		...
		-- Description label ใต้ Tab Title (ซ่อนถ้าไม่มี)
		Window.TabDescDisplay = New("TextLabel", {
			RichText = true,
			Text = "",
			...
			AutomaticSize = Enum.AutomaticSize.Y,
			Position = UDim2.fromOffset(Window.TabWidth + 26, 91),
			BackgroundTransparency = 1,
			Visible = false,
			ThemeTag = {
				TextColor3 = "SubText",
		...
		-- Divider ใต้ Tab Header (กั้นระหว่าง header กับ content)
		Window.TabHeaderDivider = New("Frame", {
			BackgroundTransparency = 0.55,
			Size = UDim2.new(1, -(Window.TabWidth + 42), 0, 1),
		...
		Window.ContainerCanvas = New("Frame", {
			Size = UDim2.new(1, -Window.TabWidth - 32, 1, -110),
			Position = UDim2.fromOffset(Window.TabWidth + 26, 100),
			BackgroundTransparency = 1,
			ClipsDescendants = true,
		}, {
		...
		table.insert(rootChildren, Window.TabDescDisplay)
		table.insert(rootChildren, Window.TabHeaderDivider)
		...
		-- เชื่อม TitleBar SearchInput กับ Window.UpdateElementVisibility
		if Window.TitleBar and Window.TitleBar.SearchInput then
			local si = Window.TitleBar.SearchInput
			Creator.AddSignal(si:GetPropertyChangedSignal("Text"), function()
				UpdateElementVisibility(si.Text or "")
			end)
			Creator.AddSignal(UserInputService.InputBegan, function(input, gp)
				if gp then return end
				if input.KeyCode == Enum.KeyCode.Escape and si:IsFocused() then
					si.Text = ""
					si:ReleaseFocus()
				end
			end)
		...
		function Window:AddTab(TabConfig)
			local tab = TabModule:New(TabConfig.Title, TabConfig.Icon, Window.TabHolder, TabConfig.Description or "")
			return tab
		end
...
local SaveManager = {} do
	...
	function SaveManager:BuildConfigSection(tab)
		...
		-- ── ข้อ 8: เหลือแค่ Auto Save Toggle ──────────────────────
		local section = tab:AddSection("Configuration", "settings", "ตั้งค่าระบบบันทึกอัตโนมัติ")

		local autoSaveEnabled = true
		local autoSaveConnection

		local function startAutoSave()
			if autoSaveConnection then autoSaveConnection:Disconnect() autoSaveConnection = nil end
			autoSaveConnection = RunService.Heartbeat:Connect(function()
				-- auto-save ทุก 30 วินาที
			end)
			-- auto-save ทันทีเมื่อเปิด
			local selectedConfig = SaveManager.Options.SaveManager_ConfigList and SaveManager.Options.SaveManager_ConfigList.Value
			if selectedConfig and selectedConfig ~= "" then
				pcall(function() SaveManager:Save(selectedConfig) end)
			end
		end
		...
		section:AddToggle("AutoSave", {
			Title = "Auto Save",
			Description = "เมื่อเปิดระบบจะบันทึกค่าให้อัตโนมัติ",
			Default = true,
			Callback = function(Value)
				...
				if Value then
					...
					self.Library:Notify({
						Title = "Config",
						Content = "Auto Save",
						SubContent = "เปิดการบันทึกอัตโนมัติแล้ว",
						Duration = 3,
					})
				else
					stopAutoSave()
					self.Library:Notify({
						Title = "Config",
						Content = "Auto Save",
						SubContent = "ปิดการบันทึกอัตโนมัติแล้ว",
						Duration = 3,
					})
				end
			end
		...
		-- auto-save เมื่อ option ใด ๆ เปลี่ยนค่า (ถ้าเปิด auto save)
		task.spawn(function()
			task.wait(2) -- รอให้ทุกอย่าง init ก่อน
			while true do
				task.wait(30)
		...
		SaveManager:SetIgnoreIndexes({ "SaveManager_ConfigList", "SaveManager_ConfigName", "AutoSave" })
...
local InterfaceManager = {} do
	...
	function InterfaceManager:LoadSettings()
		...
		if isfile(path) then
			...
			if success then
				...
				-- apply transparency ที่บันทึกไว้ทันที
				if InterfaceManager.Settings.WindowTransparency ~= nil and InterfaceManager.Library then
					task.defer(function()
						InterfaceManager.Library:SetWindowTransparency(InterfaceManager.Settings.WindowTransparency)
					end)
				end
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		section:AddSlider("WindowTransparency", {
			...
			Description = "Adjusts the background transparency.",

			Default = Settings.WindowTransparency or 1,
		...
		-- ── ข้อ 7: Custom Theme ─────────────────────────────────────
		-- Section ย่อย 1: เลือกสีหลัก
		local customSection = tab:AddSection("Custom Theme", "wand", "สร้างธีมของคุณเอง")

		-- ตาราง color key ที่ custom ได้
		local CustomColors = Settings.CustomTheme or {
			Accent         = "69D7FF",
			AcrylicMain    = "1C1D24",
			AcrylicBorder  = "373944",
			TitleBarLine   = "30323E",
			Text           = "EBEDF8",
			SubText        = "9498AF",
		}
		...
		local colorKeys = {
			...
			{ key = "AcrylicMain",   label = "Background" },
			{ key = "AcrylicBorder", label = "Border" },
			{ key = "TitleBarLine",  label = "Divider" },
			{ key = "Text",          label = "Text" },
			{ key = "SubText",       label = "Sub Text" },
		}

		local function applyCustomTheme()
			local t = Themes["Dark"] or {}
			local built = {}
			for k, v in pairs(t) do built[k] = v end
			for _, entry in ipairs(colorKeys) do
				local hex = CustomColors[entry.key]
				if hex and #hex == 6 then
					local ok, col = pcall(Color3.fromHex, hex)
					if ok then built[entry.key] = col end
				end
			end
			-- ใช้ Gradient จาก AcrylicMain
			local ok, mainCol = pcall(Color3.fromHex, CustomColors.AcrylicMain or "1C1D24")
			if ok then
				local r, g, b = mainCol.R, mainCol.G, mainCol.B
				built.AcrylicGradient = ColorSequence.new(
					Color3.new(r * 0.88, g * 0.88, b * 0.88),
					Color3.new(r, g, b)
				)
			end
			built.Name = "Custom"
			Themes["Custom"] = built
			if not table.find(Library.Themes, "Custom") then
		...
		for _, entry in ipairs(colorKeys) do
			local colorKey = entry.key
			customSection:AddInput("CustomColor_" .. colorKey, {
				Title = entry.label,
				Description = "Hex สี เช่น FF5500",
				Default = CustomColors[colorKey] or "FFFFFF",
				Placeholder = "xxxxxx",
				Callback = function(Value)
		...
		customSection:AddButton({
			...
			Description = "นำสีที่กำหนดไปใช้กับธีม",
			Callback = function()
				applyCustomTheme()
				Library:Notify({
					Title = "Custom Theme",
					Content = "Applied",
					SubContent = "ธีม Custom ถูกนำไปใช้แล้ว",
					Duration = 4,
				})
			end,
		})

		-- Section ย่อย 2: บันทึก / โหลด Custom Theme
		local customSaveSection = tab:AddSection("Saved Custom Theme", "save", "บันทึกและโหลดธีมที่ Custom ไว้")

		customSaveSection:AddButton({
			Title = "Save Current Colors",
			Description = "บันทึกสีปัจจุบันลงไฟล์",
			Callback = function()
				local path = InterfaceManager.Folder .. "/custom_theme.json"
				local ok, encoded = pcall(httpService.JSONEncode, httpService, CustomColors)
				if ok then
					writefile(path, encoded)
					Library:Notify({
						Title = "Custom Theme",
						Content = "Saved",
						SubContent = "บันทึกธีม Custom เรียบร้อย",
						Duration = 4,
					})
				end
			end,
		...
		customSaveSection:AddButton({
			...
			Description = "โหลดธีมที่เคยบันทึกไว้",
			Callback = function()
				local path = InterfaceManager.Folder .. "/custom_theme.json"
				if not isfile(path) then
					return Library:Notify({
						Title = "Custom Theme",
						Content = "Load Failed",
						SubContent = "ยังไม่มีไฟล์ธีมที่บันทึกไว้",
						Duration = 4,
					})
				end
				...
				if ok and type(decoded) == "table" then
					...
					Library:Notify({
						Title = "Custom Theme",
						Content = "Loaded",
						SubContent = "โหลดธีม Custom เรียบร้อยแล้ว",
						Duration = 4,
					})
				end
			end,
		...
		customSaveSection:AddButton({
			...
			Description = "รีเซ็ตธีม Custom กลับเป็นค่าเริ่มต้น",
			Callback = function()
				CustomColors.Accent        = "69D7FF"
				CustomColors.AcrylicMain   = "1C1D24"
				CustomColors.AcrylicBorder = "373944"
				CustomColors.TitleBarLine  = "30323E"
				CustomColors.Text          = "EBEDF8"
				CustomColors.SubText       = "9498AF"
				Settings.CustomTheme = CustomColors
				applyCustomTheme()
				Library:Notify({
					Title = "Custom Theme",
					Content = "Reset",
					SubContent = "รีเซ็ตสีกลับเป็นค่าเริ่มต้นแล้ว",
					Duration = 4,
				})
			end,
		})
...
function Library:SetWindowTransparency(Value)
	...
	-- ── Non-Acrylic path: ปรับ Background ของ AcrylicPaint Frame โดยตรง ──
	if not Library.UseAcrylic then
		local bg = Library.Window.AcrylicPaint and Library.Window.AcrylicPaint.Frame
			and Library.Window.AcrylicPaint.Frame:FindFirstChild("Background")
		if bg then
			-- Value 0 = ทึบสุด (0.45 ตามค่าเริ่มต้น AcrylicPaint), Value 3 = ใส 0.95
			local t = math.clamp(0.45 + (Value / 3) * 0.5, 0.3, 0.95)
			bg.BackgroundTransparency = t
		end
		-- ถ้ามี BackgroundImage ให้ปรับด้วย
		if Library.Window.BackgroundImage then
			Library.Window.BackgroundImage.ImageTransparency = math.clamp(0.3 + (Value / 3) * 0.6, 0.1, 0.95)
		end
