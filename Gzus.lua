...
Elements.AddTab = function(self, TabTitle, TabIcon)
	...
	if not self.SectionTabStrip then
		...
		-- [ จุดที่แก้ ] แท็บ active ไม่เปลี่ยนสีตัวหนังสือ/ไอคอนแล้ว (ตัวหนังสือคงสี SubText ตลอด)
		-- ใช้ "แถบไฮไลต์" สี Accent ที่ไถล (slide) ไปหาแท็บที่เลือกแทน ดูสะอาดตาและมีอนิเมชั่นชัดเจนกว่า
		-- ใส่ ZIndex = 0 ให้ลอยอยู่หลังปุ่มทุกปุ่ม (ปุ่มพื้นหลังโปร่งใสอยู่แล้วเลยเห็นแถบทะลุขึ้นมา)
		...
		-- คำนวณตำแหน่ง/ขนาดของแถบไฮไลต์จากปุ่มเป้าหมายจริง (อ่าน AbsolutePosition/AbsoluteSize เพราะ
		-- ปุ่มใช้ AutomaticSize.X ความกว้างไม่คงที่) แล้วไถลด้วย TweenService — Instant = true ใช้ตอน
		-- วางตำแหน่งครั้งแรก/หลังสลับ container ที่ไม่อยากให้เห็นแถบเด้งข้ามจอ
		self._MoveSectionTabIndicator = function(Button, Instant)
			if not Button or Button.AbsoluteSize.X == 0 then return end
			...
			if Instant then
				self.SectionTabIndicator.Position = goalPos
				self.SectionTabIndicator.Size = goalSize
			else
				TweenService:Create(self.SectionTabIndicator, TweenInfo.new(0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
					Position = goalPos,
					Size = goalSize,
				}):Play()
			end
	...
	-- [ จุดที่แก้ ] เลิกเปลี่ยนสีตัวหนังสือ/ไอคอนตอน active แล้ว (คงสี SubText เสมอทุกปุ่ม) —
	-- ตัวบอกว่าแท็บไหน active ย้ายไปอยู่ที่แถบไฮไลต์ Accent ที่ไถลมาหาแทน (SectionTabIndicator)
	-- พื้นหลังปุ่มเองยังคงไล่ความทึบเบาๆ ไว้เผื่อ hover state (ไม่ใช้บอก active แล้ว จึงเบากว่าเดิม)
	...
	-- ทั้งปุ่มที่เพิ่งสร้างยังไม่มี AbsoluteSize ในเฟรมนี้ (AutomaticSize ต้องรอ layout resolve ก่อน)
	-- เลื่อนไปตั้งตำแหน่งแถบไฮไลต์แบบ Instant อีกที cนหลัง defer กัน "แถบเด้ง" ให้เห็นตอนเปิด UI ครั้งแรก
	task.defer(function()
		local activeTab = self.SectionTabs[self.SelectedSectionTab]
		if activeTab then
