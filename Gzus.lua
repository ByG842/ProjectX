...
ElementsTable.SelectGroup = (function()
	...
	function Element:New(Idx, Config)
		...
		-- ค่าเริ่มต้น: Multi เก็บเป็น Dictionary / Single เก็บเป็นค่าตรงๆ
		if IsMulti then
			SG.Value = {}
		...
		SG.SetTitle = SGFrame.SetTitle
		SG.SetDesc = SGFrame.SetDesc
		SG.Visible = SGFrame.Visible
		SG.Elements = SGFrame
		...
		local Holder = New("Frame", {
			...
			New("UIListLayout", {
				FillDirection = Enum.FillDirection.Horizontal,
				Padding = UDim.new(0, 10), -- ระยะห่างระหว่างชิปกว้างขึ้นเล็กน้อย ไม่ให้ดูอัดกัน
				Wraps = true,
			}),
			New("UIPadding", { PaddingTop = UDim.new(0, 12), PaddingBottom = UDim.new(0, 10) }), -- เว้นช่องว่างจาก Title/Description มากขึ้น
		})
		...
		-- อัปเดตหน้าตาปุ่มเดียว: พื้นหลัง/ขอบ/ตัวหนังสือ + checkmark ที่ค่อยๆขยายโผล่ออกมา
		local function UpdateButton(btn, isSelected)
			local ti = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
			local popTi = TweenInfo.new(0.32, Enum.EasingStyle.Back, Enum.EasingDirection.Out)

			TweenService:Create(btn.Pill, ti, {
				BackgroundTransparency = isSelected and 0.08 or 0.93,
			}):Play()
			TweenService:Create(btn.Stroke, ti, {
				Transparency = isSelected and 0.1 or 0.82,
			}):Play()
			TweenService:Create(btn.Label, ti, {
				TextTransparency = isSelected and 0 or 0.35,
			}):Play()
			TweenService:Create(btn.CheckHolder, popTi, {
				Size = isSelected and UDim2.fromOffset(20, 16) or UDim2.fromOffset(0, 16),
			}):Play()
			TweenService:Create(btn.Check, popTi, {
				Size = isSelected and UDim2.fromOffset(12, 12) or UDim2.fromOffset(5, 5),
		...
		local function UpdateAll()
			for _, btn in pairs(Buttons) do
				local isSelected
				if IsMulti then
					isSelected = SG.Value[btn.Value] == true
				else
					isSelected = (btn.Value == SG.Value)
				end
				UpdateButton(btn, isSelected)
			end
		end

		-- ไอคอนถูกเลือก: ใช้ชุดไอคอน Lucide เดียวกับที่อื่นในไลบรารี (เครื่องหมายถูกจริงๆ ไม่ใช่รูปดาว)
		local CheckIcon = (Library and Library.GetIcon and Library:GetIcon("check")) or "rbxassetid://10734966248"
		...
		for _, opt in ipairs(Config.Options) do
			...
			-- ช่องเก็บ checkmark ที่ขยายความกว้างจาก 0 -> 20 ตอนถูกเลือก (ดันตัวหนังสือให้ขยับตามธรรมชาติ)
			...
			local BtnData = {
				Value = opt,
				Pill = Pill,
				Stroke = Pill:FindFirstChildOfClass("UIStroke"),
				Label = Label,
				Check = Check,
			...
			Creator.AddSignal(Pill.MouseButton1Click, function()
				...
				Library:SafeCallback(SG.Callback, retVal)
				Library:SafeCallback(SG.Changed, retVal)
			end)
		end

		function SG:SetValue(val)
			if IsMulti then
				SG.Value = {}
		...
		SelectGroup = {
			Save = function(idx, object)
				if object.IsMulti then
					local list = {}
					for _, opt in ipairs(object.Options) do
						if object.Value[opt] then table.insert(list, opt) end
					end
					return { type = "SelectGroup", idx = idx, value = table.concat(list, ","), multi = true }
				else
