...
Components.Section = (function()
	...
	return function(Title, Parent, Icon)
		...
		-- Section label row (uppercase, small, dimmed — like preview)
		...
		-- Wrapper: label on top, elements below (no card border)
		...
		Section.Root = New("Frame", {
			...
			New("UIPadding", { PaddingTop = UDim.new(0, GAP_TOP) }),
			CardFrame,
		})
		Section._CardFrame = CardFrame
		Creator.AddSignal(Section.Layout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
			local contentH = Section.Layout.AbsoluteContentSize.Y
			Section.Container.Size = UDim2.new(1, 0, 0, contentH)
			Section.Container.Position = UDim2.fromOffset(0, HEADER_H + CARD_PAD_V)
...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		Tab.Frame = New("TextButton", {
			...
			New("Frame", {
				...
				New("ImageLabel", {
					...
					BackgroundTransparency = 1,
					Image = Icon or "",
					Visible = (Icon ~= nil and Icon ~= ""),
					LayoutOrder = 1,
					ZIndex = 11,
					ThemeTag = { ImageColor3 = "Tab" },
				}),
				New("TextLabel", {
					Name = "TabLabel",
					Text = Title,
					...
					LayoutOrder = 2,
					ZIndex = 11,
					ThemeTag = { TextColor3 = "Tab" },
				}),
			}),
		})
		Tab.BgFrame = Tab.Frame:FindFirstChild("BgFrame")
		Tab.TabLabel = Tab.Frame:FindFirstChild("ContentRow") and Tab.Frame:FindFirstChild("ContentRow"):FindFirstChild("TabLabel")
		Tab.TabIcon  = Tab.Frame:FindFirstChild("ContentRow") and Tab.Frame:FindFirstChild("ContentRow"):FindFirstChild("TabIcon")
		...
		-- Visual state helpers for new design
		local function setTabActive(isActive)
			if Tab.BgFrame then
				TweenService:Create(Tab.BgFrame, TweenInfo.new(0.15), {
					BackgroundTransparency = isActive and 0.82 or 1,
				}):Play()
			end
			if Tab.AccentBar then
				TweenService:Create(Tab.AccentBar, TweenInfo.new(0.15), {
					BackgroundTransparency = isActive and 0 or 1,
				}):Play()
			end
			if Tab.TabLabel then
				Tab.TabLabel.FontFace = Font.new(
					"rbxasset://fonts/families/GothamSSm.json",
					isActive and Enum.FontWeight.SemiBold or Enum.FontWeight.Medium,
					Enum.FontStyle.Normal
				)
			end
		end
		Tab.SetActive = setTabActive

		Tab.Motor, Tab.SetTransparency = Creator.SpringMotor(1, Tab.Frame, "BackgroundTransparency")
		Creator.AddSignal(Tab.Frame.MouseEnter, function()
			if not Tab.Selected then
				if Tab.BgFrame then Tab.BgFrame.BackgroundTransparency = 0.93 end
			end
		end)
		Creator.AddSignal(Tab.Frame.MouseLeave, function()
			if not Tab.Selected then
				if Tab.BgFrame then Tab.BgFrame.BackgroundTransparency = 1 end
			end
		end)
		Creator.AddSignal(Tab.Frame.MouseButton1Down, function()
			if Tab.BgFrame then Tab.BgFrame.BackgroundTransparency = 0.88 end
		end)
		Creator.AddSignal(Tab.Frame.MouseButton1Up, function()
			if Tab.BgFrame then Tab.BgFrame.BackgroundTransparency = Tab.Selected and 0.82 or 0.93 end
		end)
	...
	function TabModule:SelectTab(Tab)
		...
		for _, TabObject in next, TabModule.Tabs do
			...
			if TabObject.SetActive then TabObject.SetActive(false) end
		end
		TabModule.Tabs[Tab].Selected = true
		if TabModule.Tabs[Tab].SetActive then TabModule.Tabs[Tab].SetActive(true) end
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		-- MAIN TITLEBAR FRAME — height 54px
		TitleBar.Frame = New("Frame", {
			...
			New("Frame", {
				...
				Config.Icon and New("Frame", {
					...
					BackgroundColor3 = Color3.fromRGB(105, 215, 255),
...
Components.Window = (function()
	...
	return function(Config)
		...
		Window.TabWidth = Config.TabWidth or 168
		local Selector = New("Frame", {
			Size = UDim2.fromOffset(3, 0),
			BackgroundColor3 = Color3.fromRGB(76, 194, 255),
			Position = UDim2.fromOffset(0, (Window.TabHolderTop or 47) + 0),
			AnchorPoint = Vector2.new(0, 0.5),
			ZIndex = 1,
			ThemeTag = {
				BackgroundColor3 = "Accent",
		...
		local TabFrame = New("Frame", {
			Size = UDim2.new(0, Window.TabWidth, 1, Window.ShowSearch and -65 or -33),
			Position = UDim2.new(0, 8, 0, Window.ShowSearch and 56 or 21),
			BackgroundTransparency = 1,
			ClipsDescendants = true,
		}, {
			ImageFrame,
		...
		-- Content area header: tab name + thin separator line
		Window.TabDisplay = New("TextLabel", {
			...
			TextXAlignment = "Left",
			TextYAlignment = "Center",
			Size = UDim2.new(1, -(Window.TabWidth + 50), 0, 26),
			Position = UDim2.fromOffset(Window.TabWidth + 34, 60),
			BackgroundTransparency = 1,
			ThemeTag = { TextColor3 = "Text" },
		})
		Window.TabDisplaySeparator = New("Frame", {
			Size = UDim2.new(1, -(Window.TabWidth + 34), 0, 1),
			Position = UDim2.fromOffset(Window.TabWidth + 34, 88),
		...
		Window.ContainerCanvas = New("Frame", {
			Size = UDim2.new(1, -Window.TabWidth - 40, 1, -106),
			Position = UDim2.fromOffset(Window.TabWidth + 34, 94),
			BackgroundTransparency = 1,
			ClipsDescendants = true,
		}, {
		...
		-- Sidebar background panel + right border
		local SidebarBg = New("Frame", {
			Size = UDim2.new(0, Window.TabWidth + 24, 1, -54),
		...
		local SidebarDivider = New("Frame", {
			...
			Position = UDim2.new(0, Window.TabWidth + 24, 0, 54),
			BackgroundTransparency = 0.88,
			ZIndex = 2,
			ThemeTag = { BackgroundColor3 = "TitleBarLine" },
		})
		Window.SidebarBg = SidebarBg
		Window.SidebarDivider = SidebarDivider
		table.insert(rootChildren, Window.AcrylicPaint.Frame)
		table.insert(rootChildren, SidebarBg)
		table.insert(rootChildren, SidebarDivider)
		table.insert(rootChildren, Window.TabDisplay)
		table.insert(rootChildren, Window.TabDisplaySeparator)
		...
		Window.ContainerPosMotor = Flipper.SingleMotor.new(98)
		...
		Window.SelectorPosMotor:onStep(function(Value)
			local base = Window.TabHolderTop or 47
		...
		Creator.AddSignal(UserInputService.InputChanged, function(Input)
			...
			then
				...
				local TargetSizeClamped =
					Vector2.new(math.clamp(TargetSize.X, 560, 2048), math.clamp(TargetSize.Y, 400, 2048))
				SizeMotor:setGoal({
		...
		function Window:ToggleSearch()
			...
			local searchHeight = 28
			local totalOffset = (Window.ShowSearch and searchHeight or 0) + imageOffset
			TabFrame.Size = UDim2.new(0, Window.TabWidth, 1, -(totalOffset + 33))
			if Window.UpdateTabHolderLayout then
				Window:UpdateTabHolderLayout()
			end
		end
		...
		-- AddTabGroup: adds a styled group label + tabs underneath
		-- Usage: Window:AddTabGroup({ Name = "Main", Tabs = { {Title="General", Icon="home"}, ... } })
		function Window:AddTabGroup(GroupConfig)
			local groupTabs = {}
			if GroupConfig.Name then
				New("TextLabel", {
					Text = string.upper(GroupConfig.Name),
					FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold, Enum.FontStyle.Normal),
					TextSize = 9,
					TextXAlignment = Enum.TextXAlignment.Left,
					TextColor3 = Color3.fromRGB(148, 152, 175),
					BackgroundTransparency = 1,
			...
			for _, tabCfg in ipairs(GroupConfig.Tabs or {}) do
				local tab = TabModule:New(tabCfg.Title, tabCfg.Icon, Window.TabHolder)
	...
	if button then
		...
		if draggableWhole then
			...
			Creator.AddSignal(RunService.Heartbeat, function()
				if not (isDragging and dragStart and dragOffset and holder and holder.Parent) then return end
				local mouse = LocalPlayer:GetMouse()
				local delta = Vector2.new(mouse.X, mouse.Y) - dragStart
				local viewport = workspace.Camera.ViewportSize
				local size = holder.AbsoluteSize
				holder.Position = UDim2.new(0,
					math.clamp(dragOffset.X.Offset + delta.X, 0, viewport.X - size.X),
					0,
					math.clamp(dragOffset.Y.Offset + delta.Y, 0, viewport.Y - size.Y)
				)
			end)
		end
...
-- Shared drag input handler — works for both Desktop (MouseButton1) and Mobile (Touch)
local function setupDragInput(button)
	local acceptedInputs = {
		[Enum.UserInputType.MouseButton1] = true,
		[Enum.UserInputType.Touch] = true,
	}
	Creator.AddSignal(button.InputBegan, function(Input)
		if not acceptedInputs[Input.UserInputType] then return end
		isDragging = true
		dragStart = Vector2.new(Input.Position.X, Input.Position.Y)
...
setupDragInput(MinimizeButton)
setupDragInput(MobileMinimizeButton)

Creator.AddSignal(RunService.Heartbeat, function()
	local activeMin = Library.Minimizer or Minimizer
	if not (isDragging and dragStart and dragOffset and activeMin and activeMin.Parent) then return end
	local delta = Vector2.new(Mouse.X, Mouse.Y) - dragStart
	local newX = math.clamp(dragOffset.X.Offset + delta.X, 0, workspace.Camera.ViewportSize.X - activeMin.AbsoluteSize.X)
	local newY = math.clamp(dragOffset.Y.Offset + delta.Y, 0, workspace.Camera.ViewportSize.Y - activeMin.AbsoluteSize.Y)
