		...
		function Toggle:SetValue(Value)
			...
			TweenService:Create(
				...
				{ Position = UDim2.new(0, Toggle.Value and 24 or 4, 0.5, 0),
				  BackgroundTransparency = Toggle.Value and 0 or 0.25 }
			):Play()
			TweenService:Create(
				ToggleSlider,
				tweenInfo,
			...
			TweenService:Create(
				ToggleBorder,
				tweenInfo,
				{ Transparency = Toggle.Value and 0.2 or 0.4 }
			):Play()
	...
	function Element:New(Idx, Config)
		...
		local function UpdateVisual(val)
			TweenService:Create(CheckBg, TweenInfo.new(0.2), {
				BackgroundTransparency = val and 0 or 0.85
			}):Play()
			TweenService:Create(CheckMark, TweenInfo.new(0.15), {
				ImageTransparency = val and 0 or 1,
				Size = val and UDim2.fromOffset(12, 12) or UDim2.fromOffset(6, 6),
			}):Play()
		end
	...
	function Element:New(Idx, Config)
		...
		local function UpdateRadio()
			for _, btn in pairs(Buttons) do
				...
				TweenService:Create(btn.Outer, TweenInfo.new(0.2), {
					BackgroundTransparency = isSelected and 0 or 0.9
				}):Play()
				TweenService:Create(btn.Inner, TweenInfo.new(0.15), {
					BackgroundTransparency = isSelected and 0 or 1,
					Size = isSelected and UDim2.fromOffset(8, 8) or UDim2.fromOffset(4, 4),
				}):Play()
			end
		end
...
ElementsTable.Divider = (function()
	...
	function Element:New(Idx, Config)
		...
		local Root = New("Frame", {
			Size = UDim2.new(1, 0, 0, Config.Height or 1),
			BackgroundTransparency = 0.4,
			Parent = self.Container,
			LayoutOrder = 7,
			ThemeTag = { BackgroundColor3 = "TitleBarLine" },
	...
	function Element:New(Idx, Config)
		...
		local function updateChip(item, btn)
			...
			TweenService:Create(btn, TweenInfo.new(0.18), {
				BackgroundTransparency = active and 0.5 or 0.88,
			}):Play()
		end
...
-- ============================================================
-- ✨ 10 PREMIUM NEW ELEMENTS - FluentUz Enhanced ✨
-- ============================================================

-- ═══════════════════════════════════════════════════════════
-- [E-1] SpeedDial — ปุ่มกลมลอย พร้อม sub-actions ที่ expand ขึ้น
--        ใช้งาน: Tab:AddSpeedDial({ Title="Actions", Items={{ Label="Heal", Icon="lucide-heart", Callback=fn }, ...} })
-- ═══════════════════════════════════════════════════════════
ElementsTable.SpeedDial = (function()
	local Element = {}
	Element.__index = Element
	Element.__type  = "SpeedDial"
	Element.NoIdx   = true

	function Element:New(Config)
		Config        = Config or {}
		Config.Items  = Config.Items or {}
		Config.Icon   = Config.Icon or "lucide-zap"

		local SDFrame = Components.Element(Config.Title or "Speed Dial", Config.Description, self.Container, false, Config)

		local Opened = false
		local ITEM_H = 32
		local GAP    = 6

		-- Container ที่ขยายออก
		local ExpandArea = New("Frame", {
			Size                   = UDim2.new(1, -10, 0, 0),
			Position               = UDim2.new(0, 0, 1, 4),
			BackgroundTransparency = 1,
			ClipsDescendants       = false,
			Parent                 = SDFrame.Frame,
			ZIndex                 = 20,
		}, {
			New("UIListLayout", {
		...
		-- Chevron / Trigger Button ชิดขวา
		local TriggerBtn = New("TextButton", {
			Size                   = UDim2.fromOffset(28, 28),
			Position               = UDim2.new(1, -10, 0.5, 0),
			AnchorPoint            = Vector2.new(1, 0.5),
			BackgroundTransparency = 0.82,
		...
		local TrigIco = TriggerBtn:FindFirstChild("Ico")

		-- สร้าง sub-items
		local ItemButtons = {}
		for i, item in ipairs(Config.Items) do
			local iconId = item.Icon and Library:GetIcon(item.Icon) or ""
			local Btn = New("TextButton", {
		...
		local totalExpandH = #Config.Items * (ITEM_H + GAP)

		local function Toggle()
			Opened = not Opened
			TweenService:Create(TrigIco, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {
				Rotation = Opened and 0 or 180
			}):Play()
			TweenService:Create(SDFrame.Frame, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {
				Size = UDim2.new(1, 0, 0, Opened and (44 + totalExpandH) or 44)
			}):Play()
			if Opened then
				ExpandArea.Visible = true
				for _, b in ipairs(ItemButtons) do b.Visible = true end
			else
				task.delay(0.25, function()
					if not Opened then
						ExpandArea.Visible = false
						for _, b in ipairs(ItemButtons) do b.Visible = false end
					end
				end)
			end
		end

		Creator.AddSignal(TriggerBtn.MouseButton1Click, Toggle)
...
-- ═══════════════════════════════════════════════════════════
-- [E-2] NotifBadge — ป้ายตัวเลข/label บอกสถานะ (เช่น "เปิดอยู่", count)
--        Tab:AddNotifBadge({ Title="Kill Count", Default=0 })
-- ═══════════════════════════════════════════════════════════
ElementsTable.NotifBadge = (function()
	...
	function Element:New(Idx, Config)
		...
		Config.Default = Config.Default or 0
		Config.Max     = Config.Max     or 999
		Config.Color   = Config.Color   -- optional Color3
		...
		if Config.Color then BadgeBg.BackgroundColor3 = Config.Color end
		...
		local function refresh()
			local display = NB.Count > Config.Max and (tostring(Config.Max) .. "+") or tostring(NB.Count)
			BadgeLabel.Text = display
			-- pop animation
			TweenService:Create(BadgeBg, TweenInfo.new(0.08, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
				Size = UDim2.fromOffset(BadgeBg.AbsoluteSize.X + 4, 26)
			}):Play()
			task.delay(0.12, function()
				TweenService:Create(BadgeBg, TweenInfo.new(0.1), {
					Size = UDim2.fromOffset(0, 22)
				}):Play()
		...
		function NB:SetCount(n)
			NB.Count = math.max(0, math.floor(n))
			refresh()
			Library:SafeCallback(NB.Callback, NB.Count)
			Library:SafeCallback(NB.Changed, NB.Count)
		end
		function NB:Increment(n) NB:SetCount(NB.Count + (n or 1)) end
		function NB:Decrement(n) NB:SetCount(NB.Count - (n or 1)) end
		function NB:OnChanged(Func) NB.Changed = Func Func(NB.Count) end
...
-- ═══════════════════════════════════════════════════════════
-- [E-3] TimerButton — ปุ่มที่กดแล้ว cooldown ตัวเอง (disable ชั่วคราว)
--        Tab:AddTimerButton({ Title="Teleport", Cooldown=5, Callback=fn })
-- ═══════════════════════════════════════════════════════════
ElementsTable.TimerButton = (function()
	local Element = {}
	Element.__index = Element
	Element.__type  = "TimerButton"
	Element.NoIdx   = true

	function Element:New(Config)
		Config          = Config or {}
		assert(Config.Title, "TimerButton - Missing Title")
		Config.Cooldown  = Config.Cooldown or 3
		Config.Callback  = Config.Callback or function() end

		local TB = { OnCooldown = false, Type = "TimerButton" }
		local TBFrame = Components.Element(Config.Title, Config.Description, self.Container, true, Config)

		-- Progress bar overlay ทับปุ่ม
		local ProgressOverlay = New("Frame", {
			Size                   = UDim2.fromScale(0, 1),
			BackgroundTransparency = 0.6,
		...
		Creator.AddSignal(TBFrame.Frame.MouseButton1Click, function()
			if TB.OnCooldown then return end
			TB.OnCooldown = true
			Library:SafeCallback(Config.Callback)

			ArrowIco.Visible = false
			CDLabel.Visible  = true
			ProgressOverlay.Size = UDim2.fromScale(0, 1)
			...
			task.spawn(function()
				while remaining > 0 do
					...
					remaining = math.max(0, Config.Cooldown - elapsed)
					local pct = 1 - (remaining / Config.Cooldown)
					ProgressOverlay.Size = UDim2.fromScale(pct, 1)
					CDLabel.Text = string.format("%.1fs", remaining)
				end
				-- reset
				TB.OnCooldown = false
				ProgressOverlay.Size = UDim2.fromScale(0, 1)
...
-- ═══════════════════════════════════════════════════════════
-- [E-4] RangeSlider — Slider แบบ 2 หัว (min–max range)
--        Tab:AddRangeSlider({ Title="Speed Range", Min=0, Max=100, DefaultMin=20, DefaultMax=80 })
-- ═══════════════════════════════════════════════════════════
ElementsTable.RangeSlider = (function()
	...
	function Element:New(Idx, Config)
		...
		local DisplayLabel = New("TextLabel", {
			Text                   = tostring(Config.DefaultMin) .. " – " .. tostring(Config.DefaultMax),
			FontFace               = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
			TextSize               = 12,
			Position               = UDim2.new(1, -10, 0.5, 0),
			AnchorPoint            = Vector2.new(1, 0.5),
		...
		RSFrame.Frame.Size = UDim2.new(1, 0, 0, 58)

		-- Fill (ระหว่างสอง thumb)
		local Fill = New("Frame", {
			Size                   = UDim2.fromScale(0, 1),
			BackgroundTransparency = 0.45,
			Parent                 = Rail,
		...
		local function makeThumb(themeCol)
			return New("Frame", {
				Size                   = UDim2.fromOffset(14, 14),
				AnchorPoint            = Vector2.new(0.5, 0.5),
				Position               = UDim2.new(0, 0, 0.5, 0),
				BackgroundTransparency = 0,
				Parent                 = Rail,
				ThemeTag               = { BackgroundColor3 = themeCol },
			}, {
				New("UICorner", { CornerRadius = UDim.new(1, 0) }),
				New("UIStroke", { Color = Color3.fromRGB(0,0,0), Transparency = 0.8, Thickness = 1 }),
			})
		end
		local ThumbMin = makeThumb("Accent")
		local ThumbMax = makeThumb("Accent")

		local function round(v)
			if Config.Rounding == 0 then return math.floor(v + 0.5) end
			local f = 10^Config.Rounding
		...
		local function updateVisual()
			...
			ThumbMin.Position = UDim2.new(minPct, 0, 0.5, 0)
			ThumbMax.Position = UDim2.new(maxPct, 0, 0.5, 0)
		...
		local function startDrag(thumb, isMin)
			Creator.AddSignal(thumb.InputBegan, function(Input)
				...
				conn = UserInputService.InputChanged:Connect(function(i)
					...
					local railAbs = Rail.AbsolutePosition
					local railW   = Rail.AbsoluteSize.X
					local pct     = math.clamp((i.Position.X - railAbs.X) / railW, 0, 1)
					local val     = round(RS.Min + pct * (RS.Max - RS.Min))
					if isMin then
		...
		function RS:SetValue(minV, maxV)
			RS.MinValue = math.clamp(round(minV), RS.Min, RS.Max)
			RS.MaxValue = math.clamp(round(maxV), RS.Min, RS.Max)
			if RS.MinValue > RS.MaxValue then RS.MinValue = RS.MaxValue end
			updateVisual()
			Library:SafeCallback(RS.Callback, RS.MinValue, RS.MaxValue)
			Library:SafeCallback(RS.Changed,  RS.MinValue, RS.MaxValue)
		end
...
-- ═══════════════════════════════════════════════════════════
-- [E-5] TagInput — input ที่พิมพ์แล้วกด Enter สร้าง "tag" ได้หลายอัน
--        Tab:AddTagInput({ Title="Targets", Placeholder="Add name...", Max=5 })
-- ═══════════════════════════════════════════════════════════
ElementsTable.TagInput = (function()
	...
	function Element:New(Idx, Config)
		...
		Config.Placeholder = Config.Placeholder or "Type and press Enter..."
		...
		-- ขยาย frame ให้สูงขึ้น
		TIFrame.Frame.Size = UDim2.new(1, 0, 0, 44)
		TIFrame.Frame.AutomaticSize = Enum.AutomaticSize.Y

		-- Scrolling tags row
		local TagRow = New("ScrollingFrame", {
		...
		local function addTag(text)
			text = text:match("^%s*(.-)%s*$")
			if text == "" or #TI.Tags >= Config.Max then return end
			for _, t in ipairs(TI.Tags) do if t == text then return end end
			table.insert(TI.Tags, text)
			...
			local Pill = New("TextButton", {
				...
				New("UICorner", { CornerRadius = UDim.new(1, 0) }),
				New("UIPadding", { PaddingLeft = UDim.new(0, 6), PaddingRight = UDim.new(0, 4) }),
				New("UIListLayout", { FillDirection = Enum.FillDirection.Horizontal, Padding = UDim.new(0, 4), VerticalAlignment = Enum.VerticalAlignment.Center }),
				New("TextLabel", {
					Text                   = text,
					FontFace               = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
				...
				New("ImageLabel", {
					Name                   = "X",
					Image                  = "rbxassetid://10709790298", -- check-circle icon as close
					Size                   = UDim2.fromOffset(10, 10),
					BackgroundTransparency = 1,
					ImageColor3            = Color3.fromRGB(255, 255, 255),
					ImageTransparency      = 0.3,
			...
			local tagText = text
			Creator.AddSignal(Pill.MouseButton1Click, function()
				for i, t in ipairs(TI.Tags) do
					if t == tagText then
						table.remove(TI.Tags, i)
						break
					end
				end
				Pill:Destroy()
				Library:SafeCallback(TI.Callback, TI.Tags)
				Library:SafeCallback(TI.Changed, TI.Tags)
			end)
		...
		function TI:GetTags() return TI.Tags end
		function TI:ClearTags()
			TI.Tags = {}
			for _, c in ipairs(TagRow:GetChildren()) do
				if not c:IsA("UIListLayout") then c:Destroy() end
			end
		end
		function TI:OnChanged(Func) TI.Changed = Func Func(TI.Tags) end
...
-- ═══════════════════════════════════════════════════════════
-- [E-6] InfoCard — block ข้อความหลายบรรทัด + icon (ไว้แสดงข้อมูล)
--        Tab:AddInfoCard({ Title="Note", Icon="lucide-info", Lines={"line1","line2"} })
-- ═══════════════════════════════════════════════════════════
ElementsTable.InfoCard = (function()
	local Element = {}
	Element.__index = Element
	Element.__type  = "InfoCard"
	Element.NoIdx   = true

	function Element:New(Config)
		Config        = Config or {}
		assert(Config.Title, "InfoCard - Missing Title")
		Config.Lines  = Config.Lines or {}
		Config.Icon   = Config.Icon or "lucide-info"
		Config.Accent = Config.Accent -- optional Color3

		local iconId  = Library:GetIcon(Config.Icon) or ""
		...
		if Config.Accent then AccBar.BackgroundColor3 = Config.Accent end

		local ContentFrame = New("Frame", {
			Size                   = UDim2.new(1, iconId ~= "" and -50 or -10, 0, 0),
			AutomaticSize          = Enum.AutomaticSize.Y,
			Position               = UDim2.new(0, iconId ~= "" and 40 or 8, 0, 0),
			BackgroundTransparency = 1,
			Parent                 = Root,
		}, {
...
-- ═══════════════════════════════════════════════════════════
-- [E-7] StepSlider — Slider แบบ snap ไปยัง step ที่กำหนด + แสดง label
--        Tab:AddStepSlider({ Title="Quality", Steps={"Low","Medium","High","Ultra"} })
-- ═══════════════════════════════════════════════════════════
ElementsTable.StepSlider = (function()
	local Element = {}
	Element.__index = Element
	Element.__type  = "StepSlider"

	function Element:New(Idx, Config)
		Config           = Config or {}
		assert(Config.Title,  "StepSlider - Missing Title")
		assert(Config.Steps and #Config.Steps >= 2, "StepSlider - Steps must have at least 2 entries")
		Config.Default   = Config.Default or 1  -- index 1-based

		local SS = {
			Steps    = Config.Steps,
			Index    = math.clamp(Config.Default, 1, #Config.Steps),
			Value    = Config.Steps[math.clamp(Config.Default, 1, #Config.Steps)],
			Type     = "StepSlider",
			Callback = Config.Callback or function() end,
		}

		local SSFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		SS.SetTitle = SSFrame.SetTitle
		SS.SetDesc  = SSFrame.SetDesc
		SS.Visible  = SSFrame.Visible
		SS.Elements = SSFrame

		-- Value label
		local ValLabel = New("TextLabel", {
			Text                   = tostring(Config.Steps[SS.Index]),
			FontFace               = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
			TextSize               = 12,
			Position               = UDim2.new(1, -10, 0.5, 0),
			AnchorPoint            = Vector2.new(1, 0.5),
		...
		SSFrame.Frame.Size = UDim2.new(1, 0, 0, 58)
		...
		local function updateToIndex(idx)
			idx = math.clamp(idx, 1, #SS.Steps)
			SS.Index = idx
			SS.Value = SS.Steps[idx]
			local pct = (idx - 1) / (#SS.Steps - 1)
			TweenService:Create(Thumb, TweenInfo.new(0.15, Enum.EasingStyle.Quart), { Position = UDim2.new(pct, 0, 0.5, 0) }):Play()
			TweenService:Create(Fill,  TweenInfo.new(0.15, Enum.EasingStyle.Quart), { Size = UDim2.new(pct, 0, 1, 0) }):Play()
			ValLabel.Text = tostring(SS.Value)
			Library:SafeCallback(SS.Callback, SS.Value, idx)
			Library:SafeCallback(SS.Changed,  SS.Value, idx)
		end

		Creator.AddSignal(Rail.InputBegan, function(Input)
			if Input.UserInputType ~= Enum.UserInputType.MouseButton1
			and Input.UserInputType ~= Enum.UserInputType.Touch then return end
			local function drag(i2)
				if i2.UserInputType ~= Enum.UserInputType.MouseMovement
				and i2.UserInputType ~= Enum.UserInputType.Touch then return end
				local pct = math.clamp((i2.Position.X - Rail.AbsolutePosition.X) / Rail.AbsoluteSize.X, 0, 1)
				local idx = math.floor(pct * (#SS.Steps - 1) + 0.5) + 1
				updateToIndex(idx)
			end
			local conn = UserInputService.InputChanged:Connect(drag)
			local endC
			endC = UserInputService.InputEnded:Connect(function(i2)
		...
		function SS:SetValue(indexOrVal)
			if type(indexOrVal) == "number" then
				updateToIndex(indexOrVal)
			else
				for i, s in ipairs(SS.Steps) do
					if s == indexOrVal then updateToIndex(i) return end
				end
			end
		end
		function SS:OnChanged(Func) SS.Changed = Func Func(SS.Value, SS.Index) end
		function SS:Destroy() SSFrame.Frame:Destroy() if Idx then Library.Options[Idx] = nil end end

		updateToIndex(SS.Index)
		if Idx then Library.Options[Idx] = SS end
		return SS
	end
	return Element
end)()
...
-- ═══════════════════════════════════════════════════════════
-- [E-8] ClipboardButton — ปุ่มที่กดแล้ว copy text ไป clipboard + feedback
--        Tab:AddClipboardButton({ Title="Copy Token", Value="abc123" })
-- ═══════════════════════════════════════════════════════════
ElementsTable.ClipboardButton = (function()
	local Element = {}
	Element.__index = Element
	Element.__type  = "ClipboardButton"
	Element.NoIdx   = true

	function Element:New(Config)
		Config         = Config or {}
		assert(Config.Title, "ClipboardButton - Missing Title")
		...
		-- copy icon
		local CopyIco = New("ImageLabel", {
			Image                  = "rbxassetid://10734950117",
			Size                   = UDim2.fromOffset(16, 16),
			AnchorPoint            = Vector2.new(1, 0.5),
			Position               = UDim2.new(1, -10, 0.5, 0),
			BackgroundTransparency = 1,
		...
		-- feedback label (จะ flash แล้วหาย)
		...
		Creator.AddSignal(CBFrame.Frame.MouseButton1Click, function()
			...
			-- พยายาม copy ผ่าน setclipboard (executor func)
			local success = pcall(function()
				if setclipboard then
					setclipboard(Config.Value)
				elseif toclipboard then
					toclipboard(Config.Value)
				end
			end)

			Library:SafeCallback(Config.Callback, Config.Value, success)

			-- Animate feedback
			CopyIco.ImageTransparency = 1
			TweenService:Create(FeedbackLabel, TweenInfo.new(0.15), { TextTransparency = 0 }):Play()

			task.delay(1.5, function()
				TweenService:Create(FeedbackLabel, TweenInfo.new(0.2), { TextTransparency = 1 }):Play()
				task.delay(0.2, function()
					CopyIco.ImageTransparency = 0
					copying = false
				end)
			end)
		end)
...
-- ═══════════════════════════════════════════════════════════
-- [E-9] ConfirmButton — ปุ่มที่ต้องกด 2 ครั้ง (ป้องกันกดพลาด)
--        Tab:AddConfirmButton({ Title="Reset All", Confirm="Press again!", Callback=fn })
-- ═══════════════════════════════════════════════════════════
ElementsTable.ConfirmButton = (function()
	local Element = {}
	Element.__index = Element
	Element.__type  = "ConfirmButton"
	Element.NoIdx   = true

	function Element:New(Config)
		Config             = Config or {}
		assert(Config.Title, "ConfirmButton - Missing Title")
		Config.Confirm     = Config.Confirm  or "Click again to confirm"
		Config.Timeout     = Config.Timeout  or 3
		Config.Callback    = Config.Callback or function() end

		local ConfFrame = Components.Element(Config.Title, Config.Description, self.Container, true, Config)

		local stage = 0 -- 0=idle, 1=confirming
		...
		local timer = nil
		Creator.AddSignal(ConfFrame.Frame.MouseButton1Click, function()
			if stage == 0 then
				stage = 1
				ArrowIco.ImageTransparency = 1
				TweenService:Create(WarnLabel, TweenInfo.new(0.15), { TextTransparency = 0 }):Play()
				TweenService:Create(FlashOverlay, TweenInfo.new(0.1), { BackgroundTransparency = 0.85 }):Play()
				task.delay(0.1, function()
					TweenService:Create(FlashOverlay, TweenInfo.new(0.2), { BackgroundTransparency = 1 }):Play()
				end)

				if timer then task.cancel(timer) end
				timer = task.delay(Config.Timeout, function()
					if stage == 1 then
						stage = 0
						TweenService:Create(WarnLabel, TweenInfo.new(0.2), { TextTransparency = 1 }):Play()
						task.delay(0.2, function() ArrowIco.ImageTransparency = 0 end)
					end
				end)
			elseif stage == 1 then
				stage = 0
				if timer then task.cancel(timer) end
				TweenService:Create(WarnLabel, TweenInfo.new(0.15), { TextTransparency = 1 }):Play()
				task.delay(0.15, function() ArrowIco.ImageTransparency = 0 end)
				Library:SafeCallback(Config.Callback)
			end
		end)
...
-- ═══════════════════════════════════════════════════════════
-- [E-10] KeyValueDisplay — แสดงคู่ Key: Value แบบสวย (อัปเดตได้ realtime)
--         Tab:AddKeyValueDisplay({ Title="Stats", Pairs={HP="100", Speed="16"} })
-- ═══════════════════════════════════════════════════════════
ElementsTable.KeyValueDisplay = (function()
	local Element = {}
	Element.__index = Element
	Element.__type  = "KeyValueDisplay"

	function Element:New(Idx, Config)
		Config        = Config or {}
		assert(Config.Title, "KeyValueDisplay - Missing Title")
		...
		-- Clone initial data
		for k, v in pairs(Config.Pairs) do KVD.Data[k] = v end

		local KVFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		KVD.SetTitle = KVFrame.SetTitle
		KVD.SetDesc  = KVFrame.SetDesc
		KVD.Visible  = KVFrame.Visible
		KVD.Elements = KVFrame

		-- ขยาย frame ให้สูงพอ
		KVFrame.Frame.Size = UDim2.new(1, 0, 0, 0)
		KVFrame.Frame.AutomaticSize = Enum.AutomaticSize.Y
		...
		local function addRow(key, val)
			RowCount = RowCount + 1
			local Row = New("Frame", {
				Size                   = UDim2.new(1, 0, 0, 20),
				BackgroundTransparency = RowCount % 2 == 0 and 0.93 or 1,
			...
			KVD.Labels[key] = Row:FindFirstChild("Val")
			return Row
		end

		-- Build initial rows (แก้ sorted order)
		local keys = {}
		for k in pairs(Config.Pairs) do table.insert(keys, k) end
		table.sort(keys)
		for _, k in ipairs(keys) do addRow(k, Config.Pairs[k]) end

		function KVD:Set(key, value)
			KVD.Data[key] = value
			if KVD.Labels[key] then
				local lbl = KVD.Labels[key]
				lbl.Text = tostring(value)
				-- flash update animation
				TweenService:Create(lbl, TweenInfo.new(0.08), {
		...
		function KVD:SetAll(tbl)
			for k, v in pairs(tbl) do KVD:Set(k, v) end
		end

		function KVD:Destroy() KVFrame.Frame:Destroy() if Idx then Library.Options[Idx] = nil end end

		if Idx then Library.Options[Idx] = KVD end
		return KVD
	end
	return Element
end)()
...
local SaveManager = {} do
	...
	SaveManager.Parser = {
		...
		RangeSlider = {
			...
			Load = function(idx, data)
				if SaveManager.Options[idx] then SaveManager.Options[idx]:SetValue(data.minValue or 0, data.maxValue or 100) end
			end,
		},

		TagInput = {
			Save = function(idx, object)
				return { type = "TagInput", idx = idx, tags = object.Tags }
			end,
			Load = function(idx, data)
				if SaveManager.Options[idx] then
					local ti = SaveManager.Options[idx]
					ti:ClearTags()
					if type(data.tags) == "table" then
						for _, tag in ipairs(data.tags) do
							ti:GetTags() -- ensure accessible
							table.insert(ti.Tags, tag)
						end
					end
				end
			end,
		...
		StepSlider = {
			Save = function(idx, object)
				return { type = "StepSlider", idx = idx, value = object.Index }
			end,
			Load = function(idx, data)
				if SaveManager.Options[idx] then SaveManager.Options[idx]:SetValue(data.value or 1) end
			end,
		},
