...
Components.Section = (function()
	...
	return function(Title, Parent, Icon, Description)
		...
		local DESC_H      = Description and 18 or 0  -- ความสูง description (ถ้ามี)
		...
		Section.Container = New("Frame", {
			...
			Position = UDim2.fromOffset(CARD_PAD_H, HEADER_H + DESC_H + CARD_PAD_V),
			BackgroundTransparency = 1,
		}, {
			Section.Layout,
		})
		...
		local HeaderDivider = New("Frame", {
			...
			Position = UDim2.new(0, 8, 0, HEADER_H + DESC_H),
			BackgroundTransparency = 0.7,
			ThemeTag = { BackgroundColor3 = "InElementBorder" },
		})

		-- Description label (ใต้ title, เหนือ divider)
		local DescriptionLabel = Description and New("TextLabel", {
			RichText = true,
			Text = Description,
			TextTransparency = 0,
			...
			TextYAlignment = "Top",
			TextWrapped = true,
			Size = UDim2.new(1, -20, 0, DESC_H),
			Position = UDim2.fromOffset(10, HEADER_H),
			BackgroundTransparency = 1,
		...
		local CardFrame = New("Frame", {
			...
			Size = UDim2.new(1, 0, 0, HEADER_H + DESC_H + CARD_PAD_V),
			LayoutOrder = 7,
			Parent = Parent,
		...
		Section.Root = New("Frame", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, HEADER_H + DESC_H + CARD_PAD_V + GAP_TOP),
			LayoutOrder = 7,
			Parent = Parent,
		}, {
			New("UIPadding", {
		...
		Creator.AddSignal(Section.Layout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
			...
			local totalCardH = HEADER_H + DESC_H + CARD_PAD_V + contentH + CARD_PAD_V
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		-- Search box ใน TitleBar (LayoutOrder 1 = ซ้ายสุดใน RightSection)
		...
		-- เส้นแบ่งแนวตั้งระหว่าง search กับปุ่ม
		...
		TitleBar.SearchInput = TitleSearchFrame:FindFirstChild("TitleSearchInput")
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- ซ่อน SearchFrame เก่าใน TabFrame (ย้ายไปอยู่ใน TitleBar แล้ว)
		SearchFrame.Visible = false
		Window.ShowSearch = false

		-- Wire TitleBar search input
		if Window.TitleBar.SearchInput then
			Creator.AddSignal(Window.TitleBar.SearchInput:GetPropertyChangedSignal("Text"), function()
				local searchText = Window.TitleBar.SearchInput.Text or ""
				UpdateElementVisibility(searchText)
			end)
			Creator.AddSignal(UserInputService.InputBegan, function(input, gameProcessed)
				if gameProcessed then return end
				if input.KeyCode == Enum.KeyCode.Escape and Window.TitleBar.SearchInput:IsFocused() then
					Window.TitleBar.SearchInput.Text = ""
					Window.TitleBar.SearchInput:ReleaseFocus()
				end
			end)
		end
