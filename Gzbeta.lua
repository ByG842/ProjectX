...
local StyleThemes = {
	Default = {
		-- UICorner
		WindowCorner      = UDim.new(0, 18),
		ElementCorner     = UDim.new(0, 10),
		SmallCorner       = UDim.new(0, 8),
		TinyCorner        = UDim.new(0, 6),
		PillCorner        = UDim.new(1, 0),
		-- 🍎 macOS-style card corner (Sidebar / Content card panel)
		CardCorner        = UDim.new(0, 16),
		-- UIStroke
		BorderThickness   = 1,
		-- Font (Gotham)
...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		-- 🎛️ Icon Rail: ปุ่มแท็บต้องรู้ตั้งแต่สร้างว่า sidebar ตอนนี้ย่ออยู่หรือไม่
		-- (ค่าเริ่มต้นของ Library คือ "ย่อ" แบบไอคอนล้วน) เพื่อให้ label/ไอคอนจัดตำแหน่งถูกตั้งแต่เฟรมแรก
		local StartsCollapsed = Window.SidebarCollapsed == true
		...
		local TabIcon = New("ImageLabel", {
			AnchorPoint = StartsCollapsed and Vector2.new(0.5, 0.5) or Vector2.new(0, 0.5),
			Size = UDim2.fromOffset(16, 16),
			Position = StartsCollapsed and UDim2.new(0.5, 0, 0.5, 0) or UDim2.new(0, 9, 0.5, 0),
			BackgroundTransparency = 1,
		...
		-- 💬 Tooltip: โผล่ทางขวาของไอคอนตอน sidebar ย่ออยู่ บอกชื่อ Tab แบบชัดเจน กันงงว่าไอคอนนี้คืออะไร
		...
		Creator.AddSignal(Tab.Frame.MouseEnter, function()
			if Window.SidebarCollapsed and Window.Root then
				local absPos = Tab.Frame.AbsolutePosition
				local rootPos = Window.Root.AbsolutePosition
				TabTooltip.Position = UDim2.fromOffset(
					(absPos.X - rootPos.X) + Tab.Frame.AbsoluteSize.X + 10,
					(absPos.Y - rootPos.Y) + (Tab.Frame.AbsoluteSize.Y / 2) - 13
				)
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- 🔍 Search ย้ายไปอยู่บน Header ขวาบนแล้ว (ดูจุดสร้าง SearchFrame ด้านล่าง) sidebar เลยไม่ต้องเผื่อพื้นที่ให้มันอีก
		...
		-- 📐 Sidebar collapse to icons — ค่าเริ่มต้นของ Library ตอนนี้คือ Icon Rail (ย่อเหลือไอคอนล้วน)
		-- กดปุ่ม « / » เพื่อขยาย/ย่อได้ตลอด ส่วน tooltip จะช่วยบอกชื่อ Tab ตอนย่ออยู่
		...
		-- ✅ ใช้ความกว้างแบบ Icon Rail ตั้งแต่เปิดหน้าต่างมาเลย (ไม่ต้อง animate ตอนสร้าง)
		Window:SetSidebarCollapsed(true, false)
		...
		table.insert(rootChildren, SearchFrame)
