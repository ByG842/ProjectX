...
local Themes = {
	...
	Minecraft = {
		...
		Text                = Color3.fromRGB(255, 255, 85),        -- Minecraft yellow text (ตัวจริง)
...
Components.Window = (function()
	...
	return function(Config)
		...
		if Config.BackgroundImage then
			local isTiledBG = (Config.Theme == "Minecraft") or Config.BackgroundImageTile
...
function Library:SetTheme(Value, Instant)
	...
	if Library.Window and (table.find(Library.Themes, Value) or Themes[Value]) then
		...
		if Window and Window.ManagedBackgroundImage and Window.BackgroundImage then
			...
			Window.BackgroundImage.ScaleType = isMinecraft and Enum.ScaleType.Tile or Enum.ScaleType.Stretch
			if isMinecraft then
				Window.BackgroundImage.TileSize = UDim2.fromOffset(64, 64)
			end
		end
