...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		local CHIP_COLOR_TWEEN = TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
		local CHIP_HOVER_TWEEN = TweenInfo.new(0.16, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
		local CHIP_PRESS_DOWN_TWEEN = TweenInfo.new(0.08, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
		local CHIP_PRESS_UP_TWEEN = TweenInfo.new(0.38, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
		local CHIP_ENTRANCE_TWEEN = TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
		...
		-- ฟังก์ชันอัปเดตสถานะปุ่ม (Animation ของสี ขอบ และตัวหนังสือ)
		local function updateChip(item, btn)
			local active = table.find(CH.Value, item) ~= nil
			local stroke = btn:FindFirstChildOfClass("UIStroke")

			-- อนิเมชันพื้นหลังและตัวหนังสือ (นุ่มนวลขึ้นด้วย Quint easing + ยกเลิก tween เก่าอัตโนมัติ)
			...
			-- อนิเมชันขอบ (Stroke) — เข้มขึ้นและหนาขึ้นนิดเมื่อ active
			if stroke then
				playChipTween(stroke, "stroke", CHIP_COLOR_TWEEN, {
					Transparency = active and 0.05 or 0.85,
					Thickness = active and (GetStyleProperty("BorderThickness") + 0.75) or GetStyleProperty("BorderThickness"),
				})
			end
		end

		-- อนิเมชัน "เด้ง" ตอนกด แบบสปริงมีชีวิตชีวา (กดยุบเร็ว แล้วเด้งกลับแบบ Back overshoot)
		local function bounceChip(btn)
			local scale = btn:FindFirstChildOfClass("UIScale")
			if not scale then return end
			playChipTween(scale, "scale", CHIP_PRESS_DOWN_TWEEN, { Scale = 0.88 })
			task.delay(0.08, function()
				if scale and scale.Parent then
					playChipTween(scale, "scale", CHIP_PRESS_UP_TWEEN, { Scale = 1 })
				end
			end)
		end
		...
		for index, item in ipairs(Config.Items) do
			local chip = New("TextButton", {
				...
				New("UIPadding", { PaddingLeft = UDim.new(0, 14), PaddingRight = UDim.new(0, 14) }),
				New("UIScale", { Scale = 0.85 }), -- เริ่มเล็กหน่อยแล้วเด้งโตขึ้นตอนปรากฏ ให้ความรู้สึกมีชีวิตชีวา
			})

			ChipBtns[item] = chip
			updateChip(item, chip)

			-- อนิเมชัน "ปรากฏตัว" ของแต่ละ chip แบบไล่ลำดับ (stagger) ให้ดูลื่นไหลเวลาโหลดครั้งแรก
			local entranceScale = chip:FindFirstChildOfClass("UIScale")
			if entranceScale then
				task.delay((index - 1) * 0.035, function()
					if entranceScale and entranceScale.Parent then
						playChipTween(entranceScale, "scale", CHIP_ENTRANCE_TWEEN, { Scale = 1 })
					end
				end)
			end

			-- Hover เบาๆ สำหรับชิปที่ยังไม่ได้เลือก ให้รู้สึกตอบสนอง (พื้นหลัง + ขยายนิดหน่อย)
			Creator.AddSignal(chip.MouseEnter, function()
				local hoverScale = chip:FindFirstChildOfClass("UIScale")
				if hoverScale then
					playChipTween(hoverScale, "scale", CHIP_HOVER_TWEEN, { Scale = 1.045 })
				end
			...
			Creator.AddSignal(chip.MouseLeave, function()
				local hoverScale = chip:FindFirstChildOfClass("UIScale")
				if hoverScale then
					playChipTween(hoverScale, "scale", CHIP_HOVER_TWEEN, { Scale = 1 })
				end
				if table.find(CH.Value, item) then return end
