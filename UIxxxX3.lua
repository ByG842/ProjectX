...
function Library:SetTheme(Value, Instant)
	...
	if Library.Window and (table.find(Library.Themes, Value) or Themes[Value]) then
		...
		-- 🧱 อัปเดต notification ที่เปิดค้างอยู่ให้เท็กซ์เจอร์/กรอบพิกเซล Minecraft โผล่หรือหายตามธีมที่สลับ
		if Library.ActiveNotifications then
			for _, notif in pairs(Library.ActiveNotifications) do
				if notif.MCTexture then notif.MCTexture.Visible = isMinecraft end
				if notif.MCBevel then notif.MCBevel.Visible = isMinecraft end
			end
		end
