...
Components.Window = (function()
	...
	return function(Config)
		...
		Creator.AddSignal(ResizeStartFrame.MouseEnter, function()
			local grip = ResizeStartFrame:FindFirstChild("ResizeGrip")
			if grip then grip.ImageTransparency = 0.1 end
		end)
		Creator.AddSignal(ResizeStartFrame.MouseLeave, function()
			local grip = ResizeStartFrame:FindFirstChild("ResizeGrip")
			if grip then grip.ImageTransparency = 0.55 end
		end)
		...
		function Window:Minimize()
			...
			-- 🎬 อนิเมชันย่อ/ขยายหน้าต่าง แทนการสลับ Visible ทันที
			if Window.Minimized then
				if Window.SetRootScale then
					Window.SetRootScale(0.85)
				end
				...
				if Window.RootScale then
					Window.RootScale.Scale = 0.85
				end
				if Window.SetRootScale then
					Window.SetRootScale(1)
				end
			end
