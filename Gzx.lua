...
local Themes = {
	...
	-- 💜 Gzus Theme — ม่วง-ดำเข้ม พร้อมเอฟเฟกต์ glow เรืองแสงรอบหน้าต่าง
	Gzus = {
		Name                = "Gzus",
		Accent              = Color3.fromRGB(176, 68, 255),        -- ม่วงนีออนสด
		AcrylicMain         = Color3.fromRGB(12, 6, 20),            -- เกือบดำ อมม่วง
		AcrylicBorder       = Color3.fromRGB(88, 40, 140),
		AcrylicGradient     = ColorSequence.new(Color3.fromRGB(28, 8, 48), Color3.fromRGB(4, 2, 10)),
		AcrylicNoise        = 0.94,
		TitleBarLine        = Color3.fromRGB(60, 24, 95),
		Tab                 = Color3.fromRGB(110, 60, 160),
		Element             = Color3.fromRGB(32, 16, 50),
		ElementBorder       = Color3.fromRGB(14, 6, 24),
		InElementBorder     = Color3.fromRGB(64, 30, 100),
		ElementTransparency = 0.88,
		ToggleSlider        = Color3.fromRGB(84, 44, 130),
		ToggleToggled       = Color3.fromRGB(16, 6, 28),
		SliderRail          = Color3.fromRGB(64, 30, 100),
		DropdownFrame       = Color3.fromRGB(96, 52, 150),
		DropdownHolder      = Color3.fromRGB(20, 10, 32),
		DropdownBorder      = Color3.fromRGB(14, 6, 24),
		DropdownOption      = Color3.fromRGB(64, 30, 100),
		Keybind             = Color3.fromRGB(84, 44, 130),
		Input               = Color3.fromRGB(84, 44, 130),
		InputFocused        = Color3.fromRGB(9, 4, 16),
		InputIndicator      = Color3.fromRGB(130, 72, 190),
		InputIndicatorFocus = Color3.fromRGB(176, 68, 255),
		Dialog              = Color3.fromRGB(26, 13, 42),
		DialogHolder        = Color3.fromRGB(18, 9, 30),
		DialogHolderLine    = Color3.fromRGB(11, 5, 20),
		DialogButton        = Color3.fromRGB(30, 15, 48),
		DialogButtonBorder  = Color3.fromRGB(82, 40, 125),
		DialogBorder        = Color3.fromRGB(64, 30, 100),
		DialogInput         = Color3.fromRGB(35, 18, 55),
		DialogInputLine     = Color3.fromRGB(130, 72, 190),
		Text                = Color3.fromRGB(238, 228, 248),        -- ขาวอมลาเวนเดอร์
		SubText             = Color3.fromRGB(172, 152, 196),
		Hover               = Color3.fromRGB(64, 30, 100),
		HoverChange         = 0.06,
		-- ✨ ค่าเฉพาะสำหรับเอฟเฟกต์ glow ที่ Window (ดู Window.GzusGlow ใน Components.Window)
		-- ธีมอื่นไม่มีคีย์นี้ -> GetThemeProperty จะ fallback เป็น nil -> ไม่โชว์ glow
		GlowStrength        = 0.45,
	},
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- ✨ Gzus Theme Glow Effect — ออร่าม่วงเรืองแสงพัลส์รอบขอบหน้าต่าง เปิดเฉพาะตอนธีม "Gzus"
		-- ใช้ภาพ glow แบบ 9-slice เดียวกับ DropdownGlow แต่ขยายใหญ่กว่าและ blur นุ่มกว่า
		local GzusGlow = New("ImageLabel", {
			Name = "GzusGlow",
			BackgroundTransparency = 1,
		...
		Window.GzusGlow = GzusGlow
		local GzusGlowTween = nil
		function Window:SetGzusGlowActive(Active)
			if GzusGlowTween then
				GzusGlowTween:Cancel()
				GzusGlowTween = nil
			end
			if Active then
				local target = 1 - (Themes.Gzus and Themes.Gzus.GlowStrength or 0.45)
				GzusGlow.Visible = true
				GzusGlow.ImageTransparency = 1
				GzusGlowTween = TweenService:Create(
					GzusGlow,
				...
				GzusGlow.ImageTransparency = 1
				GzusGlow.Visible = false
			end
		end
		-- sync ทันทีเผื่อ Window ถูกสร้างตอนธีมปัจจุบันเป็น Gzus อยู่แล้ว
		Window:SetGzusGlowActive(Library.Theme == "Gzus")
		...
		table.insert(rootChildren, Window.GzusGlow)
...
function Library:SetTheme(Value, Instant)
	if Library.Window and (table.find(Library.Themes, Value) or Themes[Value]) then
		...
		-- 💜 เปิด/ปิดเอฟเฟกต์ glow ของธีม Gzus ตอนสลับธีม
		if Window and Window.SetGzusGlowActive then
			Window:SetGzusGlowActive(Value == "Gzus")
		end
