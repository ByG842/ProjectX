...
Components.Window = (function()
	...
	return function(Config)
		...
		-- 🔍 Search bar relocated: now anchored to the top-right of the content pane
		-- (next to the selected-tab title) instead of stacked above the sidebar list,
		-- since the sidebar is now a narrow icon rail with no room for it.
		local SearchFrame = New("Frame", {
			Size = UDim2.new(0, 200, 0, 28),
			AnchorPoint = Vector2.new(1, 0),
			Position = UDim2.new(1, -6, 0, 56),
			BackgroundTransparency = 0.7,
			ZIndex = 10,
		...
		local imageOffset = hasImage and (imageSize + 10 + topOffset) or topOffset
		local searchHeight = 28
		local totalOffset = imageOffset

		-- 🧭 Sidebar collapse-to-icons toggle removed: the rail is permanently
		-- icon-only now, so there's nothing left to collapse/expand.
		...
		Window.TabDisplay = New("TextLabel", {
			...
			TextXAlignment = "Left",
			TextYAlignment = "Center",
			Size = UDim2.new(1, -16 - 210, 0, 28), -- 🔍 210px reserved on the right for the relocated search bar
		...
		-- 🧭 The old "collapse sidebar to icons" toggle (VS Code style « / » button) has
		-- been removed: the rail is permanently icon-only now, so there's nothing to
		-- collapse/expand between anymore.
