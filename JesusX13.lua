...
Components.Notification = (function()
	...
	function Notification:New(Config)
		...
		local iconKey     = Config.Icon or TypeIcons[Config.Type] or TypeIcons.default
		...
		-- [ จุดที่แก้ ] เดิมมีทั้งแท่งสีข้างซ้าย + badge ไอคอนสี + ตัวหนังสือหัวข้อสี accent + wash พื้นหลัง +
		-- glow หลังไอคอน + progress bar ไล่สี — บอก "type" ซ้ำกันตั้ง 5-6 ที่พร้อมกัน ดูอัดแน่น/รก
		-- ตัดแท่งสีซ้ายทิ้งทั้งหมด เหลือแค่ badge ไอคอนสีเดียวพอสื่อความหมายได้ครบ ให้พื้นที่ซ้ายโล่งขึ้นด้วย
		local TypeBar = nil
		...
		-- [ จุดที่แก้ ] เปลี่ยนจาก emoji (🔔ℹ️✅⚠️❌) เป็นไอคอนเส้น lucide + badge วงกลมมีสีพื้นจางๆ
		-- ตามสไตล์ type — Notification เป็นจุดสุดท้ายในไลบรารีนี้ที่ยังใช้ emoji อยู่ (Keybind/LiveLabel/
		-- ButtonGroup เปลี่ยนไปใช้ lucide icon กันหมดแล้ว) ตอนนี้เลยดูสอดคล้องเป็นชุดเดียวกันทั้งหมด
		-- [ จุดที่แก้ ] ตัด glow หลัง badge ทิ้ง (รอบก่อนใส่ไปพร้อม wash พื้นหลัง + เส้นขอบไล่สี ทำให้รวมกัน
		-- ดูอัดแน่นไป) badge เฉยๆ สีพื้นจาง + ไอคอนก็สื่อความหมายชัดพอแล้ว ไม่ต้องมีชั้นตกแต่งซ้อนอีก
		local IconGlow = nil
		...
		-- [ จุดที่แก้ ] ขยับลงมานิดหน่อย (32->35) และเพิ่มระยะห่างบรรทัด (2->4) ให้หายใจสะดวกขึ้น
		-- ไม่ให้ตัวหนังสือแน่นติดกันเกินไป
		...
		-- [ จุดที่แก้ ] ตัด gradient ของ progress fill ทิ้ง กลับไปสีเรียบเดียว ให้เข้ากับความเรียบง่ายที่ปรับใหม่
		-- ทั้งใบ (ตอนก่อนหน้าใส่ gradient ไว้ทำให้มีจุดไล่สีเยอะเกินไปทั้งใบพร้อมกัน)
		...
		-- [ จุดที่แก้ ] นี่คือจุดหลักที่ทำให้แผงดู "จาง/กากๆ" เห็นพื้นหลังเกมทะลุเข้ามาเกือบหมด — ของเดิม
		-- BackgroundTransparency = 0.90 (ทึบแค่ 10%) ซ้อนอยู่บนชั้น Acrylic ที่ก็โปร่งอยู่แล้ว รวมกันแล้ว
		-- เหลือทึบจริงๆ ไม่ถึงครึ่ง โดยเฉพาะตอนพื้นหลังฉากสว่างจัด (เช่นสนามหญ้า) จะดูซีดมาก
		-- ปรับเป็น 0.25 (ทึบ 75%) ให้ดูเป็นการ์ดที่มีน้ำหนัก อ่านง่ายขึ้นชัดเจน โดยยังคงลาย/noise/gradient
		-- ของ Acrylic ให้เห็นลึกอยู่ ไม่ได้ทำให้ดูแบนราบเป็นกล่องทึบธรรมดา
		...
		-- [ จุดที่แก้ ] ตัด accent wash gradient ทั่วพื้นหลังทิ้ง (รอบก่อนใส่ไว้แต่งานรวมกับ glow + gradient
		-- stroke + gradient progress ทำให้ทั้งใบดูอัดแน่น มีจุดไล่สีเยอะเกินไปพร้อมกัน) พื้นหลัง Acrylic
		-- เรียบๆ + badge สี + หัวข้อสี ก็สื่อ type ได้ชัดพอแล้ว ไม่ต้องมี wash ทั่วทั้งใบซ้อนอีกชั้น
		local AccentWash = nil

		-- [ จุดที่แก้ ] เส้นขอบกลับไปเป็นสีเดียวเรียบๆ (ตัด gradient ออก) ให้เข้ากับความเรียบง่ายที่ปรับใหม่
		...
		-- [ จุดที่แก้ ] เอา IconLabel ออกจาก TextRefs เพราะตอนนี้เป็น ImageLabel ไม่ใช่ TextLabel แล้ว
		-- (ถ้าเหลือไว้ RefreshStyle จะพยายามตั้ง .TextSize/.FontFace บน ImageLabel แล้ว error)
