...
ElementsTable.LiveLabel = (function()
	...
	-- [ จุดที่เพิ่ม ] ไอคอนต่อ type ให้จำแนกสถานะได้ไวกว่าดูแค่สี ไม่ใส่ให้ "default" เพราะไม่มี
	-- ความหมายเชิงสถานะอะไรเป็นพิเศษ ใส่ไปก็รกเปล่าๆ — override ได้ผ่าน Config.Icon/:SetIcon()
	local TypeIcons = {
		info    = "info",
	...
	function Element:New(Idx, Config)
		...
		local LL = {
			...
			Type  = "LiveLabel",
			_type = Config.Type,
			-- [ จุดที่เพิ่ม ] สถานะสี/ไอคอน custom แยกจาก type — ถ้าตั้งไว้จะ "อยู่ทน" ไม่ถูกสลับสีทับ
			-- ตอนเรียก SetType ทีหลัง (ผู้ใช้ตั้งเองแล้ว ไม่ควรมีอะไรมาแทนที่โดยไม่ได้ตั้งใจ)
			_customColor    = Config.Color or nil,
			_customIcon     = Config.Icon or nil,
			_trendEnabled   = Config.Trend == true,
			_lastNumeric    = nil,
		}
		...
		-- [ จุดที่แก้ ] เปลี่ยนจาก TextButton แทน Frame เดิม เพื่อรองรับฟีเจอร์คลิก-copy (Config.Copyable)
		-- โดยยังคงหน้าตา/พฤติกรรมเดิมทุกอย่างถ้าไม่เปิดใช้ (AutoButtonColor ปิดไว้ ไม่มี highlight แปลกๆ)
		local Pill = New("TextButton", {
			...
			New("UISizeConstraint", {
				MaxSize = Vector2.new(220, math.huge),
			}),
		})

		-- ── ไอคอนต่อ type (หรือ custom) — เว้นที่ไว้เสมอถ้ามี ซ่อนถ้าไม่มี ──────
		local PillIcon = New("ImageLabel", {
			...
			BackgroundTransparency = 1,
			LayoutOrder       = 1,
			ImageColor3       = TypeColors[Config.Type] or TypeColors.default,
			Parent            = Pill,
		})

		-- ── ลูกศร trend (ขึ้น/ลง) — โผล่เฉพาะตอนเปิด Config.Trend และมีค่าตัวเลขให้เทียบ ────
		local TrendIcon = New("ImageLabel", {
			Image             = "",
			Visible           = false,
			Size              = UDim2.fromOffset(11, 11),
			BackgroundTransparency = 1,
		...
		-- ── ไอคอน copy — โผล่เฉพาะตอน hover และเปิด Config.Copyable ไว้ ─────────
		local CopyIcon = New("ImageLabel", {
			Image             = Library:GetIcon("copy"),
			Visible           = Config.Copyable == true,
			ImageTransparency = 1, -- ซ่อนไว้ก่อน โผล่ตอน hover
			Size              = UDim2.fromOffset(11, 11),
			BackgroundTransparency = 1,
			LayoutOrder       = 4,
			ThemeTag          = { ImageColor3 = "SubText" },
		...
		-- [ จุดสำคัญ ] จุดศูนย์กลางคำนวณสีจริงที่ควรแสดง ณ ตอนนี้ — ถ้ามี custom color ใช้อันนั้นก่อน
		-- เสมอ ไม่งั้นค่อย fallback ไปสีตาม type — ทุกฟังก์ชันที่กระทบสี (SetType/SetColor/ClearColor)
		-- เรียกจุดนี้จุดเดียว กันโค้ด tween ซ้ำซ้อนหลายที่
		local function RefreshAppearance(animate)
			local col = LL._customColor or TypeColors[LL._type] or TypeColors.default
			local bg  = TypeBg[LL._type] or TypeBg.default
			local iconKey = LL._customIcon or TypeIcons[LL._type]
			...
			if animate then
				...
				TweenService:Create(PillIcon, ti, { ImageColor3 = col }):Play()
			else
				ValueLabel.TextColor3 = col
				Pill.BackgroundColor3 = bg
				if PillStroke then PillStroke.Color = col end
				PillIcon.ImageColor3 = col
			end

			if iconKey then
				PillIcon.Image   = Library:GetIcon(iconKey)
				PillIcon.Visible = true
			else
		...
		function LL:SetText(text)
			text = text or ""
			-- [ จุดที่เพิ่ม ] ค่าที่ต่างจากเดิมจริงๆ ค่อย flash เบาๆ บอกว่า "อัปเดตแล้วนะ" เข้ากับความ
			-- เป็น "Live" Label — เดิมสลับข้อความเงียบๆ ไม่รู้เลยว่าค่าเพิ่งเปลี่ยน
			local changed = (text ~= self.Value)
			self.Value      = text
			ValueLabel.Text = self.Value
			Pill.Visible    = self.Value ~= ""

			-- [ จุดที่เพิ่ม ] Trend arrow — เทียบตัวเลขที่แกะได้จากข้อความใหม่กับของเดิม โชว์ลูกศร
			-- ขึ้น/ลงสีเขียว/แดง เฉพาะตอนเปิด Config.Trend ไว้และแกะตัวเลขได้จริงทั้งคู่
			if changed and self._trendEnabled then
				local numeric = tonumber((self.Value:gsub("[^%d%.%-]", "")))
				if numeric and self._lastNumeric and numeric ~= self._lastNumeric then
					if numeric > self._lastNumeric then
						TrendIcon.Image = Library:GetIcon("trending-up")
						TrendIcon.ImageColor3 = TypeColors.success
					else
						TrendIcon.Image = Library:GetIcon("trending-down")
						TrendIcon.ImageColor3 = TypeColors.error
					end
					TrendIcon.Visible = true
				end
				if numeric then self._lastNumeric = numeric end
			end

			if changed and self.Value ~= "" then
				TweenService:Create(Pill, TweenInfo.new(0.08), { BackgroundTransparency = 0.35 }):Play()
				task.delay(0.08, function()
					if Pill and Pill.Parent then
						TweenService:Create(Pill, TweenInfo.new(0.3), { BackgroundTransparency = 0.72 }):Play()
					end
				end)
			end
		...
		-- [ จุดที่แก้ ] SetColor ตอนนี้ตั้งค่าแบบ "ถาวร" จริงๆ (เก็บไว้ใน _customColor) ไม่ใช่แค่ทับสี
		-- ทันทีแล้วหายไปตอน SetType ถูกเรียกอีกที เหมือนของเดิม — ตรงตามที่ขอ "แต่งสีเองได้"
		function LL:SetColor(color)
			self._customColor = color
			RefreshAppearance(true)
		end
		-- [ จุดที่เพิ่ม ] ล้างสี custom ทิ้ง กลับไปใช้สีตาม type ตามปกติ
		function LL:ClearColor()
			self._customColor = nil
			RefreshAppearance(true)
		end
		-- [ จุดที่เพิ่ม ] เปลี่ยนไอคอน custom ได้เองเช่นกัน (เช่นอยากใช้ "coins" แทนไอคอนตาม type)
		function LL:SetIcon(iconKey)
			self._customIcon = iconKey
		...
		-- [ จุดที่เพิ่ม ] คลิกที่ Pill เพื่อ copy ค่าปัจจุบันไปคลิปบอร์ด (ปิดไว้เป็น default ต้องเปิด
		-- Config.Copyable = true เอง กันไม่ให้ LiveLabel เดิมๆ ที่ไม่ได้ตั้งใจให้คลิกได้ เปลี่ยนพฤติกรรม
		-- โดยไม่ได้ตั้งใจ) โผล่ไอคอน copy ให้เห็นเฉพาะตอน hover เท่านั้น ไม่รกตาเวลาปกติ
		if Config.Copyable == true then
			local _, SetCopyAlpha = Creator.SpringMotor(1, CopyIcon, "ImageTransparency", true, false, { frequency = 8 })
			Creator.AddSignal(Pill.MouseEnter, function() SetCopyAlpha(0.35) end)
			Creator.AddSignal(Pill.MouseLeave, function() SetCopyAlpha(1) end)
			Creator.AddSignal(Pill.MouseButton1Click, function()
				pcall(function()
					if setclipboard then setclipboard(tostring(LL.Value))
					elseif toclipboard then toclipboard(tostring(LL.Value)) end
				end)
				CopyIcon.Image = Library:GetIcon("check")
				task.delay(0.9, function()
					if CopyIcon and CopyIcon.Parent then
						CopyIcon.Image = Library:GetIcon("copy")
					end
				end)
			end)
		end
