...
Elements.AddTab = function(self, TabTitle, TabIcon)
	...
	-- [ จุดที่แก้ ] เจอบั๊ก: resolvedIcon ถูกคำนวณไว้แต่ไม่เคยสร้าง ImageLabel มาโชว์จริงเลย! ของเดิมใส่แค่
	-- UIListLayout ไว้เฉยๆ แต่ไม่มีลูกให้มันจัดเรียง (เพราะ Text อยู่ในตัว TextButton เอง ไม่ใช่ child)
	-- ไอคอนเลยไม่เคยโผล่มาสักที — รื้อโครงสร้างใหม่ให้มี TextLabel + ImageLabel เป็น child จริงๆ
	...
	self.SectionTabs[Index] = { Button = TabButton, Container = TabContainer, SetBg = SetTabBg, SetStroke = SetTabStroke, Layout = TabContentLayout, Label = TabLabel, Icon = TabIconImg }
