...
ElementsTable.Alert = (function()
	...
	function Element:New(Idx, Config)
		...
		local Root = New("Frame", {
			...
			New("UICorner", { CornerRadius = UDim.new(0, 8) }),
			New("UIStroke", { Color = style.bg, Transparency = 0.2, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }),
			-- accent left bar
			New("Frame", {
			...
			New("UIPadding", { PaddingLeft = UDim.new(0, 14), PaddingRight = UDim.new(0, 10), PaddingTop = UDim.new(0, 9), PaddingBottom = UDim.new(0, 9) }),
			New("UIListLayout", { Padding = UDim.new(0, 8), FillDirection = Enum.FillDirection.Horizontal, VerticalAlignment = Enum.VerticalAlignment.Top }),
			New("ImageLabel", {
				Image = style.icon,
				Size = UDim2.fromOffset(16, 16),
				BackgroundTransparency = 1,
	...
	function Element:New(Idx, Config)
		...
		for _, item in ipairs(Config.Items) do
			local chip = New("TextButton", {
				...
				New("UICorner", { CornerRadius = UDim.new(1, 0) }),
				New("UIStroke", { Transparency = 0.40, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, ThemeTag = { Color = "Accent" } }),
...
function Library:CreateMinimizer(Config)
	...
	local backgroundTransparency = (typeof(Config.Transparency) == "number") and math.clamp(Config.Transparency, 0, 1) or 0.12
	...
	local function createButton(isDesktop)
		...
		return New("TextButton", {
			...
			BackgroundTransparency = backgroundTransparency or 0.12,
			...
			New("UICorner", { CornerRadius = UDim.new(0, cornerRadius or (isDesktop and 20 or 16)) }),
			...
			New("UIStroke", {
				...
				Transparency = isDesktop and 0.30 or 0.40,


				Thickness = isDesktop and 1.5 or 1.5,
