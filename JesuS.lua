...
function Library:CreateMinimizer(Config)
	...
	local function createButton(isDesktop)
		return New("TextButton", {
			...
			Size = UDim2.new(1, 0, 1, 0),
			BorderSizePixel = 0,
			-- [ จุดที่แก้ ] ของเดิม BackgroundTransparency = 1 (โปร่งใสเต็ม 100%) ทำให้ปุ่มนี้ไม่มี
			-- "รูปทรง" ของตัวเองเลย สิ่งที่เห็นคือรูปโลโก้ดิบๆ เท่านั้น ถ้าโลโก้ไม่ใช่สี่เหลี่ยมมนพอดี
			-- หรือพื้นหลังเกมสีใกล้เคียงกัน จะมองเห็นยากทันที — ใส่พื้นหลัง + กรอบ + มุมมนคงที่ให้ปุ่มมี
			-- ทรงสี่เหลี่ยมมนชัดเจนเสมอ ไม่ว่าตัวโลโก้ข้างในจะเป็นรูปทรงไหนก็ตาม (ไม่ใช่วงรี/pill)
			...
			-- [ จุดที่เพิ่ม ] มุมมนแบบ "fixed offset" (ไม่ใช่ scale) ให้เป็นสี่เหลี่ยมมนสม่ำเสมอ ไม่ว่า
			-- ปุ่มจะขนาดเท่าไหร่ก็ตาม (ต่างจาก scale-based ที่อาจดูรีถ้าไม่ใช่สี่เหลี่ยมจัตุรัสเป๊ะ)
			New("UICorner", { CornerRadius = UDim.new(0, 10) }),
			New("UIStroke", {
				Transparency = 0.4,
				Thickness = 1, -- [ จุดที่แก้ ] กำหนดตรงๆ ไม่ให้ผอมหรือหนาเกินไปตามธีม
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
				ThemeTag = { Color = "InElementBorder" },
				-- [ จุดที่แก้ ] ของเดิม UIStroke ทุกอันจะถูกจดทะเบียนเข้า style-registry อัตโนมัติ แล้วโดน
				-- ApplyStyle() ทับ Thickness เป็นค่า BorderThickness ของธีมนั้นๆ ทีหลัง (บางธีมหนาถึง 3px)
				-- ทำให้ปุ่มเล็กๆ แบบนี้ดูขอบหนาเกินไป — ใส่ _NoStyleRegister กันไว้ ให้ค่า Thickness = 1
				-- ที่ตั้งไว้ตรงนี้คงที่เสมอ ไม่ว่าจะสลับธีมไหนก็ตาม
				_NoStyleRegister = true,
			}),
			New("ImageLabel", {
				Name = "Icon",
				Image = iconAsset,
				Size = UDim2.new(0.7, 0, 0.7, 0), -- ขนาดรูป (ลดลงจาก 0.85 นิดหน่อย เผื่อพื้นที่ให้กรอบ/พื้นหลังใหม่)
				Position = UDim2.new(0.5, 0, 0.5, 0),
				AnchorPoint = Vector2.new(0.5, 0.5),
				BackgroundTransparency = 1,
