...
-- [ จุดที่แก้ ] เดิม parent ScreenGui เข้า PlayerGui ตรงๆ เสมอ แล้วหวังพึ่ง ProtectGui
-- (protectgui/syn.protect_gui) เพียงอย่างเดียว — ปัญหาคือ executor หลายตัว (เช่น Real)
-- ไม่มีฟังก์ชันนี้เลย เลยตกไปใช้ function() end (ไม่ป้องกันอะไรเลย) พอไปเจอเกมที่มีสคริปต์
-- ฝั่งเกมคอยสแกน/ลบ GUI แปลกปลอมออกจาก PlayerGui (กันโกงพื้นฐานที่หลายเกมใช้) หน้าต่างของเรา
-- จะโดนลบทิ้งทันทีโดยไม่มี error ใดๆ เลย (โค้ดรันสำเร็จทุกอย่าง แค่ GUI ถูกลบไปเงียบๆ)
--
-- แก้โดยลองหา container ที่ปลอดภัยกว่าก่อน: gethui() (container พิเศษที่ exploit หลายตัว
-- รวมถึงฝั่ง Android ให้มาเพื่อซ่อน GUI จากสคริปต์ฝั่งเกมโดยเฉพาะ) → ถ้าไม่มีลอง CoreGui
-- (สคริปต์ฝั่งเกมทั่วไปมองไม่เห็น/แก้ไขไม่ได้ตามปกติ) → ถ้าใช้ไม่ได้ทั้งคู่ค่อย fallback
-- กลับไป PlayerGui เหมือนเดิมเป็นทางเลือกสุดท้าย
local GuiParent
pcall(function()
	if gethui then
		GuiParent = gethui()
...
Elements.AddTab = function(self, TabTitle, TabIcon)
	...
	-- [ จุดที่แก้ ] Rework ตัวบอก active: เดิมเปลี่ยนสีตัวหนังสือ/ไอคอนเป็น Accent ตอนเลือก
	-- ผู้ใช้ขอให้ "ตัวหนังสือไม่เปลี่ยนสี" แต่ยังอยากได้อะไรที่สวยๆมีอนิเมชั่นบอกแทน
	-- เปลี่ยนมาใช้แถบ indicator บางๆสี Accent อยู่ขอบล่างของปุ่ม เด้ง (spring pop) + เฟดเข้าออกแทน
	...
	local IndicatorScale = Creator.New("UIScale", { Scale = 0 })
	IndicatorScale.Parent = TabIndicator
	...
	-- แถบ indicator: เฟด transparency ปกติ + เด้ง scale แบบ bouncy (dampingRatio ต่ำ) ตอนโผล่ขึ้นมา
	local _, SetIndicatorAlpha = Creator.SpringMotor(1, TabIndicator, "BackgroundTransparency", true, false, { frequency = 8 })
	local _, SetIndicatorScale = Creator.SpringMotor(0, IndicatorScale, "Scale", true, false, { frequency = 5, dampingRatio = 0.55 })
	...
	self.SectionTabs[Index] = {
		Button = TabButton, Container = TabContainer, SetBg = SetTabBg, Layout = TabContentLayout,
		Label = TabLabel, Icon = TabIconImg,
		SetIndicatorAlpha = SetIndicatorAlpha, SetIndicatorScale = SetIndicatorScale,
	}
	...
	-- [ จุดที่แก้ ] เดิมแท็บ active เปลี่ยนสีตัวหนังสือ/ไอคอนเป็น Accent — ตอนนี้เปลี่ยนมาใช้แถบ indicator
	-- บางๆใต้ปุ่มที่เด้ง+เฟดเข้าออกแทน ตัวหนังสือ/ไอคอนใช้สี SubText เดิมเสมอ ไม่เปลี่ยนสีอีกต่อไป
	...
	local function UpdateSectionTabAppearance()
		for i, t in ipairs(self.SectionTabs) do
			...
			t.SetIndicatorAlpha(active and 0 or 1)
			t.SetIndicatorScale(active and 1 or 0)
		end
		self._RecalcSectionTabLayout()
	end
