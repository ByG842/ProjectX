...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		-- ── Window control button แบบ preview (text symbol, 28×28, corner 7) ──
		local function RightBarButton(Symbol, Parent, Callback, IsClose)
			local Button = { Callback = Callback or function() end }

			-- สี hover/active ต่างกันถ้าเป็นปุ่ม Close
			local hoverBgColor  = IsClose and Color3.fromRGB(255, 60, 60)  or nil
			local hoverBgTrans  = IsClose and 0.82 or 0.88
			local normalBgTrans = 1
			...
			Button.Frame = New("TextButton", {
				...
				BackgroundTransparency = normalBgTrans,
				AutoButtonColor = false,
				Parent     = Parent,
				ThemeTag   = {
					BackgroundColor3 = IsClose and "Element" or "Hover",
					TextColor3       = "SubText",
				},
			}, {
			...
			local Motor, SetTransparency = Creator.SpringMotor(normalBgTrans, Button.Frame, "BackgroundTransparency")

			AddSignal(Button.Frame.MouseEnter, function()
				if IsClose then
					TweenService:Create(Button.Frame, TweenInfo.new(0.12), {
						BackgroundColor3 = Color3.fromRGB(220, 53, 53),
						BackgroundTransparency = 0.82,
						TextColor3 = Color3.fromRGB(255, 255, 255),
					}):Play()
				else
					SetTransparency(hoverBgTrans)
				end
			end)
			AddSignal(Button.Frame.MouseLeave, function()
				if IsClose then
					...
					Creator.OverrideTag(Button.Frame, { TextColor3 = "SubText" })
				else
					SetTransparency(normalBgTrans, true)
				end
			end)
			AddSignal(Button.Frame.MouseButton1Down, function()
				SetTransparency(IsClose and 0.70 or 0.80)
			end)
			AddSignal(Button.Frame.MouseButton1Up, function()
				SetTransparency(IsClose and hoverBgTrans or hoverBgTrans)
			end)
		...
		-- ── MAIN TITLEBAR FRAME — height 54px ตาม preview ──
		TitleBar.Frame = New("Frame", {
			Size = UDim2.new(1, 0, 0, 54),
			BackgroundTransparency = 1,
			Parent = Config.Parent,
		}, {
			-- ── LEFT SECTION: Icon + TextStack ──
			New("Frame", {
				Name = "LeftSection",
				...
				-- Icon frame: bg Accent จางๆ + border Accent + corner 9
			...
			New("Frame", {
				...
				New("UIPadding", { PaddingRight = UDim.new(0, 6) }),
			}),
		...
		-- สร้าง Window Buttons ใน RightSection
		local rightSection = TitleBar.Frame:FindFirstChild("RightSection")

		TitleBar.MinButton = RightBarButton("⊟", rightSection, function()
			Library.Window:Minimize()
		end, false)

		TitleBar.MaxButton = RightBarButton("⊡", rightSection, function()
			Config.Window.Maximize(not Config.Window.Maximized)
		end, false)

		TitleBar.CloseButton = RightBarButton("✕", rightSection, function()
		...
		-- ผูก Search Box เข้ากับระบบ search ของ Window (ถ้ามี)
		if Config.Search then
			local searchBox = TitleBar.Frame:FindFirstChild("SearchBox")
			if searchBox then
				local searchInput = searchBox:FindFirstChild("SearchInput")
				if searchInput then
					AddSignal(searchInput:GetPropertyChangedSignal("Text"), function()
						if Config.SearchCallback then
							Config.SearchCallback(searchInput.Text)
						end
					end)
					TitleBar.SearchInput = searchInput
				end
			end
		end
...
Components.Window = (function()
	...
	return function(Config)
		...
		Window.TitleBar = Components.TitleBar({
			...
			SearchCallback = function(text)
				UpdateElementVisibility(text)
			end,
