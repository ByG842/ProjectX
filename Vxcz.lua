...
Components.Element = (function()
	...
	return function(Title, Desc, Parent, Hover, Options)
		...
		-- 🧱 [REWORK] ขีดสีเน้นขอบซ้ายของแต่ละแถว — ปกติหุบเป็น 0 แล้วยืดขึ้นตอนเอาเมาส์ชี้
		-- ช่วยให้ตาไล่แถวได้ง่ายขึ้นมากในหน้าที่มี element เยอะๆ
		Element.Rail = New("Frame", {
			Name = "HoverRail",
		...
		Element.Frame = New("TextButton", {
			...
			-- ✨ [POLISH] ไล่เฉดแนวตั้งจางๆ ให้แถวดูมีมิติ (คูณกับสีธีม ไม่ล็อกสี)
			New("UIGradient", {
				Rotation = 90,
		...
		if Hover then
			...
			local RAIL_TI = TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
			local function SetRail(on)
				if Element.Rail and Element.Rail.Parent then
					TweenService:Create(Element.Rail, RAIL_TI, {
						Size = UDim2.new(0, 2, on and 0.55 or 0, 0),
					}):Play()
				end
			end
...
Components.Section = (function()
	...
	return function(Title, Parent, Icon)
		...
		local CARD_PAD_H  = 11  -- padding ซ้ายขวาภายใน card (เผื่อขีดสีขอบซ้ายของการ์ด)
		local CARD_PAD_V  = 7   -- padding บนล่างภายใน card
		local HEADER_H    = 32  -- ความสูง header row
		local GAP_TOP     = 8   -- ระยะห่างระหว่าง section card กับของข้างบน
		...
		-- 🧱 [REWORK] หัว section เดิมเป็นขีดสั้นๆ → เปลี่ยนเป็น "จุดวงแหวน" ให้อ่านเป็นหมุดหัวข้อ
		...
		local CollapseChevron = New("ImageLabel", {
			...
			BackgroundTransparency = 1,
			Rotation = 0,
			-- 🧱 [REWORK] SectionHeader ใช้ UIListLayout อยู่ ลูกทุกตัวจึงถูกจัดเรียงตาม LayoutOrder
			-- เดิม chevron ไม่ได้ตั้งค่าไว้ (=0) เลยไปโผล่หน้าสุดของแถว — ย้ายให้ไปอยู่ท้ายแถวแทน
			LayoutOrder = 10,
			ThemeTag = { ImageColor3 = "SubText" },
		})
		...
		local CardFrame = New("Frame", {
			...
			-- ✨ [POLISH] ไล่เฉดตัวการ์ด + แถบสีเน้นจางๆ ไล่หายไปทางขวาหลังหัวข้อ
			New("UIGradient", {
				Rotation = 90,
				Color = ColorSequence.new(Color3.fromRGB(255, 255, 255), Color3.fromRGB(184, 184, 184)),
			}),
			New("Frame", {
				Name = "HeaderWash",
				Size = UDim2.new(1, 0, 0, HEADER_H),
			...
			-- 🧱 [REWORK] ขีดสีเน้นเต็มความสูงขอบซ้ายของการ์ด — ทำให้แต่ละกล่องแยกกันชัดเจน
			New("Frame", {
				Name = "CardEdge",
		...
		-- ⚡ [FEATURE] ลงทะเบียนไว้ให้ปุ่ม "พับทุกกล่อง" บนแถบหัวเรื่องสั่งงานได้
		if Library.Window and Library.Window._Sections then
			table.insert(Library.Window._Sections, Section)
		end
		return Section
	end
end)()
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		-- 🧱 [REWORK] ปุ่มแท็บทรงใหม่: สูงขึ้นเป็น 42px, ไอคอนอยู่ใน "แผ่นป้าย" มุมมนแยกชิ้น
		-- (icon badge) ไม่ใช่ไอคอนลอยเปล่าๆ แบบเดิม ทำให้รางแท็บอ่านเป็นรายการชัดขึ้น
		local TabIconBadge = (not fischbypass and Icon) and New("Frame", {
			Name = "IconBadge",
			AnchorPoint = Vector2.new(0, 0.5),
		...
		Tab.Frame = New("TextButton", {
			...
			NewCorner("ElementCorner"),
			-- ✨ [POLISH] ไล่เฉดบางๆ บนปุ่มแท็บ (คูณกับสีธีม ไม่ได้ fix สีเอง) + วงแหวนสีเน้นตอนถูกเลือก
			New("UIGradient", {
				Rotation = 90,
				Color = ColorSequence.new(Color3.fromRGB(255, 255, 255), Color3.fromRGB(188, 188, 188)),
			}),
			New("UIStroke", {
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
				Transparency = 1,
			...
			New("TextLabel", {
				AnchorPoint = Vector2.new(0, 0.5),
				Position = TabIconBadge and UDim2.new(0, 42, 0.5, 0) or UDim2.new(0, 14, 0.5, 0),
				Text = Title,
				RichText = true,
				...
				TextXAlignment = "Left",
				TextYAlignment = "Center",
				Size = UDim2.new(1, TabIconBadge and -54 or -26, 1, 0),
		...
		Tab.IconId = (not fischbypass) and Icon or nil
		Tab.IconBadge = TabIconBadge
		Tab.Stroke = Tab.Frame:FindFirstChildWhichIsA("UIStroke")
		...
		Creator.AddSignal(Tab.Frame.MouseEnter, function()
			Tab.SetTransparency(Tab.Selected and 0.76 or 0.88)
		end)
		Creator.AddSignal(Tab.Frame.MouseLeave, function()
			Tab.SetTransparency(Tab.Selected and 0.82 or 0.95)
		end)
		Creator.AddSignal(Tab.Frame.MouseButton1Down, function()
			Tab.SetTransparency(Tab.Selected and 0.86 or 0.92)
		end)
	...
	function TabModule:SelectTab(Tab)
		...
		for _, TabObject in next, TabModule.Tabs do
			TabObject.SetTransparency(0.95)
			TabObject.Selected = false
		end
		TabModule.Tabs[Tab].SetTransparency(0.82)
		TabModule.Tabs[Tab].Selected = true
		Window.TabDisplay.Text = TabModule.Tabs[Tab].Name
		if Window.SetHeaderInfo then
			Window.SetHeaderInfo(TabModule.Tabs[Tab], Tab, TabModule.TabCount)
		end
		-- 🧱 [REWORK] แผ่นป้ายไอคอนของแท็บที่เลือกจะสว่างขึ้น ส่วนตัวอื่นหรี่ลง
		-- ✨ [POLISH] พร้อมวงแหวนสีเน้นรอบปุ่ม + ไอคอนคมขึ้น
		for idx, TabObject in next, TabModule.Tabs do
			local isActive = (idx == Tab)
			local ti = TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
			if TabObject.IconBadge then
				TweenService:Create(TabObject.IconBadge, ti, {
					BackgroundTransparency = isActive and 0.45 or 0.85,
				}):Play()
				local badgeIcon = TabObject.IconBadge:FindFirstChild("TabIcon")
				if badgeIcon then
					TweenService:Create(badgeIcon, ti, {
						ImageTransparency = isActive and 0 or 0.35,
					}):Play()
				end
			end
			if TabObject.Stroke then
				TweenService:Create(TabObject.Stroke, ti, {
					Transparency = isActive and 0.4 or 1,
				}):Play()
			end
		end
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		TitleBar.Frame = New("Frame", {
			...
			-- ✨ [POLISH] แสงสีเน้นจางๆ อาบอยู่หลังแถบหัว ให้หัวหน้าต่างไม่ดูตายตัว
			New("Frame", {
				Name = "TitleBloom",
				Size = UDim2.new(1, 0, 1, 0),
				Position = UDim2.fromOffset(0, 0),
			...
			-- Bottom separator line — [REWORK] ร่นเข้ามาให้เป็นเส้นลอย ไม่ชนขอบหน้าต่าง
			New("Frame", {
		...
		local function RightBarButton(Icon, Parent, Callback)
			...
			Button.Frame = New("TextButton", {
				...
				-- 🧱 [REWORK] ปุ่มควบคุมหน้าต่างเปลี่ยนเป็นทรงกลม แทนสี่เหลี่ยมมุมมน
				NewCorner("PillCorner"),
				New("ImageLabel", {
					Image = Icon,
					Size = UDim2.fromOffset(14, 14),
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- 🧭 [REWORK] ตัวชี้แท็บย้ายจาก "ขีดซ้าย" มาเป็น "พิลขอบขวา" ของราง
		-- ทำให้สายตาไล่จากไอคอน → ชื่อ → ตัวชี้ เป็นเส้นเดียวจบที่ขอบขวา
		local Selector = New("Frame", {
			Name = "TabSelector",
			Size = UDim2.fromOffset(3, 0),
			BackgroundColor3 = Color3.fromRGB(76, 194, 255),
			Position = UDim2.new(1, -3, 0, (Window.TabHolderTop or 45) + 0),
			AnchorPoint = Vector2.new(1, 0.5),
			ZIndex = 14,
			ThemeTag = {
				BackgroundColor3 = "Accent",
		...
		Window.TabHolder = New("ScrollingFrame", {
			...
			-- 🧭 [REWORK] เว้นร่องขวาไว้ให้ "พิลตัวชี้" มายืน ไม่ทับข้อความแท็บ
			New("UIPadding", {
				PaddingRight = UDim.new(0, 10),
			}),
		})
		...
		-- ✨ [POLISH] ช่องค้นหาเปลี่ยนเป็นทรงพิล + มีวงแหวนสีเน้นเรืองขึ้นตอนโฟกัส
		local SearchFocusRing = New("UIStroke", {
			ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
			Thickness = 2,
			Transparency = 1,
			_NoStyleRegister = true,
		...
		-- ✨ [POLISH] วงแหวนโฟกัสเฟดเข้า/ออก + ไอคอนแว่นขยายสว่างขึ้นตอนพิมพ์
		do
			local RING_TI = TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
			Creator.AddSignal(SearchTextbox.Input.Focused, function()
				TweenService:Create(SearchFocusRing, RING_TI, { Transparency = 0.35 }):Play()
				TweenService:Create(SearchIcon, RING_TI, { ImageTransparency = 0 }):Play()
			end)
			Creator.AddSignal(SearchTextbox.Input.FocusLost, function()
				TweenService:Create(SearchFocusRing, RING_TI, { Transparency = 1 }):Play()
				TweenService:Create(SearchIcon, RING_TI, { ImageTransparency = 0.3 }):Play()
			end)
		end
		...
		local SidebarCollapseButton = New("TextButton", {
			...
			-- 🧱 [REWORK] ปุ่มย่อรางเป็นทรงกลม ย้ายไปมุมขวาล่างของราง (เดิมอยู่ซ้าย ชนกับตัวชี้)
			NewCorner("PillCorner"),
			New("UIStroke", {
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
				Transparency = 0.75,
		...
		-- 🧱 [REWORK] "แผงด็อก" ของแถบข้าง — เดิมแถบแท็บลอยอยู่บนพื้นหลังเดียวกับเนื้อหา
		-- ทำให้หน้าต่างดูเป็นแผ่นเดียวแบนๆ ตอนนี้แยกเป็น 2 แผ่นซ้อน (ราง / เนื้อหา) ชัดเจน
		local SidebarPanel = New("Frame", {
			...
			-- ✨ [POLISH] เส้นไฮไลต์บางๆ ที่ขอบบน ให้แผงดูเป็นกระจกมีความหนา
			New("Frame", {
				Name = "GlassEdge",
				Size = UDim2.new(1, -28, 0, 1),
		...
		local TabFrame = New("Frame", {
			...
			BackgroundTransparency = 1,
			ZIndex = 2, -- อยู่เหนือแผงด็อกเสมอ (ZIndex 1)
			ClipsDescendants = true,
		}, {
			ImageFrame,
			SearchFrame,
		...
		-- ให้แผงด็อกวิ่งตาม TabFrame เสมอ (ตอนย่อ/กางแถบข้าง หรือ resize หน้าต่าง)
		local function SyncSidebarPanel()
			if not SidebarPanel or not TabFrame then
				return
			end
			local p, s = TabFrame.Position, TabFrame.Size
			-- หนีบขอบบนไม่ให้โผล่ทะลุแถบหัวเรื่อง และให้ขอบล่างต่ำกว่าราง 4px พอดี
			local topY = math.max(56, p.Y.Offset - 8)
			local extraY = (p.Y.Offset + s.Y.Offset + 4) - topY
			SidebarPanel.Position = UDim2.new(p.X.Scale, p.X.Offset - 8, p.Y.Scale, topY)
			SidebarPanel.Size = UDim2.new(s.X.Scale, s.X.Offset + 16, s.Y.Scale, extraY)
		end
		Window.SyncSidebarPanel = SyncSidebarPanel
		SyncSidebarPanel()

		Window.TabFrame = TabFrame
		-- 🧭 [REWORK] หัวข้อแท็บเดิมเป็นข้อความลอยเดี่ยวๆ → เปลี่ยนเป็น "แถบหัวเรื่อง"
		-- ขีดสีเน้น + ไอคอนแท็บ + ชื่อ + ชิปนับแท็บ (01 / 07) + ปุ่มพับทุกกล่อง + เส้นคั่นไล่จาง
		...
		Window.HeaderTick = New("Frame", {
			...
			NewCorner("PillCorner"),
			-- ✨ [POLISH] ขีดหัวเรื่องไล่จางลงด้านล่าง ไม่ใช่แท่งทึบตัน
			New("UIGradient", {
				Rotation = 90,
				Transparency = NumberSequence.new({
					NumberSequenceKeypoint.new(0, 0),
		...
		Window.HeaderIcon = New("ImageLabel", {
			Name = "HeaderIcon",
			Size = UDim2.fromOffset(18, 18),
		...
		Window.HeaderCount = New("TextLabel", {
			Name = "TabCounter",
			Text = "01 / 01",
			FontFace = GetStyleProperty("FontMedium"),
			TextSize = GetStyleProperty("TextSizeXs"),
		...
		Window.HeaderCollapseAll = New("TextButton", {
			Name = "CollapseAll",
			Text = "",
			AutoButtonColor = false,
			Size = UDim2.fromOffset(26, 26),
		...
		Window.HeaderBar = New("Frame", {
			Name = "TabHeaderBar",
			Size = UDim2.new(1, -Window.TabWidth - 32, 0, 34),
			...
			Window.TabDisplay,
			Window.HeaderCount,
			Window.HeaderCollapseAll,
			Window.HeaderRule,
		})

		-- อัปเดตไอคอน/ตัวนับบนแถบหัวเรื่องเวลาเปลี่ยนแท็บ
		Window._Sections = {}
		Window.SectionsCollapsed = false
		function Window.SetHeaderInfo(TabObject, Index, Total)
			local iconId = TabObject and TabObject.IconId
			local hasIcon = (iconId ~= nil and iconId ~= "")
			if Window.HeaderIcon then
				Window.HeaderIcon.Image = hasIcon and iconId or ""
				Window.HeaderIcon.Visible = hasIcon
			end
			if Window.TabDisplay then
				Window.TabDisplay.Position = UDim2.fromOffset(hasIcon and 40 or 15, 0)
			end
			if Window.HeaderCount then
				Window.HeaderCount.Text = string.format("%02d / %02d", Index or 0, Total or 0)
			end
		end

		-- ⚡ [FEATURE] พับ/กางกล่อง Section ทั้งหน้าในคลิกเดียว
		Creator.AddSignal(Window.HeaderCollapseAll.MouseButton1Click, function()
			Window.SectionsCollapsed = not Window.SectionsCollapsed
			local target = Window.SectionsCollapsed
			for _, sec in ipairs(Window._Sections) do
				if sec and sec.Root and sec.Root.Parent and sec.SetCollapsed then
					local onScreen = true
					local node = sec.Root
					while node do
					...
					if onScreen then
						pcall(function()
							sec:SetCollapsed(target)
						end)
					end
				end
			end
			local chev = Window.HeaderCollapseAll:FindFirstChild("Icon")
			if chev then
				TweenService:Create(chev, TweenInfo.new(0.2), { Rotation = target and -90 or 0 }):Play()
			end
		end)
		...
		local function ApplySidebarWidth(width)
			...
			if Window.HeaderBar then
				Window.HeaderBar.Position = UDim2.fromOffset(width + 26, Window.HeaderBar.Position.Y.Offset)
				Window.HeaderBar.Size = UDim2.new(1, -width - 32, Window.HeaderBar.Size.Y.Scale, Window.HeaderBar.Size.Y.Offset)
			end
			...
			if Window.SyncSidebarPanel then
				Window.SyncSidebarPanel()
			end
		end
			...
			if Window.TabHolder then
				for _, tabBtn in ipairs(Window.TabHolder:GetChildren()) do
					if tabBtn:IsA("GuiObject") then
						...
						-- 🧱 [REWORK] ตอนย่อราง ให้ "แผ่นป้ายไอคอน" ทั้งชิ้นเลื่อนไปกลาง (เดิมเลื่อนเฉพาะรูป)
						local icon = tabBtn:FindFirstChild("IconBadge") or tabBtn:FindFirstChildWhichIsA("ImageLabel", true)
						if label then
		...
		table.insert(rootChildren, Window.HeaderBar)
		table.insert(rootChildren, Window.ContainerCanvas)
		table.insert(rootChildren, SidebarPanel)
		...
		Window.SelectorPosMotor:onStep(function(Value)
			...
			Selector.Position = UDim2.new(1, -3, 0, selectorY)
			local Now = tick()
		...
		do
			...
			-- แถบแท็บด้านข้าง (Sidebar) — เลื่อนเข้ามาจากด้านซ้าย พร้อมแผงด็อกที่อยู่ข้างหลัง
			if Window.SidebarPanel then
				EntranceAnimate(Window.SidebarPanel, -60, 0, 0.12)
			end
			EntranceAnimate(TabFrame, -60, 0, 0.15)

			-- แถบหัวเรื่องของแท็บปัจจุบัน — เลื่อนลงมาจากด้านบน
			if Window.HeaderBar then
				EntranceAnimate(Window.HeaderBar, 0, -30, 0.3)
			end
