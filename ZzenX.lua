...
ElementsTable.SelectGroup = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "SelectGroup"

	function Element:New(Idx, Config)
		Config = Config or {}
		assert(Config.Title, "SelectGroup - Missing Title")
		assert(Config.Options or Config.Items, "SelectGroup - Missing Options table")

		local Options = Config.Options or Config.Items
		local IsMulti = Config.Multi == true
		local AllowEmpty = Config.AllowEmpty ~= false
		local Search = Config.Search == true
		local MaxSelected = tonumber(Config.MaxSelected)
		...
		local function buildLookup()
			...
			for _, option in ipairs(Options) do
				lookup[optionKey(option)] = option
			end
			return lookup
		end

		local OptionLookup = buildLookup()

		local function normalizeSingle(value)
			if value == nil then return nil end
			return OptionLookup[optionKey(value)] or value
		end
		...
		local function normalizeMulti(value)
			...
			if type(value) ~= "table" then
				if value ~= nil then
					local v = normalizeSingle(value)
					if v ~= nil then
						result[optionKey(v)] = true
					end
				end
				return result
			end
			...
			for _, v in ipairs(value) do
				local normalized = normalizeSingle(v)
				if normalized ~= nil and not seen[optionKey(normalized)] then
					result[optionKey(normalized)] = true
					seen[optionKey(normalized)] = true
				end
			end
			return result
		end
		...
		if IsMulti then
			Select.Value = normalizeMulti(Config.Default)
		else
			Select.Value = normalizeSingle(Config.Default)
			if Select.Value == nil and Config.Default == nil and Config.DefaultFirst ~= false then
				Select.Value = Options[1]
			end
		end

		local SGFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		Select.SetTitle = SGFrame.SetTitle
		Select.SetDesc = SGFrame.SetDesc
		Select.Visible = SGFrame.Visible
		Select.Elements = SGFrame
		...
		local Scroll = New("ScrollingFrame", {
			Size = UDim2.new(1, 0, 0, Config.MaxHeight or 118),
		...
		local EmptyLabel = New("TextLabel", {
			...
			BackgroundTransparency = 1,
			Text = Config.EmptyText or "No options found",
			Visible = false,
			LayoutOrder = 4,
			FontFace = GetStyleProperty("FontMedium"),
			TextSize = GetStyleProperty("TextSizeSm"),
		...
		local function getSelected(value)
			if IsMulti then
				return Select.Value[optionKey(value)] == true
			end
			return Select.Value ~= nil and optionKey(Select.Value) == optionKey(value)
		end
		...
		local function getCallbackValue()
			...
			local result = {}
			for _, option in ipairs(Options) do
				if Select.Value[optionKey(option)] then
		...
		local function refreshButtonVisual(value, immediate)
			local data = Buttons[optionKey(value)]
			if not data then return end

			local active = getSelected(value)
			local selectedBg = active and 0.08 or 0.88
			local selectedText = active and 0 or 0.25
			local strokeTransparency = active and 0.12 or 0.82
			local scale = active and 1.025 or 1

			if immediate then
				data.Button.BackgroundTransparency = selectedBg
				data.Button.TextTransparency = selectedText
				data.Stroke.Transparency = strokeTransparency
				data.Scale.Scale = scale
				data.Checkmark.ImageTransparency = active and 0 or 1
				return
			end

			local ti = TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
			TweenService:Create(data.Button, ti, {
				BackgroundTransparency = selectedBg,
				TextTransparency = selectedText,
			}):Play()
			TweenService:Create(data.Stroke, ti, {
				Transparency = strokeTransparency,
			}):Play()
			TweenService:Create(data.Scale, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
				Scale = scale,
			}):Play()
			TweenService:Create(data.Checkmark, ti, {
				ImageTransparency = active and 0 or 1,
			}):Play()
		end

		local function refreshAll(immediate)
			for _, option in ipairs(Options) do
				refreshButtonVisual(option, immediate)
			end
		end
		...
		local function refreshFilter()
			...
			local visible = 0
			for _, option in ipairs(Options) do
				local data = Buttons[optionKey(option)]
				if data then
					local match = query == "" or string.find(string.lower(tostring(option)), query, 1, true) ~= nil
			...
			if IsMulti then
				local count = #getCallbackValue()
				CountLabel.Text = count > 0 and (string.format("%d selected", count) .. (MaxSelected and string.format(" / %d", MaxSelected) or "")) or ""
			else
				CountLabel.Text = Select.Value ~= nil and "1 selected" or ""
			end
		end

		local function emit()
			refreshFilter()
			local value = getCallbackValue()
			Library:SafeCallback(Select.Callback, value)
			Library:SafeCallback(Select.Changed, value)
		end
		...
		for index, option in ipairs(Options) do
			...
			local Button = New("TextButton", {
				...
				New("UIStroke", {
					Transparency = 0.82,
					Thickness = math.max(1, GetStyleProperty("BorderThickness")),
					ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
					ThemeTag = { Color = "Accent" },
				}),
			})
			...
			Buttons[optionKey(option)] = data
			table.insert(ChipOrder, option)

			local baseTrans = 0.88
			local hoverTrans = 0.78
			local downTrans = 0.70
			Creator.AddSignal(Button.MouseEnter, function()
				if getSelected(option) then return end
				TweenService:Create(Button, TweenInfo.new(0.14, Enum.EasingStyle.Quint), { BackgroundTransparency = hoverTrans }):Play()
			end)
			Creator.AddSignal(Button.MouseLeave, function()
				if getSelected(option) then return end
				TweenService:Create(Button, TweenInfo.new(0.16, Enum.EasingStyle.Quint), { BackgroundTransparency = baseTrans }):Play()
			end)
			Creator.AddSignal(Button.MouseButton1Down, function()
				TweenService:Create(scaleObj, TweenInfo.new(0.1, Enum.EasingStyle.Quint), { Scale = 0.96 }):Play()
				TweenService:Create(Button, TweenInfo.new(0.08, Enum.EasingStyle.Quint), { BackgroundTransparency = downTrans }):Play()
			end)
			Creator.AddSignal(Button.MouseButton1Up, function()
				TweenService:Create(scaleObj, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = getSelected(option) and 1.025 or 1 }):Play()
			end)

			Creator.AddSignal(Button.MouseButton1Click, function()
				if IsMulti then
					local key = optionKey(option)
					local currentlySelected = Select.Value[key] == true
					if currentlySelected then
						if not AllowEmpty and #getCallbackValue() == 1 then
							return
						end
						Select.Value[key] = nil
					else
						local current = getCallbackValue()
					...
					if not AllowEmpty and getSelected(option) then
						return
					end
					Select.Value = getSelected(option) and nil or option
				end
		...
		if SearchBox then
			Creator.AddSignal(SearchBox:GetPropertyChangedSignal("Text"), refreshFilter)
		end
		...
		function Select:SetValue(value)
			if IsMulti then
				Select.Value = normalizeMulti(value)
				if MaxSelected then
					local limited = {}
					local count = 0
					for _, option in ipairs(Options) do
				...
				Select.Value = normalizeSingle(value)
				if not AllowEmpty and Select.Value == nil then
					Select.Value = Options[1]
				end
			end
			refreshAll(false)
		...
		function Select:Clear()
			if IsMulti then
				...
				Select.Value = AllowEmpty and nil or Options[1]
			end
			refreshAll(false)
			emit()
		end
		...
		function Select:Select(value)
			if IsMulti then
				local normalized = normalizeSingle(value)
				if normalized ~= nil then
					if MaxSelected and #getCallbackValue() >= MaxSelected and not Select.Value[optionKey(normalized)] then
						return
					end
					Select.Value[optionKey(normalized)] = true
				end
			else
				Select.Value = normalizeSingle(value)
			end
		...
		function Select:Deselect(value)
			if IsMulti then
				local normalized = normalizeSingle(value)
				if normalized ~= nil then
					Select.Value[optionKey(normalized)] = nil
				end
			else
				if getSelected(value) and AllowEmpty then
					Select.Value = nil
				end
			end
			refreshAll(false)
		...
		function Select:IsSelected(value)
			return getSelected(normalizeSingle(value))
		end

		function Select:OnChanged(Func)
			Select.Changed = Func
			Func(getCallbackValue())
		end

		function Select:Destroy()
			SGFrame.Frame:Destroy()
			if Idx then Library.Options[Idx] = nil end
		end

		refreshAll(true)
		refreshFilter()
		if Idx then Library.Options[Idx] = Select end
		return Select
	end
...
local SaveManager = {} do
	...
	SaveManager.Parser = {
		...
		SelectGroup = {
			Save = function(idx, object)
				return { type = "SelectGroup", idx = idx, value = object:GetValue() }
			end,
			Load = function(idx, data)
				if SaveManager.Options[idx] then
					SaveManager.Options[idx]:SetValue(data.value)
