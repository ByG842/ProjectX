...
Components.Window = (function()
	...
	return function(Config)
		...
		local ResizeGripIcon = New("ImageLabel", {
			...
			Size = UDim2.fromOffset(18, 18), -- ⬆️ ขยายให้เห็นชัดขึ้น (เดิม 12x12)
		...
		local ResizeCursorIcon = "rbxassetid://10734898934" -- lucide-move-diagonal-2 (ลูกศรเฉียง 2 หัว บอกว่า resize ได้)
		local DefaultMouseIcon = ""

		local function OnResizeHoverEnter()
			ResizeGripIcon.ImageTransparency = 0.05
			UserInputService.MouseIcon = ResizeCursorIcon
		end
		local function OnResizeHoverLeave()
			ResizeGripIcon.ImageTransparency = 0.45
			if not Resizing then
				UserInputService.MouseIcon = DefaultMouseIcon
			end
		end

		-- ✅ FIX: ผูก enter/leave กับ "ทั้ง" Frame แม่ และรูปไอคอนลูก
		-- เพราะรูปไอคอนวางทับอยู่ข้างในและมี ZIndex สูงกว่า พอเมาส์ชี้ตรงรูปพอดี
		-- Roblox จะยิง MouseLeave ของ Frame แม่ (นับว่าออกจาก Frame ไปอยู่บนรูปแทน)
		-- ถ้าไม่ผูกให้รูปลูกด้วย ไอคอนจะรีเซ็ตทันทีตอนเมาส์ขยับไปชี้ตรงรูปพอดี
