...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:SelectSubTab(SubTabIndex)
			...
			if PreviousSubTab > 0 and PreviousSubTab ~= SubTabIndex and self.SubTabs[PreviousSubTab] and self.SubTabs[SubTabIndex] then
				...
				-- ✨ AOS stagger เนื้อหาใน subtab ใหม่
				if self.SubTabs[SubTabIndex] and self.SubTabs[SubTabIndex].ContainerFrame then
					AOSStagger(self.SubTabs[SubTabIndex].ContainerFrame)
				end
	...
	-- ✨ AOS-style Stagger: แต่ละ card ค่อยๆ float ขึ้น + fade in ทีละอัน
	-- เหมือน Animate On Scroll บนเว็บ (gsap staggerFrom / aos.js)
	local function AOSStagger(ContainerFrame)
		if not ContainerFrame then return end

		-- รวบรวม children ที่เป็น GuiObject เรียงตาม LayoutOrder
		local cards = {}
		...
		-- ตั้งค่า timing — ช้าๆ สบายๆ แบบ AOS
		local FLOAT_Y    = 14    -- px ที่ลอยขึ้นมา (ขึ้นจากด้านล่าง)
		local DELAY_STEP = 0.07  -- วินาทีห่างแต่ละ card (70ms เหมือน AOS default)
		local DURATION   = 0.45  -- ความเร็ว tween แต่ละ card
		local EASE_STYLE = Enum.EasingStyle.Quart
		local EASE_DIR   = Enum.EasingDirection.Out

		-- snapshot position เดิม + เซต initial state
		local originals = {}
		for _, card in ipairs(cards) do
			originals[card] = {
				pos          = card.Position,
				transparency = card.BackgroundTransparency,
			}
			-- เริ่มต้น: ดัน Y ลงไป + ซ่อน (อย่าแตะ BackgroundTransparency เพราะ theme ใช้)
			card.Position = UDim2.new(
				card.Position.X.Scale,
				card.Position.X.Offset,
				card.Position.Y.Scale,
				card.Position.Y.Offset + FLOAT_Y
			)
		end

		-- fire stagger ทีละ card
		for i, card in ipairs(cards) do
			local delay = (i - 1) * DELAY_STEP
			local orig  = originals[card]

			task.delay(delay, function()
				if not card or not card.Parent then return end

				-- float ขึ้นมาสู่ position เดิม
				TweenService:Create(card,
					TweenInfo.new(DURATION, EASE_STYLE, EASE_DIR),
					{ Position = orig.pos }
				):Play()
			end)
		end
	...
	function TabModule:SelectTab(Tab)
		...
		if PreviousTab > 0 and PreviousTab ~= Tab and TabModule.Tabs[PreviousTab] and TabModule.Tabs[Tab] then
			...
			-- ✨ AOS stagger หลัง container slide เข้ามาแล้ว
			if TabModule.Tabs[Tab] and TabModule.Tabs[Tab].ContainerFrame then
				AOSStagger(TabModule.Tabs[Tab].ContainerFrame)
			end
			...
			-- ✨ AOS stagger สำหรับ first load (PreviousTab == 0)
			if TabModule.Tabs[Tab] and TabModule.Tabs[Tab].ContainerFrame then
				AOSStagger(TabModule.Tabs[Tab].ContainerFrame)
			end
		end
