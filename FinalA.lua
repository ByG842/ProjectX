...
-- 🔤 FontOverride — ตั้งค่าโดย Library:SetFont, ถ้าไม่ใช่ nil จะ override font
-- ของทุก style/theme (แยกอิสระจาก StyleThemes เพื่อไม่ให้ไปยุ่งกับ corner/border ของแต่ละ style)
local FontOverride = nil
local FontKeys = { FontRegular = true, FontMedium = true, FontBold = true, FontSemiBold = true }
...
local function GetStyleProperty(key)
	...
	if FontOverride and FontKeys[key] and FontOverride[key] then
		return FontOverride[key]
	end
	return style[key]
end
	...
	for _, entry in ipairs(StyleRegistry.Texts) do
		...
		if obj and obj.Parent then
			pcall(function()
				...
				local font = style[entry.fontKey] or style.FontMedium
				if FontOverride and FontKeys[entry.fontKey] and FontOverride[entry.fontKey] then
					font = FontOverride[entry.fontKey]
				end
				obj.FontFace = font
			end)
		end
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- 🔤 Font dropdown — ดึงรายชื่อฟอนต์ทั้งหมดที่ Roblox รองรับตอนนี้มาใส่ dropdown อัตโนมัติ
		-- (ใช้ Enum.Font:GetEnumItems() แทนการ hardcode list เพื่อกันชื่อฟอนต์ที่ไม่มีจริงทำให้ error)
		...
		local FontDropdown = section:AddDropdown("InterfaceFont", {
			...
			Description = "เปลี่ยนฟอนต์ของเมนูทั้งหมด มีให้เลือกเยอะมาก",

			Values = FontValues,

			Default = Settings.Font or "Default",
		...
		FontDropdown:SetValue(Settings.Font or "Default")

		-- apply saved font right away when UI loads
		task.defer(function()
			if Settings.Font and Settings.Font ~= "Default" then
		...
		-- 🌈 RGB Accent Color — ไล่สี Accent วนไปเรื่อยๆ อัตโนมัติแบบไฟปาร์ตี้
		-- ใช้ session id กันไม่ให้มี loop ค้างซ้อนกันหลายอันเวลาปิด/เปิดรัวๆ
		local RGBSessionId = 0

		local RGBAccentToggle = customThemeSection:AddToggle("RGBAccentColor", {
			Title = "RGB Accent Color",

			Description = "ไล่สี Accent วนไปเรื่อยๆ แบบไฟปาร์ตี้ 🌈 (ทับสีที่เลือกไว้ด้านบนชั่วคราว)",

			Default = Settings.RGBAccentColor or false,

			Callback = function(Value)

				Settings.RGBAccentColor = Value
				InterfaceManager:SaveSettings()

				RGBSessionId = RGBSessionId + 1
				local mySession = RGBSessionId

				if Value then
					task.spawn(function()
						while RGBSessionId == mySession and Settings.RGBAccentColor and not Library.Unloaded do
							local hue = (os.clock() * 0.15) % 1
							Library:SetAccentColor(Color3.fromHSV(hue, 0.85, 1), true)
							task.wait(0.03)
						end
					end)
				else
					-- กลับไปใช้สีที่เคยตั้งไว้เอง หรือสี default ของธีมปัจจุบัน
					if Settings.AccentColor and typeof(Settings.AccentColor) == "table" then
						local restoreColor = Color3.fromRGB(
							Settings.AccentColor.r or 200,
							Settings.AccentColor.g or 200,
							Settings.AccentColor.b or 200
						)
						Library:SetAccentColor(restoreColor, true)
					else
						Library:ResetAccentColor(true)
					end
				end
			end,
		})

		-- ถ้าเคยเปิด RGB ค้างไว้ ให้เริ่มไล่สีทันทีตอนโหลด UI
		if Settings.RGBAccentColor then
			task.defer(function()
				RGBAccentToggle:SetValue(true)
			end)
		end
...
-- 🔤 เปลี่ยนฟอนต์ของทั้ง UI โดยไม่ไปยุ่งกับ corner/border ของ style ปัจจุบัน
-- FontName = ชื่อ Enum.Font เช่น "Roboto", "Ubuntu", "Oswald" หรือ "Default" เพื่อกลับไปใช้ฟอนต์เดิมของ style
function Library:SetFont(FontName)
	...
	if not ok or typeof(enumFont) ~= "EnumItem" then
		return
	end

	local ok2, baseFont = pcall(function() return Font.fromEnum(enumFont) end)
	if not ok2 or not baseFont then
		return
	end

	local function BuildWeight(weight)
		local ok3, f = pcall(function() return Font.new(baseFont.Family, weight, baseFont.Style) end)
		if ok3 and f then return f end
		return baseFont
	end

	FontOverride = {
		FontRegular  = BuildWeight(Enum.FontWeight.Regular),
		FontMedium   = BuildWeight(Enum.FontWeight.Medium),
		FontSemiBold = BuildWeight(Enum.FontWeight.SemiBold),
		FontBold     = BuildWeight(Enum.FontWeight.Bold),
	}
