...
local StyleThemes = {
	Default = {
		-- UICorner — 🪟 ปรับให้โค้งมนขึ้นแบบ macOS (เดิม Fluent จะเหลี่ยมกว่านี้)
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		-- MAIN TITLEBAR FRAME — height 52px, เค้าโครงแบบ macOS: ปุ่มควบคุมสามดวงชิดซ้าย + ชื่อกึ่งกลาง
		TitleBar.Frame = New("Frame", {
			...
			-- 🚦 TrafficLights — โซนซ้ายสุด แทนที่ Min/Max/Close แบบ Fluent เดิม
			New("Frame", {
				Name = "LeftSection",
			...
			-- 🏷️ CenterSection — โลโก้ + ชื่อหน้าต่าง วางกึ่งกลางแถบเสมอ ไม่ผูกกับความกว้างซ้าย/ขวา
			New("Frame", {
				Name = "CenterSection",
				AnchorPoint = Vector2.new(0.5, 0.5),
				Position = UDim2.new(0.5, 0, 0.5, 0),
				...
				-- ชื่อหน้าต่าง เดี่ยว บรรทัดเดียว กึ่งกลางแบบ macOS (ไม่มี SubTitle ซ้อนใต้ชื่อ)
				New("TextLabel", {
					RichText = true,
					Text = Config.SubTitle and (Config.Title .. "  •  " .. Config.SubTitle) or Config.Title,
					FontFace = GetStyleProperty("FontSemiBold"),
					TextSize = GetStyleProperty("TextSizeSm"),
			...
			-- โซนขวา เว้นว่างไว้ให้สมดุลกับ TrafficLights ฝั่งซ้าย (แบบ macOS ไม่มีปุ่มฝั่งขวา)
			New("Frame", {
				Name = "RightSection",
				Size = UDim2.new(0, 90, 1, 0),
				AnchorPoint = Vector2.new(1, 0),
		...
		-- 🚦 ปุ่มควบคุมแบบจุดกลม 3 สี (แดง/เหลือง/เขียว) สไตล์ macOS
		-- ไอคอนซ่อนอยู่ข้างในจุด จะปรากฏให้เห็นตอน hover เท่านั้น เหมือน traffic light จริง
		local function TrafficLightButton(Icon, DotColor, Parent, Callback)
			...
			Button.Frame = New("TextButton", {
				...
				New("ImageLabel", {
					...
					BackgroundTransparency = 1,
					ImageTransparency = 1,
					ImageColor3 = Color3.fromRGB(60, 20, 0),
					Name = "Icon",
				}),
			})

			local Motor, SetIconTransparency = Creator.SpringMotor(1, Button.Frame.Icon, "ImageTransparency")

			AddSignal(Button.Frame.MouseEnter, function()
				SetIconTransparency(0.1)
			end)
			AddSignal(Button.Frame.MouseLeave, function()
				SetIconTransparency(1, true)
			end)
			AddSignal(Button.Frame.MouseButton1Click, Button.Callback)
		...
		-- ลำดับมาตรฐานของ macOS: ปิด (แดง) → ย่อ (เหลือง) → ขยาย (เขียว)
		TitleBar.CloseButton = TrafficLightButton(Components.Assets.Close, Color3.fromRGB(255, 95, 87), leftSection, function()
		...
		TitleBar.MinButton = TrafficLightButton(Components.Assets.Min, Color3.fromRGB(254, 188, 46), leftSection, function()
			Library.Window:Minimize()
		end)
		TitleBar.MaxButton = TrafficLightButton(Components.Assets.Max, Color3.fromRGB(40, 200, 64), leftSection, function()
