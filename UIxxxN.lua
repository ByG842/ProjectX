...
Components.Notification = (function()
	...
	function Notification:New(Config)
		...
		function NewNotification:Open()
			...
			-- Progress bar animation — ไล่หด + เปลี่ยนสีเป็นแดงเตือนตอนใกล้หมดเวลา ให้เห็นชัดว่ากำลังจะปิด
			if Config.Duration and Config.Duration > 0 then
				local steps    = Config.Duration * 20
				local stepSize = 1 / steps
				local warnColor = Color3.fromRGB(235, 90, 90)
				task.spawn(function()
					for i = 1, steps do
						if NewNotification.Closed then break end
						local pct = 1 - (i * stepSize)
						local fillColor = accentColor
						if pct <= 0.25 then
							-- 25% สุดท้าย: ไล่สีจาก accent ไปแดงเตือน ยิ่งใกล้หมดยิ่งแดงเข้ม
							fillColor = accentColor:Lerp(warnColor, 1 - (pct / 0.25))
						end
						TweenService:Create(ProgressFill,
							TweenInfo.new(1 / 20, Enum.EasingStyle.Linear),
							{ Size = UDim2.new(pct, 0, 1, 0), BackgroundColor3 = fillColor }
						):Play()
						task.wait(1 / 20)
					end
				end)
...
Components.Window = (function()
	...
	return function(Config)
		...
		local function UpdateElementVisibility(searchTerm)
			...
			-- 🔍 Empty state: โชว์ตอนพิมพ์ค้นหาแล้วไม่เจอ element ไหนตรงเลยสักตัว
			if Window.EmptyStateFrame then
				Window.EmptyStateFrame.Visible = (normalizedQuery ~= "" and visibleCount == 0)
			end
		...
		-- 🔍 Empty state — โชว์ตอน search แล้วไม่เจอ element ไหนตรงเลย แทนหน้าว่างเปล่า
		Window.EmptyStateFrame = New("Frame", {
			Size = UDim2.fromScale(1, 1),
			BackgroundTransparency = 1,
			Visible = false,
			ZIndex = 5,
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- 🎨 Custom Accent Color แยกจาก preset theme
		...
		local AccentColorPicker = section:AddColorpicker("CustomAccentColor", {
			Title = "Accent Color",
			Description = "ปรับสี Accent เองโดยไม่ต้องเปลี่ยนทั้งธีม",
		...
		local ResetAccentButton = section:AddActionButton("ResetAccentColor", {
...
-- 🌀 LoadingWindow — หน้าจอโหลดแยกต่างหาก โผล่ก่อนหน้าต่างหลัก ใช้ตอนสคริปต์ต้อง fetch ข้อมูล/setup อะไรก่อน
-- วิธีใช้:
--   local Loading = Library:LoadingWindow({ Title = "MyScript", SubTitle = "Loading...", Icon = true })
--   Loading:SetStatus("Fetching data...")
--   Loading:SetProgress(0.6)  -- 0 ถึง 1
--   Loading:Close()           -- ปิดตัวเอง (fade out) ก่อนค่อยเรียก Library:CreateWindow ต่อ
function Library:LoadingWindow(Config)
	Config = Config or {}

	local title    = Config.Title or "Loading"
	local subtitle = Config.SubTitle or ""
	local size     = Config.Size or UDim2.fromOffset(400, 230)
	...
	elseif type(Config.Icon) == "string" and Config.Icon ~= "" then
		local ok, resolved = pcall(function() return Library:GetIcon(Config.Icon) end)
	...
	local LW = { Closed = false }

	LW.Root = New("CanvasGroup", {
		Name = "LoadingWindow",
		Size = size,
		Position = UDim2.fromScale(0.5, 0.5),
		AnchorPoint = Vector2.new(0.5, 0.5),
	...
	-- 🌫️ Acrylic background ให้ดู premium เหมือนหน้าต่างหลัก/notification
	local LWAcrylic = Acrylic.AcrylicPaint()
	LWAcrylic.Frame.Parent = LW.Root
	LWAcrylic.Frame.ZIndex = 0

	local iconCenterY = 62

	-- 🌟 Glow เต้นเบาๆ หลังโลโก้ (breathing pulse)
	if iconAsset then
		local Glow = New("Frame", {
			Size = UDim2.fromOffset(90, 90),
			Position = UDim2.new(0.5, 0, 0, iconCenterY),
			AnchorPoint = Vector2.new(0.5, 0.5),
			BackgroundColor3 = accent,
			BackgroundTransparency = 0.82,
			ZIndex = 1,
		...
		local GlowScale = New("UIScale", { Scale = 0.85 })
		GlowScale.Parent = Glow
		task.spawn(function()
			while LW.Root and LW.Root.Parent do
				TweenService:Create(GlowScale, TweenInfo.new(1.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Scale = 1.08 }):Play()
				TweenService:Create(Glow, TweenInfo.new(1.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { BackgroundTransparency = 0.7 }):Play()
				task.wait(1.1)
				if not (LW.Root and LW.Root.Parent) then break end
				TweenService:Create(GlowScale, TweenInfo.new(1.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Scale = 0.85 }):Play()
				TweenService:Create(Glow, TweenInfo.new(1.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { BackgroundTransparency = 0.82 }):Play()
				task.wait(1.1)
			end
		end)
		...
		local Spinner = New("ImageLabel", {
			Image = "rbxassetid://10723433935", -- lucide-loader-2
		...
		TweenService:Create(Spinner, TweenInfo.new(1, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1, false), { Rotation = 360 }):Play()
	...
	local textTop = iconAsset and 118 or 30

	New("TextLabel", {
		Text = title,
		FontFace = GetStyleProperty("FontBold"),
		TextSize = GetStyleProperty("TextSizeLg"),
		Position = UDim2.new(0.5, 0, 0, textTop),
		AnchorPoint = Vector2.new(0.5, 0),
	...
	LW.StatusLabel = New("TextLabel", {
		Text = subtitle,
		FontFace = GetStyleProperty("FontMedium"),
		TextSize = GetStyleProperty("TextSizeSm"),
		Position = UDim2.new(0.5, 0, 0, textTop + 26),
		AnchorPoint = Vector2.new(0.5, 0),
	...
	local ShimmerGradient = LW.ProgressFill.Shimmer
	task.spawn(function()
		while LW.Root and LW.Root.Parent do
			ShimmerGradient.Offset = Vector2.new(-1, 0)
			TweenService:Create(ShimmerGradient, TweenInfo.new(1.1, Enum.EasingStyle.Linear), { Offset = Vector2.new(1, 0) }):Play()
			task.wait(1.1)
		end
	end)

	LW.PercentLabel = New("TextLabel", {
		Text = "0%",
		FontFace = GetStyleProperty("FontBold"),
		TextSize = GetStyleProperty("TextSizeXs"),
	...
	-- 🎬 Entrance animation: pop-in นุ่มๆ แทนโผล่มาทันที
	local EntranceScale = New("UIScale", { Scale = 0.88 })
	EntranceScale.Parent = LW.Root
	LW.Root.GroupTransparency = 1
	TweenService:Create(LW.Root, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { GroupTransparency = 0 }):Play()
	TweenService:Create(EntranceScale, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()

	local statusToken = 0
	function LW:SetStatus(Text)
		if LW.Closed then return end
		statusToken = statusToken + 1
		local myToken = statusToken
		task.spawn(function()
			TweenService:Create(LW.StatusLabel, TweenInfo.new(0.12), { TextTransparency = 1 }):Play()
			task.wait(0.12)
			if myToken ~= statusToken then return end
			LW.StatusLabel.Text = tostring(Text or "")
			TweenService:Create(LW.StatusLabel, TweenInfo.new(0.15), { TextTransparency = 0 }):Play()
		end)
	end
	...
	progressValue:GetPropertyChangedSignal("Value"):Connect(function()
		LW.PercentLabel.Text = math.floor(progressValue.Value * 100) .. "%"
	end)

	function LW:SetProgress(Pct)
		if LW.Closed then return end
		Pct = math.clamp(tonumber(Pct) or 0, 0, 1)
		TweenService:Create(LW.ProgressFill, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			Size = UDim2.new(Pct, 0, 1, 0),
		}):Play()
		TweenService:Create(progressValue, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			Value = Pct,
		}):Play()
	end
	...
	function LW:Close()
		if LW.Closed then return end
		LW.Closed = true
		local tw = TweenService:Create(LW.Root, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
			GroupTransparency = 1,
		})
		tw:Play()
		task.delay(0.25, function()
			if progressValue then progressValue:Destroy() end
			if LW.Root then LW.Root:Destroy() end
		end)
	end
