...
-- ============================================================
--  CreateMinimizer (legacy alias) → ใช้ CreateToolbar แทนแล้ว
-- ============================================================
function Library:CreateMinimizer(Config)
	return self:CreateToolbar(Config)
end

-- ============================================================
--  CreateToolbar
--  Floating vertical toolbar ที่แทน Minimizer button เดิม
--
--  Config:
--    Icon        = "rbxassetid://..."  หรือ lucide icon name (logo บนสุด)
--    Title       = "MY HUB"            ข้อความใต้ logo (optional)
--    Position    = UDim2               ตำแหน่งเริ่มต้น
--    Buttons     = {                   ปุ่มเพิ่มเติมด้านล่าง
--      { Icon = "lucide-settings",  Tooltip = "Settings", Callback = fn },
--      { Icon = "lucide-eye",       Tooltip = "ESP",      Callback = fn },
--      { Separator = true },          -- เส้นคั่น
--      { Icon = "lucide-x",         Tooltip = "Close",    Callback = fn },
--    }
--    ShowToggle  = true                แสดงปุ่ม toggle UI (default true)
--    Acrylic     = false
--
--  วิธีใช้:
--    local Toolbar = Library:CreateToolbar({
--        Icon    = "lucide-star",
--        Title   = "MyScript",
--        Buttons = {
--            { Icon = "lucide-settings", Tooltip = "Config", Callback = function()
--                Library.Window:Minimize()
--            end },
--            { Separator = true },
--            { Icon = "lucide-power", Tooltip = "Unload", Callback = function()
--                Library:Destroy()
--            end },
--        },
--    })
-- ============================================================
function Library:CreateToolbar(Config)
	Config = Config or {}

	-- ถ้ามี Toolbar อยู่แล้วให้คืนตัวเดิม
	...
	local showToggle = (Config.ShowToggle ~= false)

	-- ── Resolve logo icon ──
	local logoAsset = "rbxassetid://10734897102" -- default moon icon
	if type(Config.Icon) == "string" and Config.Icon ~= "" then
		pcall(function()
			local r = Library:GetIcon(Config.Icon)
			if r then
				logoAsset = r
			elseif Config.Icon:match("^rbxassetid://%d+$") then
				logoAsset = Config.Icon
			end
		end)
	end
	...
	local function getThemeColor(key)
		local t = Creator.Themes and Creator.Themes[Library.Theme or "Dark"]
		return t and t[key] or Color3.fromRGB(105, 215, 255)
	end

	-- ── Toolbar width / item size ──
	local TOOLBAR_W  = isMobile and 42 or 46
	local ITEM_SIZE  = isMobile and 36 or 38
	local LOGO_SIZE  = isMobile and 38 or 40
	local CORNER_R   = 14
	local PAD_V      = 6   -- padding บน-ล่างข้างใน
	local GAP        = 4   -- ระยะห่างระหว่าง item

	-- ── Toolbar holder (Auto Y) ──
	local defaultPos = isMobile
		and UDim2.new(0.5, -TOOLBAR_W/2, 0, 16)
		or  UDim2.new(0, 20, 0.5, -100)

	local Holder = New("Frame", {
		Name               = "FluentToolbar",
		Parent             = parentGui,
		Size               = UDim2.fromOffset(TOOLBAR_W, 10),
		AutomaticSize      = Enum.AutomaticSize.Y,
		Position           = Config.Position or defaultPos,
		BackgroundTransparency = 1,
		ZIndex             = 999999999,
		Visible            = (Config.Visible ~= false),
	})

	-- ── Toolbar background card ──
	local Card = New("Frame", {
		...
		New("UICorner",  { CornerRadius = UDim.new(0, CORNER_R) }),
		New("UIStroke",  {
			ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
			Transparency    = 0.45,
			Thickness       = 1.2,
		...
		-- inner glow ขอบบน (glassmorphism highlight)
		New("Frame", {
			Size               = UDim2.new(0.7, 0, 0, 1),
			Position           = UDim2.new(0.15, 0, 0, 1),
			BackgroundColor3   = Color3.fromRGB(255, 255, 255),
	...
	local ContentList = New("Frame", {
		...
		New("UIPadding", {
			PaddingTop    = UDim.new(0, PAD_V),
			PaddingBottom = UDim.new(0, PAD_V),
			PaddingLeft   = UDim.new(0, 4),
			PaddingRight  = UDim.new(0, 4),
		}),
	})

	-- ── Helper: สร้าง tooltip label ──
	...
	local function showTooltip(btn, text)
		...
		local tip = New("TextLabel", {
			...
			BackgroundTransparency = 0.1,
			TextColor3       = Color3.fromRGB(230, 232, 245),
			Size             = UDim2.fromOffset(0, 22),
			AutomaticSize    = Enum.AutomaticSize.X,
			ZIndex           = 9999999999,
			Parent           = parentGui,
		...
		ActiveTooltip = tip
		-- วางตำแหน่ง tooltip ทางขวา toolbar
		task.spawn(function()
			task.wait()
			if tip and tip.Parent then
				...
				tip.Position = UDim2.fromOffset(
					holderAbs.X + TOOLBAR_W + 8,
					abs.Y + (btn.AbsoluteSize.Y / 2) - 11
				)
			end
		end)
	end
	local function hideTooltip()
		if ActiveTooltip then
			TweenService:Create(ActiveTooltip, TweenInfo.new(0.1), { TextTransparency = 1, BackgroundTransparency = 1 }):Play()
	...
	-- ── Helper: สร้าง toolbar item button ──
	local function makeItemButton(iconAsset, tooltip, layoutOrder, isAccent)
		local accent = getThemeColor("Accent")

		local Btn = New("TextButton", {
			Name               = "ToolbarBtn",
			Size               = UDim2.fromOffset(ITEM_SIZE, ITEM_SIZE),
			BackgroundTransparency = 0.88,
			BackgroundColor3   = isAccent and accent or Color3.fromRGB(255, 255, 255),
			Text               = "",
			AutoButtonColor    = false,
			...
			New("ImageLabel", {
				...
				AnchorPoint      = Vector2.new(0.5, 0.5),
				BackgroundTransparency = 1,
				ThemeTag         = { ImageColor3 = isAccent and "AcrylicMain" or "SubText" },
			}),
		})
		...
		Creator.AddSignal(Btn.MouseEnter, function()
			...
			if ico then
				TweenService:Create(ico, TweenInfo.new(0.15), { ImageColor3 = Color3.fromRGB(255,255,255) }):Play()
			end
		...
		Creator.AddSignal(Btn.MouseLeave, function()
			...
			local ico = Btn:FindFirstChild("Icon")
			if ico then
				TweenService:Create(ico, TweenInfo.new(0.15), {
					ImageColor3 = isAccent and Color3.fromRGB(255,255,255) or getThemeColor("SubText"),
				}):Play()
			end
			hideTooltip()
		end)
		Creator.AddSignal(Btn.MouseButton1Down, function()
			TweenService:Create(Btn, TweenInfo.new(0.08), { BackgroundTransparency = 0.55 }):Play()
			-- squish scale effect
			local scale = Btn:FindFirstChildOfClass("UIScale") or New("UIScale", { Parent = Btn })
			TweenService:Create(scale, TweenInfo.new(0.08, Enum.EasingStyle.Back), { Scale = 0.9 }):Play()
		end)
		Creator.AddSignal(Btn.MouseButton1Up, function()
	...
	local function makeSeparator(order)
		New("Frame", {
			Size               = UDim2.fromOffset(TOOLBAR_W - 16, 1),
			BackgroundTransparency = 0.6,
			LayoutOrder        = order,
			Parent             = ContentList,
			ThemeTag           = { BackgroundColor3 = "InElementBorder" },
	...
	local LogoBtn = New("TextButton", {
		...
		New("UIGradient", {
			Color    = ColorSequence.new{
				...
				ColorSequenceKeypoint.new(1, getThemeColor("Accent")),
			},
			Rotation = 135,
			Transparency = NumberSequence.new{
	...
	-- breathing pulse บน logo เมื่อ minimize
	local pulseConn
	local function startLogoPulse()
		if pulseConn then return end
		local stroke = LogoBtn:FindFirstChildOfClass("UIStroke")
		if not stroke then return end
		local function doPulse()
			if not Library.Window or not Library.Window.Minimized then return end
			TweenService:Create(stroke, TweenInfo.new(0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = 0.05 }):Play()
			task.delay(0.9, function()
				if not Library.Window or not Library.Window.Minimized then
					TweenService:Create(stroke, TweenInfo.new(0.3), { Transparency = 0.35 }):Play()
					return
				end
				TweenService:Create(stroke, TweenInfo.new(0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = 0.55 }):Play()
				task.delay(0.9, doPulse)
			end)
		end
		doPulse()
	end

	-- Title ใต้ logo (optional)
	if Config.Title and Config.Title ~= "" then
		...
		New("TextLabel", {
			...
			FontFace         = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold),
			TextSize         = 8,
			TextColor3       = getThemeColor("Accent"),
			BackgroundTransparency = 1,
			Size             = UDim2.fromOffset(TOOLBAR_W - 8, 14),
			TextXAlignment   = Enum.TextXAlignment.Center,
			LayoutOrder      = 3,
			Parent           = ContentList,
			TextTransparency = 0.2,
	...
	-- ════════════════════════════════════════
	--  TOGGLE UI BUTTON (แสดง/ซ่อน window)
	-- ════════════════════════════════════════
	local toggleIconOpen   = Library:GetIcon("lucide-layout-dashboard") or "rbxassetid://10734897102"
	local toggleIconClosed = Library:GetIcon("lucide-eye-off") or "rbxassetid://10734897102"

	if showToggle then
		local ToggleBtn = makeItemButton(toggleIconOpen, "Toggle UI", 5, false)
		Creator.AddSignal(ToggleBtn.MouseButton1Click, function()
			task.wait(0.1)
			if Library.Window then
				Library.Window:Minimize()
				local ico = ToggleBtn:FindFirstChild("Icon")
				if ico then
					ico.Image = Library.Window.Minimized and toggleIconClosed or toggleIconOpen
				end
				if Library.Window.Minimized then
					startLogoPulse()
				end
			end
		end)
	...
	-- ════════════════════════════════════════
	--  CUSTOM BUTTONS จาก Config.Buttons
	-- ════════════════════════════════════════
	local btnOrder = 6
	if Config.Buttons and type(Config.Buttons) == "table" then
		for _, btnCfg in ipairs(Config.Buttons) do
			if btnCfg.Separator then
				makeSeparator(btnOrder)
			else
				local iconResolved = "rbxassetid://10734897102"
				if type(btnCfg.Icon) == "string" and btnCfg.Icon ~= "" then
					pcall(function()
						local r = Library:GetIcon(btnCfg.Icon)
						if r then iconResolved = r
						elseif btnCfg.Icon:match("^rbxassetid://%d+$") then
							iconResolved = btnCfg.Icon
						end
					end)
				end
				local ItemBtn = makeItemButton(iconResolved, btnCfg.Tooltip or "", btnOrder, btnCfg.Accent or false)
				...
				-- active state toggle (สำหรับปุ่มที่ toggle-able)
				if btnCfg.Toggle then
					local active = btnCfg.DefaultActive or false
					local function refreshActive()
						local accent = getThemeColor("Accent")
						TweenService:Create(ItemBtn, TweenInfo.new(0.18), {
							BackgroundTransparency = active and 0.55 or 0.88,
							BackgroundColor3       = active and accent or Color3.fromRGB(255,255,255),
						}):Play()
						local ico = ItemBtn:FindFirstChild("Icon")
						if ico then
							TweenService:Create(ico, TweenInfo.new(0.18), {
								ImageColor3 = active and Color3.fromRGB(255,255,255) or getThemeColor("SubText"),
							}):Play()
						end
					end
					refreshActive()
					Creator.AddSignal(ItemBtn.MouseButton1Click, function()
						...
						active = not active
						refreshActive()
						if btnCfg.Callback then Library:SafeCallback(btnCfg.Callback, active) end
					end)
				end
			end
			btnOrder = btnOrder + 1
	...
	-- ════════════════════════════════════════
	--  LOGO click → toggle UI
	-- ════════════════════════════════════════
	Creator.AddSignal(LogoBtn.MouseEnter, function()
		TweenService:Create(LogoBtn, TweenInfo.new(0.15), { BackgroundTransparency = 0.65 }):Play()
		showTooltip(LogoBtn, "Toggle UI")
	end)
	Creator.AddSignal(LogoBtn.MouseLeave, function()
		TweenService:Create(LogoBtn, TweenInfo.new(0.15), { BackgroundTransparency = 0.82 }):Play()
		hideTooltip()
	end)
	Creator.AddSignal(LogoBtn.MouseButton1Down, function()
		local s = LogoBtn:FindFirstChildOfClass("UIScale") or New("UIScale", { Parent = LogoBtn })
		TweenService:Create(s, TweenInfo.new(0.09, Enum.EasingStyle.Back), { Scale = 0.88 }):Play()
	end)
	Creator.AddSignal(LogoBtn.MouseButton1Up, function()
		local s = LogoBtn:FindFirstChildOfClass("UIScale")
		if s then TweenService:Create(s, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play() end
	end)
	Creator.AddSignal(LogoBtn.MouseButton1Click, function()
		task.wait(0.1)
		if Library.Window then
			Library.Window:Minimize()
			if Library.Window.Minimized then startLogoPulse() end
		end
	end)

	-- ════════════════════════════════════════
	--  DRAG — จับที่ Card ลากได้ทั้งแถบ
	-- ════════════════════════════════════════
	...
	-- ════════════════════════════════════════
	--  Entrance animation (slide in จากซ้าย)
	-- ════════════════════════════════════════
	task.defer(function()
		local finalPos = Holder.Position
		Holder.Position = UDim2.new(
			finalPos.X.Scale, finalPos.X.Offset - 30,
			finalPos.Y.Scale, finalPos.Y.Offset
		)
...
-- Legacy MinimizeButton / MobileMinimizeButton ถูกแทนที่ด้วย CreateToolbar แล้ว
-- (ระบบ drag + toggle ทั้งหมดอยู่ใน CreateToolbar)
