...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- ลิงก์ http(s) ธรรมดา (เช่น ลิงก์ Discord CDN, Imgur ฯลฯ) วางตรงๆ ใช้ไม่ได้ — เอนจิ้นไม่โหลดรูปจากเว็บภายนอกให้ตรงๆ
		-- ทางที่ทำได้จริงมี 3 แบบ:
		--   1) rbxassetid:// (อัปโหลดรูปขึ้น Roblox เองก่อนแล้วเอา id มาวาง) — ใช้ได้ทุก executor แน่นอนที่สุด
		--   2) วางลิงก์ http(s) แล้วให้สคริปต์โหลดมา cache ไว้ในเครื่องเอง แล้วแปลงเป็น content id ด้วย getcustomasset — ต้องมีทั้งฟังก์ชันยิง HTTP (http_request/request/syn.request) + writefile + getcustomasset ถึงจะทำได้ (ส่วนใหญ่มีครบ)
		--   3) วาง path ไฟล์ภาพที่มีอยู่ในเครื่องอยู่แล้ว แปลงผ่าน getcustomasset ตรงๆ (ไม่ต้องโหลดเน็ต)
		...
		-- โหลดรูปจากลิงก์ http(s) มา cache เป็นไฟล์ในเครื่องก่อน แล้วค่อยแปลงเป็น content id
		-- (cache ไว้ถาวรในไฟล์ ครั้งต่อไปที่รันสคริปต์ถ้าลิงก์เดิมตายไปแล้ว รูปเก่าที่เคย cache ไว้ก็ยังใช้ได้ ไม่ต้องพึ่งเน็ตซ้ำ)
		local function DownloadAndCacheImage(url)
			...
			local ok, response = pcall(HttpRequestFn, { Url = url, Method = "GET" })
			...
			local ext = (url:match("%.(%a+)%?") or url:match("%.(%a+)$") or "png"):lower()
			...
			local path = "FluentCache/custom_background." .. ext
		...
		local function ApplyCustomBackground(RawValue, Silent)
			...
			if Value:match("^rbxassetid://") or Value:match("^rbxasset://") or Value:match("^rbxthumb://") then
				-- content id ของ Roblox เอง ใช้ตรงๆ ได้เลย
				finalImage = Value
			elseif Value:match("^https?://") then
				-- ลิงก์เว็บภายนอก → ต้องโหลดมา cache ก่อน จะใช้ตรงๆ ไม่ได้
				local contentId, err = DownloadAndCacheImage(Value)
				...
				finalImage = contentId
			else
				-- ไม่ใช่ลิงก์/asset id → ลองตีความว่าเป็น path ไฟล์ในเครื่อง
				if hasLocalFileSupport and isfile(Value) then
					...
					if ok and contentId then
						...
						if not Silent then
							Library:Notify({ Title = "Custom Background", Content = "โหลดภาพจากไฟล์นี้ไม่สำเร็จ", Duration = 5 })
						end
						return
					end
				else
					if not Silent then
						local reason = hasLocalFileSupport and "ไม่พบไฟล์ตาม path ที่ให้มา" or "รูปแบบที่วางไม่ใช่ rbxassetid://, ลิงก์เว็บ, หรือ path ไฟล์ที่ใช้ได้"
						Library:Notify({ Title = "Custom Background", Content = reason, Duration = 6 })
					end
					return
				end
			end
			...
			if Library.Window then
				Library.Window:SetBackgroundImage(finalImage)
		...
		local BackgroundOpacitySlider = customBgSection:AddSlider("CustomBackgroundOpacity", {
			...
			Callback = function(Value)
				...
				if Library.Window then
					Library.Window:SetBackgroundImageTransparency(transparency)
				end
