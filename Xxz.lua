...
-- 🌐 ตัวช่วยกลาง: แปลงค่าที่ผู้ใช้กรอกมา (rbxassetid:// / ลิงก์เว็บ http(s) / path ไฟล์ในเครื่อง) ให้กลายเป็น
-- content id ที่ใช้กับ property Image ของ ImageLabel ได้จริง — ใช้ร่วมกันทั้ง Custom Background (Settings > Interface)
-- และ Element ประเภท Image เพื่อไม่ให้ต้องเขียน logic โหลด/cache รูปซ้ำหลายที่
-- คืนค่า (contentId, nil) ถ้าสำเร็จ หรือ (nil, errorMessage) ถ้าไม่สำเร็จ
-- CacheKey ใช้กันไฟล์ cache ชนกันเวลามีรูปหลายอันพร้อมกัน (เช่น "custom_background", "photo_1" ...)
function Library:ResolveImageSource(RawValue, CacheKey)
	local Value = (RawValue or ""):gsub("^%s+", ""):gsub("%s+$", "")
	if Value == "" then
		return nil, "ไม่ได้ใส่ค่าอะไรมา"
	end

	-- content id ของ Roblox เอง ใช้ตรงๆ ได้เลย ไม่ต้องแปลงอะไร
	if Value:match("^rbxassetid://") or Value:match("^rbxasset://") or Value:match("^rbxthumb://") then
		return Value, nil
	end

	-- ลิงก์เว็บภายนอก → Roblox ไม่โหลดให้ตรงๆ ต้องโหลดมา cache เป็นไฟล์ในเครื่องก่อน แล้วค่อยแปลงผ่าน getcustomasset
	if Value:match("^https?://") then
		local HttpRequestFn = (syn and syn.request) or http_request or request or (http and http.request)
		local hasUrlDownloadSupport = (typeof(HttpRequestFn) == "function") and (typeof(writefile) == "function") and (typeof(getcustomasset) == "function")
		if not hasUrlDownloadSupport then
			return nil, "executor นี้ไม่รองรับการโหลดรูปจากลิงก์ (ต้องมี HTTP request + writefile + getcustomasset) — ใช้ rbxassetid:// แทน"
		end
		local ok, response = pcall(HttpRequestFn, { Url = Value, Method = "GET" })
		if not ok or not response or not response.Body or response.Body == "" then
			return nil, "โหลดรูปจากลิงก์นี้ไม่สำเร็จ (ลิงก์อาจหมดอายุ/ผิด — ลิงก์ Discord CDN จะหมดอายุเองตามเวลา)"
		end
		local ext = (Value:match("%.(%a+)%?") or Value:match("%.(%a+)$") or "png"):lower()
		if not table.find({ "png", "jpg", "jpeg", "gif" }, ext) then ext = "png" end
		if makefolder and isfolder and not isfolder("FluentCache") then
			pcall(makefolder, "FluentCache")
		end
		local path = "FluentCache/" .. (CacheKey or "image") .. "." .. ext
		local wok = pcall(writefile, path, response.Body)
		if not wok then
			return nil, "เขียนไฟล์ภาพลง cache ไม่สำเร็จ"
		end
		local cok, contentId = pcall(getcustomasset, path)
		if not cok or not contentId then
			return nil, "แปลงไฟล์ที่โหลดมาเป็นรูปไม่สำเร็จ"
		end
		return contentId, nil
	end

	-- ไม่ใช่ทั้ง asset id และลิงก์ → ลองตีความว่าเป็น path ไฟล์ในเครื่อง
	local hasLocalFileSupport = (typeof(getcustomasset) == "function") and (typeof(isfile) == "function")
	if hasLocalFileSupport and isfile(Value) then
		local ok, contentId = pcall(getcustomasset, Value)
		if ok and contentId then
			return contentId, nil
		end
		return nil, "โหลดภาพจากไฟล์นี้ไม่สำเร็จ"
	end

	return nil, hasLocalFileSupport and "ไม่พบไฟล์ตาม path ที่ให้มา" or "รูปแบบที่วางไม่ใช่ rbxassetid://, ลิงก์เว็บ, หรือ path ไฟล์ที่ใช้ได้"
end
...
ElementsTable.Input = (function()
	...
	function Element:New(Idx, Config)
		...
		-- [ จุดที่แก้ ] เดิม DescLabel กว้างเต็มแถว (1,0) ทับกับ Textbox ที่ลอยอยู่ทางขวา
		-- ถ้า Description ยาวจนขึ้นบรรทัดที่ 2 ตัวหนังสือจะไปทับกับกล่อง input พอดี (บั๊กเดียวกับที่ Slider เคยแก้ไว้ด้วย -170)
		-- หด DescLabel ให้เหลือพื้นที่ให้ Textbox (กว้าง 160 + เว้นขอบ 10) ไม่ให้ตัวหนังสือไปทับกัน
...
-- 🖼️ Image — โชว์รูปภาพเดี่ยวๆ ในแถบ element เช่น โลโก้/แบนเนอร์/รูปที่ผู้ใช้ตั้งเอง
-- ใช้: Section:AddImage("MyPhoto", { Photo = "rbxassetid://..." }) หรือ Photo = ลิงก์เว็บ / path ไฟล์ในเครื่องก็ได้
-- (แปลงค่าผ่าน Library:ResolveImageSource ตัวเดียวกับที่ Custom Background ใน Settings > Interface ใช้)
ElementsTable.Image = (function()
	...
	function Element:New(Idx, Config)
		...
		local Img = {
			Type  = "Image",
			Value = Config.Photo or Config.Image or "",
		}

		local ImgFrame = Components.Element(Config.Title or "", Config.Description, self.Container, false, Config)
		Img.SetTitle = ImgFrame.SetTitle
		Img.SetDesc  = ImgFrame.SetDesc
		Img.Visible  = ImgFrame.Visible
		Img.Elements = ImgFrame
		-- ถ้าไม่มี Title/Description ให้ซ่อนแถบหัวไปเลย เหลือแค่กล่องรูป
		if (Config.Title == nil or Config.Title == "") and (Config.Description == nil or Config.Description == "") then
			ImgFrame.Header.Visible = false
		end

		-- ── กล่องรูป ──────────────────────────────────────────────
		-- ปกติ (ไม่ใส่ AspectRatio): เต็มความกว้างแถว สูงตาม Height
		-- ใส่ Config.AspectRatio (เช่น 1 = สี่เหลี่ยมจัตุรัส): ขนาดคงที่ กว้าง x สูง ตาม Width/Height วางกลางแถวแทน
		local PhotoHeight = Config.Height or 160
		local PhotoWidth  = Config.Width or PhotoHeight
		local isFixedSize = Config.AspectRatio ~= nil
		local PhotoBox = New("Frame", {
			Size = isFixedSize and UDim2.new(0, PhotoWidth, 0, PhotoHeight) or UDim2.new(1, 0, 0, PhotoHeight),
			Position = isFixedSize and UDim2.new(0.5, 0, 0, 0) or UDim2.new(0, 0, 0, 0),
			AnchorPoint = isFixedSize and Vector2.new(0.5, 0) or Vector2.new(0, 0),
			BackgroundTransparency = 0.5,
			ClipsDescendants = true,
			LayoutOrder = 3,
			ThemeTag = { BackgroundColor3 = "Element" },
		...
		PhotoBox.Parent = ImgFrame.LabelHolder
		...
		-- ── โหลด/แปลงรูป (async กันสคริปต์ค้างตอน HTTP request) ──────
		function Img:SetPhoto(Value)
			Img.Value = Value or ""
			Photo.Visible = false
			StatusLabel.Visible = true
			if Img.Value == "" then
				StatusLabel.Text = "No image set"
				return
			end
			StatusLabel.Text = "Loading image..."
			task.spawn(function()
				local contentId, err = Library:ResolveImageSource(Img.Value, "element_" .. tostring(Idx or "image"))
				-- กันเคส element ถูก Destroy ไปแล้วระหว่างรอโหลด
				if not PhotoBox.Parent then return end
				if contentId then
					Photo.Image = contentId
					Photo.Visible = true
					StatusLabel.Visible = false
				else
					StatusLabel.Text = err or "Failed to load image"
				end
			end)
		end

		function Img:Destroy()
			ImgFrame.Frame:Destroy()
			if Idx then Library.Options[Idx] = nil end
		end

		Img:SetPhoto(Img.Value)
		if Idx then Library.Options[Idx] = Img end
		return Img
	end
	return Element
end)()
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- 🖼️ Custom Background section — ให้ผู้เล่นตั้งภาพพื้นหลังของหน้าต่างเอง
		-- หมายเหตุ: Roblox Lua ไม่มี native "Browse..." dialog ให้เปิดไฟล์จากเครื่องเหมือนโปรแกรมทั่วไป (เช่น Real Executor ที่เป็นแอปแยก ไม่ใช่ Lua UI)
		-- อีกอย่าง: property Image ของ ImageLabel รองรับ "เฉพาะ" content id ของ Roblox เท่านั้น (rbxassetid:// / rbxasset:// / rbxthumb://)
		-- ลิงก์ http(s) ธรรมดา (เช่น ลิงก์ Discord CDN, Imgur ฯลฯ) วางตรงๆ ใช้ไม่ได้ตรงๆ — ต้องโหลด+cache ก่อน (ดู Library:ResolveImageSource)
		local customBgSection = InterfaceSubTab:AddSection("Custom Background", "image")

		-- ตัวอย่างการใช้ Element ประเภท Image เอง — โชว์รูปสอนวิธีใช้ ขนาด 1:1
		customBgSection:AddImage("MyPhoto", {
			Photo = "https://cdn.discordapp.com/attachments/1536949626924695673/1547262475504984134/ChatGPT_Image_9_.._2569_22_08_27.png?ex=6aa2c7f8&is=6aa17678&hm=e03a60d20d1807e12b22e908f315ba1e6685b8d45abf6bbf3eeb04cd1c768216&",
			Title       = "How to Use",
			Description = "วิธีใช้",
			Height      = 160,
			AspectRatio = 1, -- บังคับให้เป็นสี่เหลี่ยมจัตุรัส 1:1 (กว้าง = สูง = Height)
		})

		local hasUrlDownloadSupport = (typeof((syn and syn.request) or http_request or request or (http and http.request)) == "function")
			and (typeof(writefile) == "function") and (typeof(getcustomasset) == "function")
		local hasLocalFileSupport = (typeof(getcustomasset) == "function") and (typeof(isfile) == "function")

		local BackgroundImageInput
		local function ApplyCustomBackground(RawValue, Silent)
			local Value = (RawValue or ""):gsub("^%s+", ""):gsub("%s+$", "")
			if Value == "" then return end

			-- [ จุดที่แก้ ] ยิง HTTP ใน task.spawn เสมอ กัน UI thread ค้างช่วงที่รอโหลดลิงก์ (ตอนนี้ resolve
			-- ถูกรวมไว้ที่ Library:ResolveImageSource ตัวเดียว ใช้ร่วมกับ Element ประเภท Image ด้วย ไม่ต้องเขียนซ้ำ)
			task.spawn(function()
				local contentId, err = Library:ResolveImageSource(Value, "custom_background")
				if not contentId then
					if not Silent then
						Library:Notify({ Title = "Custom Background", Content = err, Duration = 6 })
					end
					return
				end
				if Library.Window then
					Library.Window:SetBackgroundImage(contentId)
					Library.Window.ManagedBackgroundImage = false -- กันไม่ให้ Library:SetTheme ทับภาพ custom เวลาสลับธีม
				end
				Settings.CustomBackgroundImage = Value -- เก็บ input ดิบ (ลิงก์/path/asset id) ไว้ ไม่เก็บ content id เพราะ getcustomasset คืนค่าใหม่ทุกครั้งที่รันสคริปต์
				InterfaceManager:SaveSettings()
			end)
		end

		BackgroundImageInput = customBgSection:AddInput("CustomBackgroundImage", {
			Title = "Background Image",
			Description = hasUrlDownloadSupport
				and "rbxassetid://, an image link, or a local file path. Discord links expire."
				or "Paste an rbxassetid:// — this executor can't download from links directly.",
			Placeholder = hasLocalFileSupport and "rbxassetid://... or C:\\path\\to\\image.png" or "rbxassetid://...",
			Default = Settings.CustomBackgroundImage or "",
			Finished = true, -- apply ตอนกด Enter เท่านั้น ไม่ใช่ทุกตัวอักษรที่พิมพ์
			Callback = function(Value)
				ApplyCustomBackground(Value, false)
			end,
		})

		-- ถ้าเคยตั้ง custom background ไว้ ให้ apply ทันทีตอนโหลด UI (เงียบๆ ไม่เด้ง notify ถ้าล้มเหลว เผื่อย้ายไปคนละเครื่อง/executor)
		if Settings.CustomBackgroundImage and Settings.CustomBackgroundImage ~= "" then
			ApplyCustomBackground(Settings.CustomBackgroundImage, true)
		end

		local BackgroundOpacitySlider = customBgSection:AddSlider("CustomBackgroundOpacity", {
			Title = "Background Opacity",
			Description = "How visible your custom background image is (0 = invisible, 1 = fully visible).",
			Default = Settings.CustomBackgroundImageTransparency ~= nil and (1 - Settings.CustomBackgroundImageTransparency) or 0.5,
			Min = 0,
			Max = 1,
			Rounding = 2,
			Callback = function(Value)
				local transparency = math.clamp(1 - Value, 0, 1)
				-- [ จุดที่แก้ ] บั๊กเดิม: เรียก Window:SetBackgroundImageTransparency ซึ่งไปคุม Window.AcrylicPaint
				-- (ตัวพาเนลหลักทั้งบานที่ครอบ element/ตัวหนังสือทั้งหมด) ด้วย ไม่ใช่แค่รูปพื้นหลัง — ลาก slider แล้ว
				-- ทั้งหน้าต่างเลยใสหายไปหมด ทั้งที่ตั้งใจจะปรับแค่ความชัดของรูปพื้นหลังเท่านั้น
				-- เปลี่ยนมาคุม Window.BackgroundImage ตรงๆ ไม่ไปยุ่งกับ AcrylicPaint เลย
				if Library.Window then
					Library.Window.BackgroundImageTransparency = transparency
					if Library.Window.BackgroundImage then
						Library.Window.BackgroundImage.ImageTransparency = transparency
					end
				end
				Settings.CustomBackgroundImageTransparency = transparency
				InterfaceManager:SaveSettings()
			end,
		})

		local ResetBackgroundButton = customBgSection:AddActionButton("ResetCustomBackground", {
			Title      = "Reset Background",
			ButtonText = "Reset",
			CopiedText = "✓ Reset",
			CopyText   = "",
			Callback   = function()
				Settings.CustomBackgroundImage = nil
				Settings.CustomBackgroundImageTransparency = nil
				InterfaceManager:SaveSettings()
				BackgroundImageInput:SetValue("")
				-- คืนค่าภาพพื้นหลังกลับไปเป็นของ theme ปัจจุบัน (เหมือน logic ใน Library:SetTheme)
				local Window = Library.Window
				if Window then
					local isMinecraft = (Library.Theme == "Minecraft")
					local defaultImage = isMinecraft and "rbxassetid://127892835920326" or "rbxassetid://13196113628"
					Window.ManagedBackgroundImage = true
					if Window.BackgroundImage then
						Window.BackgroundImage.Image = defaultImage
						Window.BackgroundImage.ScaleType = Enum.ScaleType.Stretch
