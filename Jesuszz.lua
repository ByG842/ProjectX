...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- วิธีที่ทำได้จริงในนี้มี 2 แบบ:
		--   1) วางลิงก์รูป / rbxassetid:// (ใช้ได้ทุก executor)
		--   2) วาง path ไฟล์ภาพในเครื่อง แล้วแปลงเป็น content id ผ่าน getcustomasset (เฉพาะ executor ที่รองรับฟังก์ชันนี้ เช่น Synapse X, Wave, Script-Ware ฯลฯ)
		...
		local function ApplyCustomBackground(RawValue, Silent)
			...
			local finalImage = Value
			local isRemote = Value:match("^rbxassetid://") or Value:match("^rbxasset://") or Value:match("^rbxthumb://") or Value:match("^https?://")
			...
			if not isRemote then
				...
				if hasLocalFileSupport and isfile(Value) then
					...
					if not Silent then
						local reason = hasLocalFileSupport and "ไม่พบไฟล์ตาม path ที่ให้มา" or "executor นี้ไม่รองรับการโหลดภาพจากไฟล์ในเครื่อง — ใช้ลิงก์รูปหรือ rbxassetid:// แทน"
			...
			Settings.CustomBackgroundImage = Value -- เก็บ input ดิบ (path/asset id) ไว้ ไม่เก็บ content id เพราะ getcustomasset คืนค่าใหม่ทุกครั้งที่รันสคริปต์
			InterfaceManager:SaveSettings()
		end

		BackgroundImageInput = customBgSection:AddInput("CustomBackgroundImage", {
			Title = "Background Image",
			Description = hasLocalFileSupport
				and "Paste an rbxassetid://, image link, or a local file path, then press Enter."
				or "Paste an rbxassetid:// or image link (e.g. rbxassetid://13196113628), then press Enter.",
