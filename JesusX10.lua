...
ElementsTable.ButtonGroup = (function()
	...
	function Element:New(Idx, Config)
		...
		-- [ จุดที่แก้ ] ของเดิมแต่ละปุ่มมีพื้นหลัง+กรอบของตัวเองแยกกัน วางชิดกัน (Padding=0) เลยเห็นเป็น
		-- เส้นกรอบคู่ตรงรอยต่อระหว่างปุ่ม ดูไม่เนียน — เปลี่ยนมาเป็น "แท่งเดียว" ที่มีพื้นหลัง/กรอบรวม
		-- อันเดียว แล้วตัดมุม (ClipsDescendants) ให้ปุ่มหัว-ท้ายโค้งตามกรอบนอกอัตโนมัติ ไม่ต้องคำนวณ
		-- มุมแต่ละปุ่มเอง (ของเดิมมีตัวแปร isFirst/isLast แต่ไม่เคยถูกใช้จริงเลยสักที่)
		...
		for i, btn in ipairs(Config.Buttons) do
			...
			-- [ จุดที่เพิ่ม ] รองรับ btn.Style = "danger" สำหรับปุ่มที่ทำลายข้อมูล/กู้คืนไม่ได้ (เช่น "Delete")
			-- ให้ตัวหนังสือ/hover เป็นโทนแดงแยกจากปุ่มอื่นในกลุ่มชัดเจน กันมือลั่นกดผิดปุ่ม
			local isDanger = btn.Style == "danger"

			-- [ จุดที่แก้ ] ปุ่มแต่ละอันไม่มีพื้นหลัง/กรอบของตัวเองแล้ว (โปร่งใสเต็ม 1 ที่พัก) ใช้พื้นหลัง
			-- รวมของ BtnRow แทน ตอน hover ค่อยไล่ความทึบขึ้นเฉพาะปุ่มนั้นปุ่มเดียว
			local B = New("TextButton", {
				...
				AutomaticSize = Enum.AutomaticSize.X,
				BackgroundTransparency = 1,
				TextColor3 = isDanger and Color3.fromRGB(235, 90, 90) or nil,
				Parent = BtnRow,
				-- [ จุดที่แก้ ] ปุ่มปกติต้องมี BackgroundColor3 ผูก ThemeTag ด้วย ("Hover" สีเดียวกับที่
				-- ใช้ hover ทั่วไปในไลบรารีนี้) ไม่งั้นตอนไล่ BackgroundTransparency ขึ้นจะโชว์สีขาวเถื่อน
				-- (ค่า default ของ Instance) เพราะไม่เคยกำหนดสีพื้นหลังไว้เลย
				ThemeTag = (not isDanger) and { TextColor3 = "Text", BackgroundColor3 = "Hover" } or nil,
			}, {
				New("UIPadding", { PaddingLeft = UDim.new(0, 14), PaddingRight = UDim.new(0, 14) }),
				-- [ จุดที่แก้ ] เส้นคั่นบางๆ ระหว่างปุ่ม (แทนกรอบเต็มรอบทุกปุ่มแบบเดิมที่ซ้อนกันเป็นเส้นคู่)
				-- เว้นระยะจากขอบบน-ล่างนิดหน่อยให้ดูนุ่มนวลกว่าเส้นเต็มความสูง ไม่ตัดกับ ClipsDescendants ด้วย
				(not isLast) and New("Frame", {
			...
			-- [ จุดที่แก้ ] danger ใช้โทนแดงตอน hover/press แทนโทนเทาเดียวกับปุ่มอื่น ให้เห็นชัดว่า "อันตราย"
			local hoverBg = isDanger and Color3.fromRGB(120, 30, 30) or nil
			if hoverBg then B.BackgroundColor3 = hoverBg end
			local Motor, SetT = Creator.SpringMotor(1, B, "BackgroundTransparency", true, false, { frequency = 8 })
			Creator.AddSignal(B.MouseEnter, function() SetT(0.82) end)
			Creator.AddSignal(B.MouseLeave, function() SetT(1) end)
			Creator.AddSignal(B.MouseButton1Down, function() SetT(0.65) end)
			Creator.AddSignal(B.MouseButton1Up, function() SetT(0.82) end)
	...
	function SaveManager:BuildConfigSection(tab)
		...
		section:AddButtonGroup("SaveManager_ConfigActions", {
			...
			Buttons = {
				...
				{
					Text = "Delete",
					-- [ จุดที่เพิ่ม ] ใช้สไตล์ "danger" ใหม่ที่เพิ่มใน ButtonGroup ปุ่มลบข้อมูลถาวรควรเด่นเป็นสีแดง
					-- แยกจาก Load/Save ข้างๆ ชัดเจน กันมือลั่นกดผิดปุ่มโดยไม่ตั้งใจ
					Style = "danger",
					Callback = function()
						local name = SaveManager.Options.SaveManager_ConfigList.Value
