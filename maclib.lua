...
function MacLib:Window(Settings)
	...
	controls.Size = UDim2.fromOffset(80, 31)
	...
	local function makeControlBtn(symbol, layoutOrder, isEnabled)
		local btn = Instance.new("TextButton")
		btn.Name = symbol == "-" and "Minimize" or (symbol == "□" and "Maximize" or "Exit")
		btn.FontFace = Font.new(assets.interFont, Enum.FontWeight.Light, Enum.FontStyle.Normal)
		btn.Text = symbol
		btn.TextColor3 = Color3.fromRGB(255, 255, 255)
		btn.TextSize = symbol == "✕" and 13 or 14
		btn.TextTransparency = isEnabled and 0.45 or 0.85
		btn.AutoButtonColor = false
		btn.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		btn.BackgroundTransparency = 1
		btn.BorderColor3 = Color3.fromRGB(0, 0, 0)
		btn.BorderSizePixel = 0
		btn.Size = UDim2.fromOffset(26, 31)
		...
		if isEnabled then
			btn.MouseEnter:Connect(function()
				Tween(btn, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
					BackgroundTransparency = symbol == "✕" and 0.8 or 0.94,
	...
	local minimizeEnabled = not (Settings.DisabledWindowControls and table.find(Settings.DisabledWindowControls, "Minimize"))
	local exitEnabled = not (Settings.DisabledWindowControls and table.find(Settings.DisabledWindowControls, "Exit"))

	local minimize = makeControlBtn("-", 0, minimizeEnabled)
	minimize.Parent = controls

	-- Mobile collapse button (□ acts as collapse/fold for touch)
	local maximize = makeControlBtn("□", 1, false)
	maximize.TextTransparency = 0.85
	maximize.Active = false
	maximize.Interactable = false
	maximize.Parent = controls

	local exit = makeControlBtn("✕", 2, exitEnabled)
	...
	-- Window icon + title/subtitle row
	...
	titleFrame.Size = UDim2.new(1, -20, 0, 0)
	...
	titleFrameUIListLayout.Padding = UDim.new(0, 8)
	...
	-- Window Icon (optional, shown only when Settings.Icon is set)
	...
	windowIcon.Image = Settings.Icon or ""
	windowIcon.ImageTransparency = Settings.Icon and 0 or 1
	windowIcon.ScaleType = Enum.ScaleType.Fit
	windowIcon.LayoutOrder = 0
	windowIcon.Visible = Settings.Icon ~= nil
	windowIcon.Parent = titleFrame

	local titleIconCorner = Instance.new("UICorner")
	titleIconCorner.CornerRadius = UDim.new(0, 6)
	titleIconCorner.Parent = windowIcon

	-- Text group (title + subtitle stacked)
	local titleTextGroup = Instance.new("Frame")
	titleTextGroup.Name = "TitleTextGroup"
	titleTextGroup.BackgroundTransparency = 1
	titleTextGroup.BorderSizePixel = 0
	titleTextGroup.AutomaticSize = Enum.AutomaticSize.Y
	titleTextGroup.Size = UDim2.new(1, Settings.Icon and -36 or 0, 0, 0)
	titleTextGroup.LayoutOrder = 1
	titleTextGroup.Parent = titleFrame

	local titleTextList = Instance.new("UIListLayout")
	titleTextList.Padding = UDim.new(0, 2)
	titleTextList.SortOrder = Enum.SortOrder.LayoutOrder
	titleTextList.Parent = titleTextGroup
	...
	subtitle.Visible = (Settings.Subtitle ~= nil and Settings.Subtitle ~= "")
	subtitle.Parent = titleTextGroup
	...
	-- DragStyle: 1 = icon only, 2 = full window, 3 (default) = sidebar/topbar header drag
	local function setupDragTarget(target)
		target.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				onDragStart(input)
			end
		end)
		target.InputChanged:Connect(onDragUpdate)
	...
	if Settings.DragStyle == 1 then
		-- icon-only drag
		setupDragTarget(interact)
	elseif Settings.DragStyle == 2 then
		-- full window drag
		setupDragTarget(base)
	else
		-- Default: drag from sidebar header and topbar (no need to find special button)
		setupDragTarget(interact) -- still support icon drag
		-- Also allow dragging from the windowControls bar (sidebar top) and topbar
		windowControls.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				-- Don't drag if clicking controls buttons
				local pos = input.Position
				local ctrlPos = controls.AbsolutePosition
				local ctrlSize = controls.AbsoluteSize
				local inControls = pos.X >= ctrlPos.X and pos.X <= ctrlPos.X + ctrlSize.X
					and pos.Y >= ctrlPos.Y and pos.Y <= ctrlPos.Y + ctrlSize.Y
				if not inControls then
					onDragStart(input)
				end
			end
		end)
		windowControls.InputChanged:Connect(onDragUpdate)
		windowControls.InputEnded:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				dragging_ = false
			end
		end)
		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					button.Size = UDim2.new(1, 0, 0, Settings.Description and 50 or 38)
					...
					buttonInteract.TextYAlignment = Settings.Description and Enum.TextYAlignment.Top or Enum.TextYAlignment.Center
					...
					if Settings.Description then
						...
						btnDesc.Name = "ButtonDescription"
						btnDesc.FontFace = Font.new(assets.interFont, Enum.FontWeight.Regular)
						btnDesc.Text = Settings.Description
						btnDesc.TextColor3 = Color3.fromRGB(200, 200, 210)
						btnDesc.TextSize = 11
						btnDesc.TextTransparency = 0.5
						btnDesc.TextWrapped = true
						btnDesc.TextXAlignment = Enum.TextXAlignment.Left
						btnDesc.AutomaticSize = Enum.AutomaticSize.Y
						btnDesc.BackgroundTransparency = 1
						btnDesc.BorderSizePixel = 0
						btnDesc.AnchorPoint = Vector2.new(0, 1)
						btnDesc.Position = UDim2.new(0, 0, 1, -4)
						btnDesc.Size = UDim2.new(1, -20, 0, 0)
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					toggleName.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
					...
					-- Optional description below toggle name
					if ToggleFunctions.Settings.Description then
						local toggleDesc = Instance.new("TextLabel")
						toggleDesc.Name = "ToggleDescription"
						toggleDesc.FontFace = Font.new(assets.interFont, Enum.FontWeight.Regular)
						toggleDesc.Text = ToggleFunctions.Settings.Description
						toggleDesc.TextColor3 = Color3.fromRGB(200, 200, 210)
						...
						toggleDesc.AnchorPoint = Vector2.new(0, 1)
						toggleDesc.Position = UDim2.new(0, 0, 1, -2)
				...
				function SectionFunctions:Header(Settings, Flag)
					...
					local headerUIList = Instance.new("UIListLayout")
					headerUIList.Padding = UDim.new(0, 3)
					headerUIList.SortOrder = Enum.SortOrder.LayoutOrder
					headerUIList.Parent = header
					...
					headerText.LayoutOrder = 0
					headerText.Size = UDim2.fromScale(1, 0)
					headerText.Parent = header

					-- Description under header title (optional)
					local headerDesc = Instance.new("TextLabel")
					headerDesc.Name = "HeaderDescription"
					headerDesc.FontFace = Font.new(assets.interFont, Enum.FontWeight.Regular)
					headerDesc.RichText = true
					headerDesc.Text = HeaderFunctions.Settings.Description or ""
					headerDesc.TextColor3 = Color3.fromRGB(200, 200, 210)
					headerDesc.TextSize = 12
					headerDesc.TextTransparency = 0.45
					headerDesc.TextWrapped = true
					headerDesc.TextXAlignment = Enum.TextXAlignment.Left
					headerDesc.AutomaticSize = Enum.AutomaticSize.Y
					headerDesc.BackgroundTransparency = 1
					headerDesc.BorderSizePixel = 0
					headerDesc.LayoutOrder = 1
					headerDesc.Size = UDim2.fromScale(1, 0)
					headerDesc.Visible = (HeaderFunctions.Settings.Description ~= nil and HeaderFunctions.Settings.Description ~= "")
					headerDesc.Parent = header

					function HeaderFunctions:UpdateName(New)
						headerText.Text = New
					end
					function HeaderFunctions:UpdateDescription(New)
						headerDesc.Text = New or ""
						headerDesc.Visible = (New ~= nil and New ~= "")
					end
				...
				-- =============================================
				-- NEW ELEMENT 1: Checkbox
				-- Usage: section:Checkbox({ Name="...", Description="...", Default=false, Callback=function(v) end })
				-- =============================================
				function SectionFunctions:Checkbox(Settings, Flag)
					local CheckboxFunctions = { Settings = Settings, IgnoreConfig = false, Class = "Checkbox" }
					...
					cbFrame.Size = UDim2.new(1, 0, 0, Settings.Description and 50 or 38)
					cbFrame.Parent = section

					local cbBtn = Instance.new("TextButton")
					cbBtn.Name = "CheckboxButton"
					cbBtn.Text = ""
					cbBtn.AutoButtonColor = false
					cbBtn.BackgroundTransparency = 1
					cbBtn.BorderSizePixel = 0
					cbBtn.Size = UDim2.fromScale(1, 1)
					...
					local checkMark = Instance.new("TextLabel")
					checkMark.Name = "CheckMark"
					checkMark.FontFace = Font.new(assets.interFont, Enum.FontWeight.Bold)
					checkMark.Text = "✓"
					checkMark.TextColor3 = Color3.fromRGB(255, 255, 255)
					...
					cbLabel.Name = "CheckboxLabel"
					cbLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
					cbLabel.Text = Settings.Name
					cbLabel.RichText = true
					cbLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
					cbLabel.TextSize = 13
					cbLabel.TextTransparency = 0.4
					cbLabel.TextXAlignment = Enum.TextXAlignment.Left
					cbLabel.TextYAlignment = Settings.Description and Enum.TextYAlignment.Top or Enum.TextYAlignment.Center
					cbLabel.AnchorPoint = Vector2.new(0, 0.5)
					cbLabel.AutomaticSize = Enum.AutomaticSize.Y
					cbLabel.BackgroundTransparency = 1
					cbLabel.BorderSizePixel = 0
					cbLabel.Position = UDim2.fromScale(0, 0.5)
					cbLabel.Size = UDim2.new(1, -28, 0, 0)
					cbLabel.Parent = cbFrame

					if Settings.Description then
						local cbDesc = Instance.new("TextLabel")
						cbDesc.Name = "CheckboxDesc"
						cbDesc.FontFace = Font.new(assets.interFont, Enum.FontWeight.Regular)
						cbDesc.Text = Settings.Description
						cbDesc.TextColor3 = Color3.fromRGB(200, 200, 210)
						cbDesc.TextSize = 11
						cbDesc.TextTransparency = 0.5
						cbDesc.TextWrapped = true
						cbDesc.TextXAlignment = Enum.TextXAlignment.Left
						cbDesc.AutomaticSize = Enum.AutomaticSize.Y
						cbDesc.BackgroundTransparency = 1
						cbDesc.BorderSizePixel = 0
						cbDesc.AnchorPoint = Vector2.new(0, 1)
						cbDesc.Position = UDim2.new(0, 0, 1, -3)
						cbDesc.Size = UDim2.new(1, -28, 0, 0)
						cbDesc.Parent = cbFrame
					end

					local function SetChecked(state, callback)
						checked = state
						CheckboxFunctions.State = state
						Tween(box, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {
							BackgroundColor3 = state and Color3.fromRGB(90, 140, 255) or Color3.fromRGB(255, 255, 255),
							BackgroundTransparency = state and 0 or 0.92
						}):Play()
						Tween(checkMark, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {
							TextTransparency = state and 0 or 1
						}):Play()
						Tween(cbLabel, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
							TextTransparency = state and 0.1 or 0.4
						}):Play()
						if callback and Settings.Callback then
					...
					cbBtn.MouseEnter:Connect(function()
						if not checked then
							Tween(box, TweenInfo.new(0.15, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.8 }):Play()
						end
					end)
					cbBtn.MouseLeave:Connect(function()
						if not checked then
							Tween(box, TweenInfo.new(0.15, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.92 }):Play()
						end
					end)
					...
					function CheckboxFunctions:SetVisibility(State)
						cbFrame.Visible = State
					end

					if Flag then MacLib.Options[Flag] = CheckboxFunctions end
					return CheckboxFunctions
				end

				-- =============================================
				-- NEW ELEMENT 2: ProgressBar (static display)
				-- Usage: section:ProgressBar({ Name="Loading", Value=75, Max=100, Color=Color3... })
				-- =============================================
				function SectionFunctions:ProgressBar(Settings, Flag)
					local PBFunctions = { Settings = Settings }
					local value = Settings.Value or 0
					local maxVal = Settings.Max or 100

					local pbFrame = Instance.new("Frame")
					pbFrame.Name = "ProgressBar"
					pbFrame.AutomaticSize = Enum.AutomaticSize.Y
					pbFrame.BackgroundTransparency = 1
					pbFrame.BorderSizePixel = 0
					pbFrame.Size = UDim2.new(1, 0, 0, 46)
					pbFrame.Parent = section

					local pbList = Instance.new("UIListLayout")
					pbList.Padding = UDim.new(0, 6)
					pbList.SortOrder = Enum.SortOrder.LayoutOrder
					pbList.Parent = pbFrame

					-- Header row: name + value label
					local headerRow = Instance.new("Frame")
					headerRow.BackgroundTransparency = 1
					headerRow.BorderSizePixel = 0
					headerRow.Size = UDim2.new(1, 0, 0, 18)
					headerRow.LayoutOrder = 0
					headerRow.Parent = pbFrame

					local pbName = Instance.new("TextLabel")
					pbName.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
					pbName.Text = Settings.Name
					pbName.RichText = true
					pbName.TextColor3 = Color3.fromRGB(255, 255, 255)
					pbName.TextSize = 13
					pbName.TextTransparency = 0.4
					pbName.TextXAlignment = Enum.TextXAlignment.Left
					pbName.BackgroundTransparency = 1
					pbName.BorderSizePixel = 0
					pbName.Size = UDim2.new(1, -40, 1, 0)
					pbName.Parent = headerRow

					local pbValueLabel = Instance.new("TextLabel")
					pbValueLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					pbValueLabel.Text = tostring(math.floor(value)) .. "/" .. tostring(maxVal)
					pbValueLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
					pbValueLabel.TextSize = 12
					pbValueLabel.TextTransparency = 0.35
					pbValueLabel.TextXAlignment = Enum.TextXAlignment.Right
					pbValueLabel.AnchorPoint = Vector2.new(1, 0)
					pbValueLabel.BackgroundTransparency = 1
					pbValueLabel.BorderSizePixel = 0
					pbValueLabel.Position = UDim2.fromScale(1, 0)
					pbValueLabel.Size = UDim2.fromOffset(40, 18)
					pbValueLabel.Parent = headerRow
					...
					barFill.BackgroundColor3 = Settings.Color or Color3.fromRGB(100, 140, 255)
					barFill.BorderSizePixel = 0
					barFill.Size = UDim2.fromScale(math.clamp(value / maxVal, 0, 1), 1)
					...
					function PBFunctions:SetValue(v)
						value = math.clamp(v, 0, maxVal)
						Tween(barFill, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {
							Size = UDim2.fromScale(value / maxVal, 1)
						}):Play()
						pbValueLabel.Text = tostring(math.floor(value)) .. "/" .. tostring(maxVal)
					end
					function PBFunctions:GetValue() return value end
					function PBFunctions:SetVisibility(State) pbFrame.Visible = State end

					if Flag then MacLib.Options[Flag] = PBFunctions end
					return PBFunctions
				end

				-- =============================================
				-- NEW ELEMENT 3: RadioGroup
				-- Usage: section:RadioGroup({ Name="Mode", Options={"Fast","Normal","Slow"}, Default="Normal", Callback=fn })
				-- =============================================
				function SectionFunctions:RadioGroup(Settings, Flag)
					local RGFunctions = { Settings = Settings, IgnoreConfig = false, Class = "RadioGroup" }
					local selected = Settings.Default or (Settings.Options and Settings.Options[1]) or ""
					...
					rgFrame.Size = UDim2.new(1, 0, 0, 0)
					rgFrame.Parent = section

					local rgList = Instance.new("UIListLayout")
					rgList.Padding = UDim.new(0, 6)
					rgList.SortOrder = Enum.SortOrder.LayoutOrder
					rgList.Parent = rgFrame

					local rgLabel = Instance.new("TextLabel")
					rgLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
					rgLabel.Text = Settings.Name
					rgLabel.RichText = true
					rgLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
					rgLabel.TextSize = 13
					rgLabel.TextTransparency = 0.4
					rgLabel.TextXAlignment = Enum.TextXAlignment.Left
					rgLabel.BackgroundTransparency = 1
					rgLabel.BorderSizePixel = 0
					rgLabel.LayoutOrder = 0
					rgLabel.Size = UDim2.new(1, 0, 0, 18)
					rgLabel.Parent = rgFrame
					...
					local function UpdateRadios(newSelected)
						selected = newSelected
						RGFunctions.Value = selected
						for opt, parts in pairs(radioButtons) do
							local isSelected = (opt == selected)
							Tween(parts.dot, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {
								BackgroundColor3 = isSelected and Color3.fromRGB(100, 140, 255) or Color3.fromRGB(255, 255, 255),
								BackgroundTransparency = isSelected and 0 or 1
							}):Play()
							Tween(parts.ring, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {
								BackgroundTransparency = isSelected and 0.6 or 0.85
							}):Play()
							Tween(parts.label, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
								TextTransparency = isSelected and 0.1 or 0.5
							}):Play()
						end
					end

					for i, opt in ipairs(Settings.Options or {}) do
						...
						ring.Position = UDim2.fromScale(0, 0.5)
						ring.Size = UDim2.fromOffset(16, 16)
						ring.Parent = optFrame
						...
						optLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Regular)
						optLabel.Text = opt
						...
						radioButtons[opt] = { dot = dot, ring = ring, label = optLabel }

						optBtn.MouseButton1Click:Connect(function()
							UpdateRadios(opt)
							if Settings.Callback then task.spawn(Settings.Callback, opt) end
						end)
					end

					UpdateRadios(selected)

					function RGFunctions:SetValue(v)
						UpdateRadios(v)
					end
					function RGFunctions:GetValue() return selected end
					function RGFunctions:SetVisibility(State) rgFrame.Visible = State end

					if Flag then MacLib.Options[Flag] = RGFunctions end
					return RGFunctions
				end

				-- =============================================
				-- NEW ELEMENT 4: Stepper (increment/decrement number)
				-- Usage: section:Stepper({ Name="Count", Default=0, Min=0, Max=10, Step=1, Callback=fn })
				-- =============================================
				function SectionFunctions:Stepper(Settings, Flag)
					local StepFunctions = { Settings = Settings, IgnoreConfig = false, Class = "Stepper" }
					local val = Settings.Default or 0
					local minVal = Settings.Min or 0
					local maxVal = Settings.Max or 100
					local step = Settings.Step or 1

					local stepFrame = Instance.new("Frame")
					stepFrame.Name = "Stepper"
					stepFrame.BackgroundTransparency = 1
					stepFrame.BorderSizePixel = 0
					stepFrame.Size = UDim2.new(1, 0, 0, 38)
					stepFrame.Parent = section

					local stepLabel = Instance.new("TextLabel")
					stepLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
					stepLabel.Text = Settings.Name
					stepLabel.RichText = true
					stepLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
					stepLabel.TextSize = 13
					stepLabel.TextTransparency = 0.4
					stepLabel.TextXAlignment = Enum.TextXAlignment.Left
					stepLabel.AnchorPoint = Vector2.new(0, 0.5)
					stepLabel.BackgroundTransparency = 1
					stepLabel.BorderSizePixel = 0
					stepLabel.Position = UDim2.fromScale(0, 0.5)
					stepLabel.Size = UDim2.new(1, -100, 1, 0)
					stepLabel.Parent = stepFrame

					-- Controls: [−] value [+]
					local controlsFrame = Instance.new("Frame")
					controlsFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					controlsFrame.BackgroundTransparency = 0.94
					controlsFrame.BorderSizePixel = 0
					controlsFrame.AnchorPoint = Vector2.new(1, 0.5)
					controlsFrame.Position = UDim2.new(1, 0, 0.5, 0)
					controlsFrame.Size = UDim2.fromOffset(90, 26)
					controlsFrame.Parent = stepFrame
					...
					controlsStroke.Color = Color3.fromRGB(255, 255, 255)
					controlsStroke.Transparency = 0.88
					controlsStroke.Parent = controlsFrame

					local minusBtn = Instance.new("TextButton")
					minusBtn.FontFace = Font.new(assets.interFont, Enum.FontWeight.Light)
					minusBtn.Text = "−"
					minusBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
					minusBtn.TextSize = 16
					minusBtn.TextTransparency = 0.4
					...
					valLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					valLabel.Text = tostring(val)
					valLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
					valLabel.TextSize = 13
					valLabel.TextTransparency = 0.15
					valLabel.AnchorPoint = Vector2.new(0.5, 0.5)
					valLabel.BackgroundTransparency = 1
					valLabel.BorderSizePixel = 0
					valLabel.Position = UDim2.fromScale(0.5, 0.5)
					valLabel.Size = UDim2.fromOffset(38, 26)
					valLabel.Parent = controlsFrame

					local plusBtn = Instance.new("TextButton")
					plusBtn.FontFace = Font.new(assets.interFont, Enum.FontWeight.Light)
					plusBtn.Text = "+"
					plusBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
					plusBtn.TextSize = 14
					plusBtn.TextTransparency = 0.4
					plusBtn.AutoButtonColor = false
					plusBtn.BackgroundTransparency = 1
					plusBtn.BorderSizePixel = 0
					plusBtn.AnchorPoint = Vector2.new(1, 0)
					plusBtn.Position = UDim2.fromScale(1, 0)
					...
					local function SetVal(v)
						...
						valLabel.Text = tostring(val)
						StepFunctions.Value = val
						if Settings.Callback then task.spawn(Settings.Callback, val) end
					end

					minusBtn.MouseButton1Click:Connect(function() SetVal(val - step) end)
					plusBtn.MouseButton1Click:Connect(function() SetVal(val + step) end)

					function StepFunctions:SetValue(v) SetVal(v) end
					function StepFunctions:GetValue() return val end
					function StepFunctions:SetVisibility(State) stepFrame.Visible = State end

					if Flag then MacLib.Options[Flag] = StepFunctions end
					return StepFunctions
				end

				-- =============================================
				-- NEW ELEMENT 5: NumberDisplay (read-only stat box)
				-- Usage: section:NumberDisplay({ Name="Ping", Value=42, Unit="ms", Color=Color3... })
				-- =============================================
				function SectionFunctions:NumberDisplay(Settings, Flag)
					local NDFunctions = { Settings = Settings }

					local ndFrame = Instance.new("Frame")
					ndFrame.Name = "NumberDisplay"
					ndFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					ndFrame.BackgroundTransparency = 0.97
					ndFrame.BorderSizePixel = 0
					ndFrame.Size = UDim2.new(1, 0, 0, 46)
					ndFrame.ClipsDescendants = true
					ndFrame.Parent = section

					local ndCorner = Instance.new("UICorner")
					ndCorner.CornerRadius = UDim.new(0, 8)
					ndCorner.Parent = ndFrame

					local ndStroke = Instance.new("UIStroke")
					ndStroke.Color = Settings.Color or Color3.fromRGB(100, 140, 255)
					ndStroke.Transparency = 0.7
					ndStroke.Parent = ndFrame

					-- Left accent
					local ndAccent = Instance.new("Frame")
					ndAccent.BackgroundColor3 = Settings.Color or Color3.fromRGB(100, 140, 255)
					ndAccent.BackgroundTransparency = 0.3
					ndAccent.BorderSizePixel = 0
					ndAccent.Size = UDim2.new(0, 3, 1, 0)
					ndAccent.Parent = ndFrame

					local ndAccentCorner = Instance.new("UICorner")
					ndAccentCorner.CornerRadius = UDim.new(0, 3)
					ndAccentCorner.Parent = ndAccent

					local ndPad = Instance.new("UIPadding")
					ndPad.PaddingLeft = UDim.new(0, 14)
					ndPad.PaddingRight = UDim.new(0, 12)
					ndPad.Parent = ndFrame

					local ndName = Instance.new("TextLabel")
					ndName.FontFace = Font.new(assets.interFont, Enum.FontWeight.Regular)
					ndName.Text = Settings.Name
					ndName.TextColor3 = Color3.fromRGB(200, 200, 210)
					ndName.TextSize = 11
					ndName.TextTransparency = 0.4
					ndName.TextXAlignment = Enum.TextXAlignment.Left
					ndName.AnchorPoint = Vector2.new(0, 0)
					ndName.BackgroundTransparency = 1
					ndName.BorderSizePixel = 0
					ndName.Position = UDim2.fromOffset(0, 8)
					ndName.Size = UDim2.new(1, 0, 0, 14)
					ndName.Parent = ndFrame

					local ndValueLabel = Instance.new("TextLabel")
					ndValueLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Bold)
					ndValueLabel.Text = tostring(Settings.Value or 0) .. (Settings.Unit and (" " .. Settings.Unit) or "")
					ndValueLabel.TextColor3 = Settings.Color or Color3.fromRGB(130, 170, 255)
					ndValueLabel.TextSize = 18
					ndValueLabel.TextTransparency = 0.05
					ndValueLabel.TextXAlignment = Enum.TextXAlignment.Left
					ndValueLabel.AnchorPoint = Vector2.new(0, 1)
					ndValueLabel.BackgroundTransparency = 1
					ndValueLabel.BorderSizePixel = 0
					ndValueLabel.Position = UDim2.new(0, 0, 1, -8)
					ndValueLabel.Size = UDim2.new(1, 0, 0, 22)
					ndValueLabel.Parent = ndFrame

					function NDFunctions:SetValue(v)
						ndValueLabel.Text = tostring(v) .. (Settings.Unit and (" " .. Settings.Unit) or "")
					end
					function NDFunctions:SetVisibility(State) ndFrame.Visible = State end

					if Flag then MacLib.Options[Flag] = NDFunctions end
					return NDFunctions
				end

				-- =============================================
				-- NEW ELEMENT 6: SeparatorLabel (divider with centered text)
				-- Usage: section:SeparatorLabel({ Text="Advanced Settings" })
				-- =============================================
				function SectionFunctions:SeparatorLabel(Settings, Flag)
					local SLFunctions = {}

					local slFrame = Instance.new("Frame")
					slFrame.Name = "SeparatorLabel"
					slFrame.AutomaticSize = Enum.AutomaticSize.Y
					slFrame.BackgroundTransparency = 1
					slFrame.BorderSizePixel = 0
					slFrame.Size = UDim2.new(1, 0, 0, 22)
					slFrame.Parent = section

					local leftLine = Instance.new("Frame")
					leftLine.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					leftLine.BackgroundTransparency = 0.88
					leftLine.BorderSizePixel = 0
					leftLine.AnchorPoint = Vector2.new(0, 0.5)
					leftLine.Position = UDim2.fromScale(0, 0.5)
					leftLine.Size = UDim2.new(0.3, -6, 0, 1)
					leftLine.Parent = slFrame

					local slText = Instance.new("TextLabel")
					slText.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
					slText.Text = Settings.Text or Settings.Name or ""
					slText.TextColor3 = Color3.fromRGB(255, 255, 255)
					slText.TextSize = 11
					slText.TextTransparency = 0.55
					slText.AnchorPoint = Vector2.new(0.5, 0.5)
					slText.BackgroundTransparency = 1
					slText.BorderSizePixel = 0
					slText.Position = UDim2.fromScale(0.5, 0.5)
					slText.Size = UDim2.fromOffset(120, 20)
					slText.Parent = slFrame

					local rightLine = Instance.new("Frame")
					rightLine.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					rightLine.BackgroundTransparency = 0.88
					rightLine.BorderSizePixel = 0
					rightLine.AnchorPoint = Vector2.new(1, 0.5)
					rightLine.Position = UDim2.fromScale(1, 0.5)
					rightLine.Size = UDim2.new(0.3, -6, 0, 1)
					rightLine.Parent = slFrame

					function SLFunctions:UpdateText(New)
						slText.Text = New
					end
					function SLFunctions:SetVisibility(State) slFrame.Visible = State end

					if Flag then MacLib.Options[Flag] = SLFunctions end
					return SLFunctions
				end

				-- =============================================
				-- NEW ELEMENT 7: Chip / Tag group (multi-select tags)
				-- Usage: section:Chips({ Name="Tags", Options={"PvP","PvE","Raid"}, Default={"PvP"}, Callback=fn })
				-- =============================================
				function SectionFunctions:Chips(Settings, Flag)
					local ChipFunctions = { Settings = Settings, IgnoreConfig = false, Class = "Chips" }
					local selected = {}
					if Settings.Default then
						for _, v in ipairs(Settings.Default) do selected[v] = true end
					end

					local chipFrame = Instance.new("Frame")
					chipFrame.Name = "Chips"
					...
					chipFrame.Parent = section

					local chipList = Instance.new("UIListLayout")
					chipList.Padding = UDim.new(0, 6)
					chipList.SortOrder = Enum.SortOrder.LayoutOrder
					chipList.Parent = chipFrame

					local chipTitle = Instance.new("TextLabel")
					chipTitle.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
					chipTitle.Text = Settings.Name
					chipTitle.RichText = true
					chipTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
					chipTitle.TextSize = 13
					chipTitle.TextTransparency = 0.4
					chipTitle.TextXAlignment = Enum.TextXAlignment.Left
					chipTitle.BackgroundTransparency = 1
					chipTitle.BorderSizePixel = 0
					chipTitle.LayoutOrder = 0
					chipTitle.Size = UDim2.new(1, 0, 0, 18)
					chipTitle.Parent = chipFrame

					local tagsRow = Instance.new("Frame")
					tagsRow.AutomaticSize = Enum.AutomaticSize.Y
					tagsRow.BackgroundTransparency = 1
					tagsRow.BorderSizePixel = 0
					tagsRow.LayoutOrder = 1
					tagsRow.Size = UDim2.new(1, 0, 0, 0)
					tagsRow.Parent = chipFrame

					local tagsFlow = Instance.new("UIListLayout")
					tagsFlow.FillDirection = Enum.FillDirection.Horizontal
					tagsFlow.Padding = UDim.new(0, 6)
					tagsFlow.SortOrder = Enum.SortOrder.LayoutOrder
					tagsFlow.Wraps = true
					tagsFlow.Parent = tagsRow

					local chipElements = {}

					for i, opt in ipairs(Settings.Options or {}) do
						local chipBtn = Instance.new("TextButton")
						chipBtn.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
						chipBtn.Text = opt
						chipBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
						chipBtn.TextSize = 12
						chipBtn.TextTransparency = selected[opt] and 0.1 or 0.55
						chipBtn.AutoButtonColor = false
						chipBtn.BackgroundColor3 = selected[opt] and Color3.fromRGB(80, 120, 255) or Color3.fromRGB(255, 255, 255)
						chipBtn.BackgroundTransparency = selected[opt] and 0.2 or 0.94
						chipBtn.BorderSizePixel = 0
						chipBtn.AutomaticSize = Enum.AutomaticSize.X
						chipBtn.Size = UDim2.fromOffset(0, 24)
						chipBtn.LayoutOrder = i
						chipBtn.Parent = tagsRow
						...
						chipPad.Parent = chipBtn

						local chipStroke = Instance.new("UIStroke")
						chipStroke.Color = Color3.fromRGB(100, 140, 255)
						chipStroke.Transparency = selected[opt] and 0.5 or 0.9
						chipStroke.Parent = chipBtn

						chipElements[opt] = { btn = chipBtn, stroke = chipStroke }

						chipBtn.MouseButton1Click:Connect(function()
							selected[opt] = not selected[opt]
							local isOn = selected[opt]
							Tween(chipBtn, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {
								BackgroundColor3 = isOn and Color3.fromRGB(80, 120, 255) or Color3.fromRGB(255, 255, 255),
								BackgroundTransparency = isOn and 0.2 or 0.94,
								TextTransparency = isOn and 0.1 or 0.55
							}):Play()
							Tween(chipStroke, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {
								Transparency = isOn and 0.5 or 0.9
							}):Play()
							if Settings.Callback then
								local result = {}
								for k, v in pairs(selected) do if v then table.insert(result, k) end end
								task.spawn(Settings.Callback, result)
							end
						end)
					end

					function ChipFunctions:GetSelected()
						local result = {}
						for k, v in pairs(selected) do if v then table.insert(result, k) end end
						return result
					end
					function ChipFunctions:SetVisibility(State) chipFrame.Visible = State end

					if Flag then MacLib.Options[Flag] = ChipFunctions end
					return ChipFunctions
				end

				-- =============================================
				-- NEW ELEMENT 8: MultiToggle (toggle group, one enabled at a time)
				-- Usage: section:MultiToggle({ Name="Speed", Options={"Slow","Normal","Fast"}, Default="Normal", Callback=fn })
				-- =============================================
				function SectionFunctions:MultiToggle(Settings, Flag)
					local MTFunctions = { Settings = Settings, IgnoreConfig = false, Class = "MultiToggle" }
					local selected = Settings.Default or (Settings.Options and Settings.Options[1]) or ""

					local mtFrame = Instance.new("Frame")
					mtFrame.Name = "MultiToggle"
					mtFrame.AutomaticSize = Enum.AutomaticSize.Y
					mtFrame.BackgroundTransparency = 1
					mtFrame.BorderSizePixel = 0
					mtFrame.Size = UDim2.new(1, 0, 0, 0)
					mtFrame.Parent = section

					local mtList = Instance.new("UIListLayout")
					mtList.Padding = UDim.new(0, 6)
					mtList.SortOrder = Enum.SortOrder.LayoutOrder
					mtList.Parent = mtFrame

					local mtTitle = Instance.new("TextLabel")
					mtTitle.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
					mtTitle.Text = Settings.Name
					mtTitle.RichText = true
					mtTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
					mtTitle.TextSize = 13
					mtTitle.TextTransparency = 0.4
					mtTitle.TextXAlignment = Enum.TextXAlignment.Left
					mtTitle.BackgroundTransparency = 1
					mtTitle.BorderSizePixel = 0
					mtTitle.LayoutOrder = 0
					mtTitle.Size = UDim2.new(1, 0, 0, 18)
					mtTitle.Parent = mtFrame

					local segRow = Instance.new("Frame")
					segRow.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					segRow.BackgroundTransparency = 0.94
					segRow.BorderSizePixel = 0
					segRow.LayoutOrder = 1
					segRow.Size = UDim2.new(1, 0, 0, 30)
					segRow.ClipsDescendants = true
					segRow.Parent = mtFrame

					local segCorner = Instance.new("UICorner")
					segCorner.CornerRadius = UDim.new(0, 8)
					segCorner.Parent = segRow

					local segStroke = Instance.new("UIStroke")
					segStroke.Color = Color3.fromRGB(255, 255, 255)
					segStroke.Transparency = 0.9
					segStroke.Parent = segRow
					...
					local function SelectOption(opt)
						selected = opt
						MTFunctions.Value = opt
						for o, parts in pairs(segButtons) do
							local isOn = (o == opt)
							Tween(parts.bg, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {
								BackgroundTransparency = isOn and 0.75 or 1
							}):Play()
							Tween(parts.lbl, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
								TextTransparency = isOn and 0.05 or 0.55
							}):Play()
						end
					end
					...
					for i, opt in ipairs(Settings.Options or {}) do
						...
						segBtn.BackgroundColor3 = Color3.fromRGB(100, 140, 255)
						segBtn.BackgroundTransparency = 1
						segBtn.BorderSizePixel = 0
						segBtn.LayoutOrder = i
						segBtn.Size = UDim2.new(1 / #Settings.Options, 0, 1, 0)
						segBtn.Parent = segRow

						local segBtnCorner = Instance.new("UICorner")
						segBtnCorner.CornerRadius = UDim.new(0, 8)
						segBtnCorner.Parent = segBtn

						local segLbl = Instance.new("TextLabel")
						segLbl.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
						segLbl.Text = opt
						segLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
						segLbl.TextSize = 12
						segLbl.TextTransparency = 0.55
						segLbl.BackgroundTransparency = 1
						segLbl.BorderSizePixel = 0
						segLbl.Size = UDim2.fromScale(1, 1)
						segLbl.Parent = segBtn

						segButtons[opt] = { bg = segBtn, lbl = segLbl }
					...
					function MTFunctions:SetValue(v) SelectOption(v) end
					function MTFunctions:GetValue() return selected end
					function MTFunctions:SetVisibility(State) mtFrame.Visible = State end

					if Flag then MacLib.Options[Flag] = MTFunctions end
					return MTFunctions
				end

				-- =============================================
				-- NEW ELEMENT 9: Badge (status indicator row)
				-- Usage: section:Badge({ Name="Status", Text="Online", Color=Color3..., Icon="rbxassetid://..." })
				-- =============================================
				function SectionFunctions:Badge(Settings, Flag)
					local BadgeFunctions = { Settings = Settings }

					local badgeFrame = Instance.new("Frame")
					badgeFrame.Name = "Badge"
					badgeFrame.BackgroundTransparency = 1
					badgeFrame.BorderSizePixel = 0
					badgeFrame.Size = UDim2.new(1, 0, 0, 32)
					badgeFrame.Parent = section

					local badgeLabel = Instance.new("TextLabel")
					badgeLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
					badgeLabel.Text = Settings.Name
					badgeLabel.RichText = true
					badgeLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
					badgeLabel.TextSize = 13
					badgeLabel.TextTransparency = 0.4
					...
					badgeLabel.Position = UDim2.fromScale(0, 0.5)
					badgeLabel.Size = UDim2.new(1, -90, 1, 0)
					badgeLabel.Parent = badgeFrame

					local badgePill = Instance.new("Frame")
					badgePill.BackgroundColor3 = Settings.Color or Color3.fromRGB(80, 200, 120)
					badgePill.BackgroundTransparency = 0.75
					badgePill.BorderSizePixel = 0
					badgePill.AnchorPoint = Vector2.new(1, 0.5)
					badgePill.Position = UDim2.new(1, 0, 0.5, 0)
					badgePill.AutomaticSize = Enum.AutomaticSize.X
					badgePill.Size = UDim2.fromOffset(0, 20)
					badgePill.Parent = badgeFrame

					local badgePillCorner = Instance.new("UICorner")
					badgePillCorner.CornerRadius = UDim.new(1, 0)
					badgePillCorner.Parent = badgePill

					local badgePillPad = Instance.new("UIPadding")
					badgePillPad.PaddingLeft = UDim.new(0, 8)
					badgePillPad.PaddingRight = UDim.new(0, 8)
					badgePillPad.Parent = badgePill

					local badgeText = Instance.new("TextLabel")
					badgeText.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					badgeText.Text = Settings.Text or "OK"
					badgeText.TextColor3 = Settings.Color or Color3.fromRGB(80, 200, 120)
					badgeText.TextSize = 11
					badgeText.TextTransparency = 0.1
					badgeText.AutomaticSize = Enum.AutomaticSize.X
					badgeText.BackgroundTransparency = 1
					badgeText.BorderSizePixel = 0
					badgeText.Size = UDim2.fromOffset(0, 20)
					badgeText.Parent = badgePill

					function BadgeFunctions:SetText(t)
						badgeText.Text = t
					end
					function BadgeFunctions:SetColor(c)
						badgePill.BackgroundColor3 = c
						badgeText.TextColor3 = c
					end
					function BadgeFunctions:SetVisibility(State) badgeFrame.Visible = State end

					if Flag then MacLib.Options[Flag] = BadgeFunctions end
					return BadgeFunctions
				end

				-- =============================================
				-- NEW ELEMENT 10: ImagePreview (display an image asset in section)
				-- Usage: section:ImagePreview({ Name="Preview", Image="rbxassetid://...", Height=80, Caption="Optional caption" })
				-- =============================================
				function SectionFunctions:ImagePreview(Settings, Flag)
					local IPFunctions = { Settings = Settings }
					local height = Settings.Height or 80

					local ipFrame = Instance.new("Frame")
					ipFrame.Name = "ImagePreview"
					ipFrame.AutomaticSize = Enum.AutomaticSize.Y
					ipFrame.BackgroundTransparency = 1
					ipFrame.BorderSizePixel = 0
					ipFrame.Size = UDim2.new(1, 0, 0, 0)
					ipFrame.Parent = section

					local ipList = Instance.new("UIListLayout")
					ipList.Padding = UDim.new(0, 5)
					ipList.SortOrder = Enum.SortOrder.LayoutOrder
					ipList.Parent = ipFrame

					if Settings.Name and Settings.Name ~= "" then
						local ipLabel = Instance.new("TextLabel")
						ipLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
						ipLabel.Text = Settings.Name
						ipLabel.RichText = true
						ipLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
						ipLabel.TextSize = 13
						ipLabel.TextTransparency = 0.4
						ipLabel.TextXAlignment = Enum.TextXAlignment.Left
						ipLabel.BackgroundTransparency = 1
						ipLabel.BorderSizePixel = 0
						ipLabel.LayoutOrder = 0
						ipLabel.Size = UDim2.new(1, 0, 0, 18)
						ipLabel.Parent = ipFrame
					end

					local imgContainer = Instance.new("Frame")
					imgContainer.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					imgContainer.BackgroundTransparency = 0.97
					imgContainer.BorderSizePixel = 0
					imgContainer.ClipsDescendants = true
					imgContainer.LayoutOrder = 1
					imgContainer.Size = UDim2.new(1, 0, 0, height)
					imgContainer.Parent = ipFrame

					local imgContCorner = Instance.new("UICorner")
					imgContCorner.CornerRadius = UDim.new(0, 8)
					imgContCorner.Parent = imgContainer

					local imgContStroke = Instance.new("UIStroke")
					imgContStroke.Color = Color3.fromRGB(255, 255, 255)
					imgContStroke.Transparency = 0.9
					imgContStroke.Parent = imgContainer
					...
					if Settings.Caption then
						local ipCaption = Instance.new("TextLabel")
						ipCaption.FontFace = Font.new(assets.interFont, Enum.FontWeight.Regular)
						ipCaption.Text = Settings.Caption
						ipCaption.TextColor3 = Color3.fromRGB(200, 200, 210)
						ipCaption.TextSize = 11
						ipCaption.TextTransparency = 0.5
						ipCaption.TextWrapped = true
						ipCaption.TextXAlignment = Enum.TextXAlignment.Left
						ipCaption.AutomaticSize = Enum.AutomaticSize.Y
						ipCaption.BackgroundTransparency = 1
						ipCaption.BorderSizePixel = 0
						ipCaption.LayoutOrder = 2
						ipCaption.Size = UDim2.new(1, 0, 0, 0)
						ipCaption.Parent = ipFrame
					end

					function IPFunctions:SetImage(id)
						img.Image = id
					end
					function IPFunctions:SetVisibility(State) ipFrame.Visible = State end

					if Flag then MacLib.Options[Flag] = IPFunctions end
					return IPFunctions
				end
	...
	function WindowFunctions:Notify(Settings)
		...
		local notifWidth = Settings.SizeX or 270
		...
		local accentColor = typeColors[Settings.Type or "Default"] or typeColors.Default

		local notification = Instance.new("Frame")
		notification.Name = "Notification"
		notification.AutomaticSize = Enum.AutomaticSize.Y
		notification.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
		...
		notificationUIStroke.Transparency = 0.92
		...
		accentBarCorner.CornerRadius = UDim.new(0, 3)
		accentBarCorner.Parent = accentBar

		-- Inner content
		local notificationInner = Instance.new("Frame")
		notificationInner.Name = "NotificationInner"
		notificationInner.AutomaticSize = Enum.AutomaticSize.Y
		notificationInner.BackgroundTransparency = 1
		notificationInner.BorderSizePixel = 0
		notificationInner.Size = UDim2.new(1, -3, 0, 0)
		notificationInner.Position = UDim2.fromOffset(3, 0)
		...
		innerPadding.Parent = notificationInner
		...
		headerRow.Parent = notificationInner

		-- Dismiss (✕) button top right
		local dismissBtn = Instance.new("TextButton")
		dismissBtn.Name = "Dismiss"
		dismissBtn.FontFace = Font.new(assets.interFont, Enum.FontWeight.Light)
		dismissBtn.Text = "✕"
		dismissBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
		dismissBtn.TextSize = 11
		dismissBtn.TextTransparency = 0.6
		...
		dismissBtn.Position = UDim2.fromScale(1, 0)
		dismissBtn.Size = UDim2.fromOffset(18, 18)
		dismissBtn.Parent = headerRow

		dismissBtn.MouseEnter:Connect(function()
			Tween(dismissBtn, TweenInfo.new(0.15, Enum.EasingStyle.Sine), { TextTransparency = 0.1 }):Play()
		end)
		dismissBtn.MouseLeave:Connect(function()
			Tween(dismissBtn, TweenInfo.new(0.15, Enum.EasingStyle.Sine), { TextTransparency = 0.6 }):Play()
		end)

		-- Notification title
		local notificationTitle = Instance.new("TextLabel")
		notificationTitle.Name = "NotificationTitle"
		notificationTitle.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
		notificationTitle.RichText = true
		notificationTitle.Text = Settings.Title
		notificationTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
		notificationTitle.TextSize = 13
		notificationTitle.TextTransparency = 0.05
		...
		notificationTitle.Size = UDim2.new(1, -20, 0, 0)
		notificationTitle.Parent = headerRow

		-- Description
		local notificationDescription = Instance.new("TextLabel")
		notificationDescription.Name = "NotificationDescription"
		notificationDescription.FontFace = Font.new(assets.interFont, Enum.FontWeight.Regular)
		notificationDescription.Text = Settings.Description or ""
		notificationDescription.TextColor3 = Color3.fromRGB(210, 210, 220)
		notificationDescription.TextSize = 12
		notificationDescription.TextTransparency = 0.25
		...
		notificationDescription.Visible = (Settings.Description ~= nil and Settings.Description ~= "")
		notificationDescription.Parent = notificationInner

		local innerList = Instance.new("UIListLayout")
		innerList.Padding = UDim.new(0, 5)
		innerList.SortOrder = Enum.SortOrder.LayoutOrder
		innerList.Parent = notificationInner

		headerRow.LayoutOrder = 0
		notificationDescription.LayoutOrder = 1

		-- Progress bar at bottom (shows lifetime countdown)
		local progressBarBg = Instance.new("Frame")
		progressBarBg.Name = "ProgressBarBg"
		progressBarBg.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		progressBarBg.BackgroundTransparency = 0.92
		progressBarBg.BorderSizePixel = 0
		progressBarBg.AnchorPoint = Vector2.new(0, 1)
		progressBarBg.Position = UDim2.fromScale(0, 1)
		...
		notificationUIScale.Scale = 0.85
		...
		local tweens = {
			...
			Out = Tween(notificationUIScale, TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
				Scale = 0.85
			}),
			FadeOut = Tween(notification, TweenInfo.new(0.2, Enum.EasingStyle.Sine), {
				BackgroundTransparency = 1
			}),
		}
		...
		-- Also allow click anywhere on notification to dismiss (optional)
		local AnimateNotification = task.spawn(function()
			tweens.In:Play()

			local lifetime = Settings.Lifetime or 4
			if lifetime ~= 0 then
				-- Animate progress bar shrinking
				Tween(progressBar, TweenInfo.new(lifetime, Enum.EasingStyle.Linear), {
		...
		function NotificationFunctions:UpdateDescription(New)
			...
			notificationDescription.Visible = (New ~= nil and New ~= "")
		end

		function NotificationFunctions:Resize(X)
			notification.Size = UDim2.fromOffset(X or 270, 0)
		end
	...
	-- Mobile collapse: fold window to just the titlebar
	local function CollapseWindow()
		isCollapsed = not isCollapsed
		maximize.Text = isCollapsed and "▣" or "□"
		if isCollapsed then
			-- Animate fold: shrink content area
			Tween(base, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
				Size = UDim2.new(base.Size.X.Scale, base.Size.X.Offset, 0, 31)
			}):Play()
		else
	...
	-- Enable mobile collapse button
	maximize.Active = true
	maximize.Interactable = true
	maximize.TextTransparency = 0.45
	maximize.MouseEnter:Connect(function()
		Tween(maximize, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
			BackgroundTransparency = 0.94, TextTransparency = 0.1
		}):Play()
	end)
	maximize.MouseLeave:Connect(function()
		Tween(maximize, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
			BackgroundTransparency = 1, TextTransparency = 0.45
		}):Play()
	end)
	maximize.MouseButton1Click:Connect(CollapseWindow)
	...
	local function ToggleMenu()
		...
		WindowFunctions:Notify({
			...
			Description = (state and "Opened " or "Hidden ") .. "the menu. Press " .. tostring(MenuKeybind.Name) .. " to toggle.",
			Lifetime = 4
		})
	end
	...
	exit.MouseButton1Click:Connect(function()
		WindowFunctions:Dialog({
			Title = Settings.Title,
			Description = "Are you sure you want to exit? You will lose any unsaved configurations.",
			Buttons = {
				{
					Name = "Confirm",
					Callback = function()
...
function MacLib:Demo()
	local Window = MacLib:Window({
		Title = "Maclib Demo",
		Subtitle = "Enhanced UI Library",
		Icon = "rbxassetid://18821914323", -- Optional window icon
	...
	sections.MainSection1:Header({
		Name = "Core Elements",
		Description = "Basic interactive controls for your script."
	})

	sections.MainSection1:Button({
		Name = "Open Dialog",
		Description = "Shows a confirmation dialog box.",
		Callback = function()
			Window:Dialog({
				Title = Window.Settings.Title,
				...
				Buttons = {
					{
						Name = "Confirm",
						Callback = function()
							Window:Notify({ Title = "Confirmed!", Description = "Action was confirmed.", Type = "Success" })
						end,
					},
					{ Name = "Cancel" }
				}
	...
	sections.MainSection1:Checkbox({
		Name = "Accept Terms",
		Description = "You must accept the terms to continue.",
		Default = false,
		Callback = function(v)
			Window:Notify({ Title = "Checkbox", Description = v and "Accepted!" or "Unchecked.", Type = v and "Success" or "Default" })
		end
	}, "Checkbox")
	...
	sections.MainSection1:Input({
		...
		AcceptedCharacters = "All",
		Callback = function(input)
			Window:Notify({ Title = "Input", Description = "Set to: " .. input, Type = "Info" })
		end,
		onChanged = function(input)
			print("Input changed: " .. input)
		end,
	}, "Input")

	sections.MainSection1:Stepper({
		Name = "Quantity",
		Default = 1,
		Min = 0,
		Max = 20,
	...
	sections.MainSection1:MultiToggle({
		Name = "Speed Mode",
		Options = {"Slow", "Normal", "Fast"},
		Default = "Normal",
		Callback = function(v)
			Window:Notify({ Title = "Speed", Description = "Set to " .. v, Type = "Info" })
		end
	}, "SpeedMode")

	sections.MainSection1:RadioGroup({
		Name = "Team",
		Options = {"Red", "Blue", "Green"},
		Default = "Blue",
		Callback = function(v)
			Window:Notify({ Title = "Team", Description = "Joined " .. v, Type = "Success" })
		end
	}, "Team")
	...
	sections.MainSection2:Header({
		...
		Description = "Display-only and information elements."
	})

	sections.MainSection2:NumberDisplay({
		Name = "Ping",
		Value = 42,
		Unit = "ms",
	...
	sections.MainSection2:ProgressBar({
		Name = "Health",
		Value = 72,
		Max = 100,
	...
	sections.MainSection2:Badge({ Name = "Connection", Text = "Online", Color = Color3.fromRGB(80, 200, 120) })
	sections.MainSection2:Badge({ Name = "Mode", Text = "Game", Color = Color3.fromRGB(100, 160, 255) })

	sections.MainSection2:SeparatorLabel({ Text = "Tags" })

	sections.MainSection2:Chips({
		Name = "Categories",
		Options = {"PvP", "PvE", "Raid", "Farm", "Boss"},
		Default = {"PvP"},
	...
	sections.MainSection2:Keybind({
		...
		Callback = function(binded)
			Window:Notify({ Title = "Keybind", Description = "Pressed: " .. tostring(binded.Name), Type = "Info", Lifetime = 3 })
		end,
		onBinded = function(bind)
			Window:Notify({ Title = "Keybind", Description = "Bound to: " .. tostring(bind.Name), Type = "Success", Lifetime = 3 })
		end,
	}, "Keybind")
