...
local SaveManager = {} do
	...
	if not RunService:IsStudio() then
		...
		function SaveManager:LoadAutoloadConfig()
			-- delay 1 frame so user script has time to register all elements into Library.Options
...
Library.CreateWindow = function(self, Config)
	...
	-- ════════════════════════════════════════════════════════════════
	-- Layout System v2
	-- พับ  → เหลือแค่ title bar ติดขอบหน้าจอ  (ตามภาพ sketch)
	-- เปิด → ดึงหน้าต่างเต็มออกมาพร้อม slide animation
	-- ใช้ Layout = "top"|"bottom"|"left"|"right" ใน CreateWindow
	-- ไม่ใส่ = หน้าต่างตรงกลางปกติ
	-- ════════════════════════════════════════════════════════════════
	if Config.Layout then
		task.defer(function()
			if not (Window.Root and Window.Root.Parent) then return end

			local layout = string.lower(tostring(Config.Layout))
			local vp     = workspace.Camera.ViewportSize
			local winW   = (Config.Size and Config.Size.X and Config.Size.X.Offset) or 620
			local winH   = (Config.Size and Config.Size.Y and Config.Size.Y.Offset) or 450

			-- ── ค้นหา TitleBar จาก Root ─────────────────────────
			-- TitleBar คือ frame แรกใน Root ที่สูงประมาณ 40-50 px
			local titleBarH = 46   -- ความสูง title bar โดยประมาณ
			local titleBarW = winW -- เต็มความกว้างหน้าต่าง

			-- ── ตำแหน่งต่างๆ ────────────────────────────────────
			-- expandedPos  = ตำแหน่งหน้าต่างเต็มติดขอบ
			-- collapsedPos = ตำแหน่งพับ (เหลือแค่ title bar)
			local expandedPos, collapsedPos

			if layout == "top" then
				expandedPos  = UDim2.fromOffset(math.floor((vp.X - winW) / 2), 0)
				collapsedPos = UDim2.fromOffset(math.floor((vp.X - winW) / 2), -(winH - titleBarH))

			elseif layout == "bottom" then
				expandedPos  = UDim2.fromOffset(math.floor((vp.X - winW) / 2), math.floor(vp.Y - winH))
				collapsedPos = UDim2.fromOffset(math.floor((vp.X - winW) / 2), math.floor(vp.Y - titleBarH))

			elseif layout == "left" then
				expandedPos  = UDim2.fromOffset(0, math.floor((vp.Y - winH) / 2))
				collapsedPos = UDim2.fromOffset(-(winW - titleBarH), math.floor((vp.Y - winH) / 2))

			elseif layout == "right" then
				expandedPos  = UDim2.fromOffset(math.floor(vp.X - winW), math.floor((vp.Y - winH) / 2))
				collapsedPos = UDim2.fromOffset(math.floor(vp.X - titleBarH), math.floor((vp.Y - winH) / 2))

			else
				return -- layout ไม่รู้จัก
			end

			-- ── วางหน้าต่างตำแหน่งพับก่อน แล้ว slide เปิด ───────
			Window.Root.Position = collapsedPos
			Window.Position      = expandedPos
			Window._layoutMode   = layout

			-- ── สร้างปุ่ม Toggle พับ/เปิด ────────────────────────
			-- ปุ่มนี้ติดอยู่กับ title bar เสมอ ไม่ว่าจะพับหรือเปิด
			-- ตำแหน่งและรูปร่างขึ้นกับทิศทาง layout

			-- ไอคอนลูกศรตาม layout และ state
			local arrowExpanded  = { top = "▼", bottom = "▲", left = "▶", right = "◀" }
			local arrowCollapsed = { top = "▲", bottom = "▼", left = "◀", right = "▶" }

			-- ขนาดและตำแหน่งปุ่มใน title bar
			-- top/bottom: ปุ่มอยู่กลางล่าง/บน title bar ขนาด 60×18
			-- left/right:  ปุ่มอยู่กลางขวา/ซ้าย title bar ขนาด 18×60
			local isHorizontal = (layout == "top" or layout == "bottom")

			local btnW = isHorizontal and 56 or 18
			local btnH = isHorizontal and 18 or 56

			-- anchor และ pos ของปุ่มบน Window.Root
			local btnAnchor, btnPos
			if layout == "top" then
				btnAnchor = Vector2.new(0.5, 1)
				btnPos    = UDim2.new(0.5, 0, 1, 0)           -- ล่างสุดของ root (ชิดขอบนอก)
			elseif layout == "bottom" then
				btnAnchor = Vector2.new(0.5, 0)
				btnPos    = UDim2.new(0.5, 0, 0, 0)           -- บนสุดของ root
			elseif layout == "left" then
				btnAnchor = Vector2.new(1, 0.5)
				btnPos    = UDim2.new(1, 0, 0.5, 0)           -- ขวาสุดของ root
			elseif layout == "right" then
				btnAnchor = Vector2.new(0, 0.5)
				btnPos    = UDim2.new(0, 0, 0.5, 0)           -- ซ้ายสุดของ root
			end

			local isExpanded = false -- เริ่มต้นพับ

			local ToggleBtn = New("TextButton", {
				Name                   = "LayoutToggleBtn",
				Text                   = arrowCollapsed[layout],
				...
				Parent                 = Window.Root,
				ThemeTag               = { BackgroundColor3 = "AcrylicMain", TextColor3 = "Accent" },
			}, {
				New("UICorner", {
					CornerRadius = isHorizontal
						and UDim.new(0, 6)
						or  UDim.new(0, 6),
				}),
				New("UIStroke", {
			...
			-- hover effect บนปุ่ม
			Creator.AddSignal(ToggleBtn.MouseEnter, function()
				TweenService:Create(ToggleBtn, TweenInfo.new(0.15), {
					BackgroundTransparency = 0.0,
				}):Play()
			end)
			Creator.AddSignal(ToggleBtn.MouseLeave, function()
				TweenService:Create(ToggleBtn, TweenInfo.new(0.15), {
					BackgroundTransparency = 0.15,
				}):Play()
			end)
			...
			local function setExpanded(expand, instant)
				isExpanded = expand
				local targetPos = expand and expandedPos or collapsedPos
				ToggleBtn.Text  = expand and arrowExpanded[layout] or arrowCollapsed[layout]

				if instant then
					Window.Root.Position = targetPos
				else
					TweenService:Create(Window.Root, tweenInfo, { Position = targetPos }):Play()
				end

				-- เปลี่ยนสี stroke ปุ่มตาม state
				local stroke = ToggleBtn:FindFirstChildOfClass("UIStroke")
				if stroke then
					TweenService:Create(stroke, TweenInfo.new(0.2), {
						Transparency = expand and 0.2 or 0.45,
					}):Play()
				end
			end
			...
			-- เริ่มพับ ไม่ animate
			setExpanded(false, true)

		end) -- end task.defer
	end
	-- ════════════════════════════════════════════════════════════════
	-- End Layout System v2
	-- ════════════════════════════════════════════════════════════════
