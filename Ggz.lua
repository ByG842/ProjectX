	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:AddSubTab(Title, Icon)
			...
			if not self.SubTabHolder then
				-- 🏷️ Row that holds the sub-tab buttons (left) AND a label with the
				-- current main Tab's name (right), so the empty space next to the
				-- sub-tabs (e.g. "Interface" / "Config") isn't left blank.
				...
				self.SubTabRow = SubTabRow

				local CurrentTabNameLabel = New("TextLabel", {
					Text = self.Name,
					RichText = true,
					FontFace = GetStyleProperty("FontMedium"),
					TextSize = GetStyleProperty("TextSizeSm"),
				...
				self.CurrentTabNameLabel = CurrentTabNameLabel
