...
ElementsTable.RadioGroup = (function()
	...
	function Element:New(Idx, Config)
		...
		local OptionsHolder = New("Frame", {
			...
			New("UIListLayout", { Padding = UDim.new(0, 4), SortOrder = Enum.SortOrder.LayoutOrder }), -- padding ลดลงนิดหน่อยเพราะแต่ละแถวมีการ์ดพื้นหลังเองแล้ว (Rework)
		...
		-- 🎨🎬 Rework: การ์ดพื้นหลังต่อแถว + label เปลี่ยนสี/หนาขึ้นตอนเลือก + dot pop แบบสปริง
		local function UpdateRadio(pulseOpt)
			for _, btn in pairs(Buttons) do
				...
				local dotTi = TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out) -- สปริงเด้งนิดๆ ตอน dot โผล่/หด แทน Quint เรียบๆ เดิม
				...
				-- การ์ดพื้นหลังทั้งแถวติดสี Accent จางๆ ตอนถูกเลือก ให้ดูเป็น "รายการที่เลือกอยู่" ชัดเจน
			...
			if pulseOpt then
				for _, btn in pairs(Buttons) do
					if btn.Value == pulseOpt then
						...
						TweenService:Create(btn.CardScale, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
					end
				end
			end
		end
		...
		for _, opt in ipairs(Config.Options) do
			...
			local Row = New("TextButton", {
				...
				BackgroundTransparency = 1, -- การ hover/selected tint คุมที่ Card แยกต่างหาก กัน property ชนกัน
				Text = "",
			...
			-- 🎨 Rework: การ์ดพื้นหลังของแถว แยกจากตัว Row (TextButton) เพื่อ tween transparency ได้อิสระจาก hover state
			local Card = Row
			...
			local BtnData = { Value = opt, Outer = Outer, Inner = Inner, Card = Card, CardStroke = CardStroke, CardScale = CardScale, Label = Label }
			table.insert(Buttons, BtnData)

			-- 🎬 Rework: hover feedback เบาๆ (การ์ดจางๆ ขึ้นมาเป็นสีเทา ถ้ายังไม่ได้เลือก)
			Creator.AddSignal(Row.MouseEnter, function()
				...
				if not isSelected then
					TweenService:Create(Card, TweenInfo.new(0.15), { BackgroundTransparency = 0.94 }):Play()
					Creator.OverrideTag(Card, { BackgroundColor3 = "Element" })
				end
			end)
			...
			Creator.AddSignal(Row.MouseButton1Click, function()
				...
				-- คืนพื้นการ์ดของทุกตัวกลับเป็นสี Accent ก่อน UpdateRadio จะ tween ทับ (กันค้างเป็นสีเทาจาก hover)
				for _, b in pairs(Buttons) do
					Creator.OverrideTag(b.Card, { BackgroundColor3 = "Accent" })
				end
...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		-- 🎨🎬 Rework: อัปเดตสถานะปุ่มด้วย spring/back easing ให้เด้งนุ่มๆ แทน Quad เรียบๆ เดิม
		-- + checkmark ไอคอนโผล่มาแบบ scale-in ตอนเลือก + glow ring รอบปุ่มตอน active
		local function updateChip(item, btn)
			local active = table.find(CH.Value, item) ~= nil
			local stroke = btn:FindFirstChildOfClass("UIStroke")
			local check = btn:FindFirstChild("CheckIcon")
			local checkScale = check and check:FindFirstChildOfClass("UIScale")
			local label = btn:FindFirstChild("ChipLabel")

			local popTi = TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
			local fadeTi = TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

			TweenService:Create(btn, popTi, {
				BackgroundTransparency = active and 0.1 or 0.92, -- กดแล้วสว่างเด่นชัดขึ้น (จาก 0.15)
			}):Play()

			if stroke then
				TweenService:Create(stroke, fadeTi, {
					Transparency = active and 0 or 0.85, -- ขอบชัดเต็มตอน active ให้ดูเหมือนมี glow ring
					Thickness = active and 1.5 or GetStyleProperty("BorderThickness"),
				}):Play()
			end

			if label then
				TweenService:Create(label, fadeTi, { TextTransparency = active and 0 or 0.4 }):Play()
				label.FontFace = active and GetStyleProperty("FontBold") or GetStyleProperty("FontMedium")
			end

			if check and checkScale then
				check.Visible = true -- คุมด้วย transparency/scale แทน visible กัน pop กระตุก
				TweenService:Create(check, fadeTi, { ImageTransparency = active and 0 or 1 }):Play()
				checkScale.Scale = active and 0.4 or 1 -- เริ่มจากตัวเล็กๆ ก่อนค่อยเด้งโต ให้ความรู้สึก pop-in จริงๆ
				TweenService:Create(checkScale, popTi, { Scale = active and 1 or 0.4 }):Play()
			end
		end

		for _, item in ipairs(Config.Items) do
			local ChipScale = New("UIScale", { Scale = 1 }) -- Rework: ใช้ทำ press-pop ตอนกด

			local CheckIcon = New("ImageLabel", { -- Rework: checkmark เล็กๆ โผล่มาตอนเลือก ให้ chip อ่านสถานะง่ายขึ้นแค่มองผ่านๆ
				Name = "CheckIcon",
				Image = Library:GetIcon("check") or "",
				Size = UDim2.fromOffset(12, 12),
				ImageTransparency = 1,
				BackgroundTransparency = 1,
				LayoutOrder = 1,
			...
			-- 🎬 Rework: hover ยกตัวขึ้นนิดๆ (สเกลโตเบาๆ) เฉพาะตอนยังไม่ได้เลือก ให้รู้สึก interactive
