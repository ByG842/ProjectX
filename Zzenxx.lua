...
local Themes = {
	...
	-- ✨ Minimal Flat — เรียบ สะอาด ไม่มีเงา/ไล่เฉด แต่ใช้โทนเทาอมฟ้า + accent สีฟ้าสดให้ดูมีดีไซน์ ไม่จืดจนไร้จุดสนใจ
	Minimal = {
		Name                = "Minimal",
		Accent              = Color3.fromRGB(94, 156, 255),        -- ฟ้าสดอ่อนๆ สไตล์ Linear/Notion
		AcrylicMain         = Color3.fromRGB(18, 18, 21),
		AcrylicBorder       = Color3.fromRGB(18, 18, 21),           -- เท่ากับพื้น = ไร้ขอบ/ไร้เงา
		AcrylicGradient     = ColorSequence.new(Color3.fromRGB(18, 18, 21), Color3.fromRGB(18, 18, 21)), -- flat สนิท ไม่ไล่เฉด
		AcrylicNoise        = 1,                                    -- ปิดพื้นผิวนอยส์
		TitleBarLine        = Color3.fromRGB(28, 28, 32),
		Tab                 = Color3.fromRGB(132, 138, 158),
		Element             = Color3.fromRGB(26, 26, 30),           -- อ่อนกว่าพื้นนิดเดียว ให้แยกชั้นได้โดยไม่ต้องมีเงา
		ElementBorder       = Color3.fromRGB(26, 26, 30),           -- ไม่มีเส้นขอบ (สีเดียวกับตัว element)
		InElementBorder     = Color3.fromRGB(40, 40, 47),
		ElementTransparency = 1,                                    -- flat เต็ม ไม่มีชั้นความโปร่งซ้อน
		ToggleSlider        = Color3.fromRGB(230, 230, 235),
		ToggleToggled       = Color3.fromRGB(94, 156, 255),         -- ใช้สี accent ตอนเปิด toggle ให้เห็นชัดว่า active
		SliderRail          = Color3.fromRGB(40, 40, 47),
		DropdownFrame       = Color3.fromRGB(26, 26, 30),
		DropdownHolder      = Color3.fromRGB(22, 22, 25),
		DropdownBorder      = Color3.fromRGB(22, 22, 25),
		DropdownOption      = Color3.fromRGB(34, 34, 39),
		Keybind             = Color3.fromRGB(34, 34, 39),
		Input               = Color3.fromRGB(26, 26, 30),
		InputFocused        = Color3.fromRGB(22, 22, 25),
		InputIndicator      = Color3.fromRGB(70, 70, 78),
		InputIndicatorFocus = Color3.fromRGB(94, 156, 255),
		Dialog              = Color3.fromRGB(22, 22, 25),
		DialogHolder        = Color3.fromRGB(18, 18, 21),
		DialogHolderLine    = Color3.fromRGB(18, 18, 21),
		DialogButton        = Color3.fromRGB(26, 26, 30),
		DialogButtonBorder  = Color3.fromRGB(26, 26, 30),
		DialogBorder        = Color3.fromRGB(22, 22, 25),
		DialogInput         = Color3.fromRGB(26, 26, 30),
		DialogInputLine     = Color3.fromRGB(70, 70, 78),
		Text                = Color3.fromRGB(240, 240, 244),
		SubText             = Color3.fromRGB(148, 150, 162),
		Hover               = Color3.fromRGB(36, 36, 42),
		HoverChange         = 0.05,                                 -- hover เบามาก ไม่มีเงาแรงๆ
		FlatStyle           = true,                                 -- flag ให้ patch ด้านล่างรู้ว่าต้องปิดเงา/ไล่เฉด
	},
...
-- ✨ Ripple / Glow effect — วงกลมขยายจางหายตอนคลิก + ขอบเรืองแสงตอน hover
-- เรียกใช้กับปุ่ม/element ที่กดได้ เช่น RippleEffect(Button.Frame)
local function RippleEffect(Object, Config)
	Config = Config or {}
	local RippleColor = Config.Color or Color3.fromRGB(255, 255, 255)
	local MaxAlpha = Config.Transparency or 0.7
	local Duration = Config.Duration or 0.45

	local GlowStroke = Instance.new("UIStroke")
	GlowStroke.Name = "__RippleGlow"
	GlowStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
	GlowStroke.Thickness = 1.25
	GlowStroke.Color = RippleColor
	GlowStroke.Transparency = 1
	GlowStroke.Parent = Object

	Object.MouseEnter:Connect(function()
		TweenService:Create(GlowStroke, TweenInfo.new(0.18, Enum.EasingStyle.Quad), { Transparency = 0.55 }):Play()
	end)
	Object.MouseLeave:Connect(function()
		TweenService:Create(GlowStroke, TweenInfo.new(0.18, Enum.EasingStyle.Quad), { Transparency = 1 }):Play()
	end)
	...
	Object.MouseButton1Down:Connect(function(x, y)
		local ok = pcall(function()
			...
			local targetDiameter = math.max(absSize.X, absSize.Y) * 2.2
			local growTween = TweenService:Create(Ripple, TweenInfo.new(Duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				Size = UDim2.fromOffset(targetDiameter, targetDiameter),
...
-- 🌌 Particle background — จุดเล็กๆ ลอยเบาๆ ด้านหลังหน้าต่าง โทน flat ไม่รบกวนสายตา
local function CreateParticleField(Count)
	Count = Count or 22
	local Field = Instance.new("Frame")
	Field.Name = "ParticleField"
	...
	for i = 1, Count do
		...
		dot.BackgroundTransparency = 0.85 + math.random() * 0.1
	...
	conn = RunService.Heartbeat:Connect(function(dt)
		...
		for _, p in ipairs(particles) do
			local pos = p.Object.Position
			local x, y = pos.X.Scale + p.SpeedX * dt, pos.Y.Scale + p.SpeedY * dt
			if x < 0 then x = 1 elseif x > 1 then x = 0 end
			if y < 0 then y = 1 elseif y > 1 then y = 0 end
			p.Object.Position = UDim2.fromScale(x, y)
		end
	end)

	Field.AncestryChanged:Connect(function(_, parent)
...
Components.Button = (function()
	...
	return function(Theme, Parent, DialogCheck)
		...
		-- ✨ ripple/glow effect ตอน hover-click
		RippleEffect(Button.Frame, { Color = Themes[Library.Theme] and Themes[Library.Theme].Accent or Color3.fromRGB(255,255,255) })
...
Components.Notification = (function()
	...
	function Notification:New(Config)
		...
		-- ✨ วงกลมสีอ่อนๆ หลังไอคอน ให้ดูมีมิติขึ้นแทนที่จะเป็นอิโมจิลอยเฉยๆ
		...
		-- ✨ pop-in เบาๆ ตอนเปิด (ย่อเล็กแล้วขยายเด้งขึ้นมา) ให้ดูมีชีวิตชีวากว่าการเลื่อนเข้าเฉยๆ
		NewNotification.PopScale = New("UIScale", { Scale = 0.9 })
		NewNotification.PopScale.Parent = NewNotification.Root
		local PopMotor, SetPopScale = Creator.SpringMotor(0.9, NewNotification.PopScale, "Scale")

		-- ⏸️ hover ค้างไว้ = พักนับเวลาปิดอัตโนมัติ ไม่ให้หลุดหายไปตอนกำลังอ่าน
		local Paused = false
		Creator.AddSignal(NewNotification.Root.MouseEnter, function() Paused = true end)
		Creator.AddSignal(NewNotification.Root.MouseLeave, function() Paused = false end)
		...
		function NewNotification:Open()
			...
			SetPopScale(Spring(1, { frequency = 5, dampingRatio = 0.7 })) -- เด้งนิดๆ ไม่บอบบาง
			...
			-- Progress bar animation (พักนับตอน hover)
		...
		if Config.Duration then
			task.spawn(function()
				local remaining = Config.Duration
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- ✨ Particle background (optional) — เปิดด้วย Config.ParticleBackground = true ตอน CreateWindow
		if Config.ParticleBackground then
			Window.ParticleField = CreateParticleField(Config.ParticleCount or 22)
			table.insert(rootChildren, Window.ParticleField)
		end
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- 🖼️ อัปโหลดพื้นหลังเอง + ให้ AI (สุ่มค่าสีเฉลี่ยจากรูป) ปรับ Accent ให้เข้ากันอัตโนมัติ
		local BackgroundPathInput = customThemeSection:AddInput("CustomBackgroundPath", {
			Title       = "Background Image Path",
			Description = "วาง path ไฟล์รูปที่อยู่ในเครื่อง (ต้องวางไฟล์ไว้ในโฟลเดอร์ที่ executor เข้าถึงได้ก่อน)",
			Placeholder = "เช่น Zzen/UI/Backgrounds/myphoto.png",
			Finished    = true,
		})

		local UploadBackgroundButton = customThemeSection:AddActionButton("UploadBackgroundImage", {
			Title       = "Upload Background",
			Description = "ตั้งรูปนี้เป็นพื้นหลังหน้าต่าง แล้วปรับสีธีมให้เข้ากับรูปอัตโนมัติ",
			ButtonText  = "Upload",
			CopiedText  = "✓ Uploaded",
			CopyText    = "",
			Callback    = function()
				local path = BackgroundPathInput.Value
				if not path or path == "" then
					Library:Notify({ Title = "Background", Content = "กรุณาใส่ path ไฟล์รูปก่อน", Duration = 4 })
					return
				end
				local contentId = Library:UploadBackgroundImage(path)
				if contentId then
		...
		local ResetBackgroundButton = customThemeSection:AddActionButton("ResetBackgroundImage", {
			Title      = "Reset Background",
			ButtonText = "Reset",
			CopiedText = "✓ Reset",
			CopyText   = "",
			Callback   = function()
				if Library.Window and Library.Window.BackgroundImage then
					Library.Window.BackgroundImage.Image = "rbxassetid://13196113628"
				end
				Library.CustomBackgroundContentId = nil
				Library:ResetAccentColor()
				Settings.AccentColor = nil
		...
		-- โหลดพื้นหลังที่เคยตั้งไว้ตอนเปิด UI ใหม่
		if Settings.CustomBackgroundPath and Settings.CustomBackgroundPath ~= "" then
			task.defer(function()
				BackgroundPathInput:SetValue(Settings.CustomBackgroundPath)
				Library:UploadBackgroundImage(Settings.CustomBackgroundPath)
			end)
		end
...
-- ==========================================================================
-- 🖼️  CUSTOM BACKGROUND UPLOAD + AUTO THEME COLOR MATCH
-- ==========================================================================
-- หมายเหตุ: "โฟลเดอร์ UI" ที่นี่คือโฟลเดอร์บนดิสก์ที่ executor เป็นคนสร้าง/อ่านให้
-- (ผ่าน makefolder / writefile / readfile) ไม่ใช่ game.Workspace ในเกม เพราะสคริปต์
-- ฝั่ง client ของ Roblox ไม่มีสิทธิ์เขียนไฟล์ลงเครื่องผู้เล่นโดยตรง ต้องพึ่งฟังก์ชัน
-- ของ executor เท่านั้น ฟีเจอร์นี้จึงใช้ได้เฉพาะ executor ที่รองรับ
-- readfile / writefile / isfile / makefolder / getcustomasset (เช่น Synapse X, Wave, Script-Ware ฯลฯ)
-- ถ้า executor ไม่รองรับ ฟังก์ชันจะแจ้งเตือนแล้ว fallback แบบไม่ error

Library.BackgroundFolder = "Zzen/UI/Backgrounds"

local function EnsureBackgroundFolder()
	if not (isfolder and makefolder) then return false end
	if not isfolder("Zzen") then makefolder("Zzen") end
	if not isfolder("Zzen/UI") then makefolder("Zzen/UI") end
	if not isfolder(Library.BackgroundFolder) then makefolder(Library.BackgroundFolder) end
	return true
end

-- อัปโหลดรูปพื้นหลังจาก path ไฟล์บนเครื่อง (ผู้ใช้ต้องรู้ path ของไฟล์ เช่น
-- วางไฟล์ไว้ในโฟลเดอร์ workspace ของ executor ก่อน แล้วพิมพ์ path ผ่าน Textbox ใน UI)
-- สำเนาไฟล์จะถูกเซฟไว้ใน Library.BackgroundFolder แล้วตั้งเป็นพื้นหลังหน้าต่างทันที
-- คืนค่า contentId ที่ใช้กับ ImageLabel ได้ หรือ nil ถ้าทำไม่สำเร็จ
function Library:UploadBackgroundImage(sourcePath)
	if not (isfile and readfile and writefile and getcustomasset) then
		self:Notify({ Title = "Background", Content = "Executor นี้ไม่รองรับการอัปโหลดรูป (ต้องมี readfile/writefile/getcustomasset)", Duration = 6 })
		return nil
	end
	if not isfile(sourcePath) then
		self:Notify({ Title = "Background", Content = "ไม่พบไฟล์: " .. tostring(sourcePath), Duration = 6 })
		return nil
	end

	EnsureBackgroundFolder()

	local ext = sourcePath:match("%.([%a%d]+)$") or "png"
	local destPath = Library.BackgroundFolder .. "/current." .. ext

	local ok, err = pcall(function()
		writefile(destPath, readfile(sourcePath))
	end)
	if not ok then
		self:Notify({ Title = "Background", Content = "เซฟไฟล์ไม่สำเร็จ: " .. tostring(err), Duration = 6 })
		return nil
	end

	local contentOk, contentId = pcall(getcustomasset, destPath)
	if not contentOk or not contentId then
		self:Notify({ Title = "Background", Content = "โหลดรูปเป็น asset ไม่สำเร็จ", Duration = 6 })
		return nil
	end

	if Library.Window and Library.Window.BackgroundImage then
		Library.Window.BackgroundImage.Image = contentId
		Library.Window.BackgroundImage.ImageTransparency = 0.08
	end

	Library.CustomBackgroundContentId = contentId
	Library.CustomBackgroundPath = destPath

	-- ลองไล่สีธีมให้เข้ากับรูปอัตโนมัติ (best-effort, ไม่ทำให้เกิด error ถ้าใช้ไม่ได้)
	task.spawn(function()
		Library:AutoThemeFromImage(contentId)
	end)

	return contentId
end

-- วิเคราะห์สีเฉลี่ย/สีเด่นจากรูปพื้นหลัง แล้วปรับ Accent ของธีมทั้งชุดให้เข้ากัน
-- ("AI" ในที่นี้คือการ sample สีเฉลี่ยแบบถ่วงน้ำหนักจากพิกเซล ไม่ใช่โมเดล AI จริง)
-- ต้องการ AssetService:CreateEditableImageAsync ซึ่งบาง executor/เกมอาจไม่เปิดให้ใช้ —
-- ถ้าใช้ไม่ได้ฟังก์ชันจะ fail เงียบๆ แล้วคงธีมเดิมไว้
function Library:AutoThemeFromImage(contentId)
	local ok1, AssetService = pcall(game.GetService, game, "AssetService")
	if not ok1 or not AssetService or not AssetService.CreateEditableImageAsync then
		return false
	end

	local ok2, editableImage = pcall(function()
		return AssetService:CreateEditableImageAsync(Content.fromUri(contentId))
	end)
	if not ok2 or not editableImage then
		return false
	end

	local ok3, r, g, b = pcall(function()
		local size = editableImage.Size
		local pixels = editableImage:ReadPixels(Vector2.zero, size) -- ค่าที่ได้อยู่ในช่วง 0-1 ต่อ channel
		local totalR, totalG, totalB, count = 0, 0, 0, 0
		local sampleTarget = 400 -- sample แค่บางจุดพอ ไม่วนทุกพิกเซล เพื่อความเร็ว
		local pixelCount = #pixels / 4
		local step = math.max(1, math.floor(pixelCount / sampleTarget))
		for i = 1, #pixels, 4 * step do
			totalR += pixels[i]
			totalG += pixels[i + 1] or 0
			totalB += pixels[i + 2] or 0
			count += 1
		end
	...
	pcall(function() editableImage:Destroy() end)

	if not ok3 or not r then
		return false
	end

	-- เพิ่มความอิ่มสี/ความสว่างเล็กน้อยให้ accent อ่านง่ายขึ้น ไม่จมกับพื้นหลัง
	local accentColor = Color3.new(math.clamp(r, 0, 1), math.clamp(g, 0, 1), math.clamp(b, 0, 1))
	local h, s, v = accentColor:ToHSV()
	s = math.clamp(s * 1.25, 0, 1)
	v = math.clamp(math.max(v, 0.55), 0, 1)
	accentColor = Color3.fromHSV(h, s, v)

	Library.CustomAccentColor = accentColor
	if Creator.UpdateThemeAnimated then
		Creator.UpdateThemeAnimated(0.35)
	elseif Creator.UpdateTheme then
		Creator.UpdateTheme()
	end

	pcall(function()
		Library:Notify({ Title = "Theme", Content = "ปรับสีธีมให้เข้ากับรูปพื้นหลังแล้ว ✨", Duration = 4 })
	end)
	return true
end
