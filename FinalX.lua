...
ElementsTable.NumberInput = (function()
	...
	function Element:New(Idx, Config)
		...
		local hasRange = NI.Min > -math.huge and NI.Max < math.huge
		...
		-- CanvasGroup แทน Frame เพื่อให้ FillBar ด้านล่างถูกตัดโค้งตาม UICorner จริงๆ
		local Holder = New("CanvasGroup", {
			Size = UDim2.fromOffset(120, 30),
			Position = UDim2.new(1, -10, 0.5, 0),
			AnchorPoint = Vector2.new(1, 0.5),
			BackgroundTransparency = 0.9,
			Active = true, -- ต้องเปิดไว้ถึงจะรับ InputChanged (สกอลล์เมาส์) ได้
		...
		-- ── แถบ fill บาง ๆ ด้านล่างของกล่อง แสดง (Value-Min)/(Max-Min) ──
		-- ให้เห็นตำแหน่งค่าปัจจุบันในช่วงแบบไว ๆ โดยไม่ต้องอ่านตัวเลข
		-- (แสดงเฉพาะตอนที่กำหนด Min/Max จำกัดไว้จริง ไม่งั้นไม่มี range ให้ทำแถบ)
		local FillBar = hasRange and New("Frame", {
		...
		local FillMotor, SetFillWidth
		if FillBar then
			FillMotor, SetFillWidth = Creator.SpringMotor(0, FillBar, "Size", true)
		end
		...
		local makeBtn = function(txt, side)
			...
			local Motor, SetT = Creator.SpringMotor(1, btn, "BackgroundTransparency", true)
		...
		-- ── ไฮไลท์ขอบตอนโฟกัสกล่อง ให้รู้สึกว่ากำลังพิมพ์อยู่ (spring glow) ──
		local FocusGlowMotor, SetFocusGlow = Creator.SpringMotor(0.5, HolderStroke, "Transparency", true)
		Creator.AddSignal(ValBox.Focused, function()
			SetFocusGlow(0.05)
			Creator.OverrideTag(HolderStroke, { Color = "Accent" })
		end)
		...
		local function set(v)
			...
			if FillBar and SetFillWidth then
				local pct = (NI.Max > NI.Min) and ((v - NI.Min) / (NI.Max - NI.Min)) or 0
				SetFillWidth(UDim2.new(math.clamp(pct, 0, 1), 0, 0, 2))
			end
		...
		-- ── Scroll wheel: ชี้เมาส์ค้างไว้แล้วหมุนสกอลล์เพื่อ +/- Step ──
		-- ลูกเล่นที่ CounterButton ไม่มี ทำให้ input ตัวนี้ปรับเลขไวขึ้นโดยไม่ต้องกดปุ่มซ้ำๆ
		Creator.AddSignal(Holder.InputChanged, function(input)
			if input.UserInputType == Enum.UserInputType.MouseWheel then
				set(NI.Value + (input.Position.Z > 0 and NI.Step or -NI.Step))
			end
		end)

		-- ── Drag-to-scrub: กดค้างที่กล่องตัวเลขแล้วลากซ้าย/ขวาเพื่อไล่ค่า ──
		-- (แบบเดียวกับ number field ใน Figma/Blender) ลากไกลขึ้น = เปลี่ยนค่าไวขึ้น
		do
			local dragging   = false
			...
			Creator.AddSignal(ValBox.InputBegan, function(input)
				if input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
				if ValBox:IsFocused() then return end -- กำลังพิมพ์อยู่ ไม่ต้อง scrub
				dragging = true
				dragMoved = false
				dragStartX = input.Position.X
				dragStartValue = NI.Value
			end)
			...
			Creator.AddSignal(UserInputService.InputChanged, function(input)
				...
				if math.abs(delta) > 3 then dragMoved = true end
				if not dragMoved then return end
				-- ทุกๆ 6px ที่ลาก = 1 step, กด Shift ค้างให้ไวขึ้น 5 เท่า
				local speedMult = UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) and 5 or 1
				local steps = math.floor(delta / 6) * speedMult
				set(dragStartValue + steps * NI.Step)
			end)
			...
			Creator.AddSignal(UserInputService.InputEnded, function(input)
				...
				if not dragMoved then
					-- ไม่ได้ลาก ถือว่าเป็นแค่คลิกเพื่อโฟกัสพิมพ์ตามปกติ
					ValBox:CaptureFocus()
				end
			end)
		end

		-- Hold to repeat (ยิ่งกดค้างนาน ยิ่งไล่ค่าไวขึ้นเรื่อยๆ)
			...
			Creator.AddSignal(b.MouseButton1Down, function()
				task.delay(0.5, function()
					...
					while b:IsDescendantOf(game) and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
						...
						task.wait(interval)
						interval = math.max(0.02, interval * 0.93) -- ค่อยๆเร่งความเร็ว
					end
				end)
			end)
		end
		...
		-- ตั้งความกว้าง fill bar เริ่มต้นแบบเงียบๆ (ไม่ยิง Callback ตอนสร้าง
		-- เพื่อคงพฤติกรรมเดิมของ NumberInput ที่ไม่ fire callback ทันทีตอน init)
		if FillBar and SetFillWidth then
			local pct = (NI.Max > NI.Min) and ((NI.Value - NI.Min) / (NI.Max - NI.Min)) or 0
			SetFillWidth(UDim2.new(math.clamp(pct, 0, 1), 0, 0, 2), true)
		end
...
ElementsTable.ActionButton = (function()
	...
	function Element:New(Idx, Config)
		...
		-- ── ระบบคอนเฟิม (optional) ─────────────────────────────────────
		-- ใส่ Config.Confirm = true เพื่อเปิดใช้งาน ก่อนรันแอคชั่นจริงจะเด้ง
		-- Dialog ถาม Confirm/Cancel ก่อนเสมอ (ใช้ Dialog component ของ Library เอง
		-- เลยได้ฟอนต์/สี/อนิเมชั่น fade+scale เข้าธีมทันทีโดยไม่ต้องประกอบ UI ใหม่)
		local confirmEnabled     = Config.Confirm and true or false
		local confirmTitle       = Config.ConfirmTitle       or "Confirm Action"
		local confirmText        = Config.ConfirmText
			or ("Are you sure you want to \"" .. tostring(Config.Title) .. "\"?")
		local confirmButtonText  = Config.ConfirmButtonText  or "Confirm"
		local cancelButtonText   = Config.CancelButtonText   or "Cancel"
		...
		-- ── ไอคอนเล็กๆ มุมขวาบนของปุ่ม บอกว่าแอคชั่นนี้ต้องคอนเฟิมก่อน ──
		local ConfirmBadge = New("TextLabel", {
			Text                   = "!",
			FontFace               = GetStyleProperty("FontBold"),
			TextSize               = 9,
			...
			AnchorPoint            = Vector2.new(1, 0),
			BackgroundColor3       = Color3.fromRGB(255, 175, 45),
		...
		-- ── แอคชั่นจริง (copy + callback + feedback ข้อความ) ──────────────
		local function RunAction()
			pcall(function()
				if setclipboard then    setclipboard(CB.Value)
				elseif toclipboard then toclipboard(CB.Value) end
			end)
			-- feedback: เปลี่ยนข้อความชั่วคราว + เด้งขนาดนิดๆให้รู้สึกว่ากดสำเร็จ
			CopyBtn.Text = copiedLabel
			local Pulse = New("UIScale", { Scale = 1, Parent = CopyBtn })
			local PulseMotor, SetPulse = Creator.SpringMotor(1, Pulse, "Scale", true)
			SetPulse(1.08, true)
			task.delay(0.09, function() SetPulse(1) end)
		...
		Creator.AddSignal(CopyBtn.MouseButton1Click, function()
			...
			-- Library.Window ถูกตั้งค่าตอน CreateWindow() ก่อนที่ elements จะถูกสร้าง
			-- อยู่แล้ว เลยเรียกใช้ Dialog ของหน้าต่างหลักได้ตรงๆ ไม่ต้องส่ง Window
			-- ผ่าน self ทุกชั้น
			if Library.Window and Library.Window.Dialog then
				Library.Window:Dialog({
					...
					Content = confirmText,
					Buttons = {
						{ Title = cancelButtonText,  Callback = function() end },
						{ Title = confirmButtonText, Callback = RunAction },
					},
				})
			else
				-- fallback กันพัง ถ้ายังไม่มี Window (ไม่ควรเกิดขึ้นในการใช้งานจริง)
				RunAction()
			end
		end)
		...
		-- เปิด/ปิดระบบคอนเฟิม และแก้ข้อความ dialog ได้หลังสร้างปุ่มแล้ว
		function CB:SetConfirm(enabled, opts)
			confirmEnabled = enabled and true or false
			opts = opts or {}
			confirmTitle      = opts.Title      or confirmTitle
			confirmText       = opts.Content    or confirmText
			confirmButtonText = opts.ConfirmText or confirmButtonText
			cancelButtonText  = opts.CancelText  or cancelButtonText
			if ConfirmBadge and ConfirmBadge.Parent then ConfirmBadge.Visible = confirmEnabled end
		end
		function CB:Destroy()
...
ElementsTable.CounterButton = (function()
	...
	function Element:New(Idx, Config)
		...
		-- ── ดีไซน์ใหม่: แคปซูล (pill) ทึบขอบมนสุด แทนที่จะเป็น 3 กล่องแยก ──
		-- ตั้งใจให้หน้าตาต่างจาก NumberInput (ซึ่งเป็นกล่องสี่เหลี่ยม+เลขพิมพ์ได้)
		-- ชัดเจน: CounterButton = แคปซูลลอย ปุ่มกลม, NumberInput = กล่องมุมมน+แถบล่าง
		-- ใช้ CanvasGroup แทน Frame เพื่อให้ ClipsDescendants ตัดขอบตาม UICorner
		-- แบบมนจริงๆ (Frame ธรรมดา ClipsDescendants จะตัดเป็นสี่เหลี่ยม มุมโผล่)
		local Holder = New("CanvasGroup", {
		...
		-- ── แถบ progress เต็มความสูง วิ่งอยู่หลังปุ่ม/เลข บอกตำแหน่งค่าในช่วง Min–Max ──
		-- ทำให้แคปซูลดูเหมือน "แบตเตอรี่/เกจ" มีลูกเล่นมากกว่าเดิมที่เป็นกล่องเปล่าล้วนๆ
		local ProgressFill = New("Frame", {
			Size                   = UDim2.new(0, 0, 1, 0),
		...
		local ProgressMotor, SetProgress = Creator.SpringMotor(0, ProgressFill, "Size", true)

		-- แถวเนื้อหา (ปุ่ม/เลข) แยกออกมาจาก Holder โดยเฉพาะ เพื่อไม่ให้ UIListLayout
		-- ไปจัดเรียง ProgressFill ปนเข้าไปด้วย (ProgressFill ต้องลอยเต็มความกว้างอิสระ)
		...
		local function makeBtn(txt, lo)
			...
			Creator.AddSignal(btn.MouseEnter,       function() SetT(0.85) end)
			Creator.AddSignal(btn.MouseLeave,       function() SetT(1) end)
			Creator.AddSignal(btn.MouseButton1Down, function() SetT(0.7) end)
		...
		local ValLabelHolder = New("Frame", {
		...
		local ValScale = New("UIScale", { Scale = 1, Parent = ValLabelHolder })
		local ValLabel = New("TextLabel", {
			...
			TextXAlignment         = Enum.TextXAlignment.Center,
			ZIndex                 = 2,
			Parent                 = ValLabelHolder,
			ThemeTag               = { TextColor3 = "Text" },
		})
		local ValScaleMotor, SetValScale = Creator.SpringMotor(1, ValScale, "Scale", true)

		local PlusBtn = makeBtn("+", 3)

		-- ── ดับเบิลคลิกที่ตัวเลข = รีเซ็ตกลับไปค่า Default ทันที (ลูกเล่นเสริม) ──
		local lastClickTime = 0
		local ValueClicker = New("TextButton", {
			BackgroundTransparency = 1,
			Text                   = "",
			Size                   = UDim2.fromScale(1, 1),
			ZIndex                 = 3,
		...
		local function set(v, skipBounce)
			v = math.clamp(math.floor(v / Config.Step + 0.5) * Config.Step, Config.Min, Config.Max)
			v = CleanFloat(v, Config.Step)
			local hitLimit = (v == CB.Value) and (v <= Config.Min or v >= Config.Max)
			CB.Value = v
			ValLabel.Text = tostring(v)

			-- progress fill ตามสัดส่วนในช่วง Min–Max
			local pct = (Config.Max > Config.Min) and ((v - Config.Min) / (Config.Max - Config.Min)) or 0
			SetProgress(UDim2.new(math.clamp(pct, 0, 1), 0, 1, 0))

			-- เด้งตัวเลขนิดๆ ทุกครั้งที่ค่าเปลี่ยน ให้รู้สึกมีชีวิตชีวา
			if not skipBounce then
				SetValScale(1.22, true)
				task.delay(0.09, function() SetValScale(1) end)
			end

			-- dim ปุ่มฝั่งที่ชนขอบ + สั่นเบาๆถ้าพยายามไปต่อทั้งที่ชนขอบอยู่แล้ว
			TweenService:Create(MinusBtn, TweenInfo.new(0.1), { BackgroundTransparency = v <= Config.Min and 0.6 or 1 }):Play()
			TweenService:Create(PlusBtn,  TweenInfo.new(0.1), { BackgroundTransparency = v >= Config.Max and 0.6 or 1 }):Play()
			if hitLimit then
				local origPos = Holder.Position
				TweenService:Create(Holder, TweenInfo.new(0.05, Enum.EasingStyle.Sine), {
					Position = UDim2.new(origPos.X.Scale, origPos.X.Offset + 3, origPos.Y.Scale, origPos.Y.Offset),
				}):Play()
				task.delay(0.05, function()
					if Holder and Holder.Parent then
						TweenService:Create(Holder, TweenInfo.new(0.05, Enum.EasingStyle.Sine), { Position = origPos }):Play()
					end
				end)
			end
		...
		Creator.AddSignal(ValueClicker.MouseButton1Click, function()
			local now = tick()
		...
		-- hold to repeat (เร่งความเร็วขึ้นเรื่อยๆเมื่อกดค้างนาน เหมือน NumberInput)
			...
			Creator.AddSignal(btn.MouseButton1Down, function()
				task.delay(0.4, function()
					...
					while btn:IsDescendantOf(game) and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
						set(CB.Value + Config.Step * dir, true)
						task.wait(interval)
						interval = math.max(0.02, interval * 0.93)
					end
				end)
			end)
		end
