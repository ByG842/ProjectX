...
Components.Section = (function()
	...
	return function(Title, Parent, Icon)
		...
		-- 🌈 แถบไล่สี (gradient) พื้นหลัง header — ไล่จากสี Accent ของธีมฝั่งซ้าย จางหายไปทางขวา
		-- สูงแค่แถว header (HEADER_H) แล้วปล่อยให้ CardFrame (ClipsDescendants + UICorner) ตัดโค้งมุมบนให้เอง
		local HeaderGradient = New("Frame", {
			...
			-- ✅ FIX: ต้องมี UICorner ของตัวเองด้วย เพราะ ClipsDescendants ของ CardFrame
			-- ตัด child เป็นกรอบสี่เหลี่ยมตรงๆ เท่านั้น ไม่ได้ตัดตามทรงโค้งของ UICorner จริง
			-- ถ้าไม่ใส่ตรงนี้ มุมเหลี่ยมของแถบนี้จะโผล่ทับมุมโค้งของ CardFrame ทำให้มุมบนดูเหลี่ยม
		...
		local CardFrame = New("Frame", {
			...
			NewCorner("TinyCorner"), -- 📉 ลดความโค้งมุม Section การ์ด (ElementCorner 8px → TinyCorner 4px, เหลี่ยมมนนิดเดียว)
			New("UIStroke", {
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
				Transparency = 0.55,
				Thickness = GetStyleProperty("BorderThickness"),
