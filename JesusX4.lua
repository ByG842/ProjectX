...
Components.Button = (function()
	local New = Creator.New
	local Spring = Flipper.Spring.new
	-- IsPrimary: ปุ่มหลัก (เช่น "ยืนยัน") จะได้สีเน้น Accent, ปุ่มรอง (เช่น "ยกเลิก") หน้าตาเดิม
	return function(Theme, Parent, DialogCheck, IsPrimary)
		DialogCheck = DialogCheck or false
		local Button = {}

		local BaseBgTransparency     = IsPrimary and 0.82 or 0
		local HoverOverlayRest       = 1
		local HoverOverlayHover      = IsPrimary and 0.82 or 0.97
		local BaseStrokeTransparency = IsPrimary and 0.15 or 0.55
		...
		local Stroke = New("UIStroke", {
			ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
			Thickness = IsPrimary and 1.3 or 1,
			Transparency = 1, -- ซ่อนไว้ก่อน รอ PlayIn()
			ThemeTag = {
		...
		local BgMotor, SetBg = Creator.SpringMotor(1, Button.Frame, "BackgroundTransparency", DialogCheck, false, { frequency = 7 })
		local StrokeMotor, SetStroke = Creator.SpringMotor(1, Stroke, "Transparency", DialogCheck, false, { frequency = 7 })
		local TextMotor, SetText = Creator.SpringMotor(1, Button.Title, "TextTransparency", DialogCheck, false, { frequency = 7 })
...
Components.Dialog = (function()
	...
	function Dialog:Create()
		...
		local TintMotor, TintTransparency =
			Creator.SpringMotor(1, NewDialog.TintFrame, "BackgroundTransparency", true, false, { frequency = 6 })
		...
		-- ── ไอคอนวงกลม โทนสี Accent อยู่ด้านบนสุด ────────────────────────
		NewDialog.IconLabel = New("TextLabel", {
			Text = "!",
		...
		NewDialog.IconScale = New("UIScale", { Scale = 0.4 })

		NewDialog.IconBadge = New("Frame", {
		...
		local IconScaleMotor, SetIconScale =
			Creator.SpringMotor(0.4, NewDialog.IconScale, "Scale", true, false, { frequency = 4.2, dampingRatio = 0.5 })

		-- ── ปุ่มปิด (X) มุมขวาบน — กดแล้วปิดแบบเดียวกับ "ยกเลิก" ─────────────
		...
		Creator.AddSignal(NewDialog.CloseX.MouseEnter, function() SetCloseXBg(0.85) end)
		Creator.AddSignal(NewDialog.CloseX.MouseLeave, function() SetCloseXBg(1) end)

		-- ── หัวข้อ อยู่กึ่งกลาง ใต้ไอคอน ───────────────────────────────────
		...
		-- dampingRatio < 1 ทำให้เด้ง (overshoot) เบาๆ ตอนเปิด ดูมีชีวิตชีวาแบบสมูท ไม่กระด้าง
		local ScaleMotor, Scale =
			Creator.SpringMotor(1.08, NewDialog.Scale, "Scale", false, false, { frequency = 4.4, dampingRatio = 0.62 })
		...
		local RootMotor, RootTransparency =
			Creator.SpringMotor(1, NewDialog.Root, "GroupTransparency", false, false, { frequency = 5.5 })
		function NewDialog:Open()
			Library.DialogOpen = true
			NewDialog.Scale.Scale = 1.08
			NewDialog.IconScale.Scale = 0.4
			TintTransparency(0.75)
			RootTransparency(0)
			Scale(1)
			-- ไอคอนเด้งเข้ามาหลังการ์ดเริ่มโผล่มานิดนึง ให้ความรู้สึก "ทยอยเข้า"
			task.delay(0.06, function()
				if NewDialog.IconBadge and NewDialog.IconBadge.Parent then
					SetIconScale(1)
				end
			end)
			-- ปุ่มไล่เฟดเข้าทีละตัว (stagger) จากซ้ายไปขวา
			for i, Btn in ipairs(NewDialog.ButtonList) do
				Btn:PlayIn(0.10 + (i - 1) * 0.06)
			end
		end
		function NewDialog:Close()
			...
			RootTransparency(1)
			Scale(1.05)
			pcall(function() NewDialog.Root.UIStroke:Destroy() end)
			task.wait(0.15)
		...
		-- ปรับไอคอนตรงกลางหัวเรื่องได้ (ตัวอักษร/สัญลักษณ์อะไรก็ได้) — ค่าเริ่มต้นคือ "!"
		function NewDialog:SetIcon(Glyph)
			if Glyph and NewDialog.IconLabel then
				NewDialog.IconLabel.Text = Glyph
			end
		end
	...
	return function(Config)
		...
		function Window:Dialog(Config)
			...
			New("UISizeConstraint", {
				MinSize = Vector2.new(320, 210),
				MaxSize = Vector2.new(620, math.huge),
				Parent = Dialog.Root,
			})

			local maxWidth = math.min(620, Window.Size.X.Offset - 120)
			local baseWidth = math.max(320, math.min(maxWidth, Content.TextBounds.X + 40))
			Dialog.Root.Size = UDim2.fromOffset(baseWidth, 210)
			ContentHolder.Size = UDim2.new(1, -40, 1, -178)
			task.defer(function()
				local contentHeight = Content.TextBounds.Y
				local desired = math.clamp(contentHeight + 178, 210, 460)
