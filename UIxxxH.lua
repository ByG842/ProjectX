...
local StyleRegistry = {
	...
	Paddings = {},   -- { object = UIPadding } สำหรับ Compact/Dense density mode
}
...
Components.Element = (function()
	...
	return function(Title, Desc, Parent, Hover, Options)
		...
		table.insert(StyleRegistry.Paddings, LabelHolderPadding)
...
Components.Section = (function()
	...
	return function(Title, Parent, Icon)
		...
		table.insert(StyleRegistry.Paddings, Section.Layout) -- 📐 ไว้ปรับระยะห่างตอนสลับ Compact/Comfortable density
...
Components.Window = (function()
	...
	return function(Config)
		...
		function Window:SetSidebarCollapsed(Collapsed, Animate)
			...
			-- ซ่อน/โชว์ label ข้อความของปุ่มแท็บแต่ละอัน (เหลือแค่ไอคอน)
			if Window.TabHolder then
				for _, tabBtn in ipairs(Window.TabHolder:GetChildren()) do
					if tabBtn:IsA("GuiObject") then
						...
						if label then
							TweenService:Create(label, TweenInfo.new(0.15), { TextTransparency = Collapsed and 1 or 0 }):Play()
						end
					end
				end
			end
		...
		-- 📁 Tab grouping — ใส่หัวข้อกลุ่ม/เส้นคั่นระหว่างชุดแท็บในไซด์บาร์ (เรียงตามลำดับที่เรียกใช้)
		function Window:AddTabGroup(Label)
			local GroupDivider = New("Frame", {
				Size = UDim2.new(1, 0, 0, Label and 22 or 9),
				BackgroundTransparency = 1,
				Parent = Window.TabHolder,
			}, {
				Label and New("TextLabel", {
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		section:AddToggleGroup("UIDensity", {
			Title   = "Density",
			Description = "ปรับระยะห่างระหว่าง element ทั้ง UI",
			Options = { "Comfortable", "Compact" },
			Default = Settings.Density or "Comfortable",
			Callback = function(Value)
...
function Library:SetDensity(Mode)
	Mode = (Mode == "Compact") and "Compact" or "Comfortable"
	Library.Density = Mode
	local isCompact = (Mode == "Compact")

	for _, obj in ipairs(StyleRegistry.Paddings) do
		local ok = pcall(function()
			if obj and obj.Parent then
				if obj:IsA("UIPadding") then
					obj.PaddingTop = UDim.new(0, isCompact and 6 or 13)
					obj.PaddingBottom = UDim.new(0, isCompact and 6 or 13)
				elseif obj:IsA("UIListLayout") then
					obj.Padding = UDim.new(0, isCompact and 2 or 4)
				end
			end
		end)
	end
