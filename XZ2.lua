...
local Themes = {
	Names = {
		...
		-- 🎮 STYLE THEMES
		"Minecraft",
		-- 🍎 macOS Sequoia style (Cascade-inspired)
		"Cascade",
	},
	Dark = {
		Name = "Dark",
	...
	Cascade = {
		-- 🍎 macOS Sequoia — System Settings vibrancy look (inspired by the Cascade UI library)
		Name = "Cascade",
		Accent = Color3.fromRGB(10, 132, 255),              -- Apple systemBlue
		AcrylicMain = Color3.fromRGB(246, 246, 248),         -- window chrome (light vibrancy)
		AcrylicBorder = Color3.fromRGB(214, 214, 219),
		AcrylicGradient = ColorSequence.new(Color3.fromRGB(250, 250, 252), Color3.fromRGB(238, 238, 242)),
		AcrylicNoise = 0.985,
		TitleBarLine = Color3.fromRGB(222, 222, 227),
		Tab = Color3.fromRGB(90, 90, 96),                    -- sidebar item text/icon
		Element = Color3.fromRGB(255, 255, 255),             -- cards / panes
		ElementBorder = Color3.fromRGB(224, 224, 229),
		InElementBorder = Color3.fromRGB(210, 210, 216),
		ElementTransparency = 0.35,
		ToggleSlider = Color3.fromRGB(255, 255, 255),        -- knob
		ToggleToggled = Color3.fromRGB(52, 199, 89),         -- Apple systemGreen (switch ON)
		SliderRail = Color3.fromRGB(210, 210, 216),
		DropdownFrame = Color3.fromRGB(228, 228, 233),
		DropdownHolder = Color3.fromRGB(255, 255, 255),
		DropdownBorder = Color3.fromRGB(220, 220, 226),
		DropdownOption = Color3.fromRGB(150, 150, 156),
		Keybind = Color3.fromRGB(150, 150, 156),
		Input = Color3.fromRGB(233, 233, 237),
		InputFocused = Color3.fromRGB(255, 255, 255),
		InputIndicator = Color3.fromRGB(150, 150, 156),
		InputIndicatorFocus = Color3.fromRGB(10, 132, 255),
		Dialog = Color3.fromRGB(255, 255, 255),
		DialogHolder = Color3.fromRGB(246, 246, 248),
		DialogHolderLine = Color3.fromRGB(224, 224, 229),
		DialogButton = Color3.fromRGB(255, 255, 255),
		DialogButtonBorder = Color3.fromRGB(220, 220, 226),
		DialogBorder = Color3.fromRGB(214, 214, 219),
		DialogInput = Color3.fromRGB(246, 246, 248),
		DialogInputLine = Color3.fromRGB(180, 180, 188),
		Text = Color3.fromRGB(28, 28, 30),                   -- macOS label color
		SubText = Color3.fromRGB(110, 110, 115),             -- macOS secondaryLabel
		Hover = Color3.fromRGB(120, 120, 128),
		HoverChange = 0.10,
		-- บอก system ว่า theme นี้ใช้ style "Sequoia" (มุมโค้งใหญ่ ขอบบาง แบบ macOS)
		StyleOverride = "Sequoia",
	},
	Aqua = {
		Name = "Aqua",
...
local StyleThemes = {
	...
	Sequoia = {
		-- 🍎 macOS Sequoia proportions — มุมโค้งใหญ่ ขอบบางมาก โปร่งแบบ vibrancy
		WindowCorner      = UDim.new(0, 16),
		ElementCorner     = UDim.new(0, 10),
		SmallCorner       = UDim.new(0, 8),
		TinyCorner        = UDim.new(0, 6),
		PillCorner        = UDim.new(1, 0),
		-- UIStroke
		BorderThickness   = 1,
		-- Font — Gotham SSm ใกล้เคียง SF Pro ที่สุดเท่าที่ Roblox มีให้ในตัว
		FontRegular       = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Regular),
		FontMedium        = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
		FontBold          = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold),
		FontSemiBold      = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold),
...
Components.TitleBar = (function()
	...
	return function(Config)
		local TitleBar = {}
		local IsMacStyle = Library.Theme == "Cascade" -- 🍎 macOS Sequoia traffic-light window controls
		...
		TitleBar.Frame = New("Frame", {
			...
			New("Frame", {
				...
				New("UIPadding", {
					PaddingLeft = UDim.new(0, IsMacStyle and 16 or 14),
				}),

				-- 🍎 macOS traffic-light cluster (Close / Minimize / Maximize) — เฉพาะ Cascade theme
				New("Frame", {
					Name = "MacControls",
			...
			New("Frame", {
				...
				BackgroundTransparency = 1,
				Visible = not IsMacStyle, -- 🍎 macOS: window controls live on the left instead
			}, {
				New("UIListLayout", {
					Padding = UDim.new(0, 4),
					FillDirection = Enum.FillDirection.Horizontal,
		...
		-- 🍎 macOS traffic-light colors, keyed by which control it is
		local MacDotColor = {
			Close     = Color3.fromRGB(255, 95, 87),   -- systemRed
			Minimize  = Color3.fromRGB(254, 188, 46),  -- systemYellow
			Maximize  = Color3.fromRGB(40, 200, 64),   -- systemGreen
		}

		-- Anchor helpers for BarButtons inside RightSection (or MacControls) using UIListLayout
		local function RightBarButton(Icon, Parent, Callback, DotKind)
			local Button = { Callback = Callback or function() end }

			if IsMacStyle then
				-- Small solid-color circle, icon only appears (faintly) — classic macOS look
				local DotColor = MacDotColor[DotKind] or Color3.fromRGB(150, 150, 150)
				Button.Frame = New("TextButton", {
					...
					New("ImageLabel", {
						...
						ImageColor3 = Color3.fromRGB(60, 20, 10),
						Name = "Icon",
					}),
				})
				local Motor, SetTransparency = Creator.SpringMotor(0.65, Button.Frame.Icon, "ImageTransparency")
				AddSignal(Button.Frame.MouseEnter, function() SetTransparency(0.1) end)
				AddSignal(Button.Frame.MouseLeave, function() SetTransparency(0.65, true) end)
		...
		local macControls = leftSection and leftSection:FindFirstChild("MacControls")
		-- macOS order is Close, Minimize, Maximize (left → right); non-mac keeps original Min/Max/Close order
		local controlsParent = IsMacStyle and macControls or rightSection

		if IsMacStyle then
			TitleBar.CloseButton = RightBarButton(Components.Assets.Close, controlsParent, function()
			...
			TitleBar.MinButton = RightBarButton(Components.Assets.Min, controlsParent, function()
				Library.Window:Minimize()
			end, "Minimize")
			TitleBar.MaxButton = RightBarButton(Components.Assets.Max, controlsParent, function()
