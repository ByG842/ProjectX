...
ElementsTable.Stepper = (function()
	...
	function Element:New(Idx, Config)
		...
		-- ลดความสูงลงให้พอดีกับปุ่มคู่ที่กะทัดรัดขึ้น (26px)
		local ValueDisplay
		if isFull then
		...
		-- ✅ FIX: AutomaticSize.Y แทนความสูงคงที่ 30 (ที่ทำให้ล้นขอบ)
		local BtnColumn = New("Frame", {
			Size                   = UDim2.fromOffset(22, 0),
			AutomaticSize          = Enum.AutomaticSize.Y,
			BackgroundTransparency = 1,
			LayoutOrder            = 2,
			Parent                 = Row,
			...
			New("UIListLayout", {
				...
				HorizontalAlignment = Enum.HorizontalAlignment.Center,
				SortOrder           = Enum.SortOrder.LayoutOrder,
				Padding             = UDim.new(0, 2), -- ← ช่องว่างระหว่างปุ่มบน/ล่าง
			}),
		})
		...
		local function makeArrowBtn(icon, lo)
			...
			-- ✅ FIX: ปุ่มเล็กลง (22x12 แทน 26x24) รวม 2 ปุ่ม + gap = 26px พอดีแถว
			local Btn = New("TextButton", {
				Text                   = "",
