...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		-- ใช้ Sine InOut สำหรับสี ให้ความรู้สึก "ค่อยๆ ไล่เข้า" นุ่มนวล ไม่กระชากเหมือน Quint/Back
		local CHIP_COLOR_TWEEN = TweenInfo.new(0.32, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
		local CHIP_STROKE_TWEEN = TweenInfo.new(0.38, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
		local CHIP_HOVER_TWEEN = TweenInfo.new(0.18, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
		local CHIP_ENTRANCE_TWEEN = TweenInfo.new(0.26, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
		...
		-- สีไล่เข้าแบบนุ่มนวลด้วย Sine InOut และให้ขอบ (Stroke) ไล่ตามหลังพื้นหลังเล็กน้อย
		-- สร้างความรู้สึก "เลเยอร์" สวยงาม แทนที่จะกระพริบเปลี่ยนสีพร้อมกันทีเดียวแบบแข็งๆ
		local function updateChip(item, btn)
			...
			if stroke then
				playChipTween(stroke, "stroke", CHIP_STROKE_TWEEN, {
					Transparency = active and 0.05 or 0.85,
					Thickness = active and (GetStyleProperty("BorderThickness") + 0.5) or GetStyleProperty("BorderThickness"),
				})
			end
		end
