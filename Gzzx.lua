...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		-- ✅ [ จุดที่แก้ ] ตั้งชื่อ (Name) ให้ label/icon ชัดเจน เพราะตอนนี้ในปุ่มแท็บมี TextLabel 2 ตัว
		-- (ตัวเต็ม + CollapsedLabel ป้ายเล็กด้านล่าง) การเดิมใช้ FindFirstChildWhichIsA จะสุ่มเจอผิดตัวได้
		...
		-- 🏷️ [ จุดที่เพิ่ม ] ป้ายชื่อเล็กๆ ที่โผล่ขึ้นมาจากขอบล่างของกล่องตอน sidebar ถูกพับ (collapsed)
		-- โชว์เมื่อเอาเมาส์ไปชี้ (hover) หรือเป็นแท็บที่กำลังเปิดอยู่ (active) กันไม่ให้เหลือแค่ไอคอนเปล่าๆ ดูไม่ออกว่าคือแท็บอะไร
		local CollapsedLabel = New("TextLabel", {
			Name = "CollapsedLabel",
		...
		Tab.Frame = New("TextButton", {
			...
			Parent = Parent,
			ZIndex = 10,
			ClipsDescendants = true, -- ✅ [ จุดที่เพิ่ม ] กัน CollapsedLabel โผล่ล้นออกนอกกล่องตอนยังไม่ถึงตำแหน่ง ให้ดูเหมือนเลื่อนขึ้นมาจากด้านล่างจริงๆ
		...
		-- ✅ [ จุดที่เพิ่ม ] ฟังก์ชันอัปเดตการโชว์/ซ่อน CollapsedLabel — เรียกตอน hover เข้า/ออก และตอนเลือก/เลิกเลือกแท็บ
		Tab.Hovering = false
		local function UpdateCollapsedReveal()
			local shouldShow = Window.SidebarCollapsed and (Tab.Hovering or Tab.Frame:GetAttribute("TabSelected") == true)
			local ti = TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
			if shouldShow then
				CollapsedLabel.Visible = true
				TweenService:Create(CollapsedLabel, ti, {
		...
		Tab.UpdateCollapsedReveal = UpdateCollapsedReveal
		Creator.AddSignal(Tab.Frame:GetAttributeChangedSignal("TabSelected"), UpdateCollapsedReveal)
		...
		Creator.AddSignal(Tab.Frame.MouseEnter, function()
			...
			Tab.Hovering = true -- ✅ [ จุดที่เพิ่ม ] โผล่ CollapsedLabel ขึ้นมาตอน hover ถ้า sidebar ถูกพับอยู่
			UpdateCollapsedReveal()
		end)
	...
	function TabModule:SelectTab(Tab)
		...
		for _, TabObject in next, TabModule.Tabs do
			...
			if TabObject.Frame then
				TabObject.Frame:SetAttribute("TabSelected", false) -- ✅ [ จุดที่เพิ่ม ] ให้ CollapsedLabel ของแท็บที่ไม่ได้เลือกซ่อนตัวลง
			end
		end
		TabModule.Tabs[Tab].SetTransparency(0.89)
		TabModule.Tabs[Tab].Selected = true
		TabModule.Tabs[Tab].Frame:SetAttribute("TabSelected", true) -- ✅ [ จุดที่เพิ่ม ] ให้ CollapsedLabel ของแท็บนี้โผล่ขึ้นมาถ้า sidebar ถูกพับอยู่
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		-- MAIN TITLEBAR FRAME — [ จุดที่แก้ ] ลดความสูงจาก 52 → 40px ให้บางลงแบบ Gzz
		TitleBar.Frame = New("Frame", {
			...
			New("Frame", {
				Name = "LeftSection",
				Size = UDim2.new(1, -290, 1, 0), -- ✅ [ จุดที่แก้ ] เผื่อที่ให้ช่องค้นหา + ปุ่ม Min/Max/Close ทางขวา
				...
				-- Logo image (ไม่มีเส้นขอบ, ขอบโค้งนิดเดียว) — [ จุดที่แก้ ] ย่อขนาดลงให้พอดีกับ header ที่บางลง
				Config.Icon and New("Frame", {
					...
					New("ImageLabel", {
						...
						Size = UDim2.fromOffset(26, 26), -- 🖼️ ย่อสัดส่วนตามเฟรมที่เล็กลง แต่ยังล้นออกมานิดหน่อยเหมือนเดิม
						Position = UDim2.fromScale(0.5, 0.5),
						AnchorPoint = Vector2.new(0.5, 0.5),
						BackgroundTransparency = 1,
			...
			New("Frame", {
				Name = "RightSection",
				Size = UDim2.new(0, 100, 1, 0), -- ✅ [ จุดที่แก้ ] แคบลงตามปุ่ม Min/Max/Close ที่เล็กลง (32→26)
				AnchorPoint = Vector2.new(1, 0),
		...
		local function RightBarButton(Icon, Parent, Callback)
			...
			Button.Frame = New("TextButton", {
				Size = UDim2.new(0, 26, 0, 26), -- ✅ [ จุดที่แก้ ] ย่อจาก 32 → 26 ให้เข้ากับ header ที่บางลง
				BackgroundTransparency = 1,
				Parent = Parent,
				Text = "",
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- ✅ [ จุดที่แก้ ] ช่องค้นหาย้ายไปอยู่ใน Title Bar แล้ว (ดูจุดสร้าง SearchFrame ด้านล่าง) จึงไม่กินพื้นที่แนวตั้งของ sidebar อีกต่อไป
		local tabHolderTop = hasImage and (imageSize + 10 + topOffset) or 45
		...
		function Window:UpdateTabHolderLayout(explicitTop)
			...
			if not newTop then
				-- ✅ [ จุดที่แก้ ] ตัดผลของ searchHeight ออก เพราะช่องค้นหาย้ายไปอยู่ใน Title Bar แล้ว ไม่กินที่ sidebar
		...
		-- 🔍 [ จุดที่แก้ ] ย้ายช่องค้นหาไปไว้บน Title Bar มุมขวา (ข้างปุ่ม Min/Max/Close) แบบ Gzz แทนที่จะฝัง
		-- เต็มความกว้างไว้เหนือรายการแท็บใน sidebar เหมือนเดิม ทำให้ sidebar ไม่เสียพื้นที่แนวตั้งอีกต่อไป
		-- (ตัว Frame นี้ถูกสร้างไว้ก่อน แล้วจะเอาไป Parent ใส่ Title Bar อีกทีตอนสร้าง Title Bar เสร็จด้านล่าง)
		...
		local TabFrame = New("Frame", {
			Size = UDim2.new(0, Window.TabWidth, 1, -31), -- ✅ [ จุดที่แก้ ] ไม่ต้องหักพื้นที่ให้ช่องค้นหาอีกต่อไป
		...
		local TabModule -- ✅ [ จุดที่เพิ่ม ] forward-declare ไว้ใช้อัปเดตป้ายชื่อ (CollapsedLabel) ของทุกแท็บตอนพับ/กาง sidebar
		...
		function Window:SetSidebarCollapsed(Collapsed, Animate)
			...
			-- ซ่อน/โชว์ label ข้อความของปุ่มแท็บแต่ละอัน + จัดไอคอนให้อยู่กึ่งกลางและย่อขนาดตอนย่อ
			-- ✅ [ จุดที่แก้ ] ค้นหาด้วยชื่อ (Name) แทน FindFirstChildWhichIsA เพราะตอนนี้ในแท็บมี TextLabel
			-- 2 ตัว (TabLabel ตัวเต็ม + CollapsedLabel ป้ายเล็กที่โผล่ตอนย่อ) การหาแบบเดิมจะสุ่มเจอผิดตัวได้
			if Window.TabHolder then
				for _, tabBtn in ipairs(Window.TabHolder:GetChildren()) do
					if tabBtn:IsA("GuiObject") then
						local label = tabBtn:FindFirstChild("TabLabel", true)
						local icon = tabBtn:FindFirstChild("TabIcon", true)
						if label then
						...
						if icon then
							...
							if Collapsed then
								...
								TweenService:Create(icon, ti, {
									-- ขยับขึ้นนิดหน่อยจากจุดกึ่งกลาง เพื่อเว้นที่ด้านล่างไว้ให้ CollapsedLabel โผล่ขึ้นมา
									Position = UDim2.new(0.5, 0, 0.38, 0),
									-- ✅ [ จุดที่แก้ ] ลดขนาดไอคอนตอนพับ sidebar ไม่ให้ดูโล่งเป็นแค่ไอคอนเปล่าๆ ลอยเดี่ยว
									Size = UDim2.fromOffset(13, 13),
								}):Play()
							else
			...
			-- ✅ [ จุดที่เพิ่ม ] อัปเดตป้ายชื่อเล็ก (CollapsedLabel) ของทุกแท็บใหม่ทันทีที่พับ/กาง sidebar
			-- (ครอบคลุมเคส "แท็บที่กำลังเปิดอยู่" ที่ต้องโชว์ป้ายชื่อค้างไว้แม้ไม่ได้ชี้เมาส์อยู่ก็ตาม)
			if TabModule then
				for _, TabObject in next, TabModule.Tabs do
					if TabObject.UpdateCollapsedReveal then
						TabObject.UpdateCollapsedReveal()
					end
				end
			end
		...
		-- 🔍 [ จุดที่แก้ ] ย้ายกล่องค้นหา (สร้างไว้ก่อนหน้านี้) เข้าไปแปะใน Title Bar จริงๆ ตอนนี้
		-- วางไว้ทางซ้ายของปุ่ม Min/Max/Close (RightSection กว้าง 100 + เว้นระยะ 8px)
		SearchFrame.Parent = Window.TitleBar.Frame
		SearchFrame.Position = UDim2.new(1, -114, 0.5, 0)
		Window.TitleBar.SearchFrame = SearchFrame
		Window.TitleBar.SearchInput = SearchInput
		...
		if Config.UserInfo then
			...
			if Config.UserInfoTop then
				...
				-- ✅ [ จุดที่แก้ ] ตัดผลของ searchHeight ออก เพราะช่องค้นหาย้ายไปอยู่ใน Title Bar แล้ว ไม่ใช่ sidebar
		...
		Window.SelectorPosMotor:onStep(function(Value)
			...
			-- ✅ [ จุดที่แก้ ] ตัดเช็ค overlap กับช่องค้นหาออก เพราะช่องค้นหาย้ายไปอยู่ใน Title Bar แล้ว ไม่ได้อยู่ใน sidebar
		...
		function Window:ToggleSearch()
			-- ✅ [ จุดที่แก้ ] ช่องค้นหาย้ายไปอยู่ใน Title Bar แล้ว ไม่ได้กินพื้นที่ sidebar อีกต่อไป
			-- จึงแค่โชว์/ซ่อนกล่องค้นหา ไม่ต้องไปคำนวณ/ปรับขนาด TabFrame หรือ TabHolder ใหม่
		...
		TabModule = Components.Tab:Init(Window) -- ✅ [ จุดที่แก้ ] ผูกกับตัวแปรที่ forward-declare ไว้ด้านบน (ไม่ใส่ local ซ้ำ)
