...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		local TabLabel = New("TextLabel", {
			AnchorPoint = Vector2.new(0, 0.5),
			Position = not fischbypass and Icon and UDim2.new(0, 34, 0.5, 0) or UDim2.new(0, 14, 0.5, 0),
			Text = Title,
			RichText = true,
			TextColor3 = Color3.fromRGB(255, 255, 255),
			TextTransparency = 0.35, -- เริ่มแบบจางไว้ก่อน (สถานะยังไม่ถูกเลือก/ไม่ได้ hover)
			FontFace = GetStyleProperty("FontMedium"),
		...
		-- ทับสีกลางๆ ตอน hover (ไม่ใช่ Accent) แยกออกจากสถานะ "ถูกเลือกอยู่" ให้ชัดเจน ไม่ปนกัน
		local TabHoverOverlay = New("Frame", {
			Size = UDim2.fromScale(1, 1),
			BackgroundTransparency = 1,
			ZIndex = 8,
		...
		-- ทับสี Accent ไว้เฉพาะแท็บที่ถูกเลือกอยู่จริงๆ เท่านั้น ให้เห็นชัดว่ากำลังอยู่แท็บไหน
		...
		Tab.Frame = New("TextButton", {
			...
			BackgroundTransparency = 1, -- พื้นฐานโปร่งใสเสมอ ให้ TabHoverOverlay/TabAccent เป็นคนคุมภาพทั้งหมดแทน
			Parent = Parent,
			ZIndex = 10,
		...
		Tab.HoverMotor, Tab.SetHover = Creator.SpringMotor(1, TabHoverOverlay, "BackgroundTransparency")
		Tab.AccentMotor, Tab.SetAccent = Creator.SpringMotor(1, TabAccent, "BackgroundTransparency")
		Tab.LabelMotor, Tab.SetLabelTransparency = Creator.SpringMotor(0.35, TabLabel, "TextTransparency")
		Tab.IconMotor, Tab.SetIconTransparency = Creator.SpringMotor(0.35, TabIcon, "ImageTransparency")
		Tab.Hovering = false

		-- คำนวณสถานะภาพรวมใหม่ทุกครั้งที่ Selected/Hovering เปลี่ยน (จุดเดียว ไม่กระจายเงื่อนไขหลายที่)
		function Tab:UpdateVisual()
			local mutedTransparency
			if self.Selected then
				self.SetAccent(0.78) -- แท็บที่เลือกอยู่: ทับสี Accent ชัดเจน
				self.SetHover(1)
				mutedTransparency = 0
			elseif self.Hovering then
				self.SetAccent(1)
				self.SetHover(0.9) -- hover เฉยๆ (ยังไม่ได้เลือก): ทับสีกลางๆ จางๆ
				mutedTransparency = 0.1
			else
				self.SetAccent(1)
				self.SetHover(1)
				mutedTransparency = 0.35 -- ปกติ: จางลงให้แท็บที่เลือกอยู่เด่นกว่า
			end
			self.SetLabelTransparency(mutedTransparency)
			self.SetIconTransparency(mutedTransparency)
		end
	...
	function TabModule:SelectTab(Tab)
		...
		for _, TabObject in next, TabModule.Tabs do
			...
			if TabObject.UpdateVisual then
				TabObject:UpdateVisual()
			end
		end
		TabModule.Tabs[Tab].Selected = true
		if TabModule.Tabs[Tab].UpdateVisual then
			TabModule.Tabs[Tab]:UpdateVisual()
		end
...
Components.Window = (function()
	...
	return function(Config)
		...
		Window.TabHolder = New("ScrollingFrame", {
			...
			-- เว้นระยะขอบล่างของลิสต์แท็บนิดหน่อย กันแท็บสุดท้ายชิดขอบล่างเกินไป
			-- (ใส่ PaddingTop ไว้เท่าค่า fallback เดิม เพราะพอมี UIPadding แล้วโค้ดคำนวณ CanvasSize
			-- จะอ่านค่าจาก UIPadding จริงแทนค่า fallback 6 ที่เคยใช้ตอนไม่มี UIPadding เลย)
			New("UIPadding", {
				PaddingTop = UDim.new(0, 6),
...
Elements.AddTab = function(self, TabTitle, TabIcon)
	...
	-- [ จุดที่แก้ ] เพิ่มความสูง SectionTabStrip จาก 22 เป็น 30 (ปุ่มเองยังสูง 22 เท่าเดิม แต่ตอนนี้
	-- จะลอยตรงกลางกล่องที่สูงกว่า ทำให้มีช่องว่างบน-ล่างรอบแถวปุ่มมากขึ้น ไม่ติดกับของข้างบน/ข้างล่างเกินไป)
	...
	-- [ จุดที่แก้ ] ของเดิม PillCorner (มนเต็มแบบเม็ดยา/วงรี) + BackgroundTransparency 0.85 (ทึบแค่ 15%)
	-- ทำให้ปุ่มมองเห็นยากและดูเป็นวงรีแทนที่จะเป็นสี่เหลี่ยมมน — เปลี่ยนเป็น ElementCorner (มนพอดี
	-- แบบสี่เหลี่ยม เข้าธีมเดียวกับปุ่มอื่นในไลบรารี) และเพิ่มความทึบให้เห็นชัดขึ้น
	local TabButton = Creator.New("TextButton", {
		...
		resolvedIcon and Creator.New("UIListLayout", {
			FillDirection = Enum.FillDirection.Horizontal,
			Padding = UDim.new(0, 4),
			VerticalAlignment = Enum.VerticalAlignment.Center,
		}) or nil,
	})
	local _, SetTabBg     = Creator.SpringMotor(0.7, TabButton, "BackgroundTransparency", true, false, { frequency = 8 })
	-- [ จุดที่แก้ ] เดิม stroke ที่พักอยู่ที่ Transparency = 1 (โปร่งใสเต็ม มองไม่เห็นเลยตอนไม่ active)
	-- ใส่ให้มีกรอบจางๆ ตลอดเวลาแทน ให้เห็นขอบเขตปุ่มชัดขึ้นแม้ตอนไม่ได้เลือก/ไม่ได้ hover
	local TabButtonStroke = Creator.New("UIStroke", {
		Transparency = 0.6,
		ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
		ThemeTag = { Color = "Accent" },
		Parent = TabButton,
	})
	local _, SetTabStroke = Creator.SpringMotor(0.6, TabButtonStroke, "Transparency", true, false, { frequency = 8 })
	...
	self.SectionTabs[Index] = { Button = TabButton, Container = TabContainer, SetBg = SetTabBg, SetStroke = SetTabStroke, Layout = TabContentLayout }
	...
	local function UpdateSectionTabAppearance()
		for i, t in ipairs(self.SectionTabs) do
			...
			t.SetBg(active and 0.4 or 0.7)
			t.SetStroke(active and 0 or 0.6)
		end
		self._RecalcSectionTabLayout()
	end
	Creator.AddSignal(TabButton.MouseEnter, function()
		if Index ~= self.SelectedSectionTab then SetTabBg(0.55) end
	end)
	Creator.AddSignal(TabButton.MouseLeave, function()
		if Index ~= self.SelectedSectionTab then SetTabBg(0.7) end
	end)
...
function Library:CreateMinimizer(Config)
	...
	local function createButton(isDesktop)
		return New("TextButton", {
			...
			Size = UDim2.new(1, 0, 1, 0),
			BorderSizePixel = 0,
			-- กลับไปใช้แบบโปร่งใสเต็ม ไม่มีกล่อง/กรอบพื้นหลัง เอาแค่โลโก้ล้วนๆ ตามที่ต้องการ
			BackgroundTransparency = 1,
			AutoButtonColor = false,
		}, {
			New("ImageLabel", {
				...
				-- ขอบมนนิดๆ ที่ตัวโลโก้เอง (ไม่มีกล่อง/พื้นหลังเหมือนเดิม แค่ทำให้ตัวรูปเองดูนุ่มขึ้น)
				New("UICorner", { CornerRadius = UDim.new(0.18, 0) }),
			}),
		})
	end
