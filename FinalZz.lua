...
Components.Element = (function()
	...
	return function(Title, Desc, Parent, Hover, Options)
		...
		if Hover then
			...
			-- Border brightens slightly on hover/press so the card feels "lit" rather
			-- than just shifting its fill — a small, cheap read of interactivity.
			local BorderMotor, SetBorderTransparency = Creator.SpringMotor(0.5, Element.Border, "Transparency", false, true)

			Creator.AddSignal(Element.Frame.MouseEnter, function()
				SetTransparency(Creator.GetThemeProperty("ElementTransparency") - Creator.GetThemeProperty("HoverChange"))
				SetBorderTransparency(0.15)
			end)
			Creator.AddSignal(Element.Frame.MouseLeave, function()
				SetTransparency(Creator.GetThemeProperty("ElementTransparency"))
				SetBorderTransparency(0.5)
			end)
...
Components.TitleBar = (function()
	...
	local BUTTON_SIZE = Mobile and 36 or 32
	local ICON_SIZE = Mobile and 16 or 14
	local DANGER_COLOR = Color3.fromRGB(235, 70, 80)
	...
	return function(Config)
		...
		-- Window control button (Min/Max/Close), laid out right-to-left by the RightSection's UIListLayout.
		-- `IsDanger` gives the button a red hover/press state (used for Close) instead of the neutral one.
		local function RightBarButton(Icon, Parent, Callback, IsDanger)
			...
			Button.Frame = New("TextButton", {
				...
				Parent = Parent,
				Text = "",
				ThemeTag = not IsDanger and { BackgroundColor3 = "Hover" } or nil,
				BackgroundColor3 = IsDanger and DANGER_COLOR or nil,
			}, {
				NewCorner("ElementCorner"),
				New("ImageLabel", {
					Image = Icon,
					...
					BackgroundTransparency = 1,
					Name = "Icon",
					ThemeTag = not IsDanger and { ImageColor3 = "SubText" } or nil,
					ImageColor3 = IsDanger and Color3.fromRGB(255, 255, 255) or nil,
				}),
			})

			local Icon_ = Button.Frame.Icon
			local BgMotor, SetBg = Creator.SpringMotor(1, Button.Frame, "BackgroundTransparency")

			if IsDanger then
				-- The icon starts dim (SubText-ish) and only turns solid white once the
				-- background is visibly red, so the button doesn't feel "armed" at rest.
				local IconMotor, SetIcon = Creator.SpringMotor(0.55, Button.Frame.Icon, "ImageTransparency")
				AddSignal(Button.Frame.MouseEnter, function() SetBg(0.15); SetIcon(0) end)
				AddSignal(Button.Frame.MouseLeave, function() SetBg(1, true); SetIcon(0.55) end)
				AddSignal(Button.Frame.MouseButton1Down, function() SetBg(0) end)
				AddSignal(Button.Frame.MouseButton1Up, function() SetBg(0.15) end)
			else
				AddSignal(Button.Frame.MouseEnter, function() SetBg(0.88) end)
				AddSignal(Button.Frame.MouseLeave, function() SetBg(1, true) end)
				AddSignal(Button.Frame.MouseButton1Down, function() SetBg(0.80) end)
				AddSignal(Button.Frame.MouseButton1Up, function() SetBg(0.88) end)
			end
...
-- Both the desktop and mobile minimizer pills share one drag/click handler; previously this
-- whole block (InputBegan + MouseButton1Click) was duplicated verbatim per button.
local Minimizer
local isDragging = false
local dragStart = nil
local dragOffset = nil

local function BindMinimizerButton(Button, AcceptedInputTypes)
	Creator.AddSignal(Button.InputBegan, function(Input)
		if not table.find(AcceptedInputTypes, Input.UserInputType) then return end
...
BindMinimizerButton(MinimizeButton, { Enum.UserInputType.MouseButton1 })
BindMinimizerButton(MobileMinimizeButton, { Enum.UserInputType.MouseButton1, Enum.UserInputType.Touch })
...
Creator.AddSignal(RunService.Heartbeat, function()
	...
	newX = math.clamp(newX, 0, math.max(0, viewportSize.X - minimizerSize.X))
	newY = math.clamp(newY, 0, math.max(0, viewportSize.Y - minimizerSize.Y))
