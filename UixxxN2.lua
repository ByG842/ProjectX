...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- 🎨 Custom Theme presets: save the current accent color under a name, then
		-- recall or delete it later. Presets persist in InterfaceManager.Settings.
		Settings.CustomThemePresets = Settings.CustomThemePresets or {}

		local function GetPresetNames()
			local names = {}
			for _, preset in ipairs(Settings.CustomThemePresets) do
				table.insert(names, preset.Name)
			end
			if #names == 0 then
				table.insert(names, "No presets saved")
			end
			return names
		end

		local PresetNameInput = customThemeSection:AddInput("CustomThemePresetName", {
			Title = "Preset Name",
			Description = "Name your current accent color, then save it as a preset.",
			Default = "",
			Placeholder = "e.g. Sunset Purple",
			MaxLength = 24,
		})

		local PresetSelectDropdown
		PresetSelectDropdown = customThemeSection:AddDropdown("CustomThemePresetSelect", {
			Title = "Saved Presets",
			Description = "Pick a saved preset to apply or delete it.",
			Values = GetPresetNames(),
			Default = GetPresetNames()[1],
		})

		local function RefreshPresetDropdown()
			local names = GetPresetNames()
			PresetSelectDropdown:SetValues(names)
			PresetSelectDropdown:SetValue(names[1])
		end

		customThemeSection:AddActionButton("SaveCustomThemePreset", {
			Title      = "Save Preset",
			Description = "Stores the current accent color under the name above.",
			ButtonText = "Save",
			CopiedText = "✓ Saved",
			CopyText   = "",
			Callback   = function()
				local Color = AccentColorPicker.Value
				local Name = (PresetNameInput.Value ~= "" and PresetNameInput.Value)
					or ("Preset " .. (#Settings.CustomThemePresets + 1))

				-- overwrite an existing preset with the same name, otherwise append
				local existing = nil
				for _, preset in ipairs(Settings.CustomThemePresets) do
				...
				if existing then
					existing.r, existing.g, existing.b = entry.r, entry.g, entry.b
				else
					table.insert(Settings.CustomThemePresets, entry)
				end

				InterfaceManager:SaveSettings()
				RefreshPresetDropdown()
				PresetSelectDropdown:SetValue(Name)
			end,
		})

		customThemeSection:AddActionButton("ApplyCustomThemePreset", {
			Title      = "Apply Preset",
			Description = "Sets the accent color to the selected preset.",
			ButtonText = "Apply",
			CopiedText = "✓ Applied",
			CopyText   = "",
			Callback   = function()
				local SelectedName = PresetSelectDropdown.Value
				for _, preset in ipairs(Settings.CustomThemePresets) do
					if preset.Name == SelectedName then
						local Color = Color3.fromRGB(preset.r, preset.g, preset.b)
						Library:SetAccentColor(Color)
						Settings.AccentColor = { r = preset.r, g = preset.g, b = preset.b }
						InterfaceManager:SaveSettings()
		...
		customThemeSection:AddActionButton("DeleteCustomThemePreset", {
			Title      = "Delete Preset",
			Description = "Removes the selected preset from your saved list.",
			ButtonText = "Delete",
			CopiedText = "✓ Deleted",
			CopyText   = "",
			Callback   = function()
				local SelectedName = PresetSelectDropdown.Value
				for i, preset in ipairs(Settings.CustomThemePresets) do
					if preset.Name == SelectedName then
						table.remove(Settings.CustomThemePresets, i)
						break
					end
				end
				InterfaceManager:SaveSettings()
