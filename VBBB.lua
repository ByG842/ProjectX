...
Components.Section = (function()
	...
	return function(Title, Parent, Icon, Description)
		...
		local CARD_PAD_V  = 8   -- padding บนล่างภายใน card
		local HEADER_H    = 28  -- ความสูง header row (title)
		local DESC_PAD_V  = 4   -- padding บน/ล่าง description
		local GAP_TOP     = 8   -- ระยะห่างระหว่าง section card กับของข้างบน

		-- คำนวณความสูง description label (ถ้ามี)
		local descLabel = nil
		local descH = 0
		...
		-- Description label (ใต้ header, ถ้ามี)
		if Description and Description ~= "" then
			...
			descLabel = New("TextLabel", {
				...
				AutomaticSize = Enum.AutomaticSize.Y,
				BackgroundTransparency = 1,
				Position = UDim2.fromOffset(20, HEADER_H + DESC_PAD_V),
				ThemeTag = { TextColor3 = "SubText" },
			})
		end

		local HEADER_TOTAL = HEADER_H + (descLabel and descH + DESC_PAD_V * 2 or 0)

		-- อัปเดต position ของ Container ให้อยู่ใต้ description
		Section.Container.Position = UDim2.fromOffset(CARD_PAD_H, HEADER_TOTAL + CARD_PAD_V)

		-- เส้นขีดคั่น header กับ content
		local HeaderDivider = New("Frame", {
			Size = UDim2.new(1, -16, 0, 1),
			Position = UDim2.new(0, 8, 0, HEADER_TOTAL),
			BackgroundTransparency = 0.7,
			ThemeTag = { BackgroundColor3 = "InElementBorder" },
		})

		-- สร้าง children list สำหรับ CardFrame
		local cardChildren = {
		...
		if descLabel then
			table.insert(cardChildren, descLabel)
		end

		-- Card wrapper — มีขอบ + bg จาง ๆ
		local CardFrame = New("Frame", {
			BackgroundTransparency = 0.93,
			Size = UDim2.new(1, 0, 0, HEADER_TOTAL + CARD_PAD_V),
			LayoutOrder = 7,
			Parent = Parent,
		...
		Section.Root = New("Frame", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, HEADER_TOTAL + CARD_PAD_V + GAP_TOP),
			LayoutOrder = 7,
			Parent = Parent,
		}, {
			New("UIPadding", {
		...
		Creator.AddSignal(Section.Layout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
			...
			local totalCardH = HEADER_TOTAL + CARD_PAD_V + contentH + CARD_PAD_V
...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent, Description)
		...
		function Tab:AddSubTab(Title, Icon)
			...
			if not self.SubTabHolder then
				...
				-- Divider คั่นระหว่าง SubTab bar กับ content (ข้อ 11)
				New("Frame", {
					Name = "SubTabDivider",
				...
				local SubTabContainerHolder = New("Frame", {
					Size = UDim2.new(1, -11, 1, -62),
					Position = UDim2.fromOffset(1, 60),
					BackgroundTransparency = 1,
	...
	function TabModule:SelectTab(Tab)
		...
		-- อัปเดต Description ของ Tab header
		local tabDesc = TabModule.Tabs[Tab].Description or ""
		if Window.TabDescDisplay then
			Window.TabDescDisplay.Text = tabDesc
			Window.TabDescDisplay.Visible = tabDesc ~= ""
		end
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		TitleBar.Frame = New("Frame", {
			...
			-- RightSection: Search + Min/Max/Close
			New("Frame", {
				Name = "RightSection",
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- Search Box สร้างไว้ก่อน จะถูก inject เข้า TitleBar RightSection ทีหลัง
		...
		-- Label "TABS" แทนช่องว่างที่ Search Box เคยอยู่
		local TabsLabel = New("TextLabel", {
			Text = "TABS",
			...
			Position = UDim2.fromOffset(4, tabHolderTop - 18),
			BackgroundTransparency = 1,
			ThemeTag = { TextColor3 = "SubText" },
		})

		local TabFrame = New("Frame", {
			Size = UDim2.new(0, Window.TabWidth, 1, -62),
		...
		-- Tab header: Title + optional Description
		Window.TabDisplayFrame = New("Frame", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1, -Window.TabWidth - 52, 0, 46),
			AutomaticSize = Enum.AutomaticSize.Y,
		...
		-- Divider ใต้ Tab header (ข้อ 10)
		Window.TabHeaderDivider = New("Frame", {
			Name = "TabHeaderDivider",
			Size = UDim2.new(1, -Window.TabWidth - 52, 0, 1),
			Position = UDim2.fromOffset(Window.TabWidth + 26, 106),
			BackgroundTransparency = 0.5,
			ThemeTag = { BackgroundColor3 = "InElementBorder" },
		})

		Window.ContainerCanvas = New("Frame", {
			Size = UDim2.new(1, -Window.TabWidth - 38, 1, -118),
			Position = UDim2.fromOffset(Window.TabWidth + 26, 114),
			BackgroundTransparency = 1,
			ClipsDescendants = true,
		}, {
		...
		table.insert(rootChildren, Window.TabDisplayFrame)
		...
		-- Inject SearchFrame เข้า TitleBar RightSection (ก่อน Min/Max/Close)
		if Window.ShowSearch and SearchFrame then
			local rightSection = Window.TitleBar.Frame:FindFirstChild("RightSection")
			if rightSection then
				SearchFrame.Parent = rightSection
		...
		if Config.UserInfo then
			...
			if Config.UserInfoTop then
				...
				local newTabHolderTop = userInfoHeight + 6 + (hasImage and (imageSize + 10 + topOffset) or topOffset)
		...
		Window.SelectorPosMotor:onStep(function(Value)
			local base = Window.TabHolderTop or 8
		...
		function Window:AddTab(TabConfig)
			local tab = TabModule:New(TabConfig.Title, TabConfig.Icon, Window.TabHolder, TabConfig.Description)
			return tab
		end
...
local SaveManager = {} do
	...
	function SaveManager:BuildConfigSection(tab)
		...
		-- AutoSave toggle (เปิดไว้เป็นค่าเริ่มต้น)
		local autoSaveEnabled = true

		-- โหลดค่า autoSave ที่บันทึกไว้ (ถ้ามี)
		if not RunService:IsStudio() then
			local autoSavePath = self.Folder .. "/autosave.txt"
			if isfile and isfile(autoSavePath) then
				local val = readfile(autoSavePath) or "true"
				autoSaveEnabled = val:match("^%s*(.-)%s*$") ~= "false"
			end
		end

		section:AddToggle("AutoSave", {
			Title = "Auto Save",
			Description = "เมื่อเปิด ระบบจะบันทึก config ให้อัตโนมัติ",
			...
			Callback = function(Value)
				...
				if not RunService:IsStudio() then
					pcall(function()
						writefile(self.Folder .. "/autosave.txt", Value and "true" or "false")
					end)
				end
			end
		})

		SaveManager.AutoSave = autoSaveEnabled
		...
		task.spawn(function()
			while true do
				...
				if SaveManager.AutoSave then
					local defaultConfig = self.Folder .. "/autosave_config.json"
					pcall(function()
						self:Save("autosave_config")
					end)
				end
			end
		end)
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- ── Section 2: Custom Theme (สร้างธีม) ───────────────────────────
		local customSection = tab:AddSection("Custom Theme", "palette", "ออกแบบสีธีมของคุณเอง แล้วบันทึกไว้ใช้งาน")

		-- โหลด custom theme ที่บันทึกไว้ (ถ้ามี)
		...
		local function saveCustomTheme(themeData)
		...
		-- ค่าเริ่มต้นจากธีมปัจจุบัน หรือธีมที่บันทึกไว้
		local saved = loadCustomTheme()
		local baseTheme = Themes[Library.Theme] or Themes["Dark"]

		local customColors = {
			Accent         = saved and saved.Accent         or baseTheme.Accent,
			AcrylicMain    = saved and saved.AcrylicMain    or baseTheme.AcrylicMain,
			Element        = saved and saved.Element        or baseTheme.Element,
			Text           = saved and saved.Text           or baseTheme.Text,
			SubText        = saved and saved.SubText        or baseTheme.SubText,
			TitleBarLine   = saved and saved.TitleBarLine   or baseTheme.TitleBarLine,
		}
		...
		if saved then
			for k, v in pairs(saved) do
				if type(v) == "table" and v.R then
					customColors[k] = tableToColor(v)
				end
			end
		end

		-- ฟังก์ชัน apply custom theme ชั่วคราว
		local function applyPreview()
			local currentThemeData = Themes[Library.Theme] or Themes["Dark"]
			-- patch เฉพาะ key ที่เราแก้
			local patched = {}
			for k, v in pairs(currentThemeData) do
				patched[k] = v
			end
			for k, v in pairs(customColors) do
				patched[k] = v
			end
			-- inject ชั่วคราวผ่าน Creator
			if Creator and Creator.Theme then
		...
		customSection:AddColorpicker("CustomAccent", {
			Title = "Accent Color",
			Description = "สีเน้น ปุ่ม, Slider, Toggle",
		...
		customSection:AddColorpicker("CustomBackground", {
			Title = "Background Color",
			Description = "สีพื้นหลังหลักของหน้าต่าง",
			Default = customColors.AcrylicMain,
			Callback = function(Value)
				customColors.AcrylicMain = Value
				applyPreview()
			end
		})

		customSection:AddColorpicker("CustomElement", {
			Title = "Element Color",
			Description = "สีพื้นหลังของ Element ต่างๆ",
			Default = customColors.Element,
			Callback = function(Value)
				customColors.Element = Value
				applyPreview()
			end
		})

		customSection:AddColorpicker("CustomText", {
			Title = "Text Color",
		...
		customSection:AddColorpicker("CustomSubText", {
			Title = "SubText Color",
			Description = "สีตัวอักษรรอง",
			Default = customColors.SubText,
			Callback = function(Value)
				customColors.SubText = Value
				applyPreview()
			end
		})

		-- ── Section 3: Save Custom Theme ──────────────────────────────────
		local saveSection = tab:AddSection("Save Custom Theme", "save", "บันทึกธีม Custom ที่ออกแบบไว้ หรือ Reset กลับธีมเดิม")

		saveSection:AddButton({
			Title = "Apply & Save",
			Description = "บันทึกธีม Custom และใช้งานทันที",
			Callback = function()
				-- บันทึกลงไฟล์
				local saveData = {}
				for k, v in pairs(customColors) do
					if typeof(v) == "Color3" then
						saveData[k] = colorToTable(v)
					end
				end
				saveCustomTheme(saveData)

				applyPreview()

				Library:Notify({
					Title = "Custom Theme",
					Content = "Theme saved and applied!",
					Duration = 4
				})
			end
		})
		...
		saveSection:AddButton({
			...
			Description = "คืนค่าสีกลับเป็นธีมที่เลือกไว้",
			Callback = function()
				Library:SetTheme(Library.Theme)
				Library:Notify({
					Title = "Custom Theme",
					Content = "Reset to " .. Library.Theme,
					Duration = 4
				})
			end
		})
...
function Library:SetWindowTransparency(Value)
	...
	local acrylicFrame = Library.Window.AcrylicPaint and Library.Window.AcrylicPaint.Frame
	local bgFrame = acrylicFrame and acrylicFrame:FindFirstChild("Background")

	if Library.UseAcrylic and acrylicFrame then
		...
		if Library.Theme == "Glass" then
			...
			if bgFrame then
				bgFrame.BackgroundTransparency = math.min(backgroundTransparency, 0.99)
			end
			...
			if bgFrame then
				bgFrame.BackgroundTransparency = math.clamp(Value * 0.3, 0, 0.99)
			end
		...
		-- ไม่ใช้ Acrylic — ปรับ BackgroundTransparency ของ Background frame โดยตรง
		if bgFrame then
			bgFrame.BackgroundTransparency = math.clamp(Value * 0.3, 0, 0.99)
		elseif acrylicFrame then
			acrylicFrame.BackgroundTransparency = math.clamp(0.85 + Value * 0.05, 0, 0.99)
		end
