...
-- ╔══════════════════════════════════════════════════════════════════════╗
-- ║  ZZEN QOL / REWORK PACK  —  100+ additions & improvements             ║
-- ║  Appended extension. Does not modify any existing behaviour above;   ║
-- ║  everything below only ADDS new capabilities to Library / Themes /   ║
-- ║  SaveManager / InterfaceManager.                                     ║
-- ╚══════════════════════════════════════════════════════════════════════╝

do
	----------------------------------------------------------------
	-- SECTION 1 : NEW THEMES  (#1 - #20)
	----------------------------------------------------------------
	...
	local function mix(c1, c2, t)
		return Color3.fromRGB(
			clampByte(math.floor(c1.R * 255 + (c2.R * 255 - c1.R * 255) * t)),
			clampByte(math.floor(c1.G * 255 + (c2.G * 255 - c1.G * 255) * t)),
			clampByte(math.floor(c1.B * 255 + (c2.B * 255 - c1.B * 255) * t))
		)
	end

	-- Builds a full theme table (same shape as the built-in themes) from
	-- just a handful of seed colors, so we can add many new themes without
	-- hand-authoring 30+ colour keys each time.
	local function BuildTheme(name, base, accent, text)
		local black = Color3.fromRGB(0, 0, 0)
		local white = Color3.fromRGB(255, 255, 255)
		return {
			Name = name,
			Accent = accent,
			AcrylicMain = mix(base, black, 0.35),
			AcrylicBorder = mix(base, white, 0.18),
			AcrylicGradient = ColorSequence.new(mix(base, black, 0.4), mix(base, black, 0.25)),
			AcrylicNoise = 0.93,
			TitleBarLine = mix(base, black, 0.25),
			Tab = mix(base, white, 0.35),
			Element = mix(base, black, 0.1),
			ElementBorder = mix(base, black, 0.4),
			InElementBorder = mix(base, white, 0.1),
			ElementTransparency = 0.88,
			ToggleSlider = mix(base, white, 0.2),
			ToggleToggled = mix(base, black, 0.45),
			SliderRail = mix(base, white, 0.1),
			DropdownFrame = mix(base, white, 0.4),
			DropdownHolder = mix(base, black, 0.2),
			DropdownBorder = mix(base, black, 0.4),
			DropdownOption = mix(base, white, 0.2),
			Keybind = mix(base, white, 0.2),
			Input = mix(base, white, 0.4),
			InputFocused = mix(base, black, 0.5),
			InputIndicator = mix(base, white, 0.3),
			InputIndicatorFocus = accent,
			Dialog = mix(base, black, 0.05),
			DialogHolder = mix(base, black, 0.25),
			DialogHolderLine = mix(base, black, 0.45),
			DialogButton = mix(base, black, 0.02),
			DialogButtonBorder = mix(base, white, 0.15),
			DialogBorder = mix(base, white, 0.1),
			DialogInput = mix(base, black, 0.1),
			DialogInputLine = mix(base, white, 0.4),
			Text = text,
			SubText = mix(text, base, 0.45),
			Hover = mix(base, white, 0.15),
			HoverChange = 0.08,
		}
	end

	local NewThemeSeeds = {
		{"Sunset",     Color3.fromRGB(40, 20, 25),  Color3.fromRGB(255, 130, 90),  Color3.fromRGB(255, 235, 225)},
		{"Forest",     Color3.fromRGB(18, 28, 20),  Color3.fromRGB(110, 190, 110), Color3.fromRGB(225, 240, 225)},
		{"Cyberpunk",  Color3.fromRGB(15, 10, 25),  Color3.fromRGB(255, 60, 180),  Color3.fromRGB(230, 230, 255)},
		{"Nord",       Color3.fromRGB(30, 35, 45),  Color3.fromRGB(136, 192, 208), Color3.fromRGB(230, 235, 240)},
		{"Dracula",    Color3.fromRGB(30, 26, 40),  Color3.fromRGB(189, 147, 249), Color3.fromRGB(240, 235, 250)},
		{"Monokai",    Color3.fromRGB(28, 28, 22),  Color3.fromRGB(166, 226, 46),  Color3.fromRGB(240, 240, 230)},
		{"Solarized",  Color3.fromRGB(20, 35, 40),  Color3.fromRGB(38, 139, 210),  Color3.fromRGB(230, 240, 240)},
		{"Crimson",    Color3.fromRGB(30, 12, 14),  Color3.fromRGB(220, 60, 70),   Color3.fromRGB(245, 225, 225)},
		{"CottonCandy",Color3.fromRGB(35, 25, 40),  Color3.fromRGB(255, 160, 220), Color3.fromRGB(250, 235, 250)},
		{"Slate",      Color3.fromRGB(28, 30, 34),  Color3.fromRGB(150, 165, 180), Color3.fromRGB(225, 230, 235)},
		{"Coffee",     Color3.fromRGB(30, 22, 16),  Color3.fromRGB(190, 140, 90),  Color3.fromRGB(240, 225, 210)},
		{"Mint",       Color3.fromRGB(14, 28, 26),  Color3.fromRGB(90, 220, 180),  Color3.fromRGB(225, 245, 240)},
		{"Lavender",   Color3.fromRGB(28, 24, 38),  Color3.fromRGB(170, 150, 230), Color3.fromRGB(235, 230, 250)},
		{"Graphite",   Color3.fromRGB(20, 20, 22),  Color3.fromRGB(160, 160, 165), Color3.fromRGB(225, 225, 225)},
		{"Neon",       Color3.fromRGB(8, 10, 18),   Color3.fromRGB(60, 255, 220),  Color3.fromRGB(220, 255, 250)},
		{"BloodMoon",  Color3.fromRGB(18, 8, 10),   Color3.fromRGB(200, 30, 40),   Color3.fromRGB(240, 210, 210)},
		{"Arctic",     Color3.fromRGB(20, 26, 32),  Color3.fromRGB(140, 210, 240), Color3.fromRGB(230, 240, 245)},
		{"Autumn",     Color3.fromRGB(30, 20, 12),  Color3.fromRGB(230, 140, 50),  Color3.fromRGB(245, 225, 200)},
		{"Royal",      Color3.fromRGB(14, 14, 34),  Color3.fromRGB(120, 100, 230), Color3.fromRGB(225, 220, 250)},
		{"Volcano",    Color3.fromRGB(22, 10, 8),   Color3.fromRGB(255, 90, 40),   Color3.fromRGB(250, 220, 205)},
	}

	for _, seed in ipairs(NewThemeSeeds) do
		local themeName, base, accent, text = seed[1], seed[2], seed[3], seed[4]
		if not Themes[themeName] then
			Themes[themeName] = BuildTheme(themeName, base, accent, text)
			table.insert(Themes.Names, themeName)
		end
	end

	----------------------------------------------------------------
	-- SECTION 2 : COLOR & MATH UTILITIES  (#21 - #30)
	----------------------------------------------------------------
	function Library:Clamp(n, min, max) -- #21
		if n < min then return min elseif n > max then return max else return n end
	end

	function Library:Lerp(a, b, t) -- #22
		return a + (b - a) * t
	end

	function Library:MapRange(n, inMin, inMax, outMin, outMax) -- #23
		if inMax == inMin then return outMin end
		return outMin + (outMax - outMin) * ((n - inMin) / (inMax - inMin))
	end

	function Library:RandomColor() -- #24
		return Color3.fromRGB(math.random(0, 255), math.random(0, 255), math.random(0, 255))
	end

	function Library:HexToColor3(hex) -- #25
		hex = hex:gsub("#", "")
	...
	function Library:Color3ToHex(color) -- #26
		return string.format("#%02X%02X%02X", math.floor(color.R * 255), math.floor(color.G * 255), math.floor(color.B * 255))
	end

	function Library:DarkenColor(color, amount) -- #27
		amount = amount or 0.2
		return mix(color, Color3.fromRGB(0, 0, 0), amount)
	end

	function Library:LightenColor(color, amount) -- #28
		amount = amount or 0.2
		return mix(color, Color3.fromRGB(255, 255, 255), amount)
	end

	function Library:MixColor(colorA, colorB, t) -- #29
		return mix(colorA, colorB, t)
	end

	function Library:InvertColor(color) -- #30
		return Color3.fromRGB(255 - math.floor(color.R * 255), 255 - math.floor(color.G * 255), 255 - math.floor(color.B * 255))
	end

	----------------------------------------------------------------
	-- SECTION 3 : FORMATTING UTILITIES  (#31 - #38)
	----------------------------------------------------------------
	function Library:FormatNumber(n) -- #31 e.g 1234567 -> "1.23M"
		n = tonumber(n) or 0
		local absN = math.abs(n)
		if absN >= 1e9 then return string.format("%.2fB", n / 1e9)
		elseif absN >= 1e6 then return string.format("%.2fM", n / 1e6)
		elseif absN >= 1e3 then return string.format("%.2fK", n / 1e3)
		else return tostring(n) end
	end

	function Library:FormatTime(seconds) -- #32 -> HH:MM:SS
	...
	function Library:FormatBytes(bytes) -- #33
	...
	function Library:CommaNumber(n) -- #34 -> "1,234,567"
	...
	function Library:TitleCase(str) -- #35
		return (str:gsub("(%a)([%w_']*)", function(first, rest) return first:upper() .. rest:lower() end))
	end

	function Library:Truncate(str, maxLen, suffix) -- #36
		suffix = suffix or "..."
		if #str <= maxLen then return str end
		return str:sub(1, maxLen) .. suffix
	end

	function Library:TrimWhitespace(str) -- #37
		return (str:gsub("^%s*(.-)%s*$", "%1"))
	end

	function Library:SplitString(str, sep) -- #38
		sep = sep or ","
		local parts = {}
	...
	----------------------------------------------------------------
	-- SECTION 4 : TABLE / DATA UTILITIES  (#39 - #46)
	----------------------------------------------------------------
	function Library:DeepCopy(tbl) -- #39
		if type(tbl) ~= "table" then return tbl end
		local result = {}
		for k, v in pairs(tbl) do
			result[k] = (type(v) == "table") and Library:DeepCopy(v) or v
		end
		return result
	end

	function Library:TableLength(tbl) -- #40
		local count = 0
		for _ in pairs(tbl) do count = count + 1 end
		return count
	end

	function Library:TableContains(tbl, value) -- #41
	...
	function Library:TableFind(tbl, value) -- #42
	...
	function Library:ShuffleTable(tbl) -- #43
		local result = Library:DeepCopy(tbl)
		for i = #result, 2, -1 do
	...
	function Library:MergeTables(t1, t2) -- #44
		local result = Library:DeepCopy(t1)
	...
	function Library:FilterTable(tbl, predicate) -- #45
		local result = {}
	...
	function Library:MapTable(tbl, fn) -- #46
		local result = {}
	...
	----------------------------------------------------------------
	-- SECTION 5 : FUNCTION UTILITIES  (#47 - #51)
	----------------------------------------------------------------
	function Library:Debounce(fn, delaySeconds) -- #47
		local waiting = false
		return function(...)
			if waiting then return end
			waiting = true
			fn(...)
			task.delay(delaySeconds or 0.2, function() waiting = false end)
		end
	end

	function Library:Throttle(fn, interval) -- #48
		local lastRun = 0
		return function(...)
			local now = os.clock()
			if now - lastRun >= (interval or 0.2) then
				lastRun = now
				fn(...)
			end
		end
	...
	function Library:SafeCall(fn, ...) -- #49
		local ok, result = pcall(fn, ...)
		if not ok then
			Library:Log("SafeCall error: " .. tostring(result))
			return nil
		end
		return result
	end

	function Library:Retry(fn, attempts, delaySeconds) -- #50
		attempts = attempts or 3
		for i = 1, attempts do
			local ok, result = pcall(fn)
			if ok then return result end
			if i < attempts then task.wait(delaySeconds or 0.5) end
		end
		return nil
	end

	function Library:Once(fn) -- #51
	...
	----------------------------------------------------------------
	-- SECTION 6 : PLAYER / GAME INFO UTILITIES  (#52 - #60)
	----------------------------------------------------------------
	local sessionStart = os.clock()

	function Library:GetFPS() -- #52
		return math.floor((1 / (RunService.RenderStepped:Wait())) + 0.5)
	end

	function Library:GetPing() -- #53
		local ok, stats = pcall(function() return game:GetService("Stats") end)
		if ok and stats then
			local ok2, val = pcall(function()
				return stats.Network.ServerStatsItem["Data Ping"]:GetValue()
			end)
			if ok2 then return math.floor(val) end
		end
		return -1
	end

	function Library:GetPlayerName() -- #54
		return LocalPlayer.Name
	end

	function Library:GetGameName() -- #55
	...
	function Library:GetElapsedTime() -- #56 seconds since library loaded
		return os.clock() - sessionStart
	end

	function Library:IsMobilePlatform() -- #57
		return Mobile
	end

	function Library:GetViewportSize() -- #58
		return Camera.ViewportSize
	end

	function Library:GetMousePosition() -- #59
		return UserInputService:GetMouseLocation()
	end

	function Library:IsKeyDown(keyCode) -- #60
		return UserInputService:IsKeyDown(keyCode)
	end

	----------------------------------------------------------------
	-- SECTION 7 : CLIPBOARD / SYSTEM  (#61 - #63)
	----------------------------------------------------------------
	function Library:CopyToClipboard(text) -- #61
	...
	function Library:HasClipboardSupport() -- #62
		return (setclipboard or toclipboard) ~= nil
	end

	function Library:Vibrate(intensity) -- #63 (mobile haptics if the executor exposes it)
		local haptic = rawget(getfenv and getfenv() or _G, "HapticFeedback")
		if haptic then pcall(haptic, intensity or 1) end
	end

	----------------------------------------------------------------
	-- SECTION 8 : NOTIFICATION SHORTHANDS  (#64 - #69)
	----------------------------------------------------------------
	function Library:Success(title, content, duration) -- #64
		return Library:Notify({ Title = title or "Success", Content = content or "", Duration = duration or 4 })
	end

	function Library:Warn(title, content, duration) -- #65
		return Library:Notify({ Title = title or "Warning", Content = content or "", Duration = duration or 5 })
	end

	function Library:Error(title, content, duration) -- #66
		return Library:Notify({ Title = title or "Error", Content = content or "", Duration = duration or 6 })
	end

	function Library:Info(title, content, duration) -- #67
		return Library:Notify({ Title = title or "Info", Content = content or "", Duration = duration or 4 })
	end

	function Library:ClearNotifications() -- #68
		if Library.ActiveNotifications then
			for _, notif in pairs(Library.ActiveNotifications) do
				if notif.Frame and notif.Frame.Destroy then
	...
	local notifiedOnceIds = {}
	function Library:NotifyOnce(id, config) -- #69 fires only the first time a given id is seen
		if notifiedOnceIds[id] then return end
		notifiedOnceIds[id] = true
		return Library:Notify(config)
	end

	----------------------------------------------------------------
	-- SECTION 9 : VISUAL / WINDOW FEATURE TOGGLES  (#70 - #76)
	----------------------------------------------------------------
	function Library:ShowFPSCounter(position) -- #70
		if Library._fpsLabel then return end
		local label = New("TextLabel", {
			Parent = GUI,
			Position = position or UDim2.new(0, 8, 0, 8),
		...
		Library._fpsLabel = label
		local frames, lastCheck = 0, os.clock()
		Library._fpsConn = RunService.RenderStepped:Connect(function()
			frames = frames + 1
			local now = os.clock()
			if now - lastCheck >= 0.5 then
				label.Text = "FPS: " .. math.floor(frames / (now - lastCheck))
				frames, lastCheck = 0, now
			end
		end)
	end

	function Library:HideFPSCounter() -- #71
		if Library._fpsConn then Library._fpsConn:Disconnect() Library._fpsConn = nil end
		if Library._fpsLabel then Library._fpsLabel:Destroy() Library._fpsLabel = nil end
	end

	function Library:ShowPingCounter(position) -- #72
		if Library._pingLabel then return end
		local label = New("TextLabel", {
			Parent = GUI,
			Position = position or UDim2.new(0, 8, 0, 36),
			Size = UDim2.new(0, 90, 0, 24),
			BackgroundColor3 = Color3.fromRGB(15, 15, 15),
			BackgroundTransparency = 0.3,
		...
		Library._pingLabel = label
		task.spawn(function()
			while Library._pingLabel do
				label.Text = "Ping: " .. tostring(Library:GetPing()) .. "ms"
				task.wait(1)
			end
		end)
	end

	function Library:HidePingCounter() -- #73
		Library._pingLabel = Library._pingLabel and (Library._pingLabel:Destroy() or nil)
	end

	function Library:SetWatermark(text, position) -- #74
		if not Library._watermark then
			Library._watermark = New("TextLabel", {
				Parent = GUI,
				Position = position or UDim2.new(0, 8, 1, -32),
				...
				Text = "",
				ZIndex = 1000,
			}, { New("UICorner", { CornerRadius = UDim.new(0, 6) }), New("UIPadding", { PaddingLeft = UDim.new(0, 8) }) })
		end
		Library._watermark.Text = text or ""
	end

	function Library:HideWatermark() -- #75
		if Library._watermark then Library._watermark:Destroy() Library._watermark = nil end
	end

	function Library:ToggleBackgroundBlur(enabled, size) -- #76
		if enabled then
			if not Library._blurEffect then
				Library._blurEffect = New("BlurEffect", { Parent = Lighting, Size = size or 12 })
			else
				Library._blurEffect.Size = size or 12
			end
		elseif Library._blurEffect then
			Library._blurEffect:Destroy()
			Library._blurEffect = nil
		end
	end

	----------------------------------------------------------------
	-- SECTION 10 : THEME TOOLS  (#77 - #81)
	----------------------------------------------------------------
	function Library:RandomizeTheme() -- #77 applies a random registered theme
		local pick = Themes.Names[math.random(1, #Themes.Names)]
		if Library.SetTheme then Library:SetTheme(pick) end
		return pick
	end

	function Library:ExportTheme(themeName) -- #78 -> JSON string of hex colors
		local theme = Themes[themeName or Library.Theme]
		if not theme then return nil end
		local out = {}
		for key, value in pairs(theme) do
			if typeof(value) == "Color3" then
				out[key] = Library:Color3ToHex(value)
			end
		end
		return httpService:JSONEncode(out)
	end

	function Library:ImportTheme(themeName, json) -- #79 applies hex colors on top of an existing theme
		local ok, decoded = pcall(function() return httpService:JSONDecode(json) end)
		if not ok or not Themes[themeName] then return false end
		for key, hex in pairs(decoded) do
			if type(hex) == "string" then
				Themes[themeName][key] = Library:HexToColor3(hex)
			end
		end
		return true
	end

	function Library:ListThemeNames() -- #80
		return Library:DeepCopy(Themes.Names)
	end

	function Library:GetCurrentThemeColors() -- #81
		return Themes[Library.Theme]
	end

	----------------------------------------------------------------
	-- SECTION 11 : SAVEMANAGER ENHANCEMENTS  (#82 - #87)
	----------------------------------------------------------------
	function SaveManager:ConfigExists(name) -- #82
		local ok, exists = pcall(function()
			return isfile(("%s/settings/%s.json"):format(SaveManager.Folder or "FluentSettings", name))
		end)
		return ok and exists or false
	end

	function SaveManager:ListConfigNames() -- #83
		local list = {}
		local ok, files = pcall(function()
			return listfiles(("%s/settings"):format(SaveManager.Folder or "FluentSettings"))
		end)
	...
	function SaveManager:DeleteConfig(name) -- #84
		local ok = pcall(function()
			delfile(("%s/settings/%s.json"):format(SaveManager.Folder or "FluentSettings", name))
		end)
		return ok
	end

	function SaveManager:ExportConfigToString(name) -- #85
		local ok, data = pcall(function()
			return readfile(("%s/settings/%s.json"):format(SaveManager.Folder or "FluentSettings", name))
		end)
		return ok and data or nil
	end

	function SaveManager:ImportConfigFromString(name, str) -- #86
		local ok = pcall(function()
			writefile(("%s/settings/%s.json"):format(SaveManager.Folder or "FluentSettings", name), str)
		end)
		return ok
	end

	function SaveManager:DuplicateConfig(sourceName, newName) -- #87
		local data = SaveManager:ExportConfigToString(sourceName)
		if not data then return false end
		return SaveManager:ImportConfigFromString(newName, data)
	end

	----------------------------------------------------------------
	-- SECTION 12 : INTERFACEMANAGER ENHANCEMENTS  (#88 - #90)
	----------------------------------------------------------------
	function InterfaceManager:GetCurrentTheme() -- #88
		return Library.Theme
	end

	function InterfaceManager:ApplyThemePreset(name) -- #89
		if Themes[name] and Library.SetTheme then
	...
	function InterfaceManager:ResetToDefault() -- #90
		if Library.SetTheme then Library:SetTheme("Dark") end
		Library.CustomAccentColor = nil
	end

	----------------------------------------------------------------
	-- SECTION 13 : DEBUG / LOGGING  (#91 - #96)
	----------------------------------------------------------------
	Library.Version = "2.0.0-rework"

	function Library:Log(message) -- #91
		print(("[Zzen] %s"):format(tostring(message)))
	end

	function Library:LogWarn(message) -- #92
		warn(("[Zzen] %s"):format(tostring(message)))
	end

	function Library:Assert(condition, message) -- #93
		if not condition then
			Library:LogWarn("Assertion failed: " .. tostring(message))
		end
		return condition
	end

	function Library:GetVersion() -- #94
		return Library.Version
	end

	function Library:PrintDebugInfo() -- #95
		Library:Log(("Version=%s Theme=%s Mobile=%s Elapsed=%.1fs"):format(
			Library.Version, tostring(Library.Theme), tostring(Mobile), Library:GetElapsedTime()
		))
	end

	function Library:SetVersionTag(tag) -- #96
		Library.Version = tag
	end

	----------------------------------------------------------------
	-- SECTION 14 : MISC QUALITY-OF-LIFE  (#97 - #102)
	----------------------------------------------------------------
	function Library:SetGlobalTransparency(value) -- #97 dims every top-level frame in the GUI
		for _, obj in ipairs(GUI:GetChildren()) do
			if obj:IsA("GuiObject") then
				pcall(function() obj.BackgroundTransparency = value end)
			end
		end
	end

	function Library:PulseElement(guiObject, pulseColor, duration) -- #98 quick highlight flash, useful for drawing attention
		if not guiObject or not guiObject:IsA("GuiObject") then return end
		local original = guiObject.BackgroundColor3
		local tween = TweenService:Create(guiObject, TweenInfo.new(duration or 0.25), { BackgroundColor3 = pulseColor or Color3.fromRGB(255, 255, 255) })
		tween:Play()
		tween.Completed:Connect(function()
			TweenService:Create(guiObject, TweenInfo.new(duration or 0.25), { BackgroundColor3 = original }):Play()
		end)
	end

	function Library:RegisterHotkeyCombo(keys, callback) -- #99 e.g. {Enum.KeyCode.LeftControl, Enum.KeyCode.K}
		local held = {}
	...
	function Library:GetLibraryInfo() -- #100
		return {
			Version = Library.Version,
			ThemeCount = #Themes.Names,
			CurrentTheme = Library.Theme,
			IsMobile = Mobile,
		}
	end

	function Library:ToggleAllElementsVisible(visible) -- #101
		if Library.Window and Library.Window.Root then
			pcall(function() Library.Window.Root.Visible = visible end)
		end
	end

	function Library:Destroy2(fadeOut) -- #102 same as Destroy but fades the whole GUI first
		if fadeOut and GUI then
	...
	Library:Log("Zzen QoL / Rework pack loaded — " .. Library.Version)
end
