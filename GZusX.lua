...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		-- เพิ่มระยะห่างแนวตั้งระหว่าง Title / Description ของ element นี้แบบพอดี ไม่ชิด ไม่ห่างเกิน
		...
		local ChipRow = New("Frame", {
			...
			-- ระยะห่างจาก Description กระชับขึ้น ไม่มีช่องว่างเยอะเกินไป
			New("UIPadding", { PaddingTop = UDim.new(0, 6), PaddingBottom = UDim.new(0, 2) }),
		})
		...
		-- อนิเมชันตอนกด: ยุบตัวไวๆ แล้วเด้งกลับแบบสปริงนุ่มๆ (Elastic) ให้ความรู้สึกมีชีวิตชีวาและลื่นไหลกว่าเดิม
		local function bounceChip(btn)
			local scale = btn:FindFirstChildOfClass("UIScale")
			if not scale then return end

			-- ยกเลิกอนิเมชันเดิมถ้ายังไม่จบ กันอาการกระตุก
			scale.Scale = 0.8
