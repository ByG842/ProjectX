...
-- ═══════════════════════════════════════════════════════════════════════════════
-- SelectGroup v2: Unified Selection Component — Fancy Edition ✨
-- RadioGroup + Chip รวมร่าง พร้อมอนิเมชั่นลื่นๆ และลูกเล่นครบ แต่โค้ดกระชับ
-- ═══════════════════════════════════════════════════════════════════════════════
...
ElementsTable.SelectGroup = (function()
	...
	-- อนิเมชั่นสปริงนุ่มๆ ใช้ร่วมกันทุกจุด (ลดโค้ดซ้ำ)
	local Spring = TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
	local Quick  = TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	local Pop    = TweenInfo.new(0.35, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out, 0, false, 0)

	local function Tween(obj, info, props)
		TweenService:Create(obj, info, props):Play()
	end

	-- 💧 Ripple effect เวลากด (ลูกเล่นเล็กๆ ที่ทำให้รู้สึกพรีเมียม)
	...
	function Element:New(Idx, Config)
		...
		assert(Config.Options, "SelectGroup - Missing Options table")

		local IsMulti = Config.Multi or false
		local Style = (Config.Style == "chip") and "chip" or "radio"
		...
		if IsMulti then
			...
			if type(Config.Default) == "table" then
				for _, v in pairs(Config.Default) do SG.Value[v] = true end
			elseif Config.Default ~= nil then
				SG.Value[Config.Default] = true
			end
		else
			SG.Value = Config.Default or Config.Options[1]
		end

		local Frame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		SG.SetTitle, SG.SetDesc, SG.Visible, SG.Elements = Frame.SetTitle, Frame.SetDesc, Frame.Visible, Frame
		...
		local Holder = New("Frame", {
			...
			Position = UDim2.fromOffset(10, 0),
			BackgroundTransparency = 1,
			Parent = Frame.LabelHolder,
			LayoutOrder = 3,
			ClipsDescendants = false,
		}, {
			New("UIListLayout", Style == "chip" and {
				FillDirection = Enum.FillDirection.Horizontal,
				Padding = UDim.new(0, 8),
				Wraps = true,
			} or {
			...
			New("UIPadding", { PaddingTop = UDim.new(0, Style == "chip" and 6 or 4), PaddingBottom = UDim.new(0, 6) }),
		})

		local Buttons = {}

		local function GetValue()
			if not IsMulti then return SG.Value end
			local out = {}
			for _, opt in ipairs(SG.Options) do
				if SG.Value[opt] then table.insert(out, opt) end
			end
			return out
		end

		-- ✨ Update พร้อมอนิเมชั่นสปริง + glow เรืองแสงตอนเลือก
		local function Refresh(onlyItem)
			for _, btn in pairs(Buttons) do
			  if not onlyItem or btn.Value == onlyItem then
				local sel = IsMulti and (SG.Value[btn.Value] == true) or (btn.Value == SG.Value)

				if Style == "radio" then
					Tween(btn.Outer, Spring, {
						BackgroundTransparency = sel and 0.05 or 0.88,
						Size = sel and UDim2.fromOffset(18, 18) or UDim2.fromOffset(16, 16),
					})
					Tween(btn.Inner, Spring, {
						BackgroundTransparency = sel and 0 or 1,
						Size = sel and UDim2.fromOffset(9, 9) or UDim2.fromOffset(5, 5),
					})
					Tween(btn.Glow, Quick, { ImageTransparency = sel and 0.6 or 1 })
					Tween(btn.Label, Quick, { TextTransparency = sel and 0 or 0.15 })
				else
					Tween(btn.Btn, Quick, {
						BackgroundTransparency = sel and 0.1 or 0.92,
						TextTransparency = sel and 0 or 0.35,
					})
					if btn.Stroke then Tween(btn.Stroke, Quick, { Transparency = sel and 0.05 or 0.85 }) end
					-- ✨ เด้งนิดๆ ตอนเลือก (pop effect)
					if sel then
						btn.Btn.Size = UDim2.fromOffset(btn.Btn.AbsoluteSize.X * 0.9, 28)
						Tween(btn.Btn, Pop, { Size = UDim2.new(0, 0, 0, 28) })
						btn.Btn.AutomaticSize = Enum.AutomaticSize.X
					end
					if btn.Check then
						Tween(btn.Check, Spring, {
							Size = sel and UDim2.fromOffset(14, 14) or UDim2.fromOffset(0, 0),
		...
		for _, opt in ipairs(Config.Options) do
			if Style == "radio" then
				...
				local Outer = New("Frame", {
					...
					-- ✨ Glow เรืองแสงรอบวงกลมตอนเลือก
					New("ImageLabel", {
						Image = "rbxassetid://131472638634488", -- soft glow blur asset (fallback: plain glow via BackgroundColor if unavailable)
						ImageTransparency = 1,
						ScaleType = Enum.ScaleType.Fit,
				...
				local Glow = Outer:FindFirstChildOfClass("ImageLabel")
				...
				local Data = { Value = opt, Outer = Outer, Inner = Inner, Glow = Glow, Label = Label, Row = Row }
				table.insert(Buttons, Data)

				-- Hover: ขยับตัวหนังสือขึ้นเล็กน้อย ให้รู้สึกมีชีวิตชีวา
				Creator.AddSignal(Row.MouseEnter, function()
					Tween(Outer, Quick, { BackgroundTransparency = (opt == SG.Value or (IsMulti and SG.Value[opt])) and 0.05 or 0.75 })
				end)
				Creator.AddSignal(Row.MouseLeave, function() Refresh(opt) end)

				Creator.AddSignal(Row.MouseButton1Click, function()
					if IsMulti then
						SG.Value[opt] = SG.Value[opt] and nil or true
					else
						SG.Value = opt
					end
					Refresh()
					Library:SafeCallback(SG.Callback, GetValue())
					Library:SafeCallback(SG.Changed, GetValue())
				end)
				...
				local Btn = New("TextButton", {
					...
					New("UIListLayout", { FillDirection = Enum.FillDirection.Horizontal, VerticalAlignment = Enum.VerticalAlignment.Center, Padding = UDim.new(0, 2) }),
				})
				local Stroke = Btn:FindFirstChildOfClass("UIStroke")

				-- ✨ Checkmark เล็กๆ โผล่ตอนเลือก (ลูกเล่นน่ารักๆ)
				...
				local Data = { Value = opt, Btn = Btn, Stroke = Stroke, Check = Check }
				table.insert(Buttons, Data)

				-- Hover เด้งขึ้นนิดนึง
				Creator.AddSignal(Btn.MouseEnter, function()
					Tween(Btn, Quick, { BackgroundTransparency = 0.75 })
				end)
				Creator.AddSignal(Btn.MouseLeave, function() Refresh(opt) end)

				Creator.AddSignal(Btn.MouseButton1Click, function(x, y)
					-- 💧 Ripple ตำแหน่งที่กด
					local relX = (x or Btn.AbsolutePosition.X + Btn.AbsoluteSize.X / 2) - Btn.AbsolutePosition.X
					local relY = (y or Btn.AbsolutePosition.Y + Btn.AbsoluteSize.Y / 2) - Btn.AbsolutePosition.Y
		...
		function SG:SetValue(v)
			self.Value = v
			Refresh()
			Library:SafeCallback(self.Callback, GetValue())
			Library:SafeCallback(self.Changed, GetValue())
		end
		function SG:OnChanged(Func) SG.Changed = Func Func(GetValue()) end
		function SG:Destroy() Frame.Frame:Destroy() if Idx then Library.Options[Idx] = nil end end

		if Idx then Library.Options[Idx] = SG end
		Refresh()
		return SG
	end
...
-- (Chip merged into SelectGroup above — see Style="chip")
...
-- 🔄 Backward-compatibility: RadioGroup และ Chip ถูกรวมเป็น SelectGroup แล้ว
-- โค้ดเก่าที่เรียก :AddRadioGroup() หรือ :AddChip() จะยังทำงานได้ปกติ
Elements.AddRadioGroup = function(self, Idx, Config)
	Config = Config or {}
	Config.Style = "radio"
	return self:AddSelectGroup(Idx, Config)
end

Elements.AddChip = function(self, Idx, Config)
	Config = Config or {}
	Config.Style = "chip"
	if Config.Multi == nil then Config.Multi = true end
	if Config.Items and not Config.Options then
		...
		SelectGroup = {
			Save = function(idx, object)
				if object.IsMulti then
					return { type = "SelectGroup", idx = idx, value = table.concat(object.Value, ",") }
				end
				return { type = "SelectGroup", idx = idx, value = object.Value }
			end,
			Load = function(idx, data)
				if SaveManager.Options[idx] then
					...
					if obj.IsMulti then
						local vals = {}
						for v in (tostring(data.value) or ""):gmatch("[^,]+") do
							table.insert(vals, v)
						end
						obj:SetValue(vals)
					else
