...
Components.Section = (function()
	...
	return function(Title, Parent, Icon)
		...
		-- [ จุดที่เพิ่ม ] Section:AddTab — มินิแท็บฝังอยู่ในการ์ด Section เดียว (คนละเรื่องกับ Tab/SubTab
		-- ของหน้าต่างที่มีอยู่แล้ว) ใช้ตอนอยากแบ่งกลุ่ม element ภายใน section เดียวกันโดยไม่ต้องสลับหน้า
		-- เช่น Section "Aimbot" มี tab ย่อย "Settings" / "Visuals" / "Advanced" อยู่ในการ์ดเดียวกัน
		-- เรียกได้หลายครั้งตามต้องการ (2 อันขึ้นไป) ครั้งแรกที่เรียกจะสร้างแถบแท็บ + ที่เก็บเนื้อหาให้อัตโนมัติ
		function Section:AddTab(TabTitle, TabIcon)
			Section.SectionTabs = Section.SectionTabs or {}
			Section.SharedGroups = Section.SharedGroups or {}
			Section.SelectedSectionTab = Section.SelectedSectionTab or 0
			Section.SectionTabCount = (Section.SectionTabCount or 0) + 1
			local Index = Section.SectionTabCount

			-- สร้างแถบแท็บ + ที่เก็บเนื้อหาแค่ครั้งแรกที่เรียก AddTab (ครั้งต่อๆ ไปใช้ของเดิมที่มีอยู่)
			if not Section.SectionTabStrip then
				Section.SectionTabStrip = New("Frame", {
				...
				Section.SectionTabContentHolder = New("Frame", {
					Size = UDim2.new(1, 0, 0, 0),
					BackgroundTransparency = 1,
					LayoutOrder = 2,
					Parent = Section.Container,
			...
			-- [ จุดที่เพิ่ม ] ฟังก์ชันคำนวณ layout กลาง สร้างครั้งเดียวต่อ Section ใช้ร่วมกันทั้ง AddTab
			-- และ AddSharedTab — คำนวณตำแหน่ง/ความสูงของ "บล็อกที่ควรโชว์ตอนนี้" เอง แบบตรงๆ ไม่พึ่ง
			-- UIListLayout จัดการให้ เพราะ block ที่ควรโชว์พร้อมกัน (tab ที่เลือก + shared group ที่ตรง)
			-- ไม่ได้เรียงกันตายตัว ต้องคำนวณสดทุกครั้งว่าใครโชว์บ้างแล้วค่อยวางซ้อนกันตามลำดับ
			if not Section._RecalcSectionTabLayout then
				function Section._RecalcSectionTabLayout()
					local y = 0
					local activeTab = Section.SectionTabs[Section.SelectedSectionTab]
					if activeTab then
						activeTab.Container.Position = UDim2.new(0, 0, 0, y)
						y = y + activeTab.Layout.AbsoluteContentSize.Y
					end
					for _, g in ipairs(Section.SharedGroups) do
						if table.find(g.Tabs, Section.SelectedSectionTab) then
					...
					Section.SectionTabContentHolder.Size = UDim2.new(1, 0, 0, y)
				end
			end
			...
			local TabButton = New("TextButton", {
				...
				Parent = Section.SectionTabStrip,
				ThemeTag = { BackgroundColor3 = "Element", TextColor3 = "SubText" },
			}, {
				NewCorner("PillCorner"),
				New("UIPadding", { PaddingLeft = UDim.new(0, 10), PaddingRight = UDim.new(0, 10) }),
				resolvedIcon and New("UIListLayout", { -- เผื่อมีไอคอน ใส่ layout ให้ icon+text เรียงแนวนอนได้
					FillDirection = Enum.FillDirection.Horizontal,
					Padding = UDim.new(0, 4),
					VerticalAlignment = Enum.VerticalAlignment.Center,
				}) or nil,
			})
			local _, SetTabBg     = Creator.SpringMotor(0.85, TabButton, "BackgroundTransparency", true, false, { frequency = 8 })
			...
			local _, SetTabStroke = Creator.SpringMotor(1, TabButtonStroke, "Transparency", true, false, { frequency = 8 })

			-- [ จุดที่แก้ ] AutomaticSize.Y ยังใช้ได้ปกติสำหรับ "วัดความสูงเนื้อหาของตัวเอง" (อ่านผ่าน
			-- TabContentLayout.AbsoluteContentSize.Y) แต่ตำแหน่ง Position จะถูก RecalcSectionTabLayout
			-- เป็นคนกำหนดแทน ไม่ปล่อยให้มันลอยอยู่ที่ (0,0) ซ้อนทับกับ container อื่นเฉยๆ แบบเดิม
			local TabContentLayout = New("UIListLayout", { Padding = UDim.new(0, 4) })
			local TabContainer = New("Frame", {
				Size = UDim2.new(1, 0, 0, 0),
				AutomaticSize = Enum.AutomaticSize.Y,
				BackgroundTransparency = 1,
				Visible = (Index == 1),
				Parent = Section.SectionTabContentHolder,
			}, {
				TabContentLayout,
			})

			Creator.AddSignal(TabContentLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
				if Section.SelectedSectionTab == Index then
					Section._RecalcSectionTabLayout()
				end
			end)

			Section.SectionTabs[Index] = { Button = TabButton, Container = TabContainer, SetBg = SetTabBg, SetStroke = SetTabStroke, Layout = TabContentLayout }

			local function UpdateSectionTabAppearance()
				for i, t in ipairs(Section.SectionTabs) do
					local active = (i == Section.SelectedSectionTab)
					t.Container.Visible = active
					t.SetBg(active and 0.75 or 0.85)
					t.SetStroke(active and 0 or 1)
				end
				Section._RecalcSectionTabLayout()
			end
			Creator.AddSignal(TabButton.MouseEnter, function()
				if Index ~= Section.SelectedSectionTab then SetTabBg(0.78) end
			end)
			Creator.AddSignal(TabButton.MouseLeave, function()
				if Index ~= Section.SelectedSectionTab then SetTabBg(0.85) end
			end)
			Creator.AddSignal(TabButton.MouseButton1Click, function()
				if Section.SelectedSectionTab == Index then return end
				Section.SelectedSectionTab = Index
				UpdateSectionTabAppearance()
			end)

			if Section.SelectedSectionTab == 0 then Section.SelectedSectionTab = Index end
			UpdateSectionTabAppearance()

			-- [ จุดสำคัญ ] SectionTab ผูก Elements metatable เหมือน Section/SubTab/Tab ทุกตัว เลยเรียก
			-- :AddToggle / :AddSlider / :AddDropdown ฯลฯ ต่อจากนี้ได้ปกติเป๊ะ (ไม่มี :AddSection ซ้อนต่อ
			-- เพราะ SectionTab เป็น container ใบสุดท้ายเหมือน Section เอง ไม่ควร nest ลึกไปกว่านี้)
			-- [ จุดที่เพิ่ม ] เก็บ _Index ไว้ในตัว handle ที่คืนกลับไปด้วย ให้ Section:AddSharedTab เอาไปอ้างอิง
			-- ได้ตรงๆ (ผู้ใช้ส่ง handle ที่ได้จาก AddTab เข้าไปได้เลย ไม่ต้องนับเลขแท็บเอง)
			local SectionTab = { Type = "SectionTab", Container = TabContainer, _Index = Index }
			setmetatable(SectionTab, Library.Elements)
			return SectionTab
		end

		-- [ จุดที่เพิ่ม ] Section:AddSharedTab — พื้นที่ element ที่โผล่ "ร่วมกัน" ในแท็บที่ระบุ (2 แท็บขึ้นไป)
		-- โดยไม่ต้องสร้าง element ซ้ำหลายชุด สร้าง element ไว้ในนี้ครั้งเดียว แล้วมันจะโผล่/หายไปอัตโนมัติ
		-- ตามว่าแท็บที่กำลังเลือกอยู่ตอนนี้อยู่ในลิสต์ที่ระบุไว้หรือเปล่า
		-- ใช้แบบนี้: local Shared = Section:AddSharedTab({ Tab1, Tab2 }) -- ส่ง handle ที่ได้จาก AddTab เข้าไป
		--            Shared:AddToggle("SomeKey", { Title = "..." })
		function Section:AddSharedTab(Tabs)
			assert(type(Tabs) == "table" and #Tabs >= 2, "Section:AddSharedTab - ต้องส่ง handle จาก AddTab อย่างน้อย 2 อันเป็น array")
			Section.SharedGroups = Section.SharedGroups or {}
			Section.SectionTabs  = Section.SectionTabs or {}
			assert(Section._RecalcSectionTabLayout, "Section:AddSharedTab - ต้องเรียก Section:AddTab อย่างน้อย 1 ครั้งก่อน")

			local TabIndices = {}
			for _, t in ipairs(Tabs) do
				assert(type(t) == "table" and t._Index, "Section:AddSharedTab - แต่ละตัวใน list ต้องเป็น handle ที่ได้จาก Section:AddTab เท่านั้น")
				table.insert(TabIndices, t._Index)
			end

			local GroupLayout = New("UIListLayout", { Padding = UDim.new(0, 4) })
			local GroupContainer = New("Frame", {
				Size = UDim2.new(1, 0, 0, 0),
				AutomaticSize = Enum.AutomaticSize.Y,
				BackgroundTransparency = 1,
			...
			table.insert(Section.SharedGroups, Group)

			-- เนื้อหาข้างในเปลี่ยนขนาด (เพิ่ม/ลบ element) ก็คำนวณใหม่ด้วย ถ้าตอนนี้กำลังโชว์อยู่
			Creator.AddSignal(GroupLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
				if table.find(TabIndices, Section.SelectedSectionTab) then
					Section._RecalcSectionTabLayout()
				end
			end)

			Section._RecalcSectionTabLayout() -- เผื่อแท็บที่เลือกอยู่ตอนนี้ตรงกับลิสต์อยู่แล้วตั้งแต่แรก

			local SharedTab = { Type = "SectionSharedTab", Container = GroupContainer }
			setmetatable(SharedTab, Library.Elements)
			return SharedTab
		end
