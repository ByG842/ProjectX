...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		-- [ จุดที่เพิ่ม ] updateChip เดี๋ยวนี้เคารพสถานะ Hovering/Pressing ด้วย
		-- เวลากด toggle แล้ว list อัปเดตทุกปุ่มพร้อมกัน ปุ่มที่เมาส์ค้างอยู่จะไม่โดนดีดกลับ
		-- ไปทับ hover state เพราะงั้นเวลาลากเมาส์ไล่กดหลายปุ่มติดกันจะไม่มีจังหวะกระตุก
		local function updateChip(item, data)
			...
			if not data.Pressed then
				if data.Hovering then
					data.SetBg(active and 0 or 0.78)
				else
		...
		for _, item in ipairs(Config.Items) do
			local chip = New("TextButton", {
				...
				AutomaticSize = Enum.AutomaticSize.X,
				BackgroundTransparency = 0.92,
				ClipsDescendants = false, -- ให้ Glow ด้านในเด้งล้นออกมาได้เล็กน้อยโดยไม่โดนตัด
			...
			-- [ จุดที่แก้ ] ผู้ใช้อยากให้ตัวปุ่มเด้งจริงๆ ไม่ใช่แค่สีกรอบ/glow เลยใส่ UIScale ที่ตัว chip เอง
			-- ผลข้างเคียง: เวลาเด้ง ปุ่มข้างๆ ใน UIListLayout จะขยับตามนิดหน่อย (เพราะ AbsoluteSize เปลี่ยนชั่วคราว)
			-- แต่ scale ปรับให้ไม่เยอะมาก (0.9 ~ 1.12) และสปริงคืนตัวไวมาก เลยแทบไม่รู้สึกว่าปุ่มข้างๆ ไหล
			local ChipScale = New("UIScale", { Scale = 1, Parent = chip })
			...
			local GlowScale = Glow:FindFirstChildOfClass("UIScale")

			-- [ จุดที่แก้ ] ใส่ IgnoreDialogCheck = true กันไว้เผื่อมี dialog อื่นเปิดค้างแล้วบัง BackgroundTransparency motor ไม่ให้ขยับ
			-- [ จุดที่ปรับ ] ขยับ frequency ของ Bg/Stroke ขึ้นเล็กน้อย (8 -> 10) ให้ตอบสนอง hover/press
			-- ไวขึ้นทันใจ แต่ยังเป็น critically-damped (dampingRatio เริ่มต้น = 1) เลยไม่มีการสั่นค้าง
			local _, SetBg     = Creator.SpringMotor(0.92, chip, "BackgroundTransparency", true, false, { frequency = 10 })
			local _, SetText   = Creator.SpringMotor(0.4,  chip, "TextTransparency", true, false, { frequency = 9 })
			local _, SetStroke = Creator.SpringMotor(0.85, stroke, "Transparency", true, false, { frequency = 10 })
			-- [ จุดที่ปรับ ] Glow ตอนนี้เป็น underdamped spring จริงๆ (dampingRatio 0.55) เลยเด้งเกินเป้าหมายนิดนึง
			-- ก่อนสปริงกลับมานิ่ง ให้ความรู้สึก "เด้ง" แบบมีน้ำหนัก ไม่ใช่แค่ไล่ค่าตรงๆ เหมือนเดิม
			local _, SetGlowScale = Creator.SpringMotor(1, GlowScale, "Scale", true, false, { frequency = 5, dampingRatio = 0.55 })
			local _, SetGlowT     = Creator.SpringMotor(1, Glow, "BackgroundTransparency", true, false, { frequency = 6 })
			-- [ จุดที่เพิ่ม ] สปริงคุมขนาดตัว chip เอง เป็น underdamped (dampingRatio 0.5) แรงและไวกว่า Glow
			-- นิดนึง เพื่อให้เห็นปุ่ม "บีบ-เด้ง" ชัดเจนจริง ไม่ใช่แค่สีเปลี่ยน
			local _, SetChipScale = Creator.SpringMotor(1, ChipScale, "Scale", true, false, { frequency = 7, dampingRatio = 0.5 })

			-- [ จุดที่เพิ่ม ] เก็บสถานะ Hovering/Pressed ต่อปุ่ม เพื่อให้ updateChip รู้ว่าตอนนี้เมาส์
			-- ค้างอยู่ปุ่มไหนบ้าง จะได้ไม่ทับ hover state เวลามีปุ่มอื่นถูกกดพร้อมกัน (เช่นโหมด Multi)
			local data = {
				Instance = chip, SetBg = SetBg, SetText = SetText, SetStroke = SetStroke,
				Active = false, Hovering = false, Pressed = false,
			}
			ChipBtns[item] = data
			updateChip(item, data)

			-- [ จุดที่เพิ่ม ] Hover feedback ที่ Chip ไม่เคยมีมาก่อน (ButtonGroup/QuickAction ข้างบนมีกันหมดแล้ว)
			-- ทำให้ Chip ตอบสนองต่อเมาส์ทันทีที่ลอยเข้า ไม่ต้องรอกดถึงจะเห็นความเคลื่อนไหว
			Creator.AddSignal(chip.MouseEnter, function()
				data.Hovering = true
				if not data.Pressed then
					SetBg(data.Active and 0 or 0.78)
				end
			end)
			Creator.AddSignal(chip.MouseButton1Down, function()
				data.Pressed = true
				SetBg(data.Active and 0.02 or 0.6)
				-- [ จุดที่เพิ่ม ] กดลง = บีบตัวปุ่มลงเล็กน้อยก่อน ให้ความรู้สึกเหมือนกดจริงๆ
				SetChipScale(0.9)
			end)
			Creator.AddSignal(chip.MouseButton1Up, function()
				data.Pressed = false
				if data.Hovering then
					SetBg(data.Active and 0 or 0.78)
				else
					SetBg(data.Active and 0.05 or 0.92)
				end
				-- [ จุดที่เพิ่ม ] ปล่อยมือ = เด้งเลยเป้าหมาย (1.12) แล้วค่อยสปริงกลับ 1
				-- เพราะ dampingRatio 0.5 เป็น underdamped อยู่แล้ว ไม่ต้องรอ delay ก็เด้งเองตามฟิสิกส์
				SetChipScale(1.12)
			...
			Creator.AddSignal(chip.MouseLeave, function()
				...
				-- [ จุดที่เพิ่ม ] ถ้าลากเมาส์ออกไปตอนกดค้าง ให้คืนขนาดกลับปกติด้วย ไม่งั้นปุ่มจะค้างเล็กอยู่
			...
			Creator.AddSignal(chip.MouseButton1Click, function()
				...
				-- [ จุดที่เพิ่ม ] Glow เด้งขึ้นเบาๆ แล้วจางหาย เป็นสปริง (ต่อเนื่อง ไม่กระตุก) และไม่กระทบขนาด chip เลย
				SetGlowScale(1.35)
				SetGlowT(0.55)
				-- [ จุดที่เพิ่ม ] toggle เสร็จให้ตัว chip เองเด้งย้ำอีกทีแรงกว่าตอนกดปกติ (1.18)
				-- แยกจาก MouseButton1Up ด้านบนที่เด้งแค่ 1.12 ไว้สำหรับกดเฉยๆ ไม่เลือก
				SetChipScale(1.18)
				task.delay(0.06, function()
					SetChipScale(1)
				end)
