...
ElementsTable.LiveLabel = (function()
	...
	-- ชุดสีสำเร็จรูป: แค่ "ทางลัด" ไม่บังคับใช้ — จะกำหนดสีเองแบบอิสระผ่าน Config.Color (Color3) ก็ได้เลย
	-- เช่น Config.Color = Color3.fromRGB(255, 120, 0) แทนการพิมพ์ Config.Type = "success" แบบเดิม
	local Presets = {
		Neutral = Color3.fromRGB(165, 168, 185),
	...
	function Element:New(Idx, Config)
		Config             = Config or {}
		Config.Text        = tostring(Config.Text or "")
		Config.Prefix      = Config.Prefix or ""
		Config.Suffix      = Config.Suffix or ""
		Config.Color       = Config.Color or (Config.Preset and Presets[Config.Preset]) or Creator.GetThemeProperty("Accent") or Presets.Neutral
		Config.MaxWidth    = Config.MaxWidth or 220
		Config.ExpandWidth = Config.ExpandWidth or 340
		Config.Bold        = Config.Bold or false
		Config.Pulse       = Config.Pulse and true or false
		Config.Animate     = (Config.Animate == nil) and true or Config.Animate
		Config.Copyable    = (Config.Copyable == nil) and true or Config.Copyable
		...
		local LL = {
			...
			Type    = "LiveLabel",
			_color  = Config.Color,
			_bg     = Config.BackgroundColor,
			_prefix = Config.Prefix,
			_suffix = Config.Suffix,
			_bold   = Config.Bold,
		}

		-- Config.Icon (ถ้ามี) จะโชว์เป็นไอคอนข้าง Title โดยอัตโนมัติ (ระบบเดิมของ Components.Element)
		...
		local function ResolveBg(color)
			return LL._bg or Darken(color, 0.55)
		end

		-- ── Pill: คลิกคัดลอกได้ / hover ขยายดูข้อความเต็ม / ClipsDescendants กันล้นกรอบ ──
		local Pill = New("TextButton", {
			AutomaticSize          = Enum.AutomaticSize.XY,
			AnchorPoint            = Vector2.new(1, 0.5),
			Position               = UDim2.new(1, -10, 0.5, 0),
			BackgroundTransparency = 0.7,
			BackgroundColor3       = ResolveBg(Config.Color),
			AutoButtonColor        = false,
			ClipsDescendants       = true,
			Text                   = "",
			...
			New("UIPadding", {
				PaddingLeft   = UDim.new(0, Config.Pulse and 16 or 8),
				PaddingRight  = UDim.new(0, 8),
				PaddingTop    = UDim.new(0, 4),
				PaddingBottom = UDim.new(0, 4),
			}),
			-- จำกัดความกว้างสูงสุดไม่ให้ทับ title (ปรับได้ผ่าน Config.MaxWidth)
			New("UISizeConstraint", {
				MaxSize = Vector2.new(Config.MaxWidth, math.huge),
			}),
		})
		local PillStroke = Pill:FindFirstChildOfClass("UIStroke")
		local PillPad    = Pill:FindFirstChildOfClass("UIPadding")
		local PillConstr = Pill:FindFirstChildOfClass("UISizeConstraint")

		-- จุดกระพริบบอกสถานะ "กำลังอัปเดตสด" (เปิดผ่าน Config.Pulse = true)
		...
		-- ── Text (prefix + value + suffix) เป็น RichText สีเดียวกับ Config.Color ──
		local ValueLabel = New("TextLabel", {
			FontFace               = GetStyleProperty(Config.Bold and "FontBold" or "FontMedium"),
			Text                   = "",
			TextColor3             = Color3.fromRGB(255, 255, 255),
			TextSize               = GetStyleProperty("TextSizeSm"),
			TextXAlignment         = Enum.TextXAlignment.Right,
			...
			AutomaticSize          = Enum.AutomaticSize.Y,
			BackgroundTransparency = 1,
			-- กว้างเต็ม Pill (padding จัดการระยะห่างแล้ว) — นี่คือกลไกที่กันไม่ให้ข้อความล้นกรอบ
			Size                   = UDim2.new(1, 0, 0, 0),
			Parent                 = Pill,
		})

		local FULL_TEXT_LIMIT = 42 -- ตัดข้อความให้สั้นลงตอนไม่ได้ hover กันล้นกรอบ, hover เพื่อดูเต็ม
		local expanded = false

		local function BuildMarkup()
			local hex    = ToHex(LL._color)
			local dimHex = ToHex(Darken(LL._color, 0.15))
			local val    = LL.Value
			if not expanded and #val > FULL_TEXT_LIMIT then
				val = val:sub(1, FULL_TEXT_LIMIT) .. "…"
			end
			local parts = {}
			if LL._prefix ~= "" then
				table.insert(parts, string.format('<font color="%s" transparency="0.35">%s</font>', dimHex, EscapeRich(LL._prefix)))
			end
			table.insert(parts, string.format('<font color="%s">%s</font>', hex, EscapeRich(val)))
			if LL._suffix ~= "" then
				table.insert(parts, string.format('<font color="%s" transparency="0.35">%s</font>', dimHex, EscapeRich(LL._suffix)))
			end
			return table.concat(parts, " ")
		end

		local function Refresh()
			ValueLabel.Text = BuildMarkup()
			Pill.Visible    = LL.Value ~= "" or LL._prefix ~= "" or LL._suffix ~= ""
		end

		-- Hover เพื่อขยาย Pill ดูข้อความเต็ม (เฉพาะตอนถูกตัดจริง ๆ)
		Creator.AddSignal(Pill.MouseEnter, function()
			if #LL.Value > FULL_TEXT_LIMIT then
				expanded = true
				if PillConstr then
					TweenService:Create(PillConstr, TweenInfo.new(0.18, Enum.EasingStyle.Quart), { MaxSize = Vector2.new(Config.ExpandWidth, math.huge) }):Play()
				end
				Refresh()
			end
		end)
		Creator.AddSignal(Pill.MouseLeave, function()
			if expanded then
				expanded = false
				if PillConstr then
					TweenService:Create(PillConstr, TweenInfo.new(0.18, Enum.EasingStyle.Quart), { MaxSize = Vector2.new(Config.MaxWidth, math.huge) }):Play()
				end
				Refresh()
			end
		end)

		-- คลิกที่ Pill เพื่อคัดลอกค่าปัจจุบัน (ปิดได้ผ่าน Config.Copyable = false)
		Creator.AddSignal(Pill.MouseButton1Click, function()
			Library:SafeCallback(Config.OnClick, LL.Value)
			if not Config.Copyable then return end
			if setclipboard then
				pcall(setclipboard, LL.Value)
				ValueLabel.Text = '<font color="#FFFFFF">คัดลอกแล้ว ✓</font>'
				task.delay(0.6, function()
					if ValueLabel and ValueLabel.Parent then Refresh() end
				end)
			end
		end)

		-- ── ปรับความสูง frame หลักตาม Pill (รองรับข้อความหลายบรรทัด) ──
		local BASE_H = 44
		local function syncFrameHeight()
			local pillH  = Pill.AbsoluteSize.Y
			local target = math.max(BASE_H, pillH + 16)
		...
		-- แฟลชสั้น ๆ ทุกครั้งที่ค่าถูกอัปเดต ให้รู้สึกว่าเป็น "live" (ปิดได้ผ่าน Config.Animate = false)
		local flashToken = 0
		local function Flash()
			if not Config.Animate then return end
			...
			task.delay(0.1, function()
				if myToken == flashToken and Pill and Pill.Parent then
					TweenService:Create(Pill, TweenInfo.new(0.35), { BackgroundTransparency = 0.7 }):Play()
				end
			end)
		end
		...
		LL.SetValue = LL.SetText

		function LL:SetColor(color)
			self._color = color
			TweenService:Create(PulseDot, TweenInfo.new(0.15), { BackgroundColor3 = color }):Play()
			if PillStroke then
				TweenService:Create(PillStroke, TweenInfo.new(0.15), { Color = color }):Play()
			end
			if not self._bg then
				TweenService:Create(Pill, TweenInfo.new(0.15), { BackgroundColor3 = Darken(color, 0.55) }):Play()
			end
			Refresh()
		end

		function LL:SetBackgroundColor(color)
			self._bg = color
			TweenService:Create(Pill, TweenInfo.new(0.15), { BackgroundColor3 = color or Darken(self._color, 0.55) }):Play()
		end

		function LL:SetPrefix(text)
			self._prefix = text or ""
			Refresh()
		end

		function LL:SetSuffix(text)
			self._suffix = text or ""
			Refresh()
		end

		function LL:SetBold(state)
			self._bold = state and true or false
			ValueLabel.FontFace = GetStyleProperty(self._bold and "FontBold" or "FontMedium")
		end

		function LL:SetPulse(state)
			state = state and true or false
			PulseDot.Visible = state
			if PillPad then
				PillPad.PaddingLeft = UDim.new(0, state and 16 or 8)
			end
			if state then StartPulse() else StopPulse() end
		end

		-- อัปเดตค่า + สี พร้อมกันในคำสั่งเดียว สะดวกกับสาย live-update เช่น:
		-- LiveLabel:Update(tostring(Coins), Coins > 1000 and LiveLabel.Presets.Success or LiveLabel.Presets.Warning)
		function LL:Update(text, color)
			if color then self:SetColor(color) end
			self:SetText(text)
		end

		function LL:OnChanged(Func)
			LL.Changed = Func
			Func(LL.Value)
		end
		...
		Refresh()
		if Config.Pulse then StartPulse() end
		...
		LiveLabel = {
			Save = function(idx, object)
				return {
					...
					idx    = idx,
					value  = object.Value,
					color  = object._color and object._color:ToHex(),
					bg     = object._bg and object._bg:ToHex(),
					prefix = object._prefix,
			...
			Load = function(idx, data)
				...
				if data.color then Opt:SetColor(Color3.fromHex(data.color)) end
				if data.bg then Opt:SetBackgroundColor(Color3.fromHex(data.bg)) end
				if data.prefix then Opt:SetPrefix(data.prefix) end
				if data.suffix then Opt:SetSuffix(data.suffix) end
				if data.bold ~= nil then Opt:SetBold(data.bold) end
				Opt:SetText(data.value or "")
			end,
		},
