...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		local function updateChip(item, btn)
			...
			-- อนิเมชันพื้นหลังและตัวหนังสือ (นุ่มนวลขึ้นด้วย Quint easing)
			TweenService:Create(btn, TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
				BackgroundTransparency = active and 0.1 or 0.9, -- กดแล้วสว่างชัดเจน, ไม่กดจะจางๆ
			...
			if stroke then
				TweenService:Create(stroke, TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
					Transparency = active and 0.1 or 0.85,
					Thickness = active and (GetStyleProperty("BorderThickness") + 0.5) or GetStyleProperty("BorderThickness"),
				}):Play()
			end
		end

		-- อนิเมชัน "เด้ง" ตอนกด ให้ความรู้สึกกดแล้วสวยและมีชีวิตชีวา
		local function bounceChip(btn)
			local scale = btn:FindFirstChildOfClass("UIScale")
			if not scale then return end
			TweenService:Create(scale, TweenInfo.new(0.09, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 0.9 }):Play()
		...
		for _, item in ipairs(Config.Items) do
			local chip = New("TextButton", {
				...
				New("UIPadding", { PaddingLeft = UDim.new(0, 14), PaddingRight = UDim.new(0, 14) }),
				New("UIScale", { Scale = 1 }), -- ใช้สำหรับอนิเมชันเด้งตอนกด
			})

			ChipBtns[item] = chip
			updateChip(item, chip)

			-- Hover เบาๆ สำหรับชิปที่ยังไม่ได้เลือก ให้รู้สึกตอบสนอง
			Creator.AddSignal(chip.MouseEnter, function()
				if table.find(CH.Value, item) then return end
				TweenService:Create(chip, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundTransparency = 0.78 }):Play()
			end)
			Creator.AddSignal(chip.MouseLeave, function()
				if table.find(CH.Value, item) then return end
				TweenService:Create(chip, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundTransparency = 0.9 }):Play()
			end)
