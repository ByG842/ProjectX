...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		-- เพิ่มระยะห่างแนวตั้งระหว่าง Title / Description ของ element นี้
		-- (แก้เฉพาะ Chip เพื่อไม่ให้ Title ชนกับ Description)
		local LabelLayout = CHFrame.LabelHolder:FindFirstChildOfClass("UIListLayout")
		if LabelLayout then
		...
		local ChipRow = New("Frame", {
			...
			New("UIListLayout", {
				FillDirection = Enum.FillDirection.Horizontal,
				Padding = UDim.new(0, 8), -- ระยะห่างระหว่างปุ่มให้ดูเป็นกลุ่มก้อน
				Wraps = true,
			}),
			-- เว้นช่องว่างด้านบนให้ห่างจาก Description ชัดเจน ไม่ติดกันอีกต่อไป
			New("UIPadding", { PaddingTop = UDim.new(0, 12), PaddingBottom = UDim.new(0, 4) }),
		})

		local ChipBtns = {}
		local ChipTweens = {} -- เก็บ tween ที่กำลังเล่นของแต่ละ object ไว้ยกเลิกของเก่าก่อนเล่นใหม่ ป้องกันอาการกระตุกเวลากดรัวๆ

		-- หมายเหตุสำคัญ: ChipRow ใช้ UIListLayout จัดเรียงปุ่ม ห้ามอนิเมท Size/UIScale ของตัวปุ่มเอง
		-- เพราะจะไป trigger ให้ layout คำนวณตำแหน่งใหม่ทุกเฟรม ทำให้ปุ่มข้างๆขยับ/แลคตามไปด้วย
		-- ฉะนั้นทุกอนิเมชันด้านล่างนี้แตะเฉพาะ Transparency / Color เท่านั้น ไม่แตะขนาดเด็ดขาด

		-- ค่าคงที่ TweenInfo กลาง ทำให้จังหวะอนิเมชันสม่ำเสมอทั้ง Chip
		local CHIP_COLOR_TWEEN = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
		local CHIP_HOVER_TWEEN = TweenInfo.new(0.15, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
		local CHIP_FLASH_IN_TWEEN = TweenInfo.new(0.06, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
		local CHIP_FLASH_OUT_TWEEN = TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
		local CHIP_ENTRANCE_TWEEN = TweenInfo.new(0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)

		-- เล่น tween พร้อมยกเลิกตัวเก่าของ object+key เดียวกันก่อนเสมอ ทำให้ transition ต่อเนื่องนุ่มนวลไม่กระตุก
		local function playChipTween(obj, key, info, goal)
			if not obj then return end
			ChipTweens[obj] = ChipTweens[obj] or {}
			local existing = ChipTweens[obj][key]
			if existing then existing:Cancel() end
			local tw = TweenService:Create(obj, info, goal)
			ChipTweens[obj][key] = tw
			tw:Play()
			return tw
		end

		-- ฟังก์ชันอัปเดตสถานะปุ่ม (Animation ของสี ขอบ และตัวหนังสือ) — ไม่แตะขนาดเลย จึงไม่กระทบ layout
		local function updateChip(item, btn)
			local active = table.find(CH.Value, item) ~= nil
			local stroke = btn:FindFirstChildOfClass("UIStroke")

			playChipTween(btn, "bg", CHIP_COLOR_TWEEN, {
				BackgroundTransparency = active and 0.08 or 0.9, -- กดแล้วสว่างชัดเจน, ไม่กดจะจางๆ
				TextTransparency = active and 0 or 0.35,
			})

			if stroke then
				playChipTween(stroke, "stroke", CHIP_COLOR_TWEEN, {
					Transparency = active and 0.05 or 0.85,
					Thickness = active and (GetStyleProperty("BorderThickness") + 0.5) or GetStyleProperty("BorderThickness"), -- ปรับหนาขึ้นนิดเดียว (Stroke ไม่กระทบ layout อยู่แล้ว)
				})
			end
		end

		-- อนิเมชัน "เด้ง" ตอนกด แบบแฟลชสว่างวาบแล้วจางลง (ใช้ overlay ซ้อนบนปุ่ม ไม่แตะขนาดปุ่มจริง จึงไม่ทำให้ปุ่มข้างๆขยับ)
		local function bounceChip(btn)
			local glow = btn:FindFirstChild("PressGlow")
			if not glow then return end
			playChipTween(glow, "flash", CHIP_FLASH_IN_TWEEN, { BackgroundTransparency = 0.55 })
			task.delay(0.06, function()
				if glow and glow.Parent then
					playChipTween(glow, "flash", CHIP_FLASH_OUT_TWEEN, { BackgroundTransparency = 1 })
				end
			end)
		end
		...
		for index, item in ipairs(Config.Items) do
			local chip = New("TextButton", {
				Text = tostring(item),
				FontFace = GetStyleProperty("FontMedium"), -- อ่านง่ายและคลีนขึ้น
				TextSize = GetStyleProperty("TextSizeSm"),
				Size = UDim2.fromOffset(0, 28), -- ความสูงปุ่มให้อักษรอยู่ตรงกลางสวยๆ
				AutomaticSize = Enum.AutomaticSize.X,
				BackgroundTransparency = 1, -- เริ่มจากจางสนิทแล้วค่อยๆ fade เข้ามาตอนโหลด (entrance)
				TextTransparency = 1,
				TextXAlignment = Enum.TextXAlignment.Center,
				TextYAlignment = Enum.TextYAlignment.Center,
				ClipsDescendants = true, -- กัน PressGlow overlay ล้นขอบมุมโค้ง
				Parent = ChipRow,
				ThemeTag = { BackgroundColor3 = "Accent", TextColor3 = "Text" },
			}, {
				-- ขอบมนนิดหน่อยแบบสี่เหลี่ยมโค้งมน ไม่แหลมและไม่เป็นเม็ดยาจนเกินไป
				NewCorner("SmallCorner"),
				New("UIStroke", { Transparency = 1, Thickness = GetStyleProperty("BorderThickness"), ApplyStrokeMode = Enum.ApplyStrokeMode.Border, ThemeTag = { Color = "Accent" } }),
				New("UIPadding", { PaddingLeft = UDim.new(0, 14), PaddingRight = UDim.new(0, 14) }),
				-- overlay สำหรับเอฟเฟกต์แฟลชตอนกด อยู่ "ข้างใน" ปุ่ม ไม่ใช่ลูกของ ChipRow จึงไม่กระทบ UIListLayout เลย
				New("Frame", {
					Name = "PressGlow",
					Size = UDim2.new(1, 0, 1, 0),
					BackgroundColor3 = Color3.fromRGB(255, 255, 255),
			...
			-- อนิเมชัน "ปรากฏตัว" ของแต่ละ chip แบบ fade-in ไล่ลำดับ (stagger) — ใช้ Transparency เท่านั้น ไม่แตะขนาด จึงไม่ทำให้แถวขยับตอนโหลด
			task.delay((index - 1) * 0.03, function()
				if chip and chip.Parent then
					updateChip(item, chip) -- เล่นซ้ำเพื่อ fade เข้าสู่ค่าจริงแบบ stagger (ตอนสร้างค่าเริ่มต้นคือโปร่งใสสนิท)
				end
			end)

			-- Hover เบาๆ สำหรับชิปที่ยังไม่ได้เลือก ให้รู้สึกตอบสนอง (เปลี่ยนแค่ความโปร่งใส ไม่แตะขนาด)
			Creator.AddSignal(chip.MouseEnter, function()
				if table.find(CH.Value, item) then return end
				playChipTween(chip, "bg", CHIP_HOVER_TWEEN, { BackgroundTransparency = 0.78 })
			end)
			Creator.AddSignal(chip.MouseLeave, function()
				if table.find(CH.Value, item) then return end
				playChipTween(chip, "bg", CHIP_HOVER_TWEEN, { BackgroundTransparency = 0.9 })
			end)
