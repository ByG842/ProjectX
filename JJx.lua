...
Elements.AddTab = function(self, TabTitle, TabIcon)
	...
	-- [ จุดที่แก้ ] ดีไซน์ใหม่ทั้งหมดตามที่ขอ — จากปุ่มลอยแยกกันแต่ละอัน (มีช่องว่างระหว่างปุ่ม, กรอบ/พื้นหลัง
	-- ของตัวเอง) เปลี่ยนเป็น "แท่งเดียว" รวมกัน (segmented bar) เหมือนที่ทำกับ ButtonGroup ไปก่อนหน้านี้ —
	-- มีพื้นหลัง/กรอบ/มุมมนรวมอันเดียว ClipsDescendants ตัดปุ่มหัว-ท้ายให้โค้งตามกรอบนอกอัตโนมัติ ปุ่มข้างใน
	-- ไม่มีพื้นหลัง/กรอบของตัวเองแล้ว มีแค่เส้นคั่นบางๆ ระหว่างปุ่มแทน ดูเป็นชิ้นเดียวกันมากขึ้น
	if not self.SectionTabStrip then
		...
		-- [ จุดที่เพิ่ม ] เส้นคั่นระหว่างแถบแท็บกับเนื้อหาด้านล่าง ตามที่ขอ ให้แยกส่วนกันชัดเจนขึ้น
	...
	local TabButton = Creator.New("TextButton", {
		...
		TabRow,
		-- [ จุดที่เพิ่ม ] เส้นคั่นบางๆ ระหว่างปุ่ม (ซ่อนไว้ให้ปุ่มสุดท้ายไม่มี ผ่าน Visible ด้านล่าง)
		Creator.New("Frame", {
			Name = "Divider",
	...
	local _, SetTabBg = Creator.SpringMotor(1, TabButton, "BackgroundTransparency", true, false, { frequency = 8 })
	...
	local TabDivider = TabButton:FindFirstChild("Divider")

	self.SectionTabs[Index] = { Button = TabButton, Container = TabContainer, SetBg = SetTabBg, Layout = TabContentLayout, Label = TabLabel, Icon = TabIconImg, Divider = TabDivider }

	-- [ จุดที่แก้ ] เส้นคั่นด้านขวาโชว์เฉพาะปุ่มที่ไม่ใช่ปุ่มสุดท้าย — ปุ่มใหม่ที่เพิ่งสร้างจะเป็นปุ่มสุดท้าย
	-- เสมอ (ซ่อนเส้นคั่นของตัวเอง) ส่วนปุ่มก่อนหน้าที่เคยเป็นปุ่มสุดท้าย ตอนนี้ไม่ใช่แล้ว โชว์เส้นคั่นให้
	if TabDivider then TabDivider.Visible = false end
	local prevTab = self.SectionTabs[Index - 1]
	if prevTab and prevTab.Divider then prevTab.Divider.Visible = true end
	...
	-- [ จุดที่แก้ ] ตัด SetStroke ออก เพราะปุ่มไม่มีกรอบของตัวเองแล้ว (แถบรวมด้านนอกดูแลกรอบให้หมดแล้ว)
	...
	Creator.AddSignal(TabButton.MouseEnter, function()
		if Index ~= self.SelectedSectionTab then SetTabBg(0.8) end
	end)
	Creator.AddSignal(TabButton.MouseLeave, function()
		if Index ~= self.SelectedSectionTab then SetTabBg(1) end
	end)
