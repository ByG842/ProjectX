...
Components.Window = (function()
	...
	return function(Config)
		...
		-- ── macOS-style layout ──────────────────────────────────────────────
		-- Sidebar ซ้าย (เข้มกว่า) | เส้นแบ่ง 1px | Content ขวา (สว่างกว่า)
		-- มุมโค้งเล็กน้อย (6px) เหมือน macOS Ventura/Sonoma
		-- ─────────────────────────────────────────────────────────────────────

		local SIDEBAR_W   = Window.TabWidth   -- ความกว้าง sidebar
		local DIVIDER_W   = 1                 -- เส้นแบ่ง
		local CORNER_R    = 6                 -- ความโค้งมุม (macOS เล็กน้อย)
		local TITLEBAR_H  = 52               -- ความสูง TitleBar

		-- Content top offset: ชื่อ Tab + padding
		local CONTENT_TOP = TITLEBAR_H + 38  -- 52 TitleBar + 38 tab label
		local CONTENT_PAD = 8                -- padding รอบเนื้อหา
		...
		-- ── macOS-style Sidebar Background (เข้มกว่าส่วน content เล็กน้อย) ─
		local SidebarBg = New("Frame", {
			...
			-- ปิดมุมขวาบน/ขวาล่างด้วย Frame ทับ (macOS sidebar ตัดแค่ซ้าย)
			New("Frame", {
				Size = UDim2.new(0, CORNER_R, 1, 0),
		...
		-- ── เส้นแบ่ง Sidebar | Content (macOS มี 1px hairline) ────────────
		local SidebarDivider = New("Frame", {
			Name = "SidebarDivider",
			Size = UDim2.new(0, DIVIDER_W, 1, -TITLEBAR_H),
		...
		Window.TabDisplay = New("TextLabel", {
			...
			TextXAlignment = "Left",
			TextYAlignment = "Center",
			Size = UDim2.new(1, -(SIDEBAR_W + DIVIDER_W + 24), 0, 22),
			Position = UDim2.fromOffset(SIDEBAR_W + DIVIDER_W + 16, TITLEBAR_H + 10),
			BackgroundTransparency = 1,
			ZIndex = 4,
			ThemeTag = { TextColor3 = "Text" },
		})
		...
		local contentLeft  = SIDEBAR_W + DIVIDER_W + CONTENT_PAD + 8
		local contentWidth = -(SIDEBAR_W + DIVIDER_W + CONTENT_PAD * 2 + 16)
		local contentTop   = CONTENT_TOP + 6

		Window.ContainerCanvas = New("Frame", {
			Name = "ContentCanvas",
			Size = UDim2.new(1, contentWidth, 1, -(contentTop + CONTENT_PAD)),
			Position = UDim2.fromOffset(contentLeft, contentTop),
			BackgroundTransparency = 1,
			ClipsDescendants = true,
			ZIndex = 2,
		}, {
		...
		-- ── macOS outer window border (UIStroke บาง) + UICorner ─────────────
		-- ใส่ไว้ใน Root ผ่าน rootChildren (จัดการ transparency ด้วย Acrylic)
		local MacWindowBorder = New("UIStroke", {
			ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
			Transparency = 0.72,
			Thickness = 1,
		...
		local MacWindowCorner = New("UICorner", {
			CornerRadius = UDim.new(0, CORNER_R),
		})

		-- ── Window outer shadow layer (macOS ทำด้วย Frame โปร่งใสขอบ) ──────
		local MacShadow = New("Frame", {
			Name = "MacShadow",
			...
			New("UICorner", { CornerRadius = UDim.new(0, CORNER_R + 2) }),
		})

		table.insert(rootChildren, MacShadow)
		...
		table.insert(rootChildren, MacWindowBorder)
		table.insert(rootChildren, MacWindowCorner)
		...
		if Config.UserInfo then
			...
			if Config.UserInfoTop then
				...
				local imageOffset = hasImage and (imageSize + 10 + topOffset) or topOffset
				-- macOS layout: TabFrame position ตาม TITLEBAR_H
				TabFrame.Position = UDim2.new(0, 0, 0, TITLEBAR_H)
				TabFrame.Size = UDim2.new(0, Window.TabWidth, 1, -(TITLEBAR_H + userInfoHeight))
				local searchOffset = hasImage and (imageSize + 10 + topOffset) or topOffset
		...
		-- macOS layout: content area เริ่มที่ TITLEBAR_H(52) + tab label(38) + 6 = 96
		Window.ContainerPosMotor = Flipper.SingleMotor.new(96)
		...
		local ContainerYValue = 96 -- macOS layout: TitleBar(52) + label(38) + 6
		...
		function Window:ToggleSearch()
			...
			local searchHeight = 28
			local totalOffset = (Window.ShowSearch and searchHeight or 0) + imageOffset
			-- macOS layout: TabFrame ไม่ขยับ position แต่ปรับ Size ให้ TabHolder
			TabFrame.Size = UDim2.new(0, Window.TabWidth, 1, -(TITLEBAR_H))
