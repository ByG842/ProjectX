...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		for _, item in ipairs(Config.Items) do
			...
			local _, SetBg     = Creator.SpringMotor(0.92, chip, "BackgroundTransparency", true)
			local _, SetText   = Creator.SpringMotor(0.4,  chip, "TextTransparency", true)
			local _, SetStroke = Creator.SpringMotor(0.85, stroke, "Transparency", true)
			-- สปริงคุม Glow ด้วย จะได้เด้งแบบสมูทต่อเนื่อง ไม่ใช่ tween ตายตัว
			local _, SetGlowScale = Creator.SpringMotor(1, GlowScale, "Scale", true)
			local _, SetGlowT     = Creator.SpringMotor(1, Glow, "BackgroundTransparency", true)
