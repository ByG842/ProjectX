...
Components.Notification = (function()
	...
	function Notification:New(Config)
		...
		NewNotification.Root = New("Frame", {
			...
			-- Border
			New("UIStroke", { Transparency = 0.5, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Color = accentColor, Thickness = GetStyleProperty("BorderThickness") }),
			TypeBar,
			IconLabel,
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- ให้ BackgroundImage (เช่นเท็กซ์เจอร์ธีม Minecraft) เป็น "พื้นหลังจริง" ได้
		-- โดยปิดชั้น tint/gradient/noise ของ AcrylicPaint ที่ปกติจะบังอยู่ด้านบนภาพ
		function Window:SetBackgroundTextureMode(enabled)
			local frame = Window.AcrylicPaint and Window.AcrylicPaint.Frame
			if not frame then return end

			local shadow     = frame:FindFirstChild("ShadowLayer")
			local gradient   = frame:FindFirstChild("GradientLayer")
			local noiseBase  = frame:FindFirstChild("NoiseBase")
			local noiseGrain = frame:FindFirstChild("NoiseGrain")

			if enabled then
				if shadow     then shadow.ImageTransparency = 1 end
				if gradient   then gradient.BackgroundTransparency = 1 end
				if noiseBase  then noiseBase.ImageTransparency = 1 end
				if noiseGrain then noiseGrain.ImageTransparency = 1 end
			else
				if shadow     then shadow.ImageTransparency = 0.7 end
				if gradient   then gradient.BackgroundTransparency = 0.4 end
				if noiseBase  then noiseBase.ImageTransparency = 0.98 end
				if noiseGrain then noiseGrain.ImageTransparency = Creator.GetThemeProperty("AcrylicNoise") or 0.9 end
			end

			Window.BackgroundTextureMode = enabled
		end
...
function Library:SetTheme(Value, Instant)
	...
	if Library.Window and (table.find(Library.Themes, Value) or Themes[Value]) then
		...
		-- ปิด/เปิดชั้น tint ของ AcrylicPaint — ธีม Minecraft ให้เท็กซ์เจอร์เป็นพื้นหลังจริง ไม่ใช่ซ้อนอยู่หลัง tint
		if Window and Window.SetBackgroundTextureMode then
			Window:SetBackgroundTextureMode(isMinecraft)
		end

		-- คง cap ความโปร่งใสของ overlay ไว้ ไม่ให้บังเท็กซ์เจอร์หมดถ้าอยู่ธีม Minecraft
		if isMinecraft then
			-- ใช้ค่าที่ให้ผลลัพธ์สวยที่สุด (เห็นเท็กซ์เจอร์ชัด พื้นหลังใส) เป็นค่าเริ่มต้นทันที
			Library:SetWindowTransparency(3)
			if InterfaceManager and InterfaceManager.Settings then
				InterfaceManager.Settings.WindowTransparency = 3
			end
			if InterfaceManager and InterfaceManager.WindowTransparencySlider then
				InterfaceManager.WindowTransparencySlider:SetValue(3)
			end
		elseif Library.WindowTransparencyValue then
			Library:SetWindowTransparency(Library.WindowTransparencyValue)
		end
...
function Library:SetWindowTransparency(Value)
	...
	-- ให้ตัวเท็กซ์เจอร์พื้นหลัง (BackgroundImage) โปร่งใสตามสไลเดอร์ไปด้วย
	-- ไม่ใช่แค่ overlay อย่างเดียว — Value 0 = เท็กซ์เจอร์จางเกือบหาย, Value 3 = เท็กซ์เจอร์ชัดสุด
	if Library.Window.BackgroundImage then
		local baseImgTransparency = Library.Window.BackgroundImageTransparency or 0.08
		local fadedImgTransparency = 0.95
		local imgTransparency = fadedImgTransparency - (Value / 3) * (fadedImgTransparency - baseImgTransparency)
		Library.Window.BackgroundImage.ImageTransparency = math.clamp(imgTransparency, 0, 1)
	end
	...
	if paint and paint.Frame then
		...
		if bg then
			-- ถ้าเปิดโหมด "เท็กซ์เจอร์คือพื้นหลัง" ไม่ต้องมี tint ทับเลย ให้ BackgroundImage โชว์เต็มๆ
			bg.BackgroundTransparency = Library.Window.BackgroundTextureMode and 1 or bgTransparency
		end
