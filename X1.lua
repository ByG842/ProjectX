...
function Library:CreateMinimizer(Config)
	...
	local function createButton(isDesktop)
		return New("TextButton", {
			...
			-- ถอด UIStroke (กรอบ) และ UICorner ออก เพราะเราจะโชว์แค่ไอคอนเพียวๆ
			New("ImageLabel", {
				Name = "Icon",
				Image = iconAsset,
				Size = UDim2.new(0.85, 0, 0.85, 0), -- ปรับให้ใหญ่ขึ้น (จากเดิม 0.52)
				Position = UDim2.new(0.5, 0, 0.5, 0),
				AnchorPoint = Vector2.new(0.5, 0.5),
				BackgroundTransparency = 1,
				ThemeTag = {
	...
	if isMobile then
		holder = New("Frame", {
			...
			Parent = parentGui,
			Size = Config.Size or UDim2.fromOffset(40, 40), -- ปรับ Base Size ให้ใหญ่ขึ้นนิดนึง
