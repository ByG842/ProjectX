...
ElementsTable.LiveLabel = (function()
	...
	-- [ จุดที่เพิ่ม ] ไอคอนต่อ type ให้จำแนกสถานะได้ไวกว่าดูแค่สี (ช่วยคนตาไม่ไวสี/แยกสีลำบากด้วย)
	-- ไม่ใส่ให้ "default" เพราะไม่มีความหมายเชิงสถานะอะไรเป็นพิเศษ ใส่ไปก็รกเปล่าๆ
	local TypeIcons = {
		info    = "info",
	...
	function Element:New(Idx, Config)
		...
		-- ── ไอคอนบอก type ── [ จุดที่เพิ่ม ] เว้นที่ไว้เสมอ (ซ่อนไว้ถ้าไม่มีไอคอนของ type นั้น เช่น default)
		-- กันไม่ให้ตำแหน่งตัวหนังสือขยับตอนสลับ type ไป-มาระหว่างมี/ไม่มีไอคอน
		local PillIcon = New("ImageLabel", {
			Image = TypeIcons[Config.Type] and Library:GetIcon(TypeIcons[Config.Type]) or "",
			Visible = TypeIcons[Config.Type] ~= nil,
			Size = UDim2.fromOffset(12, 12),
			Position = UDim2.new(0, 0, 0.5, 0),
			AnchorPoint = Vector2.new(0, 0.5),
			BackgroundTransparency = 1,
		...
		local ICON_GAP = 17 -- 12px ไอคอน + 5px ช่องว่าง เผื่อไว้เป็นค่าคงที่ใช้คำนวณตำแหน่งข้อความ

		-- ── Text ข้างใน Pill ─────────────────────────────────────
		-- [ จุดที่แก้ ] ขยับข้อความให้เริ่มหลังไอคอน (ถ้ามี) แล้วเปลี่ยนจากชิดขวาเป็นชิดซ้าย
		-- อ่านเป็นธรรมชาติกว่าเดิมตอนมีไอคอนนำหน้า (ไอคอน+ข้อความ ไหลจากซ้ายไปขวา)
		local ValueLabel = New("TextLabel", {
			...
			AutomaticSize          = Enum.AutomaticSize.Y,
			BackgroundTransparency = 1,
			-- กว้างเต็ม Pill ลบพื้นที่ไอคอน (padding จัดการระยะห่างซ้าย-ขวาอยู่แล้ว)
			Position               = UDim2.new(0, PillIcon.Visible and ICON_GAP or 0, 0, 0),
			Size                   = UDim2.new(1, PillIcon.Visible and -ICON_GAP or 0, 0, 0),
			Parent                 = Pill,
		})
		...
		function LL:SetText(text)
			text = text or ""
			-- [ จุดที่เพิ่ม ] ค่าที่ต่างจากเดิมจริงๆ ค่อย flash เบาๆ บอกว่า "อัปเดตแล้วนะ"
			-- เข้ากับความเป็น "Live" Label — เดิมสลับข้อความเงียบๆ ไม่รู้เลยว่าค่าเพิ่งเปลี่ยน
		...
		function LL:SetType(t)
			...
			-- [ จุดที่เพิ่ม ] อัปเดตไอคอนตาม type ใหม่ด้วย ซ่อน/โผล่ + ขยับข้อความตามว่ามีไอคอนไหม
			local iconKey = TypeIcons[self._type]
			PillIcon.Visible = iconKey ~= nil
			if iconKey then
				PillIcon.Image = Library:GetIcon(iconKey)
			end
			TweenService:Create(PillIcon, ti, { ImageColor3 = col }):Play()
			local gap = PillIcon.Visible and 17 or 0
			ValueLabel.Position = UDim2.new(0, gap, 0, 0)
			ValueLabel.Size     = UDim2.new(1, -gap, 0, 0)
		end
