...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		local hasIcon = (not fischbypass) and Icon and Icon ~= ""
		...
		Tab.Frame = New("TextButton", {
			...
			NewCorner("ElementCorner"),
			-- 📐 UIListLayout ใน Tab.Frame เอง: ทำให้ icon+label เรียงต่อกันแนวนอนได้ทั้งในโหมด Side (คงหน้าตาเดิม)
			-- และโหมด Up ที่ต้องใช้ AutomaticSize ตามความกว้างของเนื้อหาจริง (ไม่ fix กว้างเท่า sidebar)
			New("UIListLayout", {
			...
			New("UIPadding", {
				...
				PaddingLeft = UDim.new(0, hasIcon and 9 or 12),
		...
		-- 📐 จัดสไตล์ปุ่มแท็บให้ตรงกับโหมด layout ปัจจุบันของ Window ตั้งแต่สร้าง (รองรับ CreateTab หลังสลับโหมดด้วย)
		if Window.ApplyTabButtonLayout then
			Window.ApplyTabButtonLayout(Tab.Frame, Window.LayoutMode)
		end
...
Components.Window = (function()
	...
	return function(Config)
		local Window = {
			...
			LayoutMode = "Side", -- 📐 "Side" (แถบแท็บซ้าย, ค่าเริ่มต้น) หรือ "Up" (แถบแท็บบน แบบ Rayfield)
		}
		...
		-- ============================================================
		-- 📐 Layout Mode: "Side" (แถบแท็บซ้าย, ค่าเริ่มต้น) / "Up" (แถบแท็บบน แบบ Rayfield)
		-- สลับได้ตอน runtime ผ่าน Window:SetLayoutMode("Side" / "Up")
		-- ============================================================
		Window.SidebarCollapseButton = SidebarCollapseButton
		Window.SearchFrame = SearchFrame
		Window.Selector = Selector

		local UP_BAR_TOP = 60      -- ตำแหน่ง Y ของแถบแท็บแนวนอน (ใต้ TitleBar 52px)
		local UP_BAR_HEIGHT = 40   -- ความสูงแถบแท็บแนวนอน
		local UP_CONTENT_TOP = UP_BAR_TOP + UP_BAR_HEIGHT + 14

		-- จัดสไตล์ icon/label ภายในปุ่มแท็บแต่ละอัน ให้เข้ากับโหมดปัจจุบัน
		function Window.ApplyTabButtonLayout(TabBtn, Mode)
			if not TabBtn or not TabBtn.Parent then return end
			local Label = TabBtn:FindFirstChild("TabLabel")
			local Padding = TabBtn:FindFirstChild("TabContentPadding")
			if Mode == "Up" then
			...
			if Padding then
				-- เผื่อ padding ขวาให้กว้างขึ้นนิดในโหมด Up เพื่อความสวยงามตอนแท็บเรียงแนวนอน
				Padding.PaddingRight = UDim.new(0, Mode == "Up" and 14 or 12)
			end
		end

		local function RestyleAllTabButtons(Mode)
			if not Window.TabHolder then return end
			for _, TabBtn in ipairs(Window.TabHolder:GetChildren()) do
				if TabBtn:IsA("TextButton") then
					Window.ApplyTabButtonLayout(TabBtn, Mode)
				end
			end
		end

		function Window:SetLayoutMode(Mode)
			if Mode ~= "Up" then Mode = "Side" end
			Window.LayoutMode = Mode

			if Mode == "Up" then
				-- ย้ายแถบแท็บขึ้นไปอยู่ใต้ TitleBar เป็นแนวนอนเต็มความกว้าง
				local TabListLayout = Window.TabHolder:FindFirstChild("UIListLayout")
				if TabListLayout then
					TabListLayout.FillDirection = Enum.FillDirection.Horizontal
					TabListLayout.Padding = UDim.new(0, 6)
				end
				Window.TabHolder.ScrollingDirection = Enum.ScrollingDirection.X
				Window.TabHolder.Size = UDim2.new(1, -16, 1, 0)
				Window.TabHolder.Position = UDim2.new(0, 8, 0, 0)
				Window.TabHolder.CanvasSize = UDim2.fromScale(0, 0)

				TabFrame.Size = UDim2.new(1, -24, 0, UP_BAR_HEIGHT)
				TabFrame.Position = UDim2.new(0, 12, 0, UP_BAR_TOP)

				if ImageFrame then ImageFrame.Visible = false end
				if SearchFrame then SearchFrame.Visible = false end
				SidebarCollapseButton.Visible = false
				Selector.Visible = false

				Window.TabDisplay.Visible = false

				Window.ContainerCanvas.Size = UDim2.new(1, -24, 1, -(UP_CONTENT_TOP + 12))
				Window.ContainerCanvas.Position = UDim2.fromOffset(12, UP_CONTENT_TOP)
			else
				-- คืนค่าโหมด Side (sidebar ซ้าย) ตามเดิม
				local TabListLayout = Window.TabHolder:FindFirstChild("UIListLayout")
				if TabListLayout then
					TabListLayout.FillDirection = Enum.FillDirection.Vertical
					TabListLayout.Padding = UDim.new(0, 4)
				end
				Window.TabHolder.ScrollingDirection = Enum.ScrollingDirection.Y
				Window.TabHolder.Position = UDim2.new(0, 0, 0, Window.TabHolderTop or 45)
				Window.TabHolder.Size = UDim2.new(1, 0, 1, -((Window.TabHolderTop or 45) + 6))
				Window.TabHolder.CanvasSize = UDim2.fromScale(0, 0)

				local width = Window.SidebarCollapsed and 54 or (Window.TabWidthExpanded or Window.TabWidth)
				TabFrame.Size = UDim2.new(0, width, 1, Window.ShowSearch and -63 or -31)
				TabFrame.Position = UDim2.new(0, 12, 0, Window.ShowSearch and 54 or 19)

				if ImageFrame then ImageFrame.Visible = Window.HasImage and true or false end
				if SearchFrame then SearchFrame.Visible = Window.ShowSearch end
				SidebarCollapseButton.Visible = true
			...
			RestyleAllTabButtons(Mode)

			-- คำนวณ CanvasSize ของ TabHolder ใหม่ให้ตรงกับทิศทางปัจจุบัน (X ตอน Up / Y ตอน Side)
			task.defer(function()
				if not Window.TabHolder then return end
				local TabListLayout = Window.TabHolder:FindFirstChild("UIListLayout")
				if not TabListLayout then return end
				if Mode == "Up" then
					Window.TabHolder.CanvasSize = UDim2.new(0, TabListLayout.AbsoluteContentSize.X + 12, 0, 0)
				else
					local padding = Window.TabHolder:FindFirstChild("UIPadding")
					local paddingBottom = padding and padding.PaddingBottom.Offset or 0
					Window.TabHolder.CanvasSize = UDim2.new(0, 0, 0, TabListLayout.AbsoluteContentSize.Y + paddingBottom)
				end
			end)
		end
		...
		Creator.AddSignal(Window.TabHolder.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
			if Window.LayoutMode == "Up" then return end -- 📐 โหมด Up ใช้ CanvasSize แนวนอน คุมจาก Window:SetLayoutMode แทน
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		-- 📐 Layout Mode: Side (แถบแท็บซ้าย, ค่าเริ่มต้น) / Up (แถบแท็บบน แบบ Rayfield) — สลับได้ทันทีตอน runtime
		-- หมายเหตุ: ไม่บันทึกค่านี้ลง config ตามที่ตั้งใจไว้ เปิดโปรแกรมใหม่ทุกครั้งจะกลับไปเป็น Side เสมอ
		local LayoutModeDropdown = section:AddDropdown("InterfaceLayoutMode", {
			Title = "Layout Mode",
			Description = "สลับตำแหน่งแถบแท็บ: Side (ซ้าย) หรือ Up (บน)",
			Values = { "Side", "Up" },
			Default = "Side",
			Callback = function(Value)
				if Library.Window and Library.Window.SetLayoutMode then
					Library.Window:SetLayoutMode(Value)
				end
			end,
		})
		LayoutModeDropdown:SetValue("Side")
