...
Components.Element = function(Title, Desc, Parent, Hover, Options)
	...
	if Options.Icon then
		...
		Row.IconImage = New("ImageLabel", {
			Image = iconImage, Size = UDim2.fromOffset(15, 15),
			BackgroundTransparency = 1, LayoutOrder = 0,
			ThemeTag = { ImageColor3 = "SubText" },
		})
		Row.IconImage.Parent = Row.Header
	end
	...
	Row.Divider = New("Frame", {
		Size = UDim2.new(1, -(Tokens.Row.PadX * 2), 0, 1),
		Position = UDim2.new(0, Tokens.Row.PadX, 1, 0),
	...
	function Row:SetTitle(text)
		...
		if not hasTitle and Row.IconImage then
			...
			if not Row.DescIconImage then
				Row.DescIconImage = New("ImageLabel", {
					Image = Row.IconImage.Image, Size = UDim2.fromOffset(15, 15),
					BackgroundTransparency = 1, LayoutOrder = 0,
					ThemeTag = { ImageColor3 = "Text" },
				})
				Row.DescIconImage.Parent = Row.DescRow
			else
				Row.DescIconImage.Image = Row.IconImage.Image
				Row.DescIconImage.Parent = Row.DescRow
			end
			Row.DescLabel.Parent = Row.DescRow
			Row.DescLabel.LayoutOrder = 1
		else
			if Row.DescRow then Row.DescRow:Destroy(); Row.DescRow = nil; Row.DescIconImage = nil end
	...
	if Hover then
		local _, setTransparency = Creator.SpringMotor(1, Row.Frame, "BackgroundTransparency", false, true)
		Creator.AddSignal(Row.Frame.MouseEnter, function() setTransparency(0.92) end)
		Creator.AddSignal(Row.Frame.MouseLeave, function() setTransparency(1) end)
		Creator.AddSignal(Row.Frame.MouseButton1Down, function() setTransparency(0.85) end)
		Creator.AddSignal(Row.Frame.MouseButton1Up, function() setTransparency(0.92) end)
	end
...
Components.Section = function(Title, Parent, Icon)
	local Section = {}
	Section.Layout = New("UIListLayout", { SortOrder = Enum.SortOrder.LayoutOrder })
	...
	Creator.AddSignal(Section.Layout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
		...
		if not Section.Collapsed then
			Section.Root.Size = UDim2.new(1, 0, 0, Tokens.Group.HeaderHeight + contentH + Tokens.Group.Gap)
		end
	end)

	function Section:SetCollapsed(collapsed)
		Section.Collapsed = collapsed
		local target = collapsed and Tokens.Group.HeaderHeight or (Tokens.Group.HeaderHeight + lastContentH + Tokens.Group.Gap)
		local ti = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
		TweenService:Create(Section.Root, ti, { Size = UDim2.new(1, 0, 0, target) }):Play()
		TweenService:Create(Chevron, ti, { Rotation = collapsed and -90 or 0 }):Play()
		if collapsed then
			task.delay(0.22, function() if Section.Collapsed then Section.Container.Visible = false end end)
		else
