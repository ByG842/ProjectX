...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		-- 🎨 ทับสี Accent บางๆ ไว้ด้านหลัง เอาไว้เน้นแท็บที่ถูกเลือกอยู่ให้เห็นชัดกว่าเดิม
		-- (เดิมมีแค่ BackgroundTransparency ของ Tab.Frame เปลี่ยน 0.92 <-> 0.89 ต่างกันแทบไม่เห็น)
	...
	function TabModule:SelectTab(Tab)
		...
		for _, TabObject in next, TabModule.Tabs do
			...
			if TabObject.SetAccent then
				TabObject.SetAccent(1)
			end
		end
		TabModule.Tabs[Tab].SetTransparency(0.89)
		TabModule.Tabs[Tab].Selected = true
		if TabModule.Tabs[Tab].SetAccent then
			TabModule.Tabs[Tab].SetAccent(0.82) -- แท็บที่เลือกอยู่ ทับสี Accent จางๆ ให้เห็นชัดกว่าตัวอื่น
		end
