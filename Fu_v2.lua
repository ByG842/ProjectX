	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:AddSubTab(Title, Icon)
			...
			function SubTab:AddColumns()
				return Library._CreateColumns(SubTab.Container, SubTab.Container)
			end
		...
		function Tab:AddColumns()
			if self.SelectedSubTab > 0 and self.SubTabs[self.SelectedSubTab] then
				return self.SubTabs[self.SelectedSubTab]:AddColumns()
			end
			return Library._CreateColumns(Tab.Container, Tab.Container)
		end
...
for _, ElementComponent in pairs(ElementsTable) do
	Elements["Add" .. ElementComponent.__type] = function(self, Idx, Config)
		...
		if ElementComponent.NoIdx then
			result = ElementComponent:New(Idx)
		else
			result = ElementComponent:New(Idx, Config)
		end

		-- ── Compact mode: ลด padding element เมื่ออยู่ใน Column ──────────
		-- (_compact flag ถูก set โดย MakeColumn และ MakeCompactSection)
		if self._compact then
			local ef = result and result.Elements
			-- 1. ลด padding top/bottom ของ LabelHolder
			if ef and ef.LabelHolder then
				local pad = ef.LabelHolder:FindFirstChildOfClass("UIPadding")
				if pad then
					pad.PaddingTop    = UDim.new(0, 6)
					pad.PaddingBottom = UDim.new(0, 6)
				end
			end
			-- 2. ลด DescLabel font size ให้เล็กลง
			if ef and ef.DescLabel then
				ef.DescLabel.TextSize = 10
			end
			-- 3. ย่อ widget ที่ชิดขวา (toggle track, slider rail, dropdown inner)
			local frame = ef and ef.Frame
			if frame then
				for _, child in frame:GetChildren() do
					if (child:IsA("Frame") or child:IsA("TextButton")) then
						local ap = child.AnchorPoint
						-- widget ชิดขวา (AnchorPoint.X ≈ 1)
						if math.abs(ap.X - 1) < 0.01 then
							-- ลด UISizeConstraint MaxSize
							local cons = child:FindFirstChildOfClass("UISizeConstraint")
							if cons then
								-- slider rail maxsize → 110
								cons.MaxSize = Vector2.new(
									math.min(cons.MaxSize.X, 110),
									cons.MaxSize.Y
								)
							end
							-- toggle track: ย่อ pill 44→36, 24→19
							if child.Size.X.Offset >= 40 and child.Size.Y.Offset >= 20 and not cons then
								child.Size = UDim2.fromOffset(36, 19)
								-- ย้าย knob ให้พอดีกับ pill ใหม่
								for _, gchild in child:GetChildren() do
									if gchild:IsA("Frame") and gchild.Size.X.Offset >= 14 then
										gchild.Size = UDim2.fromOffset(12, 12)
									end
								end
							end
							-- dropdown inner: ย่อความกว้าง 160→110
							if child.Size.X.Offset >= 140 then
								child.Size = UDim2.fromOffset(110, child.Size.Y.Offset)
							end
						end
					end
				end
...
-- ════════════════════════════════════════════════════════════════════════
-- AddColumns — 2-column layout with compact elements & new mini-widgets
-- ════════════════════════════════════════════════════════════════════════
--
-- Usage:
--   local cols = Tab:AddColumns() or SubTab:AddColumns()
--   local L = cols:Left()   -- Column object (has AddSection + compact elements)
--   local R = cols:Right()
--
--   -- Section แล้ว element ปกติ (จะถูก compact อัตโนมัติ):
--   L:AddSection("Combat"):AddToggle("id", { Title = "Auto Atk", Default = false })
--
--   -- หรือใช้ compact element โดยตรงบน Column โดยไม่ต้องมี Section:
--   L:AddMiniToggle("id", { Title = "Fly", Default = false })
--   L:AddMiniSlider("id", { Title = "Speed", Min=16, Max=200, Default=16 })
--   L:AddMiniButton({ Title = "Reset", Callback = function() end })
--   L:AddMiniDropdown("id", { Title = "Mode", Values={"A","B"}, Default="A" })
--   L:AddMiniStat({ Label = "HP", Value = "100 / 100", Color = Color3.fromRGB(80,200,120) })
--   L:AddMiniLabel({ Text = "Active Features: 3", Icon = "zap" })

local function CreateColumnsInContainer(parentContainer, scrollContainer)
	local New = Creator.New

	-- ── compact sizing constants ─────────────────────────────────────────
	local COL_GAP   = 5    -- gap between left/right columns
	local SEC_H     = 20   -- compact section header height
	local SEC_PAD_H = 6    -- section card horizontal padding
	local SEC_PAD_V = 4    -- section card vertical padding
	local SEC_GAP   = 4    -- gap between section cards
	local EL_PAD    = 6    -- element top/bottom padding (was 13)

	-- ── compact Section builder ──────────────────────────────────────────
	local function MakeCompactSection(Title, ParentFrame, SectionIcon)
		local S = {}
		S.Layout = New("UIListLayout", { Padding = UDim.new(0, 3), SortOrder = Enum.SortOrder.LayoutOrder })
		S.Container = New("Frame", {
			Size = UDim2.new(1, -(SEC_PAD_H * 2), 0, 0),
			Position = UDim2.fromOffset(SEC_PAD_H, SEC_H + SEC_PAD_V),
			BackgroundTransparency = 1,
		}, { S.Layout })

		local iconImg = nil
		if SectionIcon and not fischbypass then
			local r = Library:GetIcon(SectionIcon)
			iconImg = (r and r ~= "") and r or nil
		end

		local CardFrame = New("Frame", {
			BackgroundTransparency = 0.94,
			Size = UDim2.new(1, 0, 0, SEC_H + SEC_PAD_V),
			LayoutOrder = 7, Parent = ParentFrame,
			ThemeTag = { BackgroundColor3 = "Element" },
		}, {
			New("UICorner", { CornerRadius = UDim.new(0, 7) }),
			New("UIStroke", { Transparency = 0.65, Thickness = 1,
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
				ThemeTag = { Color = "InElementBorder" } }),
			-- accent bar
			New("Frame", {
				...
				New("Frame", {
					Size = UDim2.new(0, 2, 0, 9), LayoutOrder = 1,
					ThemeTag = { BackgroundColor3 = "Accent" },
				}, { New("UICorner", { CornerRadius = UDim.new(1,0) }) }),
				iconImg and New("ImageLabel", {
					Image = iconImg, Size = UDim2.fromOffset(10,10),
					BackgroundTransparency = 1, LayoutOrder = 2,
					ThemeTag = { ImageColor3 = "SubText" },
				}) or nil,
				New("TextLabel", {
					Text = Title or "", RichText = true,
			...
			New("Frame", { -- divider
				Size = UDim2.new(1,-10,0,1), Position = UDim2.new(0,5,0,SEC_H),
				BackgroundTransparency = 0.78, ThemeTag = { BackgroundColor3 = "InElementBorder" },
			}),
			S.Container,
		})
		...
		S.Root = New("Frame", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1,0,0,SEC_H + SEC_PAD_V + SEC_GAP),
			LayoutOrder = 7, Parent = ParentFrame,
		}, {
			New("UIPadding", { PaddingTop = UDim.new(0, SEC_GAP) }),
			CardFrame,
		})

		Creator.AddSignal(S.Layout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
			local h = S.Layout.AbsoluteContentSize.Y
			S.Container.Size = UDim2.new(1, -(SEC_PAD_H * 2), 0, h)
			local cardH = SEC_H + SEC_PAD_V + h + SEC_PAD_V
			CardFrame.Size = UDim2.new(1, 0, 0, cardH)
			S.Root.Size = UDim2.new(1, 0, 0, cardH + SEC_GAP)
		end)

		return S
	end

	-- ── helper: shrink element padding after build ───────────────────────
	local function CompactifyElement(result)
		if result and result.Elements and result.Elements.LabelHolder then
			local pad = result.Elements.LabelHolder:FindFirstChildOfClass("UIPadding")
			if pad then
				pad.PaddingTop    = UDim.new(0, EL_PAD)
				pad.PaddingBottom = UDim.new(0, EL_PAD)
			end
		end
	end

	-- ── helper: find child by class and clamp size ───────────────────────
	local function ClampWidget(frame, maxW)
		-- ลด MaxSize constraint ของ widget ที่อยู่ชิดขวา
		if not frame then return end
		for _, child in frame:GetChildren() do
			if child:IsA("Frame") or child:IsA("TextButton") then
				local ap = child.AnchorPoint
				if math.abs(ap.X - 1) < 0.01 then
					-- widget ชิดขวา — ลด size constraint
					local cons = child:FindFirstChildOfClass("UISizeConstraint")
					if cons then
						cons.MaxSize = Vector2.new(maxW, cons.MaxSize.Y)
					else
						-- toggle track: ย่อขนาด pill
						if child.Size.X.Offset > maxW then
							child.Size = UDim2.fromOffset(maxW, child.Size.Y.Offset)
						end
					end
				end
			end
	...
	-- ════════════════════════════════════════════════════════════════════
	-- MINI-WIDGET BUILDERS (compact native widgets สำหรับ Column เท่านั้น)
	-- ════════════════════════════════════════════════════════════════════

	-- ── MiniToggle ───────────────────────────────────────────────────────
	local function BuildMiniToggle(Container, Idx, Config)
		local Toggle = {
			Value    = Config.Default or false,
			Type     = "MiniToggle",
			Callback = Config.Callback or function() end,
		...
		local Row = New("TextButton", {
			...
			Text = "", LayoutOrder = 7, Parent = Container,
			ThemeTag = { BackgroundColor3 = "Element" },
		}, {
			New("UICorner", { CornerRadius = UDim.new(0, 7) }),
			New("UIStroke", { Transparency = 0.65, Thickness = 1,
		...
		New("TextLabel", {
			Text = Config.Title or "", RichText = true,
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
			TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left,
			Position = UDim2.new(0, 9, 0.5, 0), AnchorPoint = Vector2.new(0, 0.5),
			Size = UDim2.new(1, -52, 1, 0), BackgroundTransparency = 1,
			TextTruncate = Enum.TextTruncate.AtEnd,
			ThemeTag = { TextColor3 = "Text" }, Parent = Row,
		})

		-- pill track (38×20 — เล็กกว่า toggle ปกติ 44×24)
		local Track = New("Frame", {
			Size = UDim2.fromOffset(38, 20),
			AnchorPoint = Vector2.new(1, 0.5),
			Position = UDim2.new(1, -8, 0.5, 0),
			BackgroundTransparency = 0.88, Parent = Row,
			ThemeTag = { BackgroundColor3 = "InElementBorder" },
		}, {
			New("UICorner", { CornerRadius = UDim.new(1, 0) }),
		})

		local TrackBorder = New("UIStroke", {
			Transparency = 0.4, Thickness = 1.2,
			ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
			ThemeTag = { Color = "ToggleSlider" }, Parent = Track,
		})

		local Fill = New("Frame", {
			Size = UDim2.fromScale(1,1), BackgroundTransparency = 1,
			Parent = Track, ThemeTag = { BackgroundColor3 = "Accent" },
		}, { New("UICorner", { CornerRadius = UDim.new(1,0) }) })
		...
		local Knob = New("Frame", {
			...
			BackgroundTransparency = 0.1, Parent = Track,
			ThemeTag = { BackgroundColor3 = "ToggleSlider" },
		}, {
			New("UICorner", { CornerRadius = UDim.new(1,0) }),
			New("UIAspectRatioConstraint", { AspectRatio = 1 }),
		...
		local function Apply(v)
			TweenService:Create(Knob, ti, { Position = UDim2.new(0, v and 21 or 3, 0.5, 0) }):Play()
			TweenService:Create(Fill, ti, { BackgroundTransparency = v and 0.35 or 1 }):Play()
			Creator.OverrideTag(TrackBorder, { Color = v and "Accent" or "ToggleSlider" })
			Creator.OverrideTag(Knob, { BackgroundColor3 = v and "ToggleToggled" or "ToggleSlider" })
		end
		...
		function Toggle:Destroy() Row:Destroy() if Idx then Library.Options[Idx] = nil end end

		Creator.AddSignal(Row.MouseButton1Click, function()
			Toggle:SetValue(not Toggle.Value)
		end)

		local M, SetT = Creator.SpringMotor(0.91, Row, "BackgroundTransparency")
		Creator.AddSignal(Row.MouseEnter, function() SetT(0.86) end)
		Creator.AddSignal(Row.MouseLeave, function() SetT(0.91) end)

		Apply(Toggle.Value)
		if Idx then Library.Options[Idx] = Toggle end
		return Toggle
	end

	-- ── MiniSlider ───────────────────────────────────────────────────────
	local function BuildMiniSlider(Container, Idx, Config)
		assert(Config.Min ~= nil and Config.Max ~= nil and Config.Default ~= nil, "MiniSlider - missing Min/Max/Default")
		...
		local Card = New("Frame", {
			...
			BackgroundTransparency = 0.91,
			LayoutOrder = 7, Parent = Container,
			ThemeTag = { BackgroundColor3 = "Element" },
		}, {
			New("UICorner", { CornerRadius = UDim.new(0, 7) }),
			New("UIStroke", { Transparency = 0.65, Thickness = 1,
		...
		local TitleLabel = New("TextLabel", {
			...
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
			TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left,
			Position = UDim2.new(0, 9, 0, 0), Size = UDim2.new(1, -18, 0, 22),
			BackgroundTransparency = 1,
			ThemeTag = { TextColor3 = "Text" }, Parent = Card,
		})
		...
		local ValLabel = New("TextLabel", {
			...
			BackgroundTransparency = 1,
			ThemeTag = { TextColor3 = "Accent" }, Parent = Card,
		})

		-- rail (full width, below title)
		...
		local Dot = New("Frame", {
			...
			New("UICorner", { CornerRadius = UDim.new(1,0) }),
			New("UIAspectRatioConstraint", { AspectRatio = 1 }),
			New("UIStroke", { Transparency = 0.55, Thickness = 1.5,
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
				ThemeTag = { Color = "AcrylicMain" } }),
		})

		local function SetVisual(v)
			local pct = (v - Slider.Min) / math.max(Slider.Max - Slider.Min, 1)
		...
		function Slider:SetValue(v)
			v = math.clamp(
				Config.Rounding == 0 and math.floor(v + 0.5) or
				math.floor(v / Config.Rounding + 0.5) * Config.Rounding,
		...
		function Slider:Destroy() Card:Destroy() if Idx then Library.Options[Idx] = nil end end

		-- drag on rail
		local function CalcValue(inputX)
			local abs = Rail.AbsolutePosition.X
			local w   = Rail.AbsoluteSize.X
			local pct = math.clamp((inputX - abs) / math.max(w, 1), 0, 1)
			return Slider.Min + pct * (Slider.Max - Slider.Min)
		end

		Creator.AddSignal(Rail.InputBegan, function(inp)
			if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
				Dragging = true
				Slider:SetValue(CalcValue(inp.Position.X))
			end
		end)
	...
	-- ── MiniButton ───────────────────────────────────────────────────────
	local function BuildMiniButton(Container, Config)
		local Row = New("TextButton", {
			Size = UDim2.new(1, 0, 0, 28),
			BackgroundTransparency = 0.91,
			Text = "", LayoutOrder = 7, Parent = Container,
		...
		local Strip = New("Frame", {
			Size = UDim2.new(0, 2, 0, 12), BackgroundTransparency = 1,
			AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, 0, 0.5, 0),
			Parent = Row, ThemeTag = { BackgroundColor3 = "Accent" },
		}, { New("UICorner", { CornerRadius = UDim.new(1,0) }) })
		...
		Creator.AddSignal(Row.MouseEnter, function()
			TweenService:Create(Strip, TweenInfo.new(0.15), { BackgroundTransparency = 0.2, Size = UDim2.new(0,3,0,15) }):Play()
			SetT(0.84)
		end)
		Creator.AddSignal(Row.MouseLeave, function()
			TweenService:Create(Strip, TweenInfo.new(0.15), { BackgroundTransparency = 1, Size = UDim2.new(0,2,0,12) }):Play()
			SetT(0.91)
		end)
		Creator.AddSignal(Row.MouseButton1Down, function() SetT(0.82) end)
		Creator.AddSignal(Row.MouseButton1Up,   function() SetT(0.88) end)
		Creator.AddSignal(Row.MouseButton1Click, function()
			Library:SafeCallback(Config.Callback)
		end)

		return { Type = "MiniButton", Frame = Row }
	end

	-- ── MiniDropdown ─────────────────────────────────────────────────────
	local function BuildMiniDropdown(Container, Idx, Config)
		local DD = {
		...
		local ValLabel = New("TextLabel", {
			Text = tostring(DD.Value or "—"), RichText = true,
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json"),
			TextSize = 10, TextXAlignment = Enum.TextXAlignment.Right,
			AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -22, 0.5, 0),
			Size = UDim2.new(0.45, 0, 1, 0), BackgroundTransparency = 1,
			TextTruncate = Enum.TextTruncate.AtEnd,
			ThemeTag = { TextColor3 = "Accent" }, Parent = Row,
		})

		local ChevronIco = New("ImageLabel", {
			Image = "rbxassetid://10709790948",
			Size = UDim2.fromOffset(12, 12),
			AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -6, 0.5, 0),
			BackgroundTransparency = 1, Rotation = 180, Parent = Row,
			ThemeTag = { ImageColor3 = "SubText" },
		})

		-- dropdown list (shows inline below row)
		...
		New("UIPadding", { PaddingLeft = UDim.new(0,4), PaddingRight = UDim.new(0,4),
			PaddingTop = UDim.new(0,4), PaddingBottom = UDim.new(0,4), Parent = ListHolder })

		-- build option rows
		for _, val in ipairs(DD.Values) do
			local OptBtn = New("TextButton", {
				Size = UDim2.new(1, 0, 0, 22),
				BackgroundTransparency = 0.92, Text = "",
				Parent = ListHolder,
				ThemeTag = { BackgroundColor3 = "Element" },
			}, {
				New("UICorner", { CornerRadius = UDim.new(0, 5) }),
				New("TextLabel", {
					Text = tostring(val), RichText = true,
					FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json"),
					TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left,
			...
			Creator.AddSignal(OptBtn.MouseButton1Click, function()
				DD.Value = val
				ValLabel.Text = tostring(val)
				DD:Close()
				Library:SafeCallback(DD.Callback, val)
				Library:SafeCallback(DD.Changed, val)
			end)

			local M2, SetT2 = Creator.SpringMotor(0.92, OptBtn, "BackgroundTransparency")
			Creator.AddSignal(OptBtn.MouseEnter, function() SetT2(0.82) end)
			Creator.AddSignal(OptBtn.MouseLeave, function() SetT2(0.92) end)
		end

		local function GetListH()
			return ListLayout.AbsoluteContentSize.Y + 8
		end

		function DD:Open()
			if DD.Opened then return end
			DD.Opened = true
			ListHolder.Visible = true
			TweenService:Create(ListHolder, TweenInfo.new(0.18, Enum.EasingStyle.Quint),
				{ Size = UDim2.new(1, 0, 0, GetListH()) }):Play()
			TweenService:Create(ChevronIco, TweenInfo.new(0.18), { Rotation = 0 }):Play()
		end

		function DD:Close()
			if not DD.Opened then return end
			DD.Opened = false
			TweenService:Create(ListHolder, TweenInfo.new(0.15, Enum.EasingStyle.Quint),
				{ Size = UDim2.new(1, 0, 0, 0) }):Play()
			TweenService:Create(ChevronIco, TweenInfo.new(0.15), { Rotation = 180 }):Play()
			task.delay(0.16, function() if not DD.Opened then ListHolder.Visible = false end end)
		end

		Creator.AddSignal(Row.MouseButton1Click, function()
			if DD.Opened then DD:Close() else DD:Open() end
		end)
		...
		function DD:SetValue(v) DD.Value = v ValLabel.Text = tostring(v) Library:SafeCallback(DD.Callback, v) end
		function DD:OnChanged(f) DD.Changed = f f(DD.Value) end
		function DD:Destroy() Row:Destroy() ListHolder:Destroy() if Idx then Library.Options[Idx] = nil end end

		if Idx then Library.Options[Idx] = DD end
		return DD
	end

	-- ── MiniStat: แสดง label + value คู่กัน ─────────────────────────────
	local function BuildMiniStat(Container, Config)
		local clr = Config.Color or Color3.fromRGB(96, 205, 255)
		local Row = New("Frame", {
		...
		New("Frame", {
			...
			BackgroundColor3 = clr, Parent = Row,
		}, { New("UICorner", { CornerRadius = UDim.new(1,0) }) })
		...
		function Stat:SetValue(v)
			ValLabel.Text = tostring(v)
		end
		function Stat:SetColor(c)
			ValLabel.TextColor3 = c
		end
		function Stat:Destroy() Row:Destroy() end
		return Stat
	end

	-- ── MiniLabel: ป้ายข้อความ + icon ──────────────────────────────────
	local function BuildMiniLabel(Container, Config)
		local iconImg = nil
		if Config.Icon and not fischbypass then
			local r = Library:GetIcon(Config.Icon)
			iconImg = (r and r ~= "") and r or nil
		end
		...
		if iconImg then
			New("ImageLabel", {
				Image = iconImg, Size = UDim2.fromOffset(11, 11),
				BackgroundTransparency = 1, LayoutOrder = 1,
				ThemeTag = { ImageColor3 = "Accent" }, Parent = Row,
			})
		end

		local Lbl = New("TextLabel", {
			Text = Config.Text or "", RichText = true,
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json"),
			TextSize = 10, TextXAlignment = Enum.TextXAlignment.Left,
			Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1,
			LayoutOrder = 2, ThemeTag = { TextColor3 = "SubText" }, Parent = Row,
		})

		local Label = { Type = "MiniLabel", Frame = Row }
		function Label:SetText(t) Lbl.Text = t end
		function Label:Destroy() Row:Destroy() end
		return Label
	end
	...
	local ColumnsRoot = New("Frame", {
		Size = UDim2.new(1, 0, 0, 0),
		AutomaticSize = Enum.AutomaticSize.Y,
		BackgroundTransparency = 1,
		LayoutOrder = 7, Parent = parentContainer,
	}, {
		New("UIListLayout", {
	...
	local function MakeColumn(layoutOrder)
		local ColFrame = New("Frame", {
			Size = UDim2.new(0.5, -(math.ceil(COL_GAP / 2) + 1), 0, 0),
			AutomaticSize = Enum.AutomaticSize.Y,
			BackgroundTransparency = 1,
			LayoutOrder = layoutOrder, Parent = ColumnsRoot,
		}, {
			New("UIListLayout", {
				Padding = UDim.new(0, 3),
				SortOrder = Enum.SortOrder.LayoutOrder,
		...
		local Col = {
			...
			_compact    = true,       -- flag สำหรับ Elements loop
		}

		-- AddSection → compact section
		function Col:AddSection(SectionTitle, SectionIcon)
			local S = MakeCompactSection(SectionTitle, ColFrame, SectionIcon)
			local SecObj = {
				Type        = "Section",
			...
			setmetatable(SecObj, Library.Elements)
			return SecObj
		end

		-- ── mini-widget shorthand methods ────────────────────────────────
		function Col:AddMiniToggle(Idx, Cfg)
			return BuildMiniToggle(ColFrame, Idx, Cfg)
		end
		function Col:AddMiniSlider(Idx, Cfg)
			return BuildMiniSlider(ColFrame, Idx, Cfg)
		end
		function Col:AddMiniButton(Cfg)
			return BuildMiniButton(ColFrame, Cfg)
		end
		function Col:AddMiniDropdown(Idx, Cfg)
			return BuildMiniDropdown(ColFrame, Idx, Cfg)
		end
		function Col:AddMiniStat(Cfg)
			return BuildMiniStat(ColFrame, Cfg)
		end
		function Col:AddMiniLabel(Cfg)
			return BuildMiniLabel(ColFrame, Cfg)
		end

		setmetatable(Col, Library.Elements)
		return Col
	end
	...
	function Cols:Left()    return self._left  end
	function Cols:Right()   return self._right end
	function Cols:Destroy() ColumnsRoot:Destroy() end

	return Cols
end

-- Inject AddColumns into Elements so Sections can also use it
Elements.AddColumns = function(self)
	return CreateColumnsInContainer(self.Container, self.ScrollFrame)
end

-- Patch Tab and SubTab via monkey-patching after Window:AddTab is called.
-- We do this by extending TabModule:New and SubTab creation with AddColumns.
-- The cleanest way: store the helper so Tab/SubTab objects can call it.
Library._CreateColumns = CreateColumnsInContainer
