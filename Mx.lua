...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		Tab.Frame = New("TextButton", {
			Size = UDim2.new(1, -16, 0, 40), -- 🏷️ spans the rail width now so the label has room next to the icon
			...
			NewCorner("ElementCorner"),
			New("TextLabel", {
				Visible = true, -- 🏷️ label restored: rail now shows the tab's name (e.g. "Farm"), not just an icon
				AnchorPoint = Vector2.new(0, 0.5),
				Position = not fischbypass and Icon and UDim2.new(0, 38, 0.5, 0) or UDim2.new(0, 12, 0.5, 0),
				Text = Title,
				RichText = true,
				...
				TextXAlignment = "Left",
				TextYAlignment = "Center",
				Size = not fischbypass and Icon and UDim2.new(1, -46, 1, 0) or UDim2.new(1, -20, 1, 0),
				BackgroundTransparency = 1,
				ZIndex = 11,
				ThemeTag = {
					TextColor3 = "Text",
			...
			New("ImageLabel", {
				AnchorPoint = Vector2.new(0, 0.5), -- 🏷️ icon back to the left edge, label sits to its right
		...
		-- 🧭 Shared floating tooltip — the button's label is hidden (icon rail), so its
		-- name shows on hover instead, just like Gzz's rail tooltip.
		do
			local Tooltip
			local function GetTooltip()
				...
				Tooltip = New("Frame", {
					...
					New("UIStroke", { Transparency = 0.5, Thickness = GetStyleProperty("BorderThickness"), ThemeTag = { Color = "ElementBorder" } }),
			...
			Creator.AddSignal(Tab.Frame.MouseEnter, function()
				...
				tip.Label.Text = Title
				local pos = Tab.Frame.AbsolutePosition
				local size = Tab.Frame.AbsoluteSize
				local rootPos = Window.Root.AbsolutePosition
				tip.Position = UDim2.fromOffset(pos.X - rootPos.X + size.X + 8, pos.Y - rootPos.Y + size.Y / 2 - 14)
				tip.Visible = true
			end)
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		-- MAIN TITLEBAR FRAME — height 44px (slimmer header)
...
Components.Window = (function()
	...
	return function(Config)
		...
		Window.TabWidth = Config.TabWidth or 150 -- 🏷️ back to a labeled rail width (icon + name), matches Gzz's sidebar
		local Selector = New("Frame", {
			Size = UDim2.fromOffset(3, 22),
			BackgroundColor3 = Color3.fromRGB(76, 194, 255),
			Position = UDim2.fromOffset(0, (Window.TabHolderTop or 8) + 20),
			AnchorPoint = Vector2.new(0, 0.5),
			ZIndex = 1,
			ThemeTag = {
				BackgroundColor3 = "Accent",
		...
		local imageSize = Window.TabWidth - 16
		local topOffset = 6
		...
		local TabHolderListLayout = New("UIListLayout", {
			Padding = UDim.new(0, 6),
			HorizontalAlignment = Enum.HorizontalAlignment.Center,
			VerticalAlignment = Enum.VerticalAlignment.Top,
		})
		...
		-- 🧭 FIX: when there are only a handful of tabs, a top-anchored list leaves one big
		-- dead gap under the last icon. Center the icons in the rail when they all fit,
		-- and only fall back to top-anchored (scrollable) once they actually overflow it.
		local function UpdateTabHolderAlignment()
			local contentHeight = TabHolderListLayout.AbsoluteContentSize.Y
			local holderHeight = Window.TabHolder.AbsoluteSize.Y
			if holderHeight > 0 and contentHeight < holderHeight then
				TabHolderListLayout.VerticalAlignment = Enum.VerticalAlignment.Center
			else
				TabHolderListLayout.VerticalAlignment = Enum.VerticalAlignment.Top
			end
		end
		Creator.AddSignal(TabHolderListLayout:GetPropertyChangedSignal("AbsoluteContentSize"), UpdateTabHolderAlignment)
		Creator.AddSignal(Window.TabHolder:GetPropertyChangedSignal("AbsoluteSize"), UpdateTabHolderAlignment)
		task.defer(UpdateTabHolderAlignment)

		-- 🔧 recompute + re-apply TabHolder's top offset/size from the window's current
		-- live state (image, user-info). Pass an explicit top to force a value,
		-- or call with no args to have it recompute from Window.HasImage/UserInfo*.
		-- (Search bar no longer lives in the sidebar, so it no longer affects this.)
		function Window:UpdateTabHolderLayout(explicitTop)
			...
			if not newTop then
				if userInfoHeight and Window.UserInfoTop then
					newTop = userInfoHeight + 6 + imageOffset
				elseif hasImage then
					newTop = imageSize + 10 + topOffset
				else
					newTop = 8
				end
			end
			...
			if userInfoHeight and Window.UserInfoTop then
				...
				Window.TabHolder.Size = UDim2.new(1, 0, 1, -newTop)
			end
		end
		-- 🔍 Search bar — same size/position formula as Gzz's content-header search:
		-- 200x30, anchored to the right edge of the content header, vertically centered.
		local SearchStroke = New("UIStroke", {
			Transparency = 0.15, -- 🔍 was 0.5 — border was nearly invisible, now clearly outlines the box
			Thickness = GetStyleProperty("BorderThickness"),
			ThemeTag = { Color = "ElementBorder" },
		})
		local SearchFrame = New("Frame", {
			Size = UDim2.new(0, 260, 0, 36), -- 🔍 was 220x32 — bigger footprint so it doesn't get lost in the header
			AnchorPoint = Vector2.new(1, 0.5),
			Position = UDim2.new(1, -18, 0.5, 0),
			BackgroundTransparency = 0.05, -- 🔍 was 0.25 — much more opaque so it stands out against the acrylic background
			ZIndex = 10,
		...
		local SearchInput = New("TextBox", {
			...
			PlaceholderColor3 = Color3.fromRGB(150, 150, 150), -- 🔍 lighter so the placeholder reads clearly at a glance
			ClearTextOnFocus = false,
			Text = "",
			Parent = SearchFrame,
			ThemeTag = {
		...
		local SearchIcon = New("ImageLabel", {
			...
			Parent = SearchFrame,
			ImageTransparency = 0, -- 🔍 fully opaque icon (was faded to 0.1)
			ThemeTag = {
				ImageColor3 = "SubText",
			},
		})
		...
		-- 🔍 Highlight the border with the accent color while focused so the box is
		-- unmistakable when the user is actually typing in it.
		...
		Creator.AddSignal(SearchInput.FocusLost, function()
			...
			Creator.OverrideTag(SearchStroke, { Color = "ElementBorder" })
		end)
		...
		-- 📐 Layout constants — same names/values as Gzz's Tokens.Rail.Width /
		-- Tokens.Header.Height / Tokens.ContentHeader.Height / divider width.
		local RAIL_W, HEADER_H, CH_H, DIV = Window.TabWidth, 44, 46, 1

		-- 🧭 Icon rail — flush against the left edge, directly under the title bar
		-- (matches Gzz's Window.Rail). The old 12px inset + « / » collapse toggle
		-- are gone: this column is permanently icon-only.
		local TabFrame = New("Frame", {
			Size = UDim2.new(0, RAIL_W, 1, -(HEADER_H + DIV)),
			Position = UDim2.fromOffset(0, HEADER_H + DIV),
			BackgroundTransparency = 1,
			ClipsDescendants = true,
		}, {
			ImageFrame,
		...
		Window.Rail = TabFrame

		local RailDivider = New("Frame", {
			Size = UDim2.new(0, 1, 1, -(HEADER_H + DIV)),
			Position = UDim2.fromOffset(RAIL_W, HEADER_H + DIV),
		...
		-- 🏷️ Content header — current tab title (left) + search bar (right), the
		-- same row, same heights as Gzz's Window.ContentHeader.
		...
		Window.ContentHeader = New("Frame", {
			Size = UDim2.new(1, -(RAIL_W + DIV), 0, CH_H),
			Position = UDim2.fromOffset(RAIL_W + DIV, HEADER_H + DIV),
			BackgroundTransparency = 1,
		}, {
			Window.TabDisplay,
			SearchFrame,
		...
		local ContentHeaderDivider = New("Frame", {
			Size = UDim2.new(1, -(RAIL_W + DIV), 0, 1),
			Position = UDim2.fromOffset(RAIL_W + DIV, HEADER_H + DIV + CH_H),
			BackgroundTransparency = 0.3,
			ThemeTag = { BackgroundColor3 = "ElementBorder" },
		})
		...
		-- (Gzz note carried over: Window.ContainerAnim + its Back/Pos/X motors were
		-- wired up but never had :setGoal() called anywhere — genuinely dead code —
		-- so they're dropped here too. ContainerHolder is all this ever needed.)
		local topOffset = HEADER_H + DIV + CH_H + DIV
		Window.ContainerCanvas = New("Frame", {
			Size = UDim2.new(1, -(RAIL_W + DIV), 1, -topOffset),
			Position = UDim2.fromOffset(RAIL_W + DIV, topOffset),
			BackgroundTransparency = 1,
			ClipsDescendants = true,
		}, {
		...
		if Config.UserInfo then
			...
			local Avatar = New("ImageLabel", {
				...
				NewCorner("PillCorner"),
				New("UIStroke", { Transparency = 0.6, Thickness = GetStyleProperty("BorderThickness"), ThemeTag = { Color = "ElementBorder" } }),
			})
			...
			-- 🧭 A 56px rail has no room for name/subtitle text next to the avatar
			-- (the old UserInfo row assumed a ~170px labeled sidebar), so — same as
			-- Gzz — that info now shows in a small tooltip that floats out to the
			-- right of the avatar on hover, instead of being permanently on-screen.
			local titleText = tostring((Config.UserInfoTitle ~= nil and Config.UserInfoTitle) or (LocalPlayer.Name or "User"))
			local subtitleText = (Config.UserInfoSubtitle ~= nil) and tostring(Config.UserInfoSubtitle) or nil
			local avatarTip
			Creator.AddSignal(Avatar.MouseEnter, function()
				avatarTip = New("Frame", {
					...
					subtitleText and New("TextLabel", {
						Text = subtitleText,
						FontFace = GetStyleProperty("FontRegular"),
						TextSize = GetStyleProperty("TextSizeXs"),
				...
				avatarTip.Position = UDim2.fromOffset(Window.TabWidth + 8, abs.Y - Window.Root.AbsolutePosition.Y + avatarSize)
			end)
			Creator.AddSignal(Avatar.MouseLeave, function()
				if avatarTip then
					avatarTip:Destroy()
					avatarTip = nil
				end
		...
		do
			...
			local function EntranceAnimate(root, offsetX, offsetY, delaySec)
				...
				task.delay(delaySec, function()
					...
					local rootTweenProps = { Position = originalPosition }
					if rootProps then
						for propName, value in pairs(rootProps) do
							rootTweenProps[propName] = value
						end
					end
					TweenService:Create(root, ti, rootTweenProps):Play()
