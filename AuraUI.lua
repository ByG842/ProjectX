...
function AcrylicPaint()
	...
	return function(props)
		...
		AcrylicPaint.Frame = New("Frame", {
			...
			-- Glassmorphism: white sheen overlay (top half lighter)
			New("Frame", {
				BackgroundColor3 = Color3.fromRGB(255, 255, 255),
			...
			-- Color tint gradient (gives depth, changes with theme)
			New("Frame", {
				BackgroundColor3 = Color3.fromRGB(255, 255, 255),
			...
			-- Noise texture for frosted glass feel
			New("ImageLabel", {
			...
			-- Outer border: soft white ring (glass edge)
			New("Frame", {
				BackgroundTransparency = 1,
				Size = UDim2.fromScale(1, 1),
				ZIndex = 2,
			...
			-- Inner highlight stroke (subtle inner glow like real glass)
			New("Frame", {
				BackgroundTransparency = 1,
...
Components.Section = (function()
	...
	return function(Title, Parent, Icon)
		...
		-- Card wrapper — AuraUI glass card
...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		Tab.Frame = New("TextButton", {
			...
			New("TextLabel", {
				AnchorPoint = Vector2.new(0, 0.5),
				Position = not fischbypass and Icon and UDim2.new(0, 24, 0.5, 0) or UDim2.new(0, 2, 0.5, 0),
				Text = Title,
				RichText = true,
		...
		local function getTabLabel()
			return Tab.Frame:FindFirstChildOfClass("TextLabel")
		end
		local function getTabPill()
			return Tab.Frame:FindFirstChild("ActivePill")
		end
		local function getTabIcon()
			return Tab.Frame:FindFirstChildOfClass("ImageLabel")
		end

		Creator.AddSignal(Tab.Frame.MouseEnter, function()
			if not Tab.Selected then
				Tab.SetTransparency(0.88)
				local lbl = getTabLabel()
				if lbl then TweenService:Create(lbl, TweenInfo.new(0.15), { TextTransparency = 0.1 }):Play() end
			end
		end)
		Creator.AddSignal(Tab.Frame.MouseLeave, function()
			if not Tab.Selected then
				...
				local lbl = getTabLabel()
				if lbl then TweenService:Create(lbl, TweenInfo.new(0.15), { TextTransparency = 0.3 }):Play() end
			end
		end)
	...
	function TabModule:SelectTab(Tab)
		...
		-- AuraUI: deactivate all tabs
		for _, TabObject in next, TabModule.Tabs do
			TabObject.SetTransparency(1)
			TabObject.Selected = false
			-- hide pill, dim text
			local pill = TabObject.Frame and TabObject.Frame:FindFirstChild("ActivePill")
			local lbl  = TabObject.Frame and TabObject.Frame:FindFirstChildOfClass("TextLabel")
			local ico  = TabObject.Frame and TabObject.Frame:FindFirstChildOfClass("ImageLabel")
			if pill then TweenService:Create(pill, TweenInfo.new(0.2, Enum.EasingStyle.Quad), { BackgroundTransparency = 1 }):Play() end
			if lbl  then TweenService:Create(lbl,  TweenInfo.new(0.2), { TextTransparency = 0.35 }):Play() end
			if ico  then TweenService:Create(ico,  TweenInfo.new(0.2), { ImageTransparency = 0.4 }):Play() end
		end

		-- AuraUI: activate selected tab
		TabModule.Tabs[Tab].SetTransparency(0.88)
		TabModule.Tabs[Tab].Selected = true
		local selTab = TabModule.Tabs[Tab]
		local selPill = selTab.Frame and selTab.Frame:FindFirstChild("ActivePill")
		local selLbl  = selTab.Frame and selTab.Frame:FindFirstChildOfClass("TextLabel")
		local selIco  = selTab.Frame and selTab.Frame:FindFirstChildOfClass("ImageLabel")
		if selPill then TweenService:Create(selPill, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { BackgroundTransparency = 0 }):Play() end
		if selLbl  then TweenService:Create(selLbl,  TweenInfo.new(0.2), { TextTransparency = 0 }):Play() end
		if selIco  then TweenService:Create(selIco,  TweenInfo.new(0.2), { ImageTransparency = 0 }):Play() end
...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		-- ══════════════════════════════════════════════════
		-- AuraUI — macOS-style Traffic Light Control Buttons
		-- ══════════════════════════════════════════════════
		local function TrafficDot(color, hoverColor, size, parent, callback)
			...
			local iconLabel = New("TextLabel", {
				...
				FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold),
				TextSize = 8,
				TextColor3 = Color3.fromRGB(40, 20, 10),
				TextTransparency = 1,
				Parent = dot,
			})
			AddSignal(dot.MouseEnter, function()
				TweenService:Create(dot, TweenInfo.new(0.15), { BackgroundColor3 = hoverColor }):Play()
				TweenService:Create(iconLabel, TweenInfo.new(0.1), { TextTransparency = 0 }):Play()
			end)
			AddSignal(dot.MouseLeave, function()
				TweenService:Create(dot, TweenInfo.new(0.15), { BackgroundColor3 = color }):Play()
				TweenService:Create(iconLabel, TweenInfo.new(0.1), { TextTransparency = 1 }):Play()
			end)
			if callback then
				AddSignal(dot.MouseButton1Click, callback)
			end
			return dot, iconLabel
		end

		-- ══════════════════════════════════════════════
		-- AURA TITLEBAR — 58px tall, full glass premium
		-- ══════════════════════════════════════════════
		...
		-- Subtle inner glow at top of titlebar (glass refraction effect)
		...
		-- Close (red)
		local closeDot, closeIcon = TrafficDot(
		...
		-- Minimize (yellow)
		local minDot, minIcon = TrafficDot(
			Color3.fromRGB(255, 189, 46),
			Color3.fromRGB(230, 160, 20),
			13, TrafficGroup,
			function()
		...
		-- Maximize (green)
		local maxDot, maxIcon = TrafficDot(
			Color3.fromRGB(40, 201, 64),
			Color3.fromRGB(20, 175, 45),
			13, TrafficGroup,
			function()
		...
		-- Center: App Title display (big elegant centered name)
		...
		-- Bottom separator — thin frosted glass line
		New("Frame", {
			Name = "Separator",
		...
		-- Expose button refs for compatibility
		TitleBar.CloseButton = { Frame = closeDot, SetCallback = function(f) AddSignal(closeDot.MouseButton1Click, f) end }
		TitleBar.MinButton   = { Frame = minDot,   SetCallback = function(f) AddSignal(minDot.MouseButton1Click, f) end }
		TitleBar.MaxButton   = { Frame = maxDot,   SetCallback = function(f) AddSignal(maxDot.MouseButton1Click, f) end }
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- AuraUI: Sidebar glass divider (vertical line separating sidebar from content)
		local SidebarDivider = New("Frame", {
			Name = "SidebarDivider",
			Size = UDim2.new(0, 1, 1, -80),
			Position = UDim2.new(0, Window.TabWidth + 20, 0, 68),
			BackgroundTransparency = 0.75,
			BackgroundColor3 = Color3.fromRGB(255, 255, 255),
			BorderSizePixel = 0,
		}, {
		...
		Window.TabDisplay = New("TextLabel", {
			...
			Position = UDim2.fromOffset(Window.TabWidth + 30, 62),
			BackgroundTransparency = 1,
			ThemeTag = {
				TextColor3 = "Text",
			},
		...
		Window.ContainerCanvas = New("Frame", {
			Size = UDim2.new(1, -Window.TabWidth - 38, 1, -110),
			Position = UDim2.fromOffset(Window.TabWidth + 30, 96),
			BackgroundTransparency = 1,
			ClipsDescendants = true,
		}, {
		...
		table.insert(rootChildren, SidebarDivider)
		...
		function Window:Minimize()
			...
			-- AuraUI: smooth scale+fade animation on minimize/restore
			if Window.Minimized then
				-- Fade out + shrink
				TweenService:Create(Window.Root, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
					GroupTransparency = 1,
				}):Play()
				task.delay(0.15, function()
					Window.Root.Visible = false
					if pcall(function() return Window.Root.GroupTransparency end) then
				...
				-- Bounce in
				TweenService:Create(Window.Root, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
					GroupTransparency = 0,
				}):Play()
			end
...
ElementsTable.Button = (function()
	...
	function Element:New(Config)
		...
		local LoadingSpinner = New("ImageLabel", {
			Image = "rbxassetid://6034506251",
			Size = UDim2.fromOffset(14, 14),
			AnchorPoint = Vector2.new(1, 0.5),
			Position = UDim2.new(1, -10, 0.5, 0),
			BackgroundTransparency = 1,
		...
		-- Animated loading spin
		local spinConn
		local function startSpin()
			LoadingSpinner.Visible = true
			ButtonIco.Visible = false
			spinConn = RunService.RenderStepped:Connect(function(dt)
				LoadingSpinner.Rotation = (LoadingSpinner.Rotation + dt * 280) % 360
			end)
		end
		...
		Creator.AddSignal(ButtonFrame.Frame.MouseButton1Click, function()
			-- Check if callback returns a Promise-like (has :andThen) or just call normally
			local result = Library:SafeCallback(Config.Callback)
		end)

		-- Expose loading API on the returned frame
		ButtonFrame.SetLoading = function(state)
...
ElementsTable.Toggle = (function()
	...
	function Element:New(Idx, Config)
		...
		function Toggle:SetValue(Value)
			...
			local tiSnap = TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)

			-- Knob slide with slight squish on activate
			TweenService:Create(ToggleCircle, ti, {
				Position = UDim2.new(0, Toggle.Value and 24 or 4, 0.5, 0),
				Size     = UDim2.fromOffset(Toggle.Value and 18 or 16, Toggle.Value and 18 or 16),
			}):Play()

			-- Accent fill fade in/out with gradient
			TweenService:Create(ToggleSlider, tiSnap, {
				BackgroundTransparency = Toggle.Value and 0.3 or 1,
			}):Play()

			-- Track border color
			Creator.OverrideTag(ToggleBorder, { Color = Toggle.Value and "Accent" or "ToggleSlider" })
			TweenService:Create(ToggleBorder, tiSnap, {
				Transparency = Toggle.Value and 0.45 or 0.5,
				Thickness = Toggle.Value and 1.8 or 1.5,
			}):Play()

			-- Knob color + subtle glow when ON
			Creator.OverrideTag(ToggleCircle, { BackgroundColor3 = Toggle.Value and "Accent" or "ToggleSlider" })
			TweenService:Create(ToggleCircle, tiSnap, {
				BackgroundTransparency = Toggle.Value and 0.0 or 0.1,
			}):Play()

			-- Track bg brightens when toggled on
			TweenService:Create(ToggleTrack, tiSnap, {
				BackgroundTransparency = Toggle.Value and 0.65 or 0.88,
			}):Play()
...
ElementsTable.Slider = (function()
	...
	function Element:New(Idx, Config)
		...
		local SliderDot = New("Frame", {
			...
			-- Realtime value bubble above knob
			New("TextLabel", {
				Name = "ValueBubble",
				Text = "0",
				FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold),
				...
				Visible = false,
				ZIndex = 20,
				ThemeTag = { TextColor3 = "Text", BackgroundColor3 = "AcrylicMain" },
			}, {
				New("UICorner", { CornerRadius = UDim.new(0, 4) }),
			}),
		})
		...
		Creator.AddSignal(SliderDot.InputBegan, function(Input)
			...
			then
				Dragging = true
				-- Grow knob
				TweenService:Create(SliderDot, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
					Size = UDim2.fromOffset(17, 17),
				}):Play()
				local bubble = SliderDot:FindFirstChild("ValueBubble")
		...
		Creator.AddSignal(SliderDot.InputEnded, function(Input)
			...
			then
				Dragging = false
				-- Shrink knob back
				TweenService:Create(SliderDot, TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
					Size = UDim2.fromOffset(13, 13),
				}):Play()
				local bubble = SliderDot:FindFirstChild("ValueBubble")
...
local InterfaceManager = {} do
	...
	InterfaceManager.Folder = "AuraSettings"
...
if getgenv then
	...
	getgenv().AuraUI = Library
	getgenv().Fluent = Library -- backward compat
...
-- ══════════════════════════════════════════════════════════════
-- AuraUI — Floating Minimizer Button (Glassmorphism pill style)
-- ══════════════════════════════════════════════════════════════
local function makeAuraMinButton(cornerRadius)
	return New("TextButton", {
		Size = UDim2.new(1, 0, 1, 0),
		BorderSizePixel = 0,
		...
		New("Frame", {
			...
			New("UICorner", { CornerRadius = UDim.new(0, cornerRadius - 1) }),
			New("UIStroke", {
				Color = Color3.fromRGB(255, 255, 255),
				Transparency = 0.82,
				Thickness = 1,
		...
		New("ImageLabel", {
			...
			AnchorPoint = Vector2.new(0.5, 0.5),
			BackgroundTransparency = 1,
			ImageColor3 = Color3.fromRGB(200, 215, 255),
...
local MinimizeButton = makeAuraMinButton(14)
local MobileMinimizeButton = makeAuraMinButton(12)
...
Creator.AddSignal(MinimizeButton.MouseEnter, function()
	TweenService:Create(MinimizeButton, TweenInfo.new(0.18, Enum.EasingStyle.Quad), { BackgroundTransparency = 0.0 }):Play()
end)
Creator.AddSignal(MinimizeButton.MouseLeave, function()
	TweenService:Create(MinimizeButton, TweenInfo.new(0.18, Enum.EasingStyle.Quad), { BackgroundTransparency = 0.08 }):Play()
end)
Creator.AddSignal(MinimizeButton.MouseButton1Down, function()
	TweenService:Create(MinimizeButton, TweenInfo.new(0.1), { BackgroundTransparency = 0.15 }):Play()
end)
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ✨ AURAUI — NEW ELEMENTS (10) + IMPROVED ELEMENTS (10) = 20 TOTAL
-- ═══════════════════════════════════════════════════════════════════════════

-- ── IMPROVED 1: Label / Paragraph upgrade — icon support + colored dot ────
ElementsTable.Label = (function()
	local Element = {}
	Element.__index = Element
	...
	function Element:New(Config)
		Config.Content = Config.Content or ""
		local color = Config.Color -- optional Color3
		local icon  = Config.Icon  -- optional lucide icon name

		local Lbl = Components.Element(Config.Title, Config.Content, self.Container, false, Config)
		Lbl.Frame.BackgroundTransparency = 0.90
		Lbl.Border.Transparency = 0.55

		-- Optional colored accent dot left
		if color then
...
-- ── IMPROVED 2: Separator — glowing accent line ────────────────────────────
ElementsTable.GlowSeparator = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "GlowSeparator"
	...
	function Element:New(Idx, Config)
		...
		-- Left line
		local lineL = Creator.New("Frame", {
			Size = UDim2.new(0.5, Config.Label ~= "" and -16 or 0, 0, 1),
			Position = UDim2.fromScale(0, 0.5),
			AnchorPoint = Vector2.new(0, 0.5),
			Parent = Root,
			BackgroundTransparency = 0,
...
ElementsTable.ProgressBar = (function()
	...
	function Element:New(Idx, Config)
		...
		local PctLabel = Creator.New("TextLabel", {
			Text = "0%",
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold),
			TextSize = 11,
			AnchorPoint = Vector2.new(1, 0.5),
		...
		function PB:SetValue(Value)
			...
			PctLabel.Text = math.floor(pct * 100) .. "%"
			Library:SafeCallback(PB.Callback, Value)
			Library:SafeCallback(PB.Changed,  Value)
		end
		function PB:OnChanged(Func) PB.Changed = Func Func(PB.Value) end
		function PB:Destroy() PBFrame.Frame:Destroy() if Idx then Library.Options[Idx] = nil end end

		PBFrame.Frame.Size = UDim2.new(1, 0, 0, 56)
		PB:SetValue(Config.Default)
...
-- ── NEW 2: Tooltip (hover info bubble) ───────────────
-- Usage: :AddTooltip({ Text = "Hover hint", Target = someElement })
ElementsTable.Tooltip = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "Tooltip"

	function Element:New(Idx, Config)
		Config = Config or {}
		assert(Config.Text,   "Tooltip - Missing Text")
		assert(Config.Target, "Tooltip - Missing Target frame")

		local TT = { Type = "Tooltip" }

		local Bubble = Creator.New("Frame", {
			Size = UDim2.fromOffset(0, 28),
			AutomaticSize = Enum.AutomaticSize.X,
			...
			Creator.New("UIStroke", { Transparency = 0.4, Thickness = 1, ThemeTag = { Color = "Accent" } }),
		...
		Creator.AddSignal(Config.Target.MouseEnter, function()
			Bubble.Visible = true
			TweenService:Create(Bubble, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
				BackgroundTransparency = 0.18,
			}):Play()
		end)
		Creator.AddSignal(Config.Target.MouseLeave, function()
			Bubble.Visible = false
		end)

		function TT:SetText(text)
		...
		if Idx then Library.Options[Idx] = TT end
		return TT
	end
	return Element
end)()
...
-- ── NEW 3: ImageDisplay ───────────────────────────────
ElementsTable.ImageDisplay = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "ImageDisplay"

	function Element:New(Idx, Config)
		Config = Config or {}
		assert(Config.Image, "ImageDisplay - Missing Image (rbxassetid://...)")
		Config.Height = Config.Height or 80

		local IM = { Type = "ImageDisplay" }

		local Root = Creator.New("Frame", {
			Size = UDim2.new(1, 0, 0, Config.Height + 16),
			BackgroundTransparency = 0.90,
			Parent = self.Container,
			LayoutOrder = 7,
			ThemeTag = { BackgroundColor3 = "Element" },
			...
			Creator.New("UIStroke", { Transparency = 0.6, Thickness = 1, ThemeTag = { Color = "InElementBorder" } }),
		})

		local Img = Creator.New("ImageLabel", {
			Image = Config.Image,
			Size = UDim2.new(1, -16, 0, Config.Height),
		...
		if Config.Caption then
			Creator.New("TextLabel", {
				...
				Position = UDim2.new(0, 8, 0, Config.Height + 4),
				TextXAlignment = Enum.TextXAlignment.Center,
				Parent = Root,
				ThemeTag = { TextColor3 = "SubText" },
			})
			Root.Size = UDim2.new(1, 0, 0, Config.Height + 30)
		end

		function IM:SetImage(id) Img.Image = id end
		function IM:Destroy() Root:Destroy() if Idx then Library.Options[Idx] = nil end end

		if Idx then Library.Options[Idx] = IM end
		return IM
	end
	return Element
end)()

-- ── NEW 4: NumberStepper (+/- buttons) ────────────────
ElementsTable.NumberStepper = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "NumberStepper"

	function Element:New(Idx, Config)
		Config = Config or {}
		assert(Config.Title, "NumberStepper - Missing Title")
		Config.Min     = Config.Min     or 0
		Config.Max     = Config.Max     or 100
		Config.Step    = Config.Step    or 1
		...
		local NSFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		NS.SetTitle = NSFrame.SetTitle
		NS.SetDesc  = NSFrame.SetDesc
		NS.Visible  = NSFrame.Visible

		local function makeBtn(label, parent)
			return Creator.New("TextButton", {
				...
				Creator.New("UICorner", { CornerRadius = UDim.new(0, 8) }),
				Creator.New("UIStroke", { Transparency = 0.5, Thickness = 1, ThemeTag = { Color = "Accent" } }),
			})
		end
		...
		local PlusBtn = makeBtn("+", Row)

		function NS:SetValue(v)
			self.Value = math.clamp(v, Config.Min, Config.Max)
		...
		function NS:OnChanged(Func) NS.Changed = Func Func(NS.Value) end
		function NS:Destroy() NSFrame.Frame:Destroy() if Idx then Library.Options[Idx] = nil end end

		Creator.AddSignal(MinusBtn.MouseButton1Click, function()
			NS:SetValue(NS.Value - Config.Step)
		end)
		Creator.AddSignal(PlusBtn.MouseButton1Click, function()
			NS:SetValue(NS.Value + Config.Step)
		end)

		if Idx then Library.Options[Idx] = NS end
		return NS
	end
	return Element
end)()
...
-- ── NEW 5: Badge / Tag ────────────────────────────────
ElementsTable.Badge = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "Badge"
	Element.NoIdx = true

	local BADGE_COLORS = {
		new     = Color3.fromRGB(96,  205, 255),
		hot     = Color3.fromRGB(255, 80,  80),
		beta    = Color3.fromRGB(160, 100, 255),
		update  = Color3.fromRGB(80,  200, 120),
		default = Color3.fromRGB(140, 140, 160),
	}

	function Element:New(Config)
		Config = Config or {}
		assert(Config.Target, "Badge - Missing Target element frame")
		Config.Text  = Config.Text  or "NEW"
		Config.Style = Config.Style or "new"

		local col = BADGE_COLORS[Config.Style:lower()] or BADGE_COLORS.default
...
-- ── NEW 6: TimerLabel (countdown or count-up) ────────
ElementsTable.TimerLabel = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "TimerLabel"

	function Element:New(Idx, Config)
		Config = Config or {}
		assert(Config.Title, "TimerLabel - Missing Title")
		Config.Duration  = Config.Duration  or 60
		Config.CountUp   = Config.CountUp   ~= nil and Config.CountUp or false
		Config.AutoStart = Config.AutoStart ~= nil and Config.AutoStart or false
		...
		local TimeDisplay = Creator.New("TextLabel", {
			Text = "00:00",
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold),
			TextSize = 14,
			Size = UDim2.fromOffset(60, 22),
		...
		function TL:Start()
			...
			conn = RunService.Heartbeat:Connect(function(dt)
				if not TL.Running then return end
				TL.Elapsed = TL.Elapsed + dt
				local display = Config.CountUp and TL.Elapsed or math.max(0, Config.Duration - TL.Elapsed)
				TimeDisplay.Text = formatTime(display)
				if not Config.CountUp and TL.Elapsed >= Config.Duration then
					TL:Stop()
					Library:SafeCallback(TL.Callback, TL.Elapsed)
				end
			end)
		end
		...
		function TL:Reset()
			self:Stop()
			TL.Elapsed = 0
			TimeDisplay.Text = formatTime(Config.CountUp and 0 or Config.Duration)
		end

		function TL:OnChanged(Func) TL.Changed = Func end
		function TL:Destroy()
			self:Stop()
			TLFrame.Frame:Destroy()
			if Idx then Library.Options[Idx] = nil end
		end

		TL:Reset()
		if Config.AutoStart then TL:Start() end
...
-- ── NEW 7: TextBadge (inline accent pill next to title) ─
ElementsTable.TextBadge = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "TextBadge"
	Element.NoIdx = true

	function Element:New(Config)
		Config = Config or {}
		assert(Config.Title, "TextBadge - Missing Title")
		Config.Badge = Config.Badge or ""

		local TBFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		TBFrame.Frame.BackgroundTransparency = 0.90
...
-- ── NEW 8: SpacerFrame (invisible height padding) ────
ElementsTable.Spacer = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "Spacer"
	Element.NoIdx = true
...
-- ── NEW 9: MultiDropdown ──────────────────────────────
-- Extends Dropdown: forces Multi=true and adds a "Select All / Clear" row
ElementsTable.MultiDropdown = (function()
	local Base = ElementsTable.Dropdown
	local Element = {}
	Element.__index = Base

	function Element:New(Idx, Config)
		Config = Config or {}
		Config.Multi     = true
		Config.AllowNull = true
		Config.Search    = (Config.Search == nil) and true or Config.Search
		return Base.New(self, Idx, Config)
	end
	return Element
end)()

-- ── NEW 10: GlassCard (decorative info card) ─────────
ElementsTable.GlassCard = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "GlassCard"

	function Element:New(Idx, Config)
		Config = Config or {}
		assert(Config.Title, "GlassCard - Missing Title")
		Config.Content = Config.Content or ""
		Config.Color   = Config.Color   or nil

		local GC = { Type = "GlassCard" }

		local accent = Config.Color or Creator.GetThemeProperty("Accent")
		...
		local Root = Creator.New("Frame", {
			...
			Creator.New("UIStroke", { Transparency = 0.5, Thickness = 1.2, ThemeTag = { Color = "InElementBorder" } }),
			-- Glass sheen
			Creator.New("Frame", {
				BackgroundColor3 = Color3.fromRGB(255, 255, 255),
				BackgroundTransparency = 0.90,
				...
				Creator.New("UICorner", { CornerRadius = UDim.new(0, 11) }),
				Creator.New("UIGradient", {
					Rotation = 180,
					Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(1, 1) }),
				}),
		...
		function GC:SetContent(text)
			local lbl = Root:FindFirstChildWhichIsA("TextLabel")
			if lbl then lbl.Text = text end
		end
		function GC:Destroy() Root:Destroy() if Idx then Library.Options[Idx] = nil end end

		if Idx then Library.Options[Idx] = GC end
		return GC
	end
	return Element
end)()
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Register all new elements into Elements metatable
-- ═══════════════════════════════════════════════════════════════════════════
local AuraElements = {
	"Label", "GlowSeparator", "ProgressBar", "Tooltip", "ImageDisplay",
	"NumberStepper", "Badge", "TimerLabel", "TextBadge", "Spacer",
	"MultiDropdown", "GlassCard",
}
for _, name in ipairs(AuraElements) do
	if ElementsTable[name] then
		Library.Elements[name] = ElementsTable[name]
	end
end

-- expose on Library for direct usage
Library.NewElement = function(self, typeName, ...)
	if Library.Elements[typeName] then
		return Library.Elements[typeName]:New(...)
	end
end
