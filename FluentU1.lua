...
Components.Window = (function()
	...
	return function(Config)
		...
		local function CenterWindow()
			-- ถ้ามี layout mode อยู่ ให้ layout จัดการตำแหน่งเอง ไม่ center
			if Window._layoutCollapsedPos then
				Window.Root.Position = Window._layoutIsExpanded
					and Window._layoutExpandedPos
					or  Window._layoutCollapsedPos
				return
			end
		...
		AutoloadButton = section:AddButton({Title = "Set as autoload", Description = "Current autoload config: none", Callback = function()
			...
			if not name or name == "" then
				return self.Library:Notify({
					...
					SubContent = "Please select a config from the list first.", Duration = 5
				})
			end
...
Library.CreateWindow = function(self, Config)
	...
	-- ════════════════════════════════════════════════════════════════
	-- Layout System v3
	-- ตอนพับ  → หน้าต่างเลื่อนออกนอกจอ เหลือแค่ title bar ติดขอบ
	-- ตอนเปิด → ดึงหน้าต่างทั้งหมดออกมา
	-- ปุ่ม toggle อยู่บน title bar ที่โผล่ออกมาเสมอ
	-- ════════════════════════════════════════════════════════════════
	if Config.Layout then
		task.defer(function()
			...
			local vp      = Camera.ViewportSize
			local winW    = Window.Root.AbsoluteSize.X
			local winH    = Window.Root.AbsoluteSize.Y
			local titleH  = 52   -- ความสูง TitleBar จริงๆ (จาก Components.TitleBar Size = 52)

			-- ── คำนวณตำแหน่ง ─────────────────────────────────────
			local expandedPos, collapsedPos

			if layout == "top" then
				-- expanded: ติดขอบบน กึ่งกลาง X
				expandedPos  = UDim2.fromOffset(math.floor((vp.X - winW) / 2), 0)
				-- collapsed: เลื่อนขึ้น เหลือแค่ titleH โผล่ที่ขอบบน
				collapsedPos = UDim2.fromOffset(expandedPos.X.Offset, -(winH - titleH))

			elseif layout == "bottom" then
				expandedPos  = UDim2.fromOffset(math.floor((vp.X - winW) / 2), math.floor(vp.Y - winH))
				collapsedPos = UDim2.fromOffset(expandedPos.X.Offset, math.floor(vp.Y - titleH))

			elseif layout == "left" then
				expandedPos  = UDim2.fromOffset(0, math.floor((vp.Y - winH) / 2))
				-- collapsed: เลื่อนซ้าย เหลือแค่ titleH โผล่ที่ขอบซ้าย
				collapsedPos = UDim2.fromOffset(-(winW - titleH), expandedPos.Y.Offset)

			elseif layout == "right" then
				expandedPos  = UDim2.fromOffset(math.floor(vp.X - winW), math.floor((vp.Y - winH) / 2))
				collapsedPos = UDim2.fromOffset(math.floor(vp.X - titleH), expandedPos.Y.Offset)

			else
				return
			end

			-- บอก CenterWindow ให้หยุด center และใช้ตำแหน่งของ layout แทน
			Window._layoutExpandedPos  = expandedPos
			Window._layoutCollapsedPos = collapsedPos
			Window._layoutIsExpanded   = false

			-- วางหน้าต่างที่ตำแหน่งพับทันที
			Window.Root.Position = collapsedPos

			-- ── ไอคอนลูกศรตาม layout และ state ────────────────
			local arrowOpen  = { top = "▼", bottom = "▲", left = "▶", right = "◀" }
			local arrowClose = { top = "▲", bottom = "▼", left = "◀", right = "▶" }

			-- ── ตำแหน่งปุ่มบน Window.Root ───────────────────────
			-- ปุ่มติดอยู่ที่ขอบด้านนอกของ title bar
			-- top:    ปุ่มอยู่ กึ่งกลางล่างสุดของ titleH → ยื่นออกมาด้านล่าง
			-- bottom: ปุ่มอยู่ กึ่งกลางบนสุด → ยื่นขึ้น
			-- left:   ปุ่มอยู่ กึ่งกลางขวาสุด → ยื่นออกขวา
			-- right:  ปุ่มอยู่ กึ่งกลางซ้ายสุด → ยื่นออกซ้าย

			local isH = layout == "top" or layout == "bottom"
			local bW  = isH and 52 or 16
			local bH  = isH and 16 or 52

			local btnAnchor, btnPos
			if layout == "top" then
				btnAnchor = Vector2.new(0.5, 0)
				btnPos    = UDim2.new(0.5, 0, 0, titleH)      -- ใต้ title bar พอดี
			elseif layout == "bottom" then
				btnAnchor = Vector2.new(0.5, 1)
				btnPos    = UDim2.new(0.5, 0, 0, 0)           -- บนสุดของ root
			elseif layout == "left" then
				btnAnchor = Vector2.new(0, 0.5)
				btnPos    = UDim2.new(0, titleH, 0.5, 0)      -- ขวาของ title bar
			elseif layout == "right" then
				btnAnchor = Vector2.new(1, 0.5)
				btnPos    = UDim2.new(0, 0, 0.5, 0)           -- ซ้ายสุด
			end

			local ToggleBtn = New("TextButton", {
				Name                   = "LayoutToggleBtn",
				Text                   = arrowOpen[layout],
			...
			Creator.AddSignal(ToggleBtn.MouseEnter, function()
				TweenService:Create(ToggleBtn, TweenInfo.new(0.12), { BackgroundTransparency = 0 }):Play()
			end)
			...
			local function applyLayout(expand, instant)
				Window._layoutIsExpanded = expand
				local pos = expand and expandedPos or collapsedPos
				ToggleBtn.Text = expand and arrowClose[layout] or arrowOpen[layout]
				if instant then
					Window.Root.Position = pos
				else
					TweenService:Create(Window.Root, tweenInfo, { Position = pos }):Play()
				end
			end

			Creator.AddSignal(ToggleBtn.MouseButton1Click, function()
				applyLayout(not Window._layoutIsExpanded)
			end)

			-- วางพับเริ่มต้น instant
			applyLayout(false, true)
		end)
	end
	-- ════════════════════════════════════════════════════════════════
	-- End Layout System v3
	-- ════════════════════════════════════════════════════════════════
