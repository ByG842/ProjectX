	...
	function Element:New(Idx, Config)
		...
		function Dropdown:BuildDropdownList()
			...
			for _, Value in ipairs(Values) do
				...
				-- Section separator: { Sec = true, Label = "..." }
				if type(Value) == "table" and Value.Sec == true then
					if not hasRenderedItem then continue end

					local secLabel = tostring(Value.Label or "")
					...
					if secLabel ~= "" then
						...
						New("Frame", {
							Size = UDim2.new(0.22, 0, 0, 1),
							Position = UDim2.new(0.78, -8, 0.5, 0),
							AnchorPoint = Vector2.new(0, 0.5),
							BackgroundTransparency = 0.4,
							BorderSizePixel = 0,
							Parent = dividerFrame,
...
local SaveManager = {} do
	...
	function SaveManager:BuildConfigSection(tab)
		...
		-- Autoload toggle: ON = set selected config as autoload, OFF = clear autoload
		local autoloadCurrentName = ""
		if isfile(self.Folder .. "/autoload.txt") then
			autoloadCurrentName = (readfile(self.Folder .. "/autoload.txt") or ""):match("^%s*(.-)%s*$") or ""
		end
		local autoloadDefaultOn = autoloadCurrentName ~= ""

		local AutoloadToggle
		AutoloadToggle = section:AddToggle("SaveManager_AutoloadToggle", {
			Title = "Set as autoload",
			Description = autoloadDefaultOn and ("Autoload: " .. autoloadCurrentName) or "Autoload: none",
			Default = autoloadDefaultOn,
			Callback = function(Value)
				if Value then
					local name = SaveManager.Options.SaveManager_ConfigList.Value
					if not name or name == "" then
						-- revert toggle off if no config selected
						task.defer(function()
					...
					AutoloadToggle:SetDesc("Autoload: " .. name)
					self.Library:Notify({
						Title = "Interface",
						Content = "Config loader",
					...
					AutoloadToggle:SetDesc("Autoload: none")
					self.Library:Notify({
						Title = "Interface",
						Content = "Config loader",
		...
		SaveManager:SetIgnoreIndexes({ "SaveManager_ConfigList", "SaveManager_ConfigName", "SaveManager_AutoloadToggle" })
