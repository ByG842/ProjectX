	...
	return function(props)
		...
		AcrylicPaint.Frame = New("Frame", {
			...
			New("ImageLabel", {
				...
				ImageTransparency = 0.65, -- 🌑 เงาใต้กระจกเข้มขึ้นนิดนึง ให้ความรู้สึกลอยเหนือพื้น (depth) มากขึ้น
			}),
			...
			-- ✨ Glass sheen: แถบแสงพาดทแยงมุมบนซ้าย จำลองแสงสะท้อนบนผิวกระจกจริง (100% ปลอดภัย ไม่กระทบจอเกม)
			New("Frame", {
				Name = "GlassSheen",
				BackgroundColor3 = Color3.fromRGB(255, 255, 255),
				BackgroundTransparency = 1,
			...
			New("Frame", {
				...
				NewCorner("WindowCorner"),
				New("UIStroke", {
					Transparency = 0.4, -- ขอบคมขึ้นนิดนึงให้ดูเหมือนขอบกระจกจริง
					Thickness = GetStyleProperty("BorderThickness"),
					ThemeTag = {
						Color = "AcrylicBorder",
					},
				}, {
					-- 💎 Gradient บนเส้นขอบ: สว่างกว่าที่มุมบนซ้าย (รับแสง) ค่อยๆ จางลงมุมล่างขวา จำลองแสงกระทบขอบกระจก
					New("UIGradient", {
						Rotation = 45,
						Transparency = NumberSequence.new({
...
Components.Tab = (function()
	...
	-- 🎬 เลื่อน element/section ด้านในทีละอันแบบ stagger (ทยอยเข้าไม่พร้อมกัน ช้าๆ)
	-- ใช้ตอนสลับ Tab/SubTab ให้เนื้อหาข้างในดูมีชีวิตชีวา แทนที่จะเลื่อนมาเป็นก้อนเดียวแข็งๆ
	local function StaggerRevealChildren(Container, OffsetY, StaggerStep, Duration)
		...
		for Index, Item in ipairs(Items) do
			...
			task.delay((Index - 1) * StaggerStep, function()
				if not Item or not Item.Parent then return end
				local ti = TweenInfo.new(Duration, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
				TweenService:Create(Item, ti, { Position = OriginalPosition }):Play()
			end)
		end
	end
	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:SelectSubTab(SubTabIndex)
			...
			if PreviousSubTab > 0 and PreviousSubTab ~= SubTabIndex and self.SubTabs[PreviousSubTab] and self.SubTabs[SubTabIndex] then
				...
				-- 🎬 element ข้างในซับแท็บทยอยเลื่อนเข้าทีละอัน ช้าๆ
				if NewSubTab.Container then
					StaggerRevealChildren(NewSubTab.Container, 26, 0.045, 0.45)
				end
				...
				if self.SubTabs[SubTabIndex] and self.SubTabs[SubTabIndex].Container then
					StaggerRevealChildren(self.SubTabs[SubTabIndex].Container, 26, 0.045, 0.45)
				end
			end
		end
	...
	function TabModule:SelectTab(Tab)
		...
		if PreviousTab > 0 and PreviousTab ~= Tab and TabModule.Tabs[PreviousTab] and TabModule.Tabs[Tab] then
			...
			-- 🎬 element ข้างในทยอยเลื่อนเข้าทีละอัน ช้าๆ (แยกจากการเลื่อนก้อนคอนเทนเนอร์ด้านบน)
			if NewTab.ContainerFrame then
				StaggerRevealChildren(NewTab.ContainerFrame, 26, 0.045, 0.45)
			end
			...
			if TabModule.Tabs[Tab] and TabModule.Tabs[Tab].ContainerFrame then
				StaggerRevealChildren(TabModule.Tabs[Tab].ContainerFrame, 26, 0.045, 0.45)
			end
		end
	end
