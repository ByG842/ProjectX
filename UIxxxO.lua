...
Library.CreateWindow = function(self, Config)
	...
	-- 🎬 Entrance animation: แต่ละส่วนสไลด์เข้ามาประกอบกันเป็นหน้าต่าง แทนโผล่มาทันทีแบบโต้งๆ
	-- หน่วงไว้ ~1.8 วิก่อนเล่น เพราะช่วงแรกที่สคริปต์รันมักมีอาการกระตุก (โหลด GUI/ต่อ connection ต่างๆ)
	-- รอให้ทุกอย่าง settle ก่อนค่อยเล่นอนิเมชันให้ลื่นจริง
	if Window.RootWrapper and Window.RootScale then
		local titleBarFrame     = Window.TitleBar and Window.TitleBar.Frame
		local tabFrameObj       = Window.TabFrame
		local containerCanvasObj = Window.ContainerCanvas

		local origTitlePos     = titleBarFrame and titleBarFrame.Position
		local origTabPos       = tabFrameObj and tabFrameObj.Position
		local origContainerPos = containerCanvasObj and containerCanvasObj.Position

		-- ซ่อน/ขยับออกจากตำแหน่งจริงไว้ก่อน รอจนกว่าจะถึงเวลาเล่นอนิเมชัน
		Window.RootWrapper.GroupTransparency = 1
		Window.RootScale.Scale = 0.94

		if titleBarFrame then
			titleBarFrame.Position = origTitlePos - UDim2.fromOffset(0, 16)
		end
		if tabFrameObj then
			tabFrameObj.Position = origTabPos - UDim2.fromOffset(24, 0)
		end
		if containerCanvasObj then
			containerCanvasObj.Position = origContainerPos + UDim2.fromOffset(0, 16)
		end

		-- 💫 Shimmer skeleton — โชว์ระหว่างช่วงหน่วง 1.8 วิ กันจอว่างเปล่าดูเหมือนค้าง
		local ShimmerCard = New("Frame", {
		...
		local ShimmerGradient = ShimmerCard.ShimmerSweep
		local shimmerRunning = true
		task.spawn(function()
			while shimmerRunning do
				ShimmerGradient.Offset = Vector2.new(-1, 0)
				TweenService:Create(ShimmerGradient, TweenInfo.new(0.9, Enum.EasingStyle.Linear), { Offset = Vector2.new(1, 0) }):Play()
				task.wait(0.9)
			end
		end)

		task.spawn(function()
			task.wait(1.8) -- ⏱️ หน่วงกันกระตุกตอนสคริปต์เพิ่งรัน

			shimmerRunning = false
			local shimmerFade = TweenService:Create(ShimmerCard, TweenInfo.new(0.18), { BackgroundTransparency = 1 })
			shimmerFade:Play()
			shimmerFade.Completed:Connect(function()
				ShimmerCard:Destroy()
			end)

			TweenService:Create(Window.RootWrapper, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { GroupTransparency = 0 }):Play()
			TweenService:Create(Window.RootScale, TweenInfo.new(0.38, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()

			if titleBarFrame then
				TweenService:Create(titleBarFrame, TweenInfo.new(0.32, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { Position = origTitlePos }):Play()
			end
			task.wait(0.06)
			if tabFrameObj then
				TweenService:Create(tabFrameObj, TweenInfo.new(0.32, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { Position = origTabPos }):Play()
			end
			task.wait(0.06)
			if containerCanvasObj then
				TweenService:Create(containerCanvasObj, TweenInfo.new(0.32, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { Position = origContainerPos }):Play()
			end
		end)
	end
