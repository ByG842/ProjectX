...
-- ─── AddColumns: 2-column layout ───────────────────────────────────────
-- Usage:
--   local cols = Tab:AddColumns() or SubTab:AddColumns()
--   local left  = cols:Left()   -- returns a Section-like object
--   local right = cols:Right()  -- returns a Section-like object
--   left:AddToggle("MyToggle", { Title = "Hello", Default = false })

local function CreateColumnsInContainer(parentContainer, scrollContainer)
	local New = Creator.New

	-- Outer wrapper that sits inside the tab's ScrollingFrame
	...
	local function MakeColumn(layoutOrder)
		...
		-- Column acts like a mini-Tab: has Container = ColFrame and AddSection
		local Col = {
			Type = "Column",
			Container = ColFrame,
		...
		function Col:AddSection(SectionTitle, SectionIcon)
			...
			local SectionFrame = Components.Section(SectionTitle, ColFrame, Icon)
			Section.Container = SectionFrame.Container
			Section.ScrollFrame = scrollContainer
