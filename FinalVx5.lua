...
Components.Window = (function()
	...
	return function(Config)
		...
		function Window:Dialog(Config)
			...
			-- ── กันหัวข้อ (Title) ล้นกรอบ ────────────────────────────────
			-- วัดความกว้างจริงของ Title (แบบไม่ตัดคำ) เพื่อเอามาใช้คำนวณความกว้าง
			-- กล่องร่วมกับ Content แล้วค่อย wrap เมื่อยาวเกิน maxWidth
			-- หมายเหตุ: ใช้ TextLabel ชั่วคราววัดแทน TextService:GetTextSize เพราะ
			-- ตัวหลังรับเฉพาะ Enum.Font (ฟอนต์แบบเก่า) แต่ FontFace ในไลบรารีนี้
			-- เป็น Font object (ฟอนต์แบบใหม่) ส่งเข้าไปตรงๆ จะ error "Unable to cast Font to Font"
			local TitleMeasure = New("TextLabel", {
				FontFace = GetStyleProperty("FontSemiBold"),
				Text = Dialog.Title.Text or "",
				TextSize = GetStyleProperty("TextSizeTitle"),
				TextWrapped = false,
				AutomaticSize = Enum.AutomaticSize.XY,
			...
			local titleBounds = TitleMeasure.TextBounds
			TitleMeasure:Destroy()

			local maxWidth = math.min(620, Window.Size.X.Offset - 120)
			local baseWidth = math.max(300, math.min(maxWidth, math.max(Content.TextBounds.X, titleBounds.X) + 40))
			Dialog.Root.Size = UDim2.fromOffset(baseWidth, 165)
			ContentHolder.Size = UDim2.new(1, -40, 1, -110)
			task.defer(function()
				-- Title.TextBounds.Y คือความสูงจริง "หลัง wrap" ตามความกว้างกล่องที่ตั้งไปแล้ว
				local titleHeight = Dialog.Title.TextBounds.Y
				local topOffset = 18 + titleHeight + 14 -- padding บน + สูง title + ช่องว่างก่อน content
				ContentHolder.Position = UDim2.fromOffset(20, topOffset)
				ContentHolder.Size = UDim2.new(1, -40, 1, -(topOffset + 50))

				local contentHeight = Content.TextBounds.Y
				local desired = math.clamp(topOffset + contentHeight + 90, 165, 460)
...
ElementsTable.ActionButton = (function()
	...
	function Element:New(Idx, Config)
		...
		-- ── ระบบคอนเฟิม (Confirm) ──────────────────────────────────────
		-- Config.Confirm = true   -> เปิดใช้งานด้วยข้อความ default
		-- Config.Confirm = {      -> เปิดใช้งานพร้อมกำหนดข้อความเอง
		--     Title       = "หัวข้อ Dialog",
		--     Text        = "เนื้อหาที่ถาม (ยาวแค่ไหนก็ได้ ไม่ล้นกรอบ — ตัว Dialog
		--                    ของไลบรารีนี้ wrap คำ + เลื่อนสกอร์ลอัตโนมัติให้อยู่แล้ว)",
		--     ConfirmText = "ข้อความปุ่มยืนยัน",
		--     CancelText  = "ข้อความปุ่มยกเลิก",
		-- }
		local ConfirmCfg = nil
		if Config.Confirm == true then
			ConfirmCfg = {}
		elseif type(Config.Confirm) == "table" then
			ConfirmCfg = Config.Confirm
		end
		...
		-- การกระทำจริง (คัดลอก + เปลี่ยนข้อความปุ่มชั่วคราว + ยิง Callback)
		-- แยกออกมาเป็นฟังก์ชันเดียว เพื่อให้ทั้งโหมดปกติ และโหมดกด "ยืนยัน" ใน Dialog
		-- เรียกใช้โค้ดชุดเดียวกัน ไม่ต้องเขียนซ้ำ
		...
		Creator.AddSignal(CopyBtn.MouseButton1Click, function()
			if ConfirmCfg then
				-- ใช้ระบบ Dialog กลางของไลบรารี (Library.Window:Dialog) ซึ่งมี
				-- อนิเมชั่น fade + scale spring ให้อยู่แล้ว, มีฉากหลังมืด (tint),
				-- และเนื้อหายาวแค่ไหนก็ wrap + เลื่อนสกอร์ลได้ ไม่ล้นกรอบ
				Library.Window:Dialog({
					Title = ConfirmCfg.Title or ("ยืนยัน" .. (Config.Title and (" — " .. Config.Title) or "")),
					Content = ConfirmCfg.Text
						or ("คุณแน่ใจหรือไม่ว่าต้องการดำเนินการ \"" .. tostring(Config.Title) .. "\"?"),
					Buttons = {
						{ Title = ConfirmCfg.ConfirmText or "ยืนยัน", Callback = PerformAction },
						{ Title = ConfirmCfg.CancelText  or "ยกเลิก" },
					},
				})
			else
				PerformAction()
