...
-- 📱 คำนวณขนาดหน้าต่างสำหรับมือถือจากขนาดจอจริง ครอบคลุมทุกรุ่น/ทุกทิศทาง
-- แยกสูตร portrait/landscape เพราะสัดส่วนจอมือถือต่างกันมาก (จอสูงยาว 19.5:9 ไปจนถึงจอเก่า 16:9)
-- และหัก GuiInset (status bar / notch / home indicator) ออกก่อนคำนวณ ไม่ให้หน้าต่างโดนบัง
local function ComputeMobileWindowSize(viewportSize)
	if not viewportSize then return nil end

	local inset = GuiService:GetGuiInset() -- Vector2 ของพื้นที่ระบบที่กินไปด้านบน (และบางเครื่องด้านล่าง)
	local usableX = math.max(viewportSize.X - inset.X, 200)
	local usableY = math.max(viewportSize.Y - inset.Y, 200)

	local isLandscape = usableX > usableY

	local targetW, targetH
	if isLandscape then
		-- แนวนอน: พื้นที่สูงจำกัดกว่ามาก ให้เตี้ยลงแต่กว้างพอสมควร
		targetW = math.clamp(usableX * 0.62, 360, 640)
		targetH = math.clamp(usableY * 0.86, 240, 520)
	else
		-- แนวตั้ง: กว้างเกือบเต็มจอ, สูงพอประมาณเผื่อแป้นพิมพ์/thumb reach
		targetW = math.clamp(usableX * 0.94, 300, 480)
		targetH = math.clamp(usableY * 0.72, 340, 620)
	end

	-- กันเหนียวรอบสุดท้าย ห้ามเกินพื้นที่ใช้งานจริงไม่ว่ากรณีใด (เผื่อมือถือจอเล็ก/บาง fold ที่ผิดปกติ)
	targetW = math.min(targetW, usableX - 12)
	targetH = math.min(targetH, usableY - 12)

	return UDim2.fromOffset(math.floor(targetW), math.floor(targetH))
end

-- 📱 คำนวณสัดส่วนย่อเนื้อหาภายในหน้าต่าง (UIScale) เทียบกับขนาดที่นักพัฒนาออกแบบไว้ตอนแรก
-- แก้ปัญหา: หน้าต่างถูกบีบเล็กลงพอดีจอมือถือแล้ว แต่ปุ่ม/แถว/ตัวอักษรข้างในยังเป็นพิกเซลตายตัว ไม่ย่อตาม ล้น/ทับกัน
local function ComputeContentScale(designSize, actualSize)
	if not Mobile or not designSize or not actualSize then return 1 end
	local dw, dh = designSize.X.Offset, designSize.Y.Offset
	if dw <= 0 or dh <= 0 then return 1 end
	local scale = math.min(actualSize.X.Offset / dw, actualSize.Y.Offset / dh)
	-- ไม่ให้ย่อจนเล็กเกินอ่าน (0.55) และไม่ขยายเกินขนาดต้นฉบับ (1)
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- + ใช้ตัวเดียวกันย่อเนื้อหาทั้งหมดตามสัดส่วนจอมือถือ (แก้ปัญหาข้างในไม่ย่อตามหน้าต่าง)
		Window.ContentScale = ComputeContentScale(Config.DesignSize, Window.Size)
		...
		-- 📱 มือถือ: คำนวณขนาดหน้าต่างใหม่ทุกครั้งที่จอเปลี่ยน (หมุนแนวตั้ง/แนวนอน, split-screen, ฯลฯ)
		-- ไม่ทำตอนผู้ใช้กำลังลากปรับขนาดเองอยู่ (Resizing) กันชนกัน
		if Mobile then
			Creator.AddSignal(Camera:GetPropertyChangedSignal("ViewportSize"), function()
				if Resizing then return end
				local newSize = ComputeMobileWindowSize(Camera.ViewportSize)
				...
				-- ย่อ/ขยายเนื้อหาข้างในให้พอดีกับขนาดหน้าต่างใหม่ด้วย ไม่งั้นจะเหมือนก่อนแก้ (ล้น/ทับกัน)
				Window.ContentScale = ComputeContentScale(Config.DesignSize, newSize)
...
Library.CreateWindow = function(self, Config)
	...
	-- 📱 ขนาดหน้าต่างบนมือถือ: คิดจากขนาดจอจริงเสมอ ไม่อิง Config.Size ของเดสก์ท็อป
	...
	if Mobile then
		...
		local mobileSize = ComputeMobileWindowSize(vp)
		if mobileSize then
