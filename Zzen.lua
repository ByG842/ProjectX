...
-- 📱 คำนวณ "สเกลเดียว" ที่ใช้ทั้งย่อขนาดหน้าต่างและย่อเนื้อหาข้างในไปพร้อมกัน
-- (เดิมแยกคำนวณคนละสูตร ทำให้กรอบหน้าต่างกับเนื้อหาที่ย่อแล้วไม่ match กัน เหลือที่ว่างไม่เต็ม)
-- คงสัดส่วน (aspect ratio) เดิมของ design ไว้เป๊ะ แค่ย่อสัดส่วนเดียวกันทั้งกว้าง-สูง
-- รับประกันว่าเนื้อหาข้างใน (ที่ถูก UIScale ด้วยสเกลเดียวกัน) จะเต็มพอดีกรอบหน้าต่างเสมอ ไม่มีที่ว่างเหลือ
local function ComputeMobileScale(viewportSize, designSize)
	if not viewportSize or not designSize then return 1 end
	local dw, dh = designSize.X.Offset, designSize.Y.Offset
	if dw <= 0 or dh <= 0 then return 1 end

	local inset = GuiService:GetGuiInset() -- หัก status bar / notch / home indicator ออกก่อน
	local margin = 16 -- เผื่อขอบจอทั้งสองด้าน
	local usableX = math.max(viewportSize.X - inset.X - margin * 2, 200)
	local usableY = math.max(viewportSize.Y - inset.Y - margin * 2, 200)

	-- หาสเกลสูงสุดที่ใส่ design ทั้งก้อนลงในพื้นที่ใช้งานได้พอดี โดยไม่บิดสัดส่วน (letterbox-fit)
	-- ครอบคลุมทั้ง portrait/landscape อัตโนมัติ เพราะมิติที่ "จำกัดกว่า" จะเป็นตัวกำหนดสเกลเอง
	local scale = math.min(usableX / dw, usableY / dh, 1)
	return math.clamp(scale, 0.5, 1)
end

-- 📱 ขนาดหน้าต่างบนมือถือ = design size คูณด้วยสเกลที่คำนวณได้ (ไม่ใช่เปอร์เซ็นต์จอลอยๆ แบบเดิม)
local function ComputeMobileWindowSize(viewportSize, designSize, scale)
	if not viewportSize or not designSize then return nil end
	scale = scale or ComputeMobileScale(viewportSize, designSize)
	local dw, dh = designSize.X.Offset, designSize.Y.Offset
	return UDim2.fromOffset(math.floor(dw * scale), math.floor(dh * scale))
end

-- 📱 สัดส่วนย่อเนื้อหาภายในหน้าต่าง (UIScale) — ใช้สเกลเดียวกับที่คำนวณขนาดหน้าต่าง เพื่อให้ fit กันเป๊ะ
local function ComputeContentScale(viewportSize, designSize)
	if not Mobile or not viewportSize or not designSize then return 1 end
	return ComputeMobileScale(viewportSize, designSize)
end
...
Components.Window = (function()
	...
	return function(Config)
		...
		-- + ใช้ตัวเดียวกันย่อเนื้อหาทั้งหมดตามสัดส่วนจอมือถือ (สเกลเดียวกับที่ใช้คำนวณขนาดหน้าต่าง ให้ fit กันเป๊ะ)
		Window.ContentScale = ComputeContentScale(Camera.ViewportSize, Config.DesignSize)
		Window.RootScale = New("UIScale", { Scale = Window.ContentScale })
		Window.RootScale.Parent = Window.Root
		local RootScaleMotor, SetRootScale = Creator.SpringMotor(Window.ContentScale, Window.RootScale, "Scale")
		...
		-- 📱 มือถือ: คำนวณขนาดหน้าต่าง+สเกลเนื้อหาใหม่ทุกครั้งที่จอเปลี่ยน (หมุนแนวตั้ง/แนวนอน, split-screen, ฯลฯ)
		-- ใช้สเกลตัวเดียวกันคำนวณทั้งคู่ (ComputeMobileScale) รับประกันว่าเนื้อหาเต็มพอดีกรอบเสมอ ไม่มีที่ว่างเหลือ
		-- ไม่ทำตอนผู้ใช้กำลังลากปรับขนาดเองอยู่ (Resizing) กันชนกัน
		if Mobile then
			Creator.AddSignal(Camera:GetPropertyChangedSignal("ViewportSize"), function()
				if Resizing then return end
				local vp = Camera.ViewportSize
				local scale = ComputeMobileScale(vp, Config.DesignSize)
				local newSize = ComputeMobileWindowSize(vp, Config.DesignSize, scale)
				if not newSize then return end

				Window.Size = newSize
				SizeMotor:setGoal({
					X = Instant(newSize.X.Offset),
					Y = Instant(newSize.Y.Offset),
				})

				Window.ContentScale = scale
				if Window.RootScaleMotor then
					local minimizedMultiplier = Window.Minimized and 0.8 or 1
					Window.RootScaleMotor:setGoal(Instant(Window.ContentScale * minimizedMultiplier))
				end
		...
		function Window:Minimize()
			...
			if Window.Minimized then
				if Window.RootScaleMotor then
					Window.RootScaleMotor:setGoal(Spring(0.8 * (Window.ContentScale or 1), SmoothSpringParams))
				end
				...
				if Window.RootScale then
					Window.RootScale.Scale = 0.8 * (Window.ContentScale or 1)
				end
				if Window.RootWrapper then
					Window.RootWrapper.GroupTransparency = 1
				end
				if Window.RootScaleMotor then
					Window.RootScaleMotor:setGoal(Spring(1 * (Window.ContentScale or 1), SmoothSpringParams))
				end
				if Window.RootFadeMotor then
...
Library.CreateWindow = function(self, Config)
	...
	-- 📱 ขนาดหน้าต่างบนมือถือ: คิดจาก DesignSize (ขนาดต้นฉบับ) คูณสเกลเดียวที่พอดีจอ
	-- (แก้ปัญหาเดิมที่ Config.Size คงที่ เช่น 760x540 ล้นจอมือถือ/ต้องซูมตลอดเวลา
	--  และแก้ปัญหาที่ขนาดหน้าต่างกับสเกลเนื้อหาคำนวณคนละสูตร ทำให้เนื้อหาไม่เต็มพอดีกรอบ)
	-- รองรับทุกรุ่น/ทุกความละเอียด เพราะคำนวณจาก ViewportSize จริง ไม่ใช่เลขคงที่
	local DesignSize = Config.Size -- ขนาดที่นักพัฒนาตั้งใจออกแบบ layout ไว้ (ใช้เทียบสัดส่วนย่อเนื้อหา)
	local ResolvedSize = Config.Size
	if Mobile then
		local vp = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize
		local mobileSize = ComputeMobileWindowSize(vp, DesignSize)
		if mobileSize then
			ResolvedSize = mobileSize
		end
	end
